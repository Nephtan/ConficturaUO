# SOURCE-BATCH-366 DDRelicCloth Guard Repair Target
...
Add a local guard to `DDRelicCloth.OnDoubleClick(Mobile from)` so stale/null interaction state cannot send the existing informational relic-identification message through a null or deleted mobile, or from a deleted source relic item.
