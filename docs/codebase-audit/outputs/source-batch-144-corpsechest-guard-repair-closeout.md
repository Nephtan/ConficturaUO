# SOURCE-BATCH-144 CorpseChest Guard Repair Closeout

## Summary

Implemented `SB144-CAND-001` from `source-batch-144-candidate-discovery.csv`.

`Data/Scripts/Items/Containers/CorpseChest.cs` now guards stale/null mobile and deleted source container state before reading `from.Luck`, changing delayed-fill state, or filling the container.

## Source Changes

- `CorpseChest.OnDragLift(Mobile from)` now returns `false` for null/deleted mobiles or deleted source containers.
...
- POST-BATCH-Y gate hits for `Data/Scripts/Items/Containers/CorpseChest.cs`: `0`.
- Active overlay rows for `Data/Scripts/Items/Containers/CorpseChest.cs`: `0`.
...
SOURCE-BATCH-144 is complete and ready to commit. `SOURCE-BATCH-145+` remains pending the next concrete non-gated source target from `source-batch-144-candidate-discovery.csv`.
