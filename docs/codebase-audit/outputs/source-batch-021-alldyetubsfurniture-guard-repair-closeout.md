# SOURCE-BATCH-021 Closeout: AllDyeTubsFurniture Guard Repair

Reviewed at: 2026-06-16T19:24:10.9554142-05:00

## Summary

`SOURCE-BATCH-021` implemented `SB017-CAND-005`, a non-gated `AllDyeTubsFurniture` guard repair in `Data/Scripts/Items/Misc/Dyes/AllDyeTubsFurniture.cs`.
...
- `AllDyeTubsFurnitureTarget.OnTarget(Mobile from, object targeted)` now returns for null/deleted mobiles, null/deleted source tubs, and deleted target items.
...
- Targeted source scan confirmed `AllowPack`, target range, `FurnitureAttribute.Check(item)`, `ConsumeTotal(typeof(Gold), 100)`, hue assignment, charged deletion/decrement, sound `0x23F`, and serialization markers remain present.
