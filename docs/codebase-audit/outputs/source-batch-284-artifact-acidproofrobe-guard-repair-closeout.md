# SOURCE-BATCH-284 Artifact_AcidProofRobe Guard Repair Closeout

## Summary

`SOURCE-BATCH-284` implemented `SB284-CAND-001` in `Data/Scripts/Items/Magical/Artifacts/Artifact_AcidProofRobe.cs`.

`Artifact_AcidProofRobe.OnDoubleClick(Mobile from)` now returns immediately for null/deleted mobiles or deleted robes before cooldown math, parent checks, bottle consumption, acid creation, or messages, and missing backpacks now use the existing empty-bottle failure message. `OnDragLift(Mobile from)` now returns `false` for null/deleted mobiles or deleted robes before drag-lift messaging.
...
- Passed: changed-file scan found only `Data/Scripts/Items/Magical/Artifacts/Artifact_AcidProofRobe.cs` as a source change, with no project/config/data changes.
