# SOURCE-BATCH-145 CorpseSailor Guard Repair Closeout

## Summary

Implemented `SB144-CAND-002` from `source-batch-144-candidate-discovery.csv`.

`Data/Scripts/Items/Containers/CorpseSailor.cs` now guards stale/null mobile and deleted source container state before reading `from.Luck`, changing delayed-fill state, or filling the container.

## Source Changes

- `CorpseSailor.OnDragLift(Mobile from)` now returns `false` for null/deleted mobiles or deleted source containers.
...
- POST-BATCH-Y gate hits for `Data/Scripts/Items/Containers/CorpseSailor.cs`: `0`.
- Active overlay rows for `Data/Scripts/Items/Containers/CorpseSailor.cs`: `0`.
...
SOURCE-BATCH-145 is complete and ready to commit. `SOURCE-BATCH-146+` remains pending the next concrete non-gated source target from `source-batch-144-candidate-discovery.csv`.
