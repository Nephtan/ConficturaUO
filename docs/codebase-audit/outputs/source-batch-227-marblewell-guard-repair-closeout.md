# SOURCE-BATCH-227 MarbleWell Guard Repair Closeout

## Summary

`SOURCE-BATCH-227` created fresh candidate discovery and implemented `SB227-CAND-001` in `Data/Scripts/Items/Construction/Wells/marblewell.cs`.

`MarbleWellPiece.OnDoubleClick(Mobile from)` now guards stale/null/deleted interaction state before range, thirst, message, or parent-addon state reads.
...
- POST-BATCH-Y exact-file gate hits for `Data/Scripts/Items/Construction/Wells/marblewell.cs`: `0`
