# SOURCE-BATCH-024 Closeout: AllDyeTubsMountEthereal Guard Repair

Reviewed at: 2026-06-16T19:40:06.2516530-05:00

## Summary

`SOURCE-BATCH-024` implemented `SB022-CAND-003`, a non-gated `AllDyeTubsMountEthereal` guard repair in `Data/Scripts/Items/Misc/Dyes/AllDyeTubsMountEthereal.cs`.
...
- `AllDyeTubsMountEtherealTarget.OnTarget(Mobile from, object targeted)` now returns for null/deleted mobiles, null/deleted source tubs, and deleted target items.
...
- Targeted source scan confirmed `AllowPack`, target range, `EtherealMount`, `ConsumeTotal(typeof(Gold), 100)`, hue assignment, charged deletion/decrement, sound `0x23F`, the `Select an ethereal to dye` message, and serialization markers remain present.
