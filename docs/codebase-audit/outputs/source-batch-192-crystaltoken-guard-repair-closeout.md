# SOURCE-BATCH-192 CrystalToken Guard Repair Closeout

## Summary

`SOURCE-BATCH-192` implemented `SB192-CAND-001` in `Data/Scripts/Items/Gifts/Crystal Token.cs`.

`CrystalToken.OnDoubleClick(Mobile from)` now returns immediately for null/deleted mobiles or deleted source tokens before checking backpack state or opening `CrystalTokenGump`. Missing backpacks are handled through the existing localized backpack-use failure message.

## Preserved Behavior

- Valid in-backpack double-click still opens `CrystalTokenGump`.
...
- POST-BATCH-Y exact-file gate hits for `Data/Scripts/Items/Gifts/Crystal Token.cs`: `0`
- Exact-file active overlay rows: `0`
- No gated approval crossed.

## Verification

- Candidate CSV import: passed; `SB192-CAND-001` imported with `PostBatchYGateHitCount=0` and `ActiveOverlayRows=0`.
- Targeted source scan: passed; mobile/source guard, backpack guard, localized message `1042001`, `CrystalTokenGump` open behavior, `Serialize`, and `Deserialize` remain present.
