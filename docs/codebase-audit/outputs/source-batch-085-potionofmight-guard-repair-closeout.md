# SOURCE-BATCH-085 PotionOfMight Guard Repair Closeout

## Summary

`SOURCE-BATCH-085` implemented `SB084-CAND-002` from `source-batch-084-candidate-discovery.csv`.

`Data/Scripts/Items/Potions/Special/PotionOfMight.cs` now guards null/deleted mobiles, deleted source potions, missing backpacks, and out-of-backpack source potions before stat-cap, chance, `RawStr`, drink effect, or potion consume state is evaluated.
...
- POST-BATCH-Y gate hits for `Data/Scripts/Items/Potions/Special/PotionOfMight.cs`: `0`
...
`SOURCE-BATCH-085` is closed. `SOURCE-BATCH-086+` remains pending the next concrete non-gated source target from `source-batch-084-candidate-discovery.csv`.
