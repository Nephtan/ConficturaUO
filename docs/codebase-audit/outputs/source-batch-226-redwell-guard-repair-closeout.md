# SOURCE-BATCH-226 RedWell Guard Repair Closeout

## Summary

`SOURCE-BATCH-226` created fresh candidate discovery and implemented `SB226-CAND-001` in `Data/Scripts/Items/Construction/Wells/redwell.cs`.

`RedWellPiece.OnDoubleClick(Mobile from)` now guards stale/null/deleted interaction state before range, thirst, message, or parent-addon state reads.
...
- POST-BATCH-Y exact-file gate hits for `Data/Scripts/Items/Construction/Wells/redwell.cs`: `0`
