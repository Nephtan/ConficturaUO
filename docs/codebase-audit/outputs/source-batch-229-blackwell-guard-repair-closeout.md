# SOURCE-BATCH-229 BlackWell Guard Repair Closeout

## Summary

`SOURCE-BATCH-229` created fresh candidate discovery and implemented `SB229-CAND-001` in `Data/Scripts/Items/Construction/Wells/blackwell.cs`.

`BlackWellPiece.OnDoubleClick(Mobile from)` now guards stale/null/deleted interaction state before range, thirst, message, or parent-addon state reads.
...
- POST-BATCH-Y exact-file gate hits for `Data/Scripts/Items/Construction/Wells/blackwell.cs`: `0`
