# SOURCE-BATCH-063 OilTopaz Guard Repair Closeout
...
- POST-BATCH-Y gate hits for `Data/Scripts/Items/Potions/Oils/OilTopaz.cs`: 0
- Active overlay rows for `Data/Scripts/Items/Potions/Oils/OilTopaz.cs`: 0
- Gate crossed: none
...
SOURCE-BATCH-063 is closed as a committed non-gated source batch. The `source-batch-049-candidate-discovery.csv` queue is exhausted, and `SOURCE-BATCH-064+` requires a new candidate discovery pass.
