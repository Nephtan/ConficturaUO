# SOURCE-BATCH-369 DDRelicJewels Guard Repair Target
...
Add a local guard to `DDRelicJewels.OnDoubleClick(Mobile from)` so stale/null interaction state cannot send the existing informational relic-identification message through a null or deleted mobile, or from a deleted source relic item.
