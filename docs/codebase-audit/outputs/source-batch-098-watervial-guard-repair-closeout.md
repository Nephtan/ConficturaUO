# SOURCE-BATCH-098 WaterVial Guard Repair Closeout

## Summary

`SOURCE-BATCH-098` implemented `SB093-CAND-006`, the `WaterVial` guard repair from `source-batch-093-candidate-discovery.csv`.

`Data/Scripts/Items/Food/WaterVial.cs` now guards null/deleted mobiles and deleted source vials before dispatching to `DrinkingFunctions.OnDrink`.
...
- POST-BATCH-Y gate hits for `Data/Scripts/Items/Food/WaterVial.cs`: `0`
...
- Targeted source scan confirmed the new mobile/source-vial guard and preserved `DrinkingFunctions.OnDrink(this, from)` delegation plus deserialize reset evidence.
...
`SOURCE-BATCH-098` is closed. The `source-batch-093-candidate-discovery.csv` implementation queue is exhausted; `SOURCE-BATCH-099+` requires fresh candidate discovery.
