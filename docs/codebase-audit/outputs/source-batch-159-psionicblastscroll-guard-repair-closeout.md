# SOURCE-BATCH-159 PsionicBlastScroll Guard Repair Closeout

## Summary

`SOURCE-BATCH-159` implemented `SB154-CAND-006` in `Data/Scripts/Magic/Mystic/Scrolls/PsionicBlastScroll.cs`.

`PsionicBlastScroll.OnDoubleClick` now returns immediately for null/deleted mobiles or a deleted source scroll before comparing owner, sending messages, or deleting the scroll. `PsionicBlastScroll.OnDragLift` now returns `false` for the same stale state before comparing owner, sending the crumble message, or deleting the scroll.
...
- POST-BATCH-Y gate hits for `PsionicBlastScroll.cs`: 0
- Active overlay rows for `PsionicBlastScroll.cs`: 0
...
`SOURCE-BATCH-160+` is pending `SB154-CAND-007` / `PsychicWallScroll` fresh preflight.
