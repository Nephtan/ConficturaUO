# SOURCE-BATCH-457 LearnReagentsBook Guard Repair Target

## Target

- Batch: `SOURCE-BATCH-457`
- Candidate: `SB457-CAND-001`
- Behavior: add stale/null mobile, deleted source item, and missing-backpack guards to `LearnReagentsBook.OnDoubleClick(Mobile e)`.
- System: `Items:Books / LearnReagentsBook`
- File: `Data/Scripts/Items/Books/LearnReagents.cs`

## Fence Result

- POST-BATCH-Y exact-file gate hits: `0`
- Active overlay exact-file rows: `0`
- No staff/access, command policy, balance/economy, region/map, serializer migration, project/config/data, XML/config/data, or reorganization approval is crossed.

## Must Stay Unchanged

- book item ID, name, weight, and property text
- reagents guide text
- `LearnReagentsGump` layout
- sound `0x249`
- `Server.Gumps.MyLibrary.readBook(this, e)` behavior
- existing backpack-use failure message
- serialization layout/versioning
- namespace/type/file layout
- project/config/data files, staff/access behavior, economy/reward tuning, region/map policy, and reorganization state

## Ready Goal

`/goal SOURCE-BATCH-457 LearnReagentsBook Guard Repair`
