# SOURCE-BATCH-146 LootBag Guard Repair Closeout

## Summary

Implemented `SB144-CAND-003` from `source-batch-144-candidate-discovery.csv`.

`Data/Scripts/Items/Containers/LootBag.cs` now guards stale/null mobile and deleted source container state before reading `from.Luck`, changing delayed-fill state, or filling the container.

## Source Changes

- `LootBag.OnDragLift(Mobile from)` now returns `false` for null/deleted mobiles or deleted source containers.
...
- POST-BATCH-Y gate hits for `Data/Scripts/Items/Containers/LootBag.cs`: `0`.
- Active overlay rows for `Data/Scripts/Items/Containers/LootBag.cs`: `0`.
...
SOURCE-BATCH-146 is complete and ready to commit. `SOURCE-BATCH-147+` remains pending the next concrete non-gated source target from `source-batch-144-candidate-discovery.csv`.
