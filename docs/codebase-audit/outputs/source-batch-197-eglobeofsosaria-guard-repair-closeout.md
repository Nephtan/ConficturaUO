# SOURCE-BATCH-197 EGlobeOfSosaria Guard Repair Closeout

## Summary

`SOURCE-BATCH-197` implemented `SB195-CAND-003` in `Data/Scripts/Items/Gifts/ShadowDeeds/EGlobeOfSosaria.cs`.

`EGlobeOfSosariaComponent.OnDoubleClick(Mobile from)` now returns immediately for null/deleted mobiles or deleted source components before checking range.
...
- POST-BATCH-Y exact-file gate hits for `Data/Scripts/Items/Gifts/ShadowDeeds/EGlobeOfSosaria.cs`: `0`
- Exact-file active overlay rows: `0`
- No gated approval crossed.

## Verification

- Candidate CSV import: passed; `SB195-CAND-003` imported with `PostBatchYGateHitCount=0` and `ActiveOverlayRows=0`.
