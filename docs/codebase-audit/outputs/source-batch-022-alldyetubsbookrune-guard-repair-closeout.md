# SOURCE-BATCH-022 Closeout: AllDyeTubsBookRune Guard Repair

Reviewed at: 2026-06-16T19:31:49.9389645-05:00

## Summary

`SOURCE-BATCH-022` implemented `SB022-CAND-001`, a non-gated `AllDyeTubsBookRune` guard repair in `Data/Scripts/Items/Misc/Dyes/AllDyeTubsBookRune.cs`.
...
- `AllDyeTubsBookRuneTarget.OnTarget(Mobile from, object targeted)` now returns for null/deleted mobiles, null/deleted source tubs, and deleted target items.
...
- Targeted source scan confirmed `AllowPack`, target range, `Runebook`, `ConsumeTotal(typeof(Gold), 100)`, hue assignment, charged deletion/decrement, sound `0x23F`, and serialization markers remain present.
