...
| Startup script compile smoke | Unavailable | Not run in this batch because the checkout contains a `Saves` tree and no safe no-load/no-listen startup flag has been established. |
...
Next safe action: run the startup smoke only in an isolated local copy or after the operator confirms this checkout and port configuration are safe for a short-lived server start.
