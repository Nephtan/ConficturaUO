# SOURCE-BATCH-074 StoneMiningBook Guard Repair Target

## Target

- Batch: `SOURCE-BATCH-074`
- Candidate: `SB073-CAND-002`
- Behavior: add stale/null/mobile/backpack/source-book guards to `StoneMiningBook.OnDoubleClick(Mobile from)`.
- System: `Items:Trades / Specialized / StoneMiningBook`
- Source file: `Data/Scripts/Items/Trades/Specialized/StoneMiningBook.cs`

## Fence Result

- POST-BATCH-Y gate hits for `StoneMiningBook.cs`: `0`
...
```text
/goal SOURCE-BATCH-074 StoneMiningBook Guard Repair

Implement SB073-CAND-002 from source-batch-073-candidate-discovery.csv. Add stale/null/mobile/backpack/source-book guards to Data/Scripts/Items/Trades/Specialized/StoneMiningBook.cs while preserving learning behavior, messages, deletion, serialization, layout, and all gated policy surfaces. Verify gate hits=0, active overlay rows=0, serializer diff clean, forbidden surfaces clean, Server.csproj Debug/x86 build, runtime compile-only, git diff --check, then commit as: fix: guard StoneMiningBook interactions.
```
