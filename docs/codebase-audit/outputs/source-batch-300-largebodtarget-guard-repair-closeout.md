# SOURCE-BATCH-300 LargeBODTarget Guard Repair Closeout

## Summary

`SOURCE-BATCH-300` implemented `SB300-CAND-001` in `Data/Scripts/Trades/Bulk Orders/LargeBODTarget.cs`.

`LargeBODTarget.OnTarget(Mobile from, object targeted)` now returns safely for null/deleted mobiles, null/deleted source deeds, or missing backpacks before checking deed containment or forwarding to `LargeBOD.EndCombine`.
...
- Passed: changed-file scan found only `Data/Scripts/Trades/Bulk Orders/LargeBODTarget.cs` as a tracked source change, with no project/config/data changes.
