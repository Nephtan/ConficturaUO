# SOURCE-BATCH-367 DDRelicFur Guard Repair Target
...
Add a local guard to `DDRelicFur.OnDoubleClick(Mobile from)` so stale/null interaction state cannot send the existing informational relic-identification message through a null or deleted mobile, or from a deleted source relic item.
