...
| `post-batch-b-save-compatibility-triage.csv` | Post-audit | Scope the 304 P0 critical save-compatibility rows and record source-reviewed decisions as `POST-BATCH-B` proceeds; 300 rows are reviewed after `Magic:Druidism` batch `POST-BATCH-B-30A`; blocked pending Druidism transient-effect save policy. | Blocked |
