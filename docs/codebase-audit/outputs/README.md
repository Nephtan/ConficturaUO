...
| `post-batch-b-save-compatibility-triage.csv` | Post-audit | Scope the 304 P0 critical save-compatibility rows and record source-reviewed decisions as `POST-BATCH-B` proceeds; 298 rows are reviewed after `Items:Weapons` batch `POST-BATCH-B-29A`. | InProgress |
