...
| `post-batch-b-save-compatibility-triage.csv` | Post-audit | Scope the 304 P0 critical save-compatibility rows and record source-reviewed decisions as `POST-BATCH-B` proceeds; 231 rows are reviewed after `Trades:Bulk Orders` batch `POST-BATCH-B-09A`. | InProgress |
