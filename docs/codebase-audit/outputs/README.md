...
| `post-batch-b-save-compatibility-triage.csv` | Post-audit | Scope the 304 P0 critical save-compatibility rows and record source-reviewed decisions as `POST-BATCH-B` proceeds; 250 rows are reviewed after `Items:Special` batch `POST-BATCH-B-11A`. | InProgress |
