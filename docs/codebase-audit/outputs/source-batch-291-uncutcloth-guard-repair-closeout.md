# SOURCE-BATCH-291 UncutCloth Guard Repair Closeout

## Summary

`SOURCE-BATCH-291` implemented `SB291-CAND-001` in `Data/Scripts/Items/Trades/Resources/Tailor/UncutCloth.cs`.

`UncutCloth.OnSingleClick(Mobile from)` now returns immediately for null/deleted mobiles or deleted folded cloth before localized folded-cloth label packet send.
...
- Passed: targeted source scan confirmed the stale/null/mobile/source-item guard and preserved localized label numbers, amount string, `MessageLocalized` packet values, `ICommodity` members, existing `Dye`/`Scissor` behavior, construction metadata, and serializer methods.
...
- Passed: changed-file scan found only `Data/Scripts/Items/Trades/Resources/Tailor/UncutCloth.cs` as a source change, with no project/config/data changes.
