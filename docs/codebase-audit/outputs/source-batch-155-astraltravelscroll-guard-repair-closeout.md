# SOURCE-BATCH-155 AstralTravelScroll Guard Repair Closeout

## Summary

`SOURCE-BATCH-155` implemented `SB154-CAND-002` in `Data/Scripts/Magic/Mystic/Scrolls/AstralTravelScroll.cs`.

`AstralTravelScroll.OnDoubleClick` now returns immediately for null/deleted mobiles or a deleted source scroll before comparing owner, sending messages, or deleting the scroll. `AstralTravelScroll.OnDragLift` now returns `false` for the same stale state before comparing owner, sending the crumble message, or deleting the scroll.
...
- POST-BATCH-Y gate hits for `AstralTravelScroll.cs`: 0
- Active overlay rows for `AstralTravelScroll.cs`: 0
...
`SOURCE-BATCH-156+` is pending `SB154-CAND-003` / `CreateRobeScroll` fresh preflight.
