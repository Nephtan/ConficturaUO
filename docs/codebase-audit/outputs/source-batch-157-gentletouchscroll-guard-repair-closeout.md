# SOURCE-BATCH-157 GentleTouchScroll Guard Repair Closeout

## Summary

`SOURCE-BATCH-157` implemented `SB154-CAND-004` in `Data/Scripts/Magic/Mystic/Scrolls/GentleTouchScroll.cs`.

`GentleTouchScroll.OnDoubleClick` now returns immediately for null/deleted mobiles or a deleted source scroll before comparing owner, sending messages, or deleting the scroll. `GentleTouchScroll.OnDragLift` now returns `false` for the same stale state before comparing owner, sending the crumble message, or deleting the scroll.
...
- POST-BATCH-Y gate hits for `GentleTouchScroll.cs`: 0
- Active overlay rows for `GentleTouchScroll.cs`: 0
...
`SOURCE-BATCH-158+` is pending `SB154-CAND-005` / `LeapScroll` fresh preflight.
