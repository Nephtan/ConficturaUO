# SOURCE-BATCH-020 Closeout: AllDyeTubsWeapon Guard Repair

Reviewed at: 2026-06-16T19:19:55.8367442-05:00

## Summary

`SOURCE-BATCH-020` implemented `SB017-CAND-004`, a non-gated `AllDyeTubsWeapon` guard repair in `Data/Scripts/Items/Misc/Dyes/AllDyeTubsWeapon.cs`.
...
- `AllDyeTubsWeaponTarget.OnTarget(Mobile from, object targeted)` now returns for null/deleted mobiles, null/deleted source tubs, and deleted target items.
...
- Targeted source scan confirmed `AllowPack`, target range, `BaseWeapon`, `ConsumeTotal(typeof(Gold), 100)`, hue assignment, charged deletion/decrement, sound `0x23F`, and serialization markers remain present.
