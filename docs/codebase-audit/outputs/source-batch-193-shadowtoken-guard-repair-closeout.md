# SOURCE-BATCH-193 ShadowToken Guard Repair Closeout

## Summary

`SOURCE-BATCH-193` implemented `SB193-CAND-001` in `Data/Scripts/Items/Gifts/Shadow Token.cs`.

`ShadowToken.OnDoubleClick(Mobile from)` now returns immediately for null/deleted mobiles or deleted source tokens before checking backpack state or opening `ShadowTokenGump`. Missing backpacks are handled through the existing localized backpack-use failure message.

## Preserved Behavior

- Valid in-backpack double-click still opens `ShadowTokenGump`.
...
- POST-BATCH-Y exact-file gate hits for `Data/Scripts/Items/Gifts/Shadow Token.cs`: `0`
- Exact-file active overlay rows: `0`
- No gated approval crossed.

## Verification

- Candidate CSV import: passed; `SB193-CAND-001` imported with `PostBatchYGateHitCount=0` and `ActiveOverlayRows=0`.
- Targeted source scan: passed; mobile/source guard, backpack guard, localized message `1042001`, `ShadowTokenGump` open behavior, `Serialize`, and `Deserialize` remain present.
