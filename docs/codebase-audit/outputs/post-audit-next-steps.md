...
Generated: 2026-06-06T22:50:21.3318360-05:00
...
- `post-audit-active-backlog-status.csv` also maps 102 reviewed save-compatibility rows from `POST-BATCH-B-02A`, `POST-BATCH-B-02B`, `POST-BATCH-B-02C`, and `POST-BATCH-B-03A` through `POST-BATCH-B-03L` to the save triage artifact.
...
- Source-reviewed decisions cover the 19 `ServerCore` high-blast-radius rows, 10 `POST-BATCH-B-02A` XMLSpawner central persistence rows, 20 `POST-BATCH-B-02B` remaining XMLSpawner serializer rows, 28 `POST-BATCH-B-02C` `System:Obsolete` rows, and 44 `POST-BATCH-B-03*` `Custom:Mobiles` rows.
- Current reviewed decisions are 25 `FalsePositive`, 16 `IntentionalLegacy`, and 80 `SafeNoChange`.
- The remaining 183 rows are queued for later source review and do not approve source edits.
