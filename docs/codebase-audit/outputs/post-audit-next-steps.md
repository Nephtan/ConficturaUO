...
Generated: 2026-06-06T22:32:41.2393702-05:00
...
- `post-audit-active-backlog-status.csv` also maps 87 reviewed save-compatibility rows from `POST-BATCH-B-02A`, `POST-BATCH-B-02B`, `POST-BATCH-B-02C`, and `POST-BATCH-B-03A` through `POST-BATCH-B-03G` to the save triage artifact.
...
- Source-reviewed decisions cover the 19 `ServerCore` high-blast-radius rows, 10 `POST-BATCH-B-02A` XMLSpawner central persistence rows, 20 `POST-BATCH-B-02B` remaining XMLSpawner serializer rows, 28 `POST-BATCH-B-02C` `System:Obsolete` rows, and 29 `POST-BATCH-B-03*` `Custom:Mobiles` rows.
- Current reviewed decisions are 25 `FalsePositive`, 16 `IntentionalLegacy`, and 65 `SafeNoChange`.
- The remaining 198 rows are queued for later source review and do not approve source edits.
