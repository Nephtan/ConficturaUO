# SOURCE-BATCH-161 PurityOfBodyScroll Guard Repair Closeout

## Summary

`SOURCE-BATCH-161` implemented `SB154-CAND-008` in `Data/Scripts/Magic/Mystic/Scrolls/PurityOfBodyScroll.cs`.

`PurityOfBodyScroll.OnDoubleClick` now returns immediately for null/deleted mobiles or a deleted source scroll before comparing owner, sending messages, or deleting the scroll. `PurityOfBodyScroll.OnDragLift` now returns `false` for the same stale state before comparing owner, sending the crumble message, or deleting the scroll.
...
- POST-BATCH-Y gate hits for `PurityOfBodyScroll.cs`: 0
- Active overlay rows for `PurityOfBodyScroll.cs`: 0
...
`SOURCE-BATCH-162+` is pending `SB154-CAND-009` / `QuiveringPalmScroll` fresh preflight.
