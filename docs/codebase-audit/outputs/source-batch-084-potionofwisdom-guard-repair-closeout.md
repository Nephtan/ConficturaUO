# SOURCE-BATCH-084 PotionOfWisdom Guard Repair Closeout

## Summary

`SOURCE-BATCH-084` completed fresh candidate discovery, selected `SB084-CAND-001`, and implemented the `PotionOfWisdom` guard repair.

`Data/Scripts/Items/Potions/Special/PotionOfWisdom.cs` now guards null/deleted mobiles, deleted source potions, missing backpacks, and out-of-backpack source potions before stat-cap, chance, `RawInt`, drink effect, or potion consume state is evaluated.
...
- POST-BATCH-Y gate hits for `Data/Scripts/Items/Potions/Special/PotionOfWisdom.cs`: `0`
- Active overlay rows for `Data/Scripts/Items/Potions/Special/PotionOfWisdom.cs`: `0`
- No gated approval was crossed.

## Verification

- `source-batch-084-candidate-discovery.csv` imported successfully.
...
`SOURCE-BATCH-084` is closed. `SOURCE-BATCH-085+` remains pending the next concrete non-gated source target from `source-batch-084-candidate-discovery.csv`.
