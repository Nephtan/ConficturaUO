# SOURCE-BATCH-068 SkeletonsKey Guard Repair Target

## Target

- Batch: `SOURCE-BATCH-068`
- Candidate: `SB068-CAND-001`
- Behavior: add stale/null/mobile/backpack/source-key guards to `SkeletonsKey.OnDoubleClick(Mobile from)` and `UnlockTarget.OnTarget(Mobile from, object targeted)`.
- System: `Items:Containers / SkeletonsKey`
- Source file: `Data/Scripts/Items/Containers/SkeltonsKey.cs`

## Fence Result

- POST-BATCH-Y gate hits for `SkeltonsKey.cs`: `0`
...
```text
/goal SOURCE-BATCH-068 SkeletonsKey Guard Repair

Implement SB068-CAND-001 from source-batch-068-candidate-discovery.csv. Add stale/null/mobile/backpack/source-key guards to Data/Scripts/Items/Containers/SkeltonsKey.cs while preserving lock/unlock eligibility, messages, sounds, key consumption, serialization, layout, and all gated policy surfaces. Verify gate hits=0, active overlay rows=0, serializer diff clean, forbidden surfaces clean, Server.csproj Debug/x86 build, runtime compile-only, git diff --check, then commit as: fix: guard SkeletonsKey interactions.
```
