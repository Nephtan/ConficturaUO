# SOURCE-BATCH-200 EShadowFirePit Guard Repair Closeout

## Summary

`SOURCE-BATCH-200` implemented `SB198-CAND-003` in `Data/Scripts/Items/Gifts/ShadowDeeds/EShadowFirePit.cs`.

`EShadowFirePitComponent.OnDoubleClick(Mobile from)` now returns immediately for null/deleted mobiles or deleted source components before checking range.
...
- POST-BATCH-Y exact-file gate hits for `Data/Scripts/Items/Gifts/ShadowDeeds/EShadowFirePit.cs`: `0`
- Exact-file active overlay rows: `0`
- No gated approval crossed.

## Verification

- Candidate CSV import: passed; `SB198-CAND-003` imported with `PostBatchYGateHitCount=0` and `ActiveOverlayRows=0`.
- Targeted source scan: passed; mobile/source guard, range check, localized overhead message `1019045`, light setting, addon components, deed metadata, deserialize item-id correction, `Serialize`, and `Deserialize` remain present.
