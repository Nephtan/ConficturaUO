# SOURCE-BATCH-273 PortableSmelter Guard Repair Closeout

## Summary

`SOURCE-BATCH-273` implemented `SB273-CAND-001` in `Data/Scripts/Items/Technology/PortableSmelter.cs`.

`PortableSmelter.OnDoubleClick(Mobile from)` now returns immediately for null/deleted mobiles or deleted source smelters before movable and backpack checks. It also uses the existing backpack-use failure when the mobile has no backpack. `InternalTarget.OnTarget(Mobile from, object targeted)` now returns immediately for null/deleted mobiles and uses the existing backpack-use failure for null/deleted/out-of-backpack source smelter state before evaluating ore, range, skills, ingot conversion, or charge consumption.
...
- Passed: targeted source scan confirmed the stale/null/mobile/source-tool/backpack guards and preserved target prompt, target range, `BaseOre` eligibility, distance message, difficulty switch, Mining skill check, ingot placement, charge consumption calls, broken-tool message, and serializer methods.
...
- Passed: changed-file scan found only `Data/Scripts/Items/Technology/PortableSmelter.cs` as a source change, with no project/config/data changes.
