# SOURCE-BATCH-199 EObsidianRock Guard Repair Closeout

## Summary

`SOURCE-BATCH-199` implemented `SB198-CAND-002` in `Data/Scripts/Items/Gifts/ShadowDeeds/EObsidianRock.cs`.

`EObsidianRockComponent.OnDoubleClick(Mobile from)` now returns immediately for null/deleted mobiles or deleted source components before checking range.
...
- POST-BATCH-Y exact-file gate hits for `Data/Scripts/Items/Gifts/ShadowDeeds/EObsidianRock.cs`: `0`
- Exact-file active overlay rows: `0`
- No gated approval crossed.

## Verification

- Candidate CSV import: passed; `SB198-CAND-002` imported with `PostBatchYGateHitCount=0` and `ActiveOverlayRows=0`.
