# SOURCE-BATCH-394 MonsterManual Guard Repair Target
...
Add local guards to `MonsterManual.OnDoubleClick(Mobile from)` and `BookTarget.OnTarget(Mobile from, object targeted)` so stale/null interaction state cannot dereference an invalid mobile, use a deleted source book, or open lookup gumps for deleted target mobiles.
...
MonsterManual item ID, name, weight, Dungeons & Dragons property label, target range, target eligibility messages, `PlayersHandbook.IsPeople` delegation, `DruidismGump` construction, sound `0x55` behavior, serialization layout/versioning, namespace/type/file layout, project files, XML/config/data files, staff/access behavior, economy/reward tuning, region/map policy, and reorganization state must stay unchanged.
