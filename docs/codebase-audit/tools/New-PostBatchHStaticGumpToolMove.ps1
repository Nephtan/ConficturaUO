...
$Closeout.Add("| Serialized Static Gump Tool rows | `$($StaticSerializationRows.Count)` |") | Out-Null
...
$Closeout.Add('') | Out-Null
$Closeout.Add("Move `$TargetDir` back to `$OldDir`, restore the previous `Scripts.csproj` include/content paths, rerun project truth and path-sensitive audit generators, and revert Static Gump Tool documentation/source-trace paths.") | Out-Null
