    ...
    public class Syth : BaseCreature
    {
        ...
        public static void SpecialItem(Item item, Mobile m)
        {
            ...
            if (item is BaseHat)
            {
                ...
                BaseRunicTool.ApplyAttributesTo(hat, false, 0, props, min, max);
            }
            else if (item is BaseClothing)
            {
                BaseClothing cloth = (BaseClothing)item;
                BaseRunicTool.ApplyAttributesTo(cloth, false, 0, props, min, max);
            }
            else if (item is BaseArmor)
            {
                BaseArmor armor = (BaseArmor)item;
                BaseRunicTool.ApplyAttributesTo(armor, false, 0, props, min, max);
            }
            else if (item is BaseWeapon)
            {
                BaseWeapon weapon = (BaseWeapon)item;
                BaseRunicTool.ApplyAttributesTo(weapon, false, 0, props, min, max);
            }
        }
