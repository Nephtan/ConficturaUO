...
namespace Server.Mobiles
{
    ...
    public class VendorBackpack : Backpack
    {
        ...
        )
        {
            ...
            if (!BaseHouse.NewVendorSystem && Parent is PlayerVendor)
            {
                ...
                if (
                    house != null
                    && house.IsAosRules
                    && !house.CheckAosStorage(1 + item.TotalItems + plusItems)
                )
                {
                    if (message)
        ...
        public override void GetChildContextMenuEntries(
            Mobile from,
    ...
    public class PlayerVendor : Mobile
    {
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            if (version < 1 || newVendorSystemActivated)
            {
                if (version < 1)
                {
                    ...
                    Timer.DelayCall(
                        TimeSpan.Zero,
                        new TimerStateCallback(UpgradeFromVersion0),
                        newVendorSystemActivated
                    );
                }
                else
                {
        ...
        public override bool CheckNonlocalDrop(Mobile from, Item item, Item target)
        {
            if (IsOwner(from))
            {
                if (GetVendorItem(item) == null)
                {
                    ...
                    Timer.DelayCall(
                        TimeSpan.Zero,
                        new TimerStateCallback(NonLocalDropCallback),
        ...
        public static void TryToBuy(Item item, Mobile from)
        {
            ...
            {
                from.SendMessage(
                    "You cannot buy this item right now.  Please wait one minute and try again."
                );
            }
            else
            {
        ...
        public void CollectGold(Mobile to)
        {
            if (HoldGold > 0)
            {
                SayTo(
                    to,
                    "How much of the {0} that I'm holding would you like?",
                    HoldGold.ToString()
                );
