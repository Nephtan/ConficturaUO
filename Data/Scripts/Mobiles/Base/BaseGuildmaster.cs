...
namespace Server.Mobiles
{
    public abstract class BaseGuildmaster : BaseVendor
    {
        ...
        public virtual void SayWelcomeTo(Mobile m)
        {
            SayTo(
                m,
                "Welcome to the guild! Thou shalt find it beneficial to your future endeavors."
            );
        }
        ...
        public static void SayPriceTo(Mobile m, Mobile guildmaster)
        {
            m.Send(
                new MessageLocalizedAffix(
                    ...
                    guildmaster.Body,
                    MessageType.Regular,
                    guildmaster.SpeechHue,
                    3,
                    1008052,
                    guildmaster.Name,
                    AffixType.Append,
                    " " + JoiningFee(m).ToString(),
                    ""
                )
            );
        }
        ...
        public class JoinEntry : ContextMenuEntry
        {
            ...
            public JoinEntry(Mobile from, Mobile guildmaster)
                : base(6116, 3)
            {
                m_Mobile = from;
        ...
        public class ResignEntry : ContextMenuEntry
        {
            ...
            public ResignEntry(Mobile from, Mobile guildmaster)
                : base(6115, 3)
            {
                m_Mobile = from;
                m_Guildmaster = guildmaster;
        ...
        public static void JoinGuild(Mobile player, Mobile guildmaster)
        {
            ...
            if (player.Blessed)
                ...
                ((PlayerMobile)player).Profession == 1
                && ((BaseVendor)guildmaster).NpcGuild != NpcGuild.AssassinsGuild
                && ((BaseVendor)guildmaster).NpcGuild != NpcGuild.NecromancersGuild
                && ((BaseVendor)guildmaster).NpcGuild != NpcGuild.ThievesGuild
            )
        ...
        public void ProcessGuild(Mobile from, Item dropped)
        {
            ...
            if (
                pm.Profession == 1
                && NpcGuild != NpcGuild.AssassinsGuild
                && NpcGuild != NpcGuild.NecromancersGuild
