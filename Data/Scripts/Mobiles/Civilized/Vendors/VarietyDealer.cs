    ...
    public class VarietyDealer : BaseVendor
    {
                    ...
                    if (!mobile.HasGump(typeof(SpeechGump)))
                    {
                        ...
                        mobile.SendGump(
                            new SpeechGump(
                                mobile,
                                "The Hunt For Relics",
                                SpeechFunctions.SpeechText(m_Giver, m_Mobile, "Variety")
                            )
                        );
                    }
                }
        ...
        public override bool OnDragDrop(Mobile from, Item dropped)
        {
            if (dropped is Gold)
            {
                ...
                {
                    if (
                        Server.Misc.PlayerSettings.GetDiscovered(from, "the Land of Sosaria")
                        && Server.Misc.PlayerSettings.GetDiscovered(from, "the Land of Lodoria")
                        && Server.Misc.PlayerSettings.GetDiscovered(
                            from,
                    ...
                    )
                    {
                        if (AlreadyHasBook(from))
                        {
                            this.PublicOverheadMessage(
                                ...
                                0,
                                false,
                                string.Format("Here. I see you already have a book.")
                            );
                        }
                        else if (PlayerSettings.GetKeys(from, "Antiques"))
                        {
                            this.PublicOverheadMessage(
                                ...
                                0,
                                false,
                                string.Format("Thank you, but you already done that for me.")
                            );
                        }
                        else
                        {
                            ...
                            this.PublicOverheadMessage(
                                ...
                                0,
                                false,
                                string.Format("Good luck with the search.")
                            );
                    ...
                    {
                        sMessage =
                            "You need to discover the nine lands before I share this with you.";
            ...
            else if (dropped is MuseumBook)
            {
                ...
                else if (MuseumBook.GetNext(book) > 60)
                {
                    ...
                    sMessage =
                        "You have done the museum a great service. Here is "
