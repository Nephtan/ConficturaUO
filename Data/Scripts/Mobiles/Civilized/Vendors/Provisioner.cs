...
namespace Server.Mobiles
{
    public class Provisioner : BaseVendor
    {
        ...
        public override void InitSBInfo()
        {
            ...
            if (CurrentMonth == "12")
            {
                m_SBInfos.Add(new SBHolidayXmas());
            }
            if (CurrentMonth == "10")
            {
                m_SBInfos.Add(new SBHolidayHalloween());
            }
                    ...
                    if (!mobile.HasGump(typeof(SpeechGump)))
                    {
                        ...
                        mobile.SendGump(
                            new SpeechGump(
                                ...
                                SpeechFunctions.SpeechText(m_Giver, m_Mobile, "Provisioner")
                            )
                        );
                    }
                }
        ...
        private class FixEntry : ContextMenuEntry
        {
            ...
            public FixEntry(Provisioner Provisioner, Mobile from)
                : base(6120, 12)
            {
        ...
        public void BeginRepair(Mobile from)
        {
            ...
            if (BeggingPose(from) > 0) // LET US SEE IF THEY ARE BEGGING
            {
                ...
                SayTo(
                    from,
                    "Since you are begging, which unusual item shall I examine, for "
                        + idCost.ToString()
                        + " gold?"
                );
            }
            else
            {
                SayTo(
                    from,
                    "Which unusual item shall I examine, for " + idCost.ToString() + " gold?"
                );
            }
        ...
        private class RepairTarget : Target
        {
            ...
            public RepairTarget(Provisioner provisioner)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is Item)
                {
                    ...
                    if (Server.Misc.RelicItems.IsRelicItem(examine))
                    {
                        ...
                        {
                            string toSay = Server.Misc.RelicItems.IdentifyRelicValue(
                                m_Provisioner,
                                from,
                                examine
                            );
                        ...
                        else
                        {
                            m_Provisioner.SayTo(
                                from,
                                "It would cost you {0} gold for me to examine that.",
                                toConsume
                            );
