...
namespace Server.Mobiles
{
    public class Jester : BaseVendor
    {
                    ...
                    if (!mobile.HasGump(typeof(SpeechGump)))
                    {
                        ...
                        mobile.SendGump(
                            new SpeechGump(
                                mobile,
                                "Surely You Jest",
                                SpeechFunctions.SpeechText(m_Giver, m_Mobile, "Jester")
                            )
                        );
                    }
                }
        ...
        private class FixEntry : ContextMenuEntry
        {
            ...
            public FixEntry(Jester Jester, Mobile from)
                : base(6120, 12)
            {
                m_Jester = Jester;
                m_From = from;
        ...
        public void BeginRepair(Mobile from)
        {
            ...
            if (BeggingPose(from) > 0) // LET US SEE IF THEY ARE BEGGING
            {
                ...
                SayTo(
                    from,
                    "Since you are begging, do you still want me to alter your hat, robe, or shoes to look more foolish as it will only cost you "
                        + nCost.ToString()
                        + " gold?"
                );
            }
            else
            {
                SayTo(
                    from,
                    "If you want me to alter your hat, robe, or shoes to be more foolish, it will cost you "
                        + nCost.ToString()
                        + " gold."
                );
            }
        ...
        private class RepairTarget : Target
        {
            ...
            public RepairTarget(Jester tailor)
                ...
                )
                {
                    ...
                    if (pack.ConsumeTotal(typeof(Gold), toConsume))
                    {
                        ...
                        )
                        {
                            ...
                            {
                                ba.ItemID = 0x4C17;
                                ba.Name = adj + cls + "suit";
                            }
                            else if (ba.ItemID == 0x4C17)
                            {
                                ba.ItemID = 0x2B6B;
                                ba.Name = adj + cls + "coat";
                            }
                            else
                            {
                                ba.ItemID = 0x4C16;
                                ba.Name = adj + cls + "garb";
                            }
                        }
                    }
                    else
