...
namespace Server.Mobiles
{
    public class Scribe : BaseVendor
    {
                    ...
                    if (!mobile.HasGump(typeof(SpeechGump)))
                    {
                        ...
                        mobile.SendGump(
                            new SpeechGump(
                                mobile,
                                "The Written Word",
                                SpeechFunctions.SpeechText(m_Giver, m_Mobile, "Scribe")
                            )
                        );
                    }
                }
                    ...
                    if (from.Skills[SkillName.Inscribe].Value >= 30)
                    {
                        if (Server.Misc.Research.AlreadyHasBag(from))
                        {
                            this.PublicOverheadMessage(
                                ...
                                0,
                                false,
                                string.Format("Here. You already have a pack.")
                            );
                        }
                        else
                        {
        ...
        private class FixEntry : ContextMenuEntry
        {
            ...
            public FixEntry(Scribe Scribe, Mobile from)
                : base(6120, 12)
            {
                m_Scribe = Scribe;
                m_From = from;
        ...
        public void BeginRepair(Mobile from)
        {
            ...
            if (BeggingPose(from) > 0) // LET US SEE IF THEY ARE BEGGING
            {
                ...
                SayTo(
                    from,
                    "Since you are begging, do you still want me to identify an unknown scroll, it will only cost you  "
                        + nCost.ToString()
                        + " gold?"
                );
            }
            else
            {
                SayTo(
                    from,
                    "If you want me to identify an unknown scroll, it will cost you  "
                        + nCost.ToString()
                        + " gold."
                );
            }
        ...
        private class RepairTarget : Target
        {
            ...
            public RepairTarget(Scribe mage)
