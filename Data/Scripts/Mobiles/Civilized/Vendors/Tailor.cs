...
namespace Server.Mobiles
{
    public class Tailor : BaseVendor
    {
                ...
                PlayerMobile mobile = (PlayerMobile)m_Mobile;
                {
                    if (!mobile.HasGump(typeof(SpeechGump)))
                    {
                        ...
                        mobile.SendGump(
                            new SpeechGump(
                                mobile,
                                "Altering Cloaks And Robes",
                                SpeechFunctions.SpeechText(m_Giver, m_Mobile, "Tailor")
                            )
                        );
                    }
                }
        ...
        private class FixEntry : ContextMenuEntry
        {
            ...
            public FixEntry(Tailor Tailor, Mobile from)
                : base(6120, 12)
            {
                m_Tailor = Tailor;
                m_From = from;
        ...
        public void BeginRepair(Mobile from)
        {
            ...
            if (BeggingPose(from) > 0) // LET US SEE IF THEY ARE BEGGING
            {
                ...
                nCostH = nCostH - (int)((from.Skills[SkillName.Begging].Value * 0.005) * nCostH);
                if (nCostH < 1)
                {
                    nCostH = 1;
                }
                idCost = idCost - (int)((from.Skills[SkillName.Begging].Value * 0.005) * idCost);
                if (idCost < 1)
                {
                    idCost = 1;
                }
                SayTo(
                    from,
                    "Since you are begging, do you still want me to tailor your robe or cloak to look normal, it will only cost you "
                        + nCost.ToString()
                        + " gold? Maybe repair a hat for at least "
                        + nCostH.ToString()
                        + " gold per durability? Or maybe identify an item for "
                        + idCost.ToString()
                        + " gold?"
                );
            }
            else
            {
                SayTo(
                    from,
                    "If you want me to tailor your robe or cloak to look normal, it will cost you "
                        + nCost.ToString()
                        + " gold. Maybe repair a hat at "
                        + nCostH.ToString()
                        + " gold per durability? Or maybe identify an item for "
                        + idCost.ToString()
                        + " gold?"
        ...
        private class RepairTarget : Target
        {
            ...
            public RepairTarget(Tailor tailor)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if ((targeted is MagicRobe || targeted is BaseOuterTorso) && from.Backpack != null)
                {
                    ...
                    if (ba.ItemID != 0x1F03 && ba.ItemID != 0x1F04)
                    {
                        ...
                        if (BeggingPose(from) > 0) // LET US SEE IF THEY ARE BEGGING
                        {
                            nCost =
                                nCost
                                - (int)((from.Skills[SkillName.Begging].Value * 0.005) * nCost);
                            if (nCost < 1)
                            {
                                nCost = 1;
                            }
                    ...
                    else
                    {
                        m_Tailor.SayTo(
                            from,
                            "It would cost you {0} gold to have that done.",
                            toConsume
                        );
                ...
                /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    ...
                    (targeted is BaseHat || targeted is BaseLevelHat || targeted is BaseGiftHat)
                    && from.Backpack != null
                    && ((Item)targeted).ItemID != 0x1545
                    && ((Item)targeted).ItemID != 0x1546
                    && ((Item)targeted).ItemID != 0x1547
                    && ((Item)targeted).ItemID != 0x1548
                    && ((Item)targeted).ItemID != 0x2B6D
                    && ((Item)targeted).ItemID != 0x3164
                )
                {
                    ...
                    else
                    {
                        m_Tailor.SayTo(
                            from,
                            "It would cost you {0} gold to have that repaired.",
                            toConsume
                        );
                        from.SendMessage("You do not have enough gold.");
                    }
