...
namespace Server.Mobiles
{
    public class Thief : BaseVendor
    {
        ...
        public override void InitOutfit()
        {
            ...
            {
                case 0:
                    AddItem(new Server.Items.Bandana(color));
                    break;
                case 1:
                    AddItem(new Server.Items.SkullCap(color));
                    break;
                case 2:
                    AddItem(new Server.Items.ClothCowl(color));
                    AddItem(new Server.Items.Cloak(color));
                    break;
                case 3:
                    AddItem(new Server.Items.ClothHood(color));
                    AddItem(new Server.Items.Cloak(color));
                    break;
                case 4:
                    AddItem(new Server.Items.FancyHood(color));
                    AddItem(new Server.Items.Cloak(color));
                    break;
                case 5:
                    AddItem(new Server.Items.HoodedMantle(color));
                    AddItem(new Server.Items.Cloak(color));
                    break;
            }
        }
                    ...
                    if (!mobile.HasGump(typeof(SpeechGump)))
                    {
                        ...
                        mobile.SendGump(
                            new SpeechGump(
                                mobile,
                                "The Art Of Thievery",
                                SpeechFunctions.SpeechText(m_Giver, m_Mobile, "Thief")
                            )
                        );
                    }
                }
        ...
        private class FixEntry : ContextMenuEntry
        {
            ...
            public FixEntry(Thief Thief, Mobile from)
                : base(6120, 12)
            {
                m_Thief = Thief;
                m_From = from;
        ...
        public void BeginRepair(Mobile from)
        {
            ...
            if (BeggingPose(from) > 0) // LET US SEE IF THEY ARE BEGGING
            {
                ...
                SayTo(
                    from,
                    "Since you are begging, do you still want me to unlock a box? It will only cost you "
                        + nCost.ToString()
                        + "."
                );
            }
            else
            {
                SayTo(
                    from,
                    "If you want me to unlock a box, it will cost you "
                        + nCost.ToString()
                        + " gold."
                );
            }
        ...
        private class RepairTarget : Target
        {
            ...
            public RepairTarget(Thief thief)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                else if (targeted is LockableContainer && from.Backpack != null)
                {
                    ...
                    else
                    {
                        m_Thief.SayTo(
                            from,
                            "It would cost you {0} gold to have that unlocked.",
                            toConsume
                        );
