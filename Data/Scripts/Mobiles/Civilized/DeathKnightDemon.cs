...
namespace Server.Mobiles
{
    public class DeathKnightDemon : BasePerson
    {
        ...
        public override bool OnDragDrop(Mobile from, Item dropped)
        {
            if (
                dropped is Gold
                && dropped.Amount == 10000
                && from.Karma <= -5000
                && from.Skills[SkillName.Knightship].Base >= 100
            )
            {
            ...
            else if (dropped is BaseClothing || dropped is BaseArmor)
            {
                if (
                    ...
                    && dropped.ItemID != 0x2FC5
                    && dropped.ItemID != 0x317B
                )
                {
                    ...
                    if (pack.ConsumeTotal(typeof(Gold), cost))
                    {
                        ...
                        switch (Utility.RandomMinMax(0, 4))
                        {
                            case 0:
                                ...
                                break;
                            case 1:
                                dropped.Name = "daemon wings";
                                break;
                            case 2:
                                dropped.Name = "demonic wings";
                                break;
                            case 3:
                                dropped.Name = "devil wings";
                                break;
                            case 4:
                                dropped.Name = "devlish wings";
                                break;
                        }
                    }
                    else
                    {
                        this.SayTo(
                            from,
                            "It would cost you {0} gold for demon wings you fool!",
                            cost
                        );
                    ...
                    dropped.Layer == Layer.Cloak
                    && (dropped.ItemID == 0x2FC5 || dropped.ItemID == 0x317B)
                )
                {
                    ...
                    if (!mobile.HasGump(typeof(SpeechGump)))
                    {
                        mobile.SendGump(
                            new SpeechGump(
                                mobile,
                                "The Black Heart",
                                SpeechFunctions.SpeechText(m_Giver, m_Mobile, "DeathKnight")
                            )
                        );
                    }
                }
        ...
        public DeathKnightDemon(Serial serial)
            : base(serial) { }
