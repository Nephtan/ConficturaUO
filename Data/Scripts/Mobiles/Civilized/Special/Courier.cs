...
namespace Server.Mobiles
{
    public class Courier : BasePerson
    {
            ...
            : base()
        {
            ...
            {
                case 0:
                    AddItem(new FeatheredHat(Utility.RandomColor(0)));
                    break;
                case 1:
                    AddItem(new FloppyHat(Utility.RandomColor(0)));
                    break;
                case 2:
                    AddItem(new StrawHat(Utility.RandomColor(0)));
                    break;
                case 3:
                    AddItem(new WideBrimHat(Utility.RandomColor(0)));
                    break;
                case 4:
                    AddItem(new TallStrawHat(Utility.RandomColor(0)));
                    break;
            }
                    ...
                    if (!mobile.HasGump(typeof(SpeechGump)))
                    {
                        ...
                        mobile.SendGump(
                            new SpeechGump(
                                mobile,
                                "Message Deliveries",
                                SpeechFunctions.SpeechText(m_Giver, m_Mobile, "Courier")
                            )
                        );
                    }
                }
        ...
        private class CourierEntry : ContextMenuEntry
        {
            ...
            public CourierEntry(Courier Courier, Mobile from)
                : base(2141, 3)
            {
                m_Courier = Courier;
                m_From = from;
                ...
                foreach (Item item in World.Items.Values)
                    if (item is CourierMail)
                    {
                        if (((CourierMail)item).owner == m)
                        {
                            ...
                            SayTo(
                                m,
                                "Hmmm...I already gave you a message from "
                                    + msgQuest
                                    + ". Here is a another if you lost it."
                            );
                        }
                    }
            }
            ...
            if (GiveMail)
            {
                ...
                foreach (Mobile msg in World.Mobiles.Values)
                    if (msg is EpicCharacter && msg.Name != "the Great Earth Serpent")
                    {
                        ...
                        if (
                            ((EpicCharacter)msg).MyAlignment == "evil"
                            && PlayerSettings.GetDiscovered(m, tWorld)
                            && (m.Karma < 0 || ((PlayerMobile)m).KarmaLocked == true)
                        )
                        {
                            npcs.Add(msg);
                            c++;
                            ...
                            ((EpicCharacter)msg).MyAlignment == "good"
                            && PlayerSettings.GetDiscovered(m, tWorld)
                            && m.Karma >= 0
                        )
                        {
                            ...
                            ((EpicCharacter)msg).MyAlignment == "neutral"
                            && PlayerSettings.GetDiscovered(m, tWorld)
                        )
                        {
                            npcs.Add(msg);
                ...
                while (!foundNPC)
                {
                    ...
                    for (int i = 0; i < npcs.Count; ++i)
                    {
                        ...
                        if (i == o)
                        {
                            ...
                            myHomeWorld = Server.Misc.Worlds.GetMyWorld(
                                WhoMap,
                                WhoLoc,
                                dude.MyX,
                                dude.MyY
        ...
        public static void PickSearchLocation(
            CourierMail scroll,
            string DungeonNow,
            Mobile from,
            string alignment,
        ...
        )
        {
            ...
            foreach (Item target in World.Items.Values)
                if (
                    ...
                    && (
                        Worlds.GetDifficultyLevel(target.Location, target.Map)
                        <= GetPlayerInfo.GetPlayerDifficulty(from)
                    )
                )
                {
                    ...
                    else if (PlayerSettings.GetDiscovered(from, tWorld))
                    {
                        targets.Add(target);
                        aCount++;
                    }
            ...
            for (int i = 0; i < targets.Count; ++i)
            {
                ...
                if (xCount == aCount)
                {
                    ...
                    realMap = finding.Map;
                    thisMap = Server.Misc.Worlds.GetMyDefaultMap(thisWorld);
                    thisPlace = Server.Misc.Worlds.GetRegionName(finding.Map, finding.Location);
                    scroll.MsgComplete = 0;
                    scroll.MsgReward =
                        Server.Misc.Worlds.GetDifficultyLevel(finding.Location, finding.Map) + 2;
            ...
            {
                case 1:
                    Word4 = "thousands of years ago";
                    break;
                case 2:
                    Word4 = "decades ago";
                    break;
                case 3:
                    Word4 = "millions of years ago";
                    break;
                case 4:
                    Word4 = "many years ago";
                    break;
            }
            ...
            string heard =
                "I have heard that you could perhaps help me with something of the utmost importance.";
                ...
                switch (Utility.RandomMinMax(0, 5))
                {
                    ...
                    case 1:
                        heard =
                            ...
                            + " has told me about you, and that maybe you can help.";
                        break;
                    case 2:
                        heard =
                            "After speaking to my friend, "
                            + RandomThings.GetRandomName()
                            + ", they mentioned that maybe you can assist me with something.";
                        break;
                    case 3:
                        heard =
                            "I hear that you are one I could trust for this important task ahead.";
                        break;
                    case 4:
                        heard =
                            "The "
                            ...
                            + " mentioned that you could perhaps help me with something.";
                        break;
                    case 5:
                        heard = "There is a dire situation I think you may be able to help with.";
                        break;
                }
            }
            else if (alignment == "evil")
            {
                ...
                {
                    case 0:
                        heard = "I have heard that you are one that can serve me in my purposes.";
                        break;
                    case 1:
                        heard =
                            RandomThings.GetRandomName()
                            + " has told me about you, and that you would serve me well.";
                        break;
                    case 2:
                        heard =
                            "After speaking to my servant, "
                            + RandomThings.GetRandomName()
                            + ", they mentioned that maybe you would do my bidding.";
                        break;
                    case 3:
                        heard =
                            "I hear whispers of your ambitions, and that maybe we can both benefit from what I am about to ask.";
                        break;
                    case 4:
                        heard =
                            "Those in "
                            + RandomThings.GetRandomCity()
                            + " sometimes speak your name in hush curses, which is why I have sent this message to you.";
                        break;
                    case 5:
                        heard =
                            "There is an item I need for my plans, and I think you are one that can obtain it with little notice by others.";
                        break;
                }
            }
            ...
            string EntranceLocation = Worlds.GetAreaEntrance(scroll.SearchDungeon, realMap);

            scroll.SearchMessage =
                intro
                + " "
                + reward
                + " "
            ...
            scroll.SearchMessage =
                scroll.SearchMessage
                + " "
                + scroll.SearchDungeon
                + " "
                + Word4
                + " in "
                + scroll.SearchWorld
                + " at the below sextant coordinates.<br><br>"
                + EntranceLocation;

            scroll.SearchMessage =
                scroll.SearchMessage
                + "<br><br>When you find it, bring this message back to me. I am in "
                + homeworld
                + " at the below sextant coordinates.<br><br>"
                + scroll.ForWhere;
