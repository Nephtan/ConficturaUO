...
namespace Server.Mobiles
{
    public class CartographersGuildmaster : BaseGuildmaster
    {
        ...
        [Constructable]
        public CartographersGuildmaster()
            : base("cartographer")
        {
        ...
        private class FixEntry : ContextMenuEntry
        {
            ...
            public FixEntry(CartographersGuildmaster CartographersGuildmaster, Mobile from)
                : base(6120, 12)
            {
        ...
        private class RepairTarget : Target
        {
            ...
            public RepairTarget(CartographersGuildmaster CartographersGuildmaster)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is TreasureMap && from.Backpack != null)
                {
                    ...
                    {
                        m_CartographersGuildmaster.SayTo(
                            from,
                            "That map has already been deciphered."
                        );
                    }
                    else if (pack.ConsumeTotal(typeof(Gold), toConsume))
                    {
                        ...
                        if (tmap.Level == 1)
                        {
                            m_CartographersGuildmaster.SayTo(
                                from,
                                "This map was really quite simple."
                            );
                        }
                        else if (tmap.Level == 2)
                        {
                            m_CartographersGuildmaster.SayTo(
                                from,
                                "Seemed pretty easy...so here it is."
                            );
                        }
                        else if (tmap.Level == 3)
                        {
                            m_CartographersGuildmaster.SayTo(
                                from,
                                "This map was a bit of a challenge."
                            );
                        }
                        else if (tmap.Level == 4)
                        {
                            m_CartographersGuildmaster.SayTo(
                                from,
                                "Whoever drew this map, did not want it found."
                            );
                        }
                        else if (tmap.Level == 5)
                        {
                            m_CartographersGuildmaster.SayTo(
                                from,
                                "This took more research than normal."
                            );
                        }
                        else
                        {
        ...
        public CartographersGuildmaster(Serial serial)
            : base(serial) { }
