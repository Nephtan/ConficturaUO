...
namespace Server.Mobiles
{
    public class LibrarianGuildmaster : BaseGuildmaster
    {
        ...
        public override bool OnDragDrop(Mobile from, Item dropped)
        {
                    ...
                    if (from.Skills[SkillName.Inscribe].Value >= 30)
                    {
                        if (Server.Misc.Research.AlreadyHasBag(from))
                        {
                            this.PublicOverheadMessage(
                                ...
                                0,
                                false,
                                string.Format("Good luck with your research.")
                            );
                        }
                        else
                        {
            ...
            else if (dropped is Runebook)
            {
                ...
                this.PrivateOverheadMessage(
                    ...
                    1153,
                    false,
                    "I have changed the cover of your book.",
                    from.NetState
                );
            }
                    ...
                    if (!mobile.HasGump(typeof(SpeechGump)))
                    {
                        ...
                        mobile.SendGump(
                            new SpeechGump(
                                ...
                                SpeechFunctions.SpeechText(m_Giver, m_Mobile, "Sage")
                            )
                        );
                    }
                }
        ...
        private class FixEntry : ContextMenuEntry
        {
            ...
            public FixEntry(LibrarianGuildmaster LibrarianGuildmaster, Mobile from)
                : base(6120, 12)
            {
        ...
        public void BeginRepair(Mobile from)
        {
            ...
            if (BeggingPose(from) > 0) // LET US SEE IF THEY ARE BEGGING
            {
                ...
                SayTo(
                    from,
                    "Since you are begging, I will ask for less to identify an item, wands cost "
                        + nCost.ToString()
                        + " gold and artifacts cost "
                        + nCost2.ToString()
                        + " gold. Or maybe identify another type item for "
                        + idCost.ToString()
                        + " gold?"
                );
            }
            else
            {
                SayTo(
                    from,
                    "If you want me to identify an item, wands cost "
                        + nCost.ToString()
                        + " gold and artifacts cost "
                        + nCost2.ToString()
                        + " gold. Or maybe identify another type item for "
        ...
        private class RepairTarget : Target
        {
            ...
            public RepairTarget(LibrarianGuildmaster mage)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (BeggingPose(from) > 0) // LET US SEE IF THEY ARE BEGGING
                {
                    nCost = nCost - (int)((from.Skills[SkillName.Begging].Value * 0.005) * nCost);
                    nCost2 =
                        nCost2 - (int)((from.Skills[SkillName.Begging].Value * 0.005) * nCost2);
                }
                ...
                else if (targeted is ScrollClue)
                {
                    ...
                    else
                    {
                        m_Sage.SayTo(
                            from,
                            "It would cost you {0} gold to have that deciphered.",
                            toConsume
                        );
                        from.SendMessage("You do not have enough gold.");
                    }
        ...
        public LibrarianGuildmaster(Serial serial)
            : base(serial) { }
