...
namespace Server.Mobiles
{
    public class AssassinGuildmaster : BaseGuildmaster
    {
        ...
        public override void InitOutfit()
        {
            ...
            {
                case 0:
                    AddItem(new Server.Items.ClothCowl(2411));
                    break;
                case 1:
                    AddItem(new Server.Items.ClothHood(2411));
                    break;
                case 2:
                    AddItem(new Server.Items.FancyHood(2411));
                    break;
                case 3:
                    AddItem(new Server.Items.HoodedMantle(2411));
                    break;
            }
                ...
                PlayerMobile mobile = (PlayerMobile)m_Mobile;
                {
                    ...
                    mobile.SendGump(
                        new SpeechGump(
                            mobile,
                            "Death and Taxes",
                            SpeechFunctions.SpeechText(m_Giver, m_Mobile, "Assassin")
                        )
                    );
                }
            }
        ...
        private class HireEntry : ContextMenuEntry
        {
            ...
            public HireEntry(Mobile from, AssassinGuildmaster giver)
                : base(6120, 12)
            {
                m_From = from;
                m_Giver = giver;
        ...
        public void Bribery(Mobile from)
        {
            ...
            if (pm.Profession == 1)
            {
                SayTo(
                    from,
                    "You are a bit too famous in the land to pursuade the guards to forget your crimes."
                );
                paid = true;
            }
            ...
            else if (packs.ConsumeTotal(typeof(Gold), cost))
            {
                SayTo(
                    ...
                    "I will use your "
                        + cost.ToString()
                        + " gold to pursuade the guards to look the other way."
                );
            ...
            else
            {
                ...
                if (cont != null && cont.ConsumeTotal(typeof(Gold), cost))
                {
                    SayTo(
                        ...
                        "I will use  "
                            + cost.ToString()
                            + " gold from your bank box to pursuade the guards to look the other way."
                    );
        ...
        public AssassinGuildmaster(Serial serial)
            : base(serial) { }
