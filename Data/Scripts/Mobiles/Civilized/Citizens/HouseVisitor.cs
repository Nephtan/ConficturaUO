...
namespace Server.Mobiles
{
    public class HouseVisitor : Citizens
    {
        ...
        public DateTime NextChat
        {
            get { return m_NextChat; }
            set { m_NextChat = value; }
        }
        ...
        public HouseVisitor(Serial serial)
            : base(serial) { }
        ...
        {
            if (
                DateTime.Now >= m_NextChat
                && (
                    Server.Items.TavernTable.isShantyVisitor(this)
                    || Server.Items.TavernTable.isLawnVisitor(this)
                    || Server.Items.TavernTable.CountPatrons(this) > 1
                )
            )
            {
