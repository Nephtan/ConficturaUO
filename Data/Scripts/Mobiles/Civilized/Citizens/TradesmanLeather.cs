...
namespace Server.Mobiles
{
    public class TradesmanLeather : Citizens
    {
        ...
        public TradesmanLeather(Serial serial)
            : base(serial) { }
...
namespace Server.Items
{
    ...
    public class LeatherHit : Item
    {
        ...
        public LeatherHit(Serial serial)
            : base(serial) { }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (from is Citizens)
            {
                ...
                {
                    ItemID = Utility.RandomList(0x1069, 0x106A);
                }
                else
                {
                    ItemID = Utility.RandomList(0x107A, 0x107B);
                }
...
namespace Server.Items
{
    public class CrateOfLeather : Item
    {
        ...
        public CrateOfLeather(Serial serial)
            : base(serial) { }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else
            {
                ...
                if (CrateItem == "deep sea")
                {
                    from.AddToBackpack(new SpinedLeather(CrateQty));
                }
                else if (CrateItem == "lizard")
                {
                    from.AddToBackpack(new HornedLeather(CrateQty));
                }
                else if (CrateItem == "serpent")
                {
                    from.AddToBackpack(new BarbedLeather(CrateQty));
                }
                else if (CrateItem == "necrotic")
                {
                    from.AddToBackpack(new NecroticLeather(CrateQty));
                }
                else if (CrateItem == "volcanic")
                {
                    from.AddToBackpack(new VolcanicLeather(CrateQty));
                }
                else if (CrateItem == "frozen")
                {
                    from.AddToBackpack(new FrozenLeather(CrateQty));
                }
                else if (CrateItem == "goliath")
                {
                    from.AddToBackpack(new GoliathLeather(CrateQty));
                }
                else if (CrateItem == "draconic")
                {
                    from.AddToBackpack(new DraconicLeather(CrateQty));
                }
                else if (CrateItem == "hellish")
                {
                    from.AddToBackpack(new HellishLeather(CrateQty));
                }
                else if (CrateItem == "dinosaur")
                {
                    from.AddToBackpack(new DinosaurLeather(CrateQty));
                }
                else if (CrateItem == "alien")
                {
                    from.AddToBackpack(new AlienLeather(CrateQty));
                }
                else
                {
                    from.AddToBackpack(new Leather(CrateQty));
                }
                ...
                from.PrivateOverheadMessage(
                    ...
                    0x14C,
                    false,
                    "You separate the leather into your backpack",
                    from.NetState
                );
                this.Delete();
            }
