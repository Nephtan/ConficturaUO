...
namespace Server.Mobiles
{
    public class TradesmanSmith : Citizens
    {
        ...
        public TradesmanSmith(Serial serial)
            : base(serial) { }
    ...
    public class AnvilHit : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (
                ...
                && from.FindItemOnLayer(Layer.OneHanded) is BaseWeapon
                && from is Citizens
            )
            {
...
namespace Server.Items
{
    public class CrateOfMetal : Item
    {
        ...
        public CrateOfMetal(Serial serial)
            : base(serial) { }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else
            {
                ...
                if (CrateItem == "dull copper")
                {
                    from.AddToBackpack(new DullCopperIngot(CrateQty));
                }
                else if (CrateItem == "shadow iron")
                {
                    from.AddToBackpack(new ShadowIronIngot(CrateQty));
                }
                else if (CrateItem == "copper")
                {
                    from.AddToBackpack(new CopperIngot(CrateQty));
                }
                else if (CrateItem == "bronze")
                {
                    from.AddToBackpack(new BronzeIngot(CrateQty));
                }
                else if (CrateItem == "gold")
                {
                    from.AddToBackpack(new GoldIngot(CrateQty));
                }
                else if (CrateItem == "agapite")
                {
                    from.AddToBackpack(new AgapiteIngot(CrateQty));
                }
                else if (CrateItem == "verite")
                {
                    from.AddToBackpack(new VeriteIngot(CrateQty));
                }
                else if (CrateItem == "valorite")
                {
                    from.AddToBackpack(new ValoriteIngot(CrateQty));
                }
                else if (CrateItem == "nepturite")
                {
                    from.AddToBackpack(new NepturiteIngot(CrateQty));
                }
                else if (CrateItem == "obsidian")
                {
                    from.AddToBackpack(new ObsidianIngot(CrateQty));
                }
                else if (CrateItem == "steel")
                {
                    from.AddToBackpack(new SteelIngot(CrateQty));
                }
                else if (CrateItem == "brass")
                {
                    from.AddToBackpack(new BrassIngot(CrateQty));
                }
                else if (CrateItem == "mithril")
                {
                    from.AddToBackpack(new MithrilIngot(CrateQty));
                }
                else if (CrateItem == "xormite")
                {
                    from.AddToBackpack(new XormiteIngot(CrateQty));
                }
                else if (CrateItem == "dwarven")
                {
                    from.AddToBackpack(new DwarvenIngot(CrateQty));
                }
                else
                {
                    from.AddToBackpack(new IronIngot(CrateQty));
                }
                ...
                from.PrivateOverheadMessage(
                    ...
                    0x14C,
                    false,
                    "You separate the ingots into your backpack",
                    from.NetState
                );
                this.Delete();
            }
