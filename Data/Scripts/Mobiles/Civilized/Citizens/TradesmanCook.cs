...
namespace Server.Mobiles
{
    public class TradesmanCook : Citizens
    {
        ...
        public TradesmanCook(Serial serial)
            : base(serial) { }
    ...
    public class StoveHit : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (from is Citizens)
            {
                ...
                else if (ItemID == 0x568C)
                {
                    ItemID = 0x568B;
                    from.PlaySound(Utility.RandomList(0x5CF, 0x5CA, 0x345));
                }
                else if (ItemID == 0x568D)
                {
                    ItemID = 0x568E;
                    from.PlaySound(Utility.RandomList(0x5CF, 0x5CA, 0x345));
                }
                else if (ItemID == 0x568E)
                {
                    ItemID = Utility.RandomList(0x568C, 0x568D);
                    from.PlaySound(Utility.RandomList(0x059, 0x057));
                }
...
namespace Server.Items
{
    public class CrateOfFood : Item
    {
        ...
        public CrateOfFood(Serial serial)
            : base(serial) { }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else
            {
                ...
                if (ItemID == 0x508B)
                {
                    from.AddToBackpack(new FishSteak(CrateQty));
                }
                else if (ItemID == 0x508C)
                {
                    from.AddToBackpack(new LambLeg(CrateQty));
                }
                else if (ItemID == 0x508D)
                {
                    from.AddToBackpack(new Ribs(CrateQty));
                }
                else if (ItemID == 0x50BA)
                {
                    from.AddToBackpack(new BreadLoaf(CrateQty));
                }
                ...
                from.PrivateOverheadMessage(
                    ...
                    0x14C,
                    false,
                    "You separate the food into your backpack",
                    from.NetState
                );
                this.Delete();
            }
