...
namespace Server.Mobiles
{
    public class TradesmanButcher : Citizens
    {
        ...
        public TradesmanButcher(Serial serial)
            : base(serial) { }
...
namespace Server.Items
{
    public class ButcherHit : Item
    {
            ...
            : base(0x13)
        {
            ...
            if (ItemID == 0x63D8 || ItemID == 0x63D9)
            {
                Hue = Utility.RandomList(0x7D1, 0x7D2, 0x7D3, 0x7D4, 0x7D5, 0x7D6);
            }
        }

        public ButcherHit(Serial serial)
            : base(serial) { }
...
namespace Server.Items
{
    public class CrateOfMeats : Item
    {
        ...
        public CrateOfMeats(Serial serial)
            : base(serial) { }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else
            {
                ...
                if (ItemID == 0x508B)
                {
                    from.AddToBackpack(new RawFishSteak(CrateQty));
                }
                else if (ItemID == 0x508C)
                {
                    from.AddToBackpack(new RawLambLeg(CrateQty));
                }
                else if (ItemID == 0x508D)
                {
                    from.AddToBackpack(new RawRibs(CrateQty));
                }
                ...
                from.PrivateOverheadMessage(
                    ...
                    0x14C,
                    false,
                    "You separate the meat into your backpack",
                    from.NetState
                );
                this.Delete();
            }
