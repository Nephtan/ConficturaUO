...
namespace Server.Items
{
    public class PackBeastItem : Item
    {
        ...
        public int Porter_Serial
        {
            get { return PorterSerial; }
            set
            {
                PorterSerial = value;
        ...
        [CommandProperty(AccessLevel.Owner)]
        public int Porter_Owner
        {
            get { return PorterOwner; }
            set
            {
                PorterOwner = value;
                InvalidateProperties();
        ...
        [CommandProperty(AccessLevel.Owner)]
        public int Porter_Type
        {
            get { return PorterType; }
            set
            {
                PorterType = value;
                InvalidateProperties();
        ...
        [CommandProperty(AccessLevel.Owner)]
        public string Porter_Name
        {
            get { return PorterName; }
            set
            {
                PorterName = value;
                InvalidateProperties();
            ...
            : base(0x2126)
        {
            ...
            else if (PorterType == 23)
            {
                ItemID = 0x20DB;
            }
            else if (PorterType == 177)
            {
                ItemID = 0x20CF;
            }
            else if (PorterType == 179)
            {
                ItemID = 0x20E1;
            }
            else if (PorterType == 291)
            {
                ItemID = 0x2126;
            }
        }

        public PackBeastItem(Serial serial)
            : base(serial) { }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.SendMessage(
                    "You need to be near a druid or animal handler to call this pack animal!"
                );
            }
            else
            {
                ...
                {
                    from.SendMessage(
                        "Your pack animal needs to be tended to by a druid guildmaster."
                    );
                }
                ...
                else
                {
                    ...
                    if (friend.Body == 213)
                    {
                        friend.BaseSoundID = 0xA3;
                    }
                    else if (friend.Body == 292)
                    {
                        friend.BaseSoundID = 0x3F3;
                    }
                    else if (friend.Body == 291)
                    {
                        friend.BaseSoundID = 0xA8;
                    }
                    ...
                    if (PorterName != null)
                    {
                        friend.Name = PorterName;
                    }
                    else
                    {
                        friend.Name = "a pack animal";
                    }
        ...
        public override void AddNameProperties(ObjectPropertyList list)
        {
            ...
            {
                sType = "a pack llama";
                if (PorterName != "a pack animal")
                {
                    sType = PorterName + " the pack llama";
                }
            }
            else if (PorterType == 23)
            {
                ...
                if (PorterName != "a pack animal")
                {
                    sType = PorterName + " the pack bear";
                }
            }
            else if (PorterType == 177)
            {
            ...
            else if (PorterType == 291)
            {
                ...
                if (PorterName != "a pack animal")
                {
                    sType = PorterName + " the pack horse";
                }
            }
