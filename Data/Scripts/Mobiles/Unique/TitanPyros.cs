...
namespace Server.Mobiles
{
    ...
    public class TitanPyros : BaseCreature
    {
            ...
            else if (CanKillIt == 0)
            {
                ...
                if (winner is PlayerMobile)
                {
                    LoggingFunctions.LogGenericQuest(
                        winner,
                        "has obtained the power of the fire titan"
                    );
                }
                ...
                if (winner != null)
                {
                    ...
                    if (winner is PlayerMobile && !winner.Blessed)
                    {
                        ...
                        if (p != null)
                        {
                            ...
                            winner.StatCap < 300
                            && !Server.Misc.PlayerSettings.GetSpecialsKilled(winner, "TitanPyros")
                        )
                        {
                            Server.Misc.PlayerSettings.SetSpecialsKilled(
                                winner,
                ...
                if (GetPlayerInfo.LuckyKiller(winner.Luck) && Utility.RandomMinMax(1, 10) == 1)
                {
                    ...
                    switch (Utility.RandomMinMax(0, 3))
                    {
                        case 1:
                            Arty.Delete();
                            Arty = new Artifact_MantleofPyros();
                            break;
                        case 2:
                            Arty.Delete();
                            Arty = new Artifact_RobeofPyros();
                            break;
                        case 3:
                            Arty.Delete();
                            Arty = new Artifact_PyrosGrimoire();
                            break;
                    }
                    AddItem(Arty);
                }
        ...
        public TitanPyros(Serial serial)
            : base(serial) { }
