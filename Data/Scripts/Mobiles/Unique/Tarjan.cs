...
namespace Server.Mobiles
{
    ...
    public class Tarjan : BaseCreature
    {
        ...
        public class TarjanTimer : Timer
        {
            ...
            public TarjanTimer(Mobile from)
...
namespace Server.Items
{
    public class TarjanBox : Item
    {
        ...
        public TarjanBox(Serial serial)
            : base(serial) { }

        public override void OnDoubleClick(Mobile from)
        {
            if (
                PlayerSettings.GetBardsTaleQuest(from, "BardsTaleKylearanKey")
                && PlayerSettings.GetBardsTaleQuest(from, "BardsTaleSpectreEye")
            )
            {
            ...
            else
            {
                ...
                from.PrivateOverheadMessage(
                    ...
                    1150,
                    false,
                    "You found a key with a symbol of a unicorn on it.",
                    from.NetState
                );
                ...
                from.SendGump(
                    new Server.Gumps.ClueGump(
                        from,
                        "You found a key with a symbol of a unicorn on it, along with a magical book of items.",
                        "Tarjan's Death"
                    )
                );
...
namespace Server.Items
{
    public class TarjanStatue : Item
    {
        ...
        public TarjanStatue(Serial serial)
            : base(serial) { }
            ...
            if (from.InRange(this.GetWorldLocation(), 2))
            {
                ...
                if (PlayerSettings.GetBardsTaleQuest(from, "BardsTaleKylearanKey"))
                {
                    from.PrivateOverheadMessage(
                        ...
                        1150,
                        false,
                        "This statue still has the eye you placed in it.",
                        from.NetState
                    );
                }
                else if (
                ...
                )
                {
                    from.PrivateOverheadMessage(
                        ...
                        1150,
                        false,
                        "You place the mysterious eye into the statue.",
                        from.NetState
                    );
                    SpawnTarjan(from);
                    this.Delete();
                ...
                else
                {
                    from.PrivateOverheadMessage(
                        ...
                        1150,
                        false,
                        "This statue seems to be missing an eye.",
                        from.NetState
                    );
                }
            }
