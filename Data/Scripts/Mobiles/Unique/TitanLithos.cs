...
namespace Server.Mobiles
{
    ...
    public class TitanLithos : BaseCreature
    {
        ...
        {
            base.BreathDealDamage(target, 41);
        }
            ...
            else if (CanKillIt == 0)
            {
                ...
                if (winner is PlayerMobile)
                {
                    LoggingFunctions.LogGenericQuest(
                        winner,
                        "has obtained the power of the earth titan"
                    );
                }
                        ...
                        if (p != null)
                        {
                            foreach (PartyMemberInfo pmi in p.Members)
                            {
                                if (
                                    ...
                                    && !pmi.Mobile.Blessed
                                    && pmi.Mobile.StatCap < 300
                                    && !Server.Misc.PlayerSettings.GetSpecialsKilled(
                                        pmi.Mobile,
                                        "TitanLithos"
                                    )
                                )
                                {
                                    Server.Misc.PlayerSettings.SetSpecialsKilled(
                                        pmi.Mobile,
                                        "TitanLithos",
                                        true
                                    );
                            ...
                            winner.StatCap < 300
                            && !Server.Misc.PlayerSettings.GetSpecialsKilled(winner, "TitanLithos")
                        )
                        {
                            Server.Misc.PlayerSettings.SetSpecialsKilled(
                                winner,
                ...
                if (GetPlayerInfo.LuckyKiller(winner.Luck) && Utility.RandomMinMax(1, 10) == 1)
                {
                    ...
                    switch (Utility.RandomMinMax(0, 3))
                    {
                        case 1:
                            Arty.Delete();
                            Arty = new Artifact_MantleofLithos();
                            break;
                        case 2:
                            Arty.Delete();
                            Arty = new Artifact_RobeofLithos();
                            break;
                        case 3:
                            Arty.Delete();
                            Arty = new Artifact_LithosTome();
                            break;
                    }
                    AddItem(Arty);
                }
        ...
        public TitanLithos(Serial serial)
            : base(serial) { }
