    ...
    public class EtherealMount : Item, IMount, IMountItem, Engines.VeteranRewards.IRewardItem
    {
        ...
        public virtual bool Validate(Mobile from)
        {
            if (Parent == null)
            {
                ...
                (this is PaladinWarhorse)
                && (from.Skills[SkillName.Knightship].Base < 100 || from.Karma < 0)
            )
            {
            ...
            {
                from.SendMessage(
                    "Only a grandmaster in necromancy and magery may ride this evil being."
                );
                return false;
            }
            else if (
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            if (
                ...
                && Server.Mobiles.AnimalTrainer.IsNoMountRegion(from, from.Region)
            )
            {
                ...
                (
                    ...
                    && Server.Misc.Worlds.InBuilding(from)
                ) || (from.Region is HouseRegion && MyServerSettings.NoMountsInHouses())
            )
            {
        ...
        private class EtherealSpell : Spell
        {
            ...
            public EtherealSpell(EtherealMount mount, Mobile rider, Mobile owner)
            ...
            public override void OnDisturb(DisturbType type, bool message)
            {
                if (message && !m_Stop)
                    Caster.SendMessage(
                        "You have been disrupted while attempting to summon your mount!"
                    );
            }
