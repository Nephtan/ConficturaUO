...
namespace Server.Regions
{
    ...
    {
        public ProtectedRegion(XmlElement xml, Map map, Region parent)
            : base(xml, map, parent) { }
            ...
            if (this.Name == "the Chamber of the Codex")
            {
                ...
                for (int i = 0; i < targets.Count; ++i)
                {
                    ...
                    if (item is CodexWisdom)
                    {
                        ...
                        if (
                            (
                                codex.SkillFirst == codex.PreviousFirst
                                || codex.SkillFirst == codex.PreviousSecond
                            )
                            && (
                                codex.SkillSecond == codex.PreviousFirst
                                || codex.SkillSecond == codex.PreviousSecond
                            )
                        )
                            DestroyLense = false;
