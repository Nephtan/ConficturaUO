    ...
    public class HouseRegion : BaseRegion
    {
        ...
        public override bool AllowHarmful(Mobile from, Mobile target)
        {
            if (
                ...
                && (
                    m_House.IsOwner(target) || m_House.IsCoOwner(target) || m_House.IsFriend(target)
                )
            )
                return true;
        ...
        public override void OnLocationChanged(Mobile m, Point3D oldLocation)
        {
            ...
            if (m is BaseCreature && ((BaseCreature)m).NoHouseRestrictions) { }
                ...
                m is BaseCreature
                && ((BaseCreature)m).IsHouseSummonable
                && !(BaseCreature.Summoning || m_House.IsInside(oldLocation, 16))
            ) { }
            else if (
                (m_House.Public || !m_House.IsAosRules)
                && m_House.IsBanned(m)
                && m_House.IsInside(m)
            )
            {
            ...
            )
            {
                ...
                m_House.IsCombatRestricted(m)
                && m_House.IsInside(m)
                && !m_House.IsInside(oldLocation, 16)
            )
            {
                m.Location = m_House.BanLocation;
        ...
        public override bool OnMoveInto(
            Mobile from,
            Direction d,
            Point3D newLocation,
            Point3D oldLocation
        )
        {
            ...
            else if (from is BaseCreature && !((BaseCreature)from).Controlled) // Untamed creatures cannot enter public houses
            {
                ...
                && ((BaseCreature)from).IsHouseSummonable
                && !(BaseCreature.Summoning || m_House.IsInside(oldLocation, 16))
            )
            {
                return false;
            ...
            )
            {
                ...
                (m_House.Public || !m_House.IsAosRules)
                && m_House.IsBanned(from)
                && m_House.IsInside(newLocation, 16)
            )
            {
            ...
            )
            {
                ...
                m_House.IsCombatRestricted(from)
                && !m_House.IsInside(oldLocation, 16)
                && m_House.IsInside(newLocation, 16)
            )
            {
            ...
            if (
                m_House.InternalizedVendors.Count > 0
                && m_House.IsInside(from)
                && !m_House.IsInside(oldLocation, 16)
                && m_House.IsOwner(from)
                && from.Alive
                && !from.HasGump(typeof(NoticeGump))
            )
            {
                /* This house has been customized recently, and vendors that work out of this
                 * house have been temporarily relocated.  You must now put your vendors back to work.
        ...
        {
            if (Server.Misc.MyServerSettings.HouseStorage())
                return false;
            else if (
                (m_House.IsLockedDown(item) || m_House.IsSecure(item)) && m_House.IsInside(item)
            )
                return false;
            else
