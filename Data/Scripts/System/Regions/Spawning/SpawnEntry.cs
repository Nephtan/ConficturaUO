...
namespace Server.Regions
{
    public class SpawnEntry : ISpawner
    {
        ...
        {
            get { return m_SpawnedObjects.Count >= m_Max; }
        }
        public bool Spawning
        {
            get { return m_Running && !this.Complete; }
        }
        ...
        public SpawnEntry(
            ...
            int max,
            TimeSpan minSpawnTime,
            TimeSpan maxSpawnTime
        )
        {
            m_ID = id;
            m_Region = region;
        ...
        private void InternalDeleteSpawnedObjects()
        {
            foreach (ISpawnable spawnable in m_SpawnedObjects)
            {
                ...
                bool uncontrolled =
                    !(spawnable is BaseCreature) || !((BaseCreature)spawnable).Controlled;
        ...
        public static void Initialize()
        {
            ...
            CommandSystem.Register(
                ...
                new CommandEventHandler(RespawnRegion_OnCommand)
            );
            CommandSystem.Register(
                "DelAllRegionSpawns",
                AccessLevel.Administrator,
                new CommandEventHandler(DelAllRegionSpawns_OnCommand)
            );
            CommandSystem.Register(
                "DelRegionSpawns",
                AccessLevel.GameMaster,
                new CommandEventHandler(DelRegionSpawns_OnCommand)
            );
            CommandSystem.Register(
            ...
            CommandSystem.Register(
                "StartRegionSpawns",
                AccessLevel.GameMaster,
                new CommandEventHandler(StartRegionSpawns_OnCommand)
            );
            CommandSystem.Register(
                "StopAllRegionSpawns",
                AccessLevel.Administrator,
                new CommandEventHandler(StopAllRegionSpawns_OnCommand)
            );
            CommandSystem.Register(
                "StopRegionSpawns",
                AccessLevel.GameMaster,
                new CommandEventHandler(StopRegionSpawns_OnCommand)
            );
        }
