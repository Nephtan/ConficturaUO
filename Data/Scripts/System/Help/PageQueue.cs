    ...
    public class PageQueue
    {
        ...
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(Pages_OnCommand)
            );
        }
        ...
        public static void Enqueue(PageEntry entry)
        {
            ...
            foreach (NetState ns in NetState.Instances)
            {
                ...
                if (
                    ...
                    && m.AccessLevel >= AccessLevel.Counselor
                    && m.AutoPageNotify
                    && m.LastMoveTime >= (DateTime.Now - TimeSpan.FromMinutes(10.0))
                )
                    isStaffOnline = true;
            }
        ...
        private static void SendEmail(PageEntry entry)
        {
            ...
            {
                writer.WriteLine(
                    "RunUO Speech Log Page - {0}",
                    PageQueue.GetPageTypeName(entry.Type)
                );
                writer.WriteLine();
