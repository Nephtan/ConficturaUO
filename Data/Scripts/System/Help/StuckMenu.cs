...
namespace Server.Menus.Questions
{
    ...
    public class StuckMenu : Gump
    {
        ...
        {
            new StuckMenuEntry("Sosaria", new Point3D[] { new Point3D(3213, 3673, 0) })
        };
        private static StuckMenuEntry[] m_LunaEntries = new StuckMenuEntry[]
        {
            new StuckMenuEntry("Luna", new Point3D[] { new Point3D(5884, 2864, 0) })
        };
        private static StuckMenuEntry[] m_AmbrosiaEntries = new StuckMenuEntry[]
        {
            new StuckMenuEntry("Ambrosia", new Point3D[] { new Point3D(3325, 3934, 0) })
        };
        private static StuckMenuEntry[] m_UmberEntries = new StuckMenuEntry[]
        {
            new StuckMenuEntry("Umber Veil", new Point3D[] { new Point3D(2982, 3696, 0) })
        };
        private static StuckMenuEntry[] m_SerpentEntries = new StuckMenuEntry[]
        {
            new StuckMenuEntry("Serpent Island", new Point3D[] { new Point3D(2191, 315, 0) })
        };
        private static StuckMenuEntry[] m_LodorEntries = new StuckMenuEntry[]
        {
            new StuckMenuEntry("Lodoria", new Point3D[] { new Point3D(5727, 3467, 0) })
        };
        private static StuckMenuEntry[] m_DreadEntries = new StuckMenuEntry[]
        {
            new StuckMenuEntry("Isles of Dread", new Point3D[] { new Point3D(237, 324, 3) })
        };
        private static StuckMenuEntry[] m_SavageEntries = new StuckMenuEntry[]
        {
            new StuckMenuEntry("Savaged Empire", new Point3D[] { new Point3D(1110, 2541, -1) })
        };
        private static StuckMenuEntry[] m_KuldarEntries = new StuckMenuEntry[]
        {
            new StuckMenuEntry("Bottle World", new Point3D[] { new Point3D(6714, 705, 0) })
        };
        private static StuckMenuEntry[] m_BardEntries = new StuckMenuEntry[]
        {
            new StuckMenuEntry("Skara Brae", new Point3D[] { new Point3D(7041, 213, 0) })
        };
        private static StuckMenuEntry[] m_UnderworldEntries = new StuckMenuEntry[]
        {
            new StuckMenuEntry("Underworld", new Point3D[] { new Point3D(755, 393, 0) })
        };
            ...
            : base(50, 50)
        {
            ...
            string myWorld = Worlds.GetMyWorld(
                m_Mobile.Map,
                m_Mobile.Location,
                m_Mobile.X,
                m_Mobile.Y
            ...
            {
                entries = m_UnderworldEntries;
                m_Map = Map.Underworld;
            }
            else if (myWorld == "the Serpent Island")
            {
                entries = m_SerpentEntries;
                m_Map = Map.SerpentIsland;
            }
            else if (myWorld == "the Isles of Dread")
            {
                entries = m_DreadEntries;
                m_Map = Map.IslesDread;
            }
            else if (myWorld == "the Savaged Empire")
            {
                entries = m_SavageEntries;
                m_Map = Map.SavagedEmpire;
            }
            else if (myWorld == "the World of Atlantis")
            {
                entries = m_Entries;
                m_Map = Map.Sosaria;
            }
            ...
            AddHtml(
                ...
                245,
                20,
                @"<BODY><BASEFONT Color=" + color + ">STUCK IN THE WORLD</BASEFONT></BODY>",
                (bool)false,
                (bool)false
            );
        ...
        private void Teleport(StuckMenuEntry entry)
        {
            if (m_MarkUse)
            {
                ...
                new TeleportTimer(
                    m_Mobile,
                    entry,
                    TimeSpan.FromSeconds(10.0 + (Utility.RandomDouble() * 110.0)),
                    m_Map
                ).Start();
