    ...
    public class VisibilityList
    {
        public static void Initialize()
        {
            ...
            CommandSystem.Register(
                ...
                new CommandEventHandler(Vis_OnCommand)
            );
            CommandSystem.Register(
                "VisList",
                AccessLevel.Counselor,
                new CommandEventHandler(VisList_OnCommand)
            );
            CommandSystem.Register(
                "VisClear",
                AccessLevel.Counselor,
                new CommandEventHandler(VisClear_OnCommand)
            );
        }
                ...
                if (from is PlayerMobile && targeted is Mobile)
                {
                    ...
                    if (targ.AccessLevel <= from.AccessLevel)
                    {
                        ...
                        if (list.Contains(targ))
                        {
                            list.Remove(targ);
                            from.SendMessage(
                                "{0} has been removed from your visibility list.",
                                targ.Name
                            );
                        }
                        else
                        {
                            list.Add(targ);
                            from.SendMessage(
                                "{0} has been added to your visibility list.",
                                targ.Name
                            );
                        }
