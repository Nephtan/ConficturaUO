    ...
    public sealed class ObjectConditional
    {
        ...
        public bool IsMobile
        {
            get
            {
                return (
                    m_ObjectType == null
                    || m_ObjectType == typeofMobile
                    || m_ObjectType.IsSubclassOf(typeofMobile)
                );
            }
        }
        ...
        public void Compile(ref AssemblyEmitter emitter)
        {
            ...
            for (int i = 0; i < m_Conditionals.Length; ++i)
                m_Conditionals[i] = ConditionalCompiler.Compile(
                    emitter,
                    m_ObjectType,
                    m_Conditions[i],
                    i
        ...
        )
        {
            ...
            if (objectType == null)
                throw new Exception(
                    String.Format("No type with that name ({0}) was found.", args[offset + index])
                );
            ...
            while (index < size)
            {
                ...
                if (condition == null)
                    throw new InvalidOperationException(
                        String.Format("Unrecognized operator (\"{0}\").", oper)
                    );
