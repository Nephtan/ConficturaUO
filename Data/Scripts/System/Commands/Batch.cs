    ...
    public class Batch : BaseCommand
    {
        ...
        public override void ExecuteList(CommandEventArgs e, ArrayList list)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < m_BatchCommands.Count; ++i)
                {
                    ...
                    {
                        e.Mobile.SendMessage(
                            "You do not have access to that command: {0}.",
                            commandString
                        );
                        return;
                    }
                ...
                for (int i = 0; i < commands.Length; ++i)
                {
                    ...
                    else
                    {
                        ...
                        for (int j = 0; j < list.Count; ++j)
                        {
                            ...
                            PropertyInfo endProp = Properties.GetPropertyInfo(
                                ref obj,
                                chain,
                                ref failReason
                            );
        ...
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(Batch_OnCommand)
            );
        }
        ...
        public void Render()
        {
            ...
            for (int i = 0; i < m_Batch.BatchCommands.Count; ++i)
            {
                ...
                AddImageTiled(
                    ...
                    CurrentY + 2,
                    2,
                    EntryHeight + OffsetSize + EntryHeight - 4,
                    0x24A8
                );
                AddImageTiled(
                    CurrentX,
                    CurrentY + EntryHeight + OffsetSize + EntryHeight - 2,
                    9,
                    2,
                    0x24A8
                );
                AddImageTiled(
                    ...
                    CurrentY + 3,
                    6,
                    EntryHeight + EntryHeight - 4 - OffsetSize,
                    HeaderGumpID
                );
            ...
            if (SplitButtonID(info.ButtonID, 1, out type, out index))
            {
                switch (type)
                {
                    case 0:
                    {
                        ...
                        {
                            BaseCommandImplementor impl = BaseCommandImplementor.Implementors[
                                index
                            ];
