    ...
    public class OpenBrowserCommand : BaseCommand
    {
        ...
        public static void OpenBrowser_Callback(Mobile from, bool okay, object state)
        {
            ...
            else
            {
                ...
                gm.SendMessage(
                    "{0} : has chosen not to open their web browser to : {1}",
                    from.Name,
                    url
                );
            }
                    ...
                    else
                    {
                        ...
                        mob.SendGump(
                            new WarningGump(
                                ...
                                320,
                                240,
                                new WarningGumpCallback(OpenBrowser_Callback),
                                new object[] { from, url }
                            )
                        );
                    }
        ...
        public override bool ValidateArgs(BaseCommandImplementor impl, CommandEventArgs e)
        {
            if (e.Length >= 1)
            {
                ...
                if (t == null)
                {
                    ...
                    {
                        e.Mobile.SendGump(
                            new AddGump(e.Mobile, match, 0, AddGump.Match(match).ToArray(), true)
                        );
                    }
                }
                else
    ...
    public class SetCommand : BaseCommand
    {
        ...
        public override void Execute(CommandEventArgs e, object obj)
        {
            if (e.Length >= 2)
            {
                ...
                {
                    string result = Properties.SetValue(
                        e.Mobile,
                        obj,
                        e.GetString(i),
                        e.GetString(i + 1)
            ...
            if (list.Count > 1)
            {
                e.Mobile.SendGump(
                    new WarningGump(
                        ...
                        420,
                        280,
                        new WarningGumpCallback(OnConfirmCallback),
                        new object[] { e, list }
                    )
                );
            ...
            if (from.AccessLevel > targ.AccessLevel)
            {
                ...
                if (fromState != null && targState != null)
                {
                    ...
                    if (fromAccount != null && targAccount != null)
                    {
                        ...
                        AddResponse(
                            String.Format("They have been {0}.", m_Ban ? "banned" : "kicked")
                        );
