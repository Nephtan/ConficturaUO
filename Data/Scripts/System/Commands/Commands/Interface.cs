...
namespace Server.Commands.Generic
{
    ...
    public class InterfaceGump : BaseGridGump
    {
        ...
        public void Render()
        {
            ...
            AddEntryHtml(
                ...
                Center(
                    String.Format(
                        "Page {0} of {1}",
                        m_Page + 1,
                        (m_List.Count + EntriesPerPage - 1) / EntriesPerPage
                    )
                )
            );
                ...
                for (int i = 0; i < m_Columns.Length; ++i)
                {
                    if (i > 0 && m_List.Count > 0)
                    {
                        ...
                        if (obj != null)
                        {
                            ...
                            PropertyInfo[] chain = Properties.GetPropertyInfoChain(
                                m_From,
                                obj.GetType(),
                                m_Columns[i],
                                PropertyAccess.Read,
            ...
            for (
                int i = m_Page * EntriesPerPage, line = 0;
                line < EntriesPerPage && i < m_List.Count;
                ++i, ++line
            )
            {
                AddNewLine();
                ...
                else
                {
                    for (int j = 1; j < m_Columns.Length; ++j)
                    {
                        ...
                        {
                            PropertyInfo p = Properties.GetPropertyInfo(
                                ref src,
                                chain,
                                ref failReason
                            );
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            switch (info.ButtonID)
            {
                case 1:
                {
                    if (m_Page > 0)
                        m_From.SendGump(
                            new InterfaceGump(m_From, m_Columns, m_List, m_Page - 1, m_Select)
                        );
                ...
                default:
                {
                    ...
                    if (v >= 0 && v < m_List.Count)
                    {
                        ...
                        if (!BaseCommand.IsAccessible(m_From, obj))
                        {
                            ...
                            m_From.SendGump(
                                new InterfaceGump(m_From, m_Columns, m_List, m_Page, m_Select)
                            );
                            break;
                        }
                        ...
                        if (obj is Item && !((Item)obj).Deleted)
                            m_From.SendGump(
                                new InterfaceItemGump(m_From, m_Columns, m_List, m_Page, (Item)obj)
                            );
                        else if (obj is Mobile && !((Mobile)obj).Deleted)
                            m_From.SendGump(
                                new InterfaceMobileGump(
                                    m_From,
                                    m_Columns,
                                    m_List,
                                    m_Page,
    ...
    public class InterfaceItemGump : BaseGridGump
    {
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                {
                    m_From.SendGump(
                        new InterfaceItemGump(m_From, m_Columns, m_List, m_Page, m_Item)
                    );
    ...
    public class InterfaceMobileGump : BaseGridGump
    {
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                {
                    m_From.SendGump(
                        new InterfaceMobileGump(m_From, m_Columns, m_List, m_Page, m_Mobile)
                    );
                ...
                case 3: // Delete
                {
                    if (!m_Mobile.Player)
                    {
                        ...
                        m_Mobile.Delete();
                        m_From.SendGump(
                            new InterfaceGump(m_From, m_Columns, m_List, m_Page, m_Mobile)
                        );
                    }
