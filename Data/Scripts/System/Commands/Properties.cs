    ...
    public class Properties
    {
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(Props_OnCommand)
            );
        }
        ...
        {
            PropertyInfo[] chain = GetPropertyInfoChain(
                from,
                obj.GetType(),
                propertyName,
                access,
        ...
        public static string IncreaseValue(Mobile from, object o, string[] args)
        {
            ...
            for (int i = 0; i < realProps.Length; ++i)
            {
                ...
                try
                {
                    ...
                    realProps[i].SetValue(
                        realObjs[i],
                        Convert.ChangeType(v, realProps[i].PropertyType),
                        null
                    );
                }
                catch
        ...
        private static string InternalGetValue(object o, PropertyInfo p, PropertyInfo[] chain)
        {
            ...
            if (value == null)
                ...
                toString = (
                    (string)value == "null" ? @"@""null""" : String.Format("\"{0}\"", value)
                );
            else
