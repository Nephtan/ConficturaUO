...
namespace Server.Commands
{
    ...
    {
        private static Dictionary<string, CommandInfo> m_HelpInfos =
        ...
        {
            get { return m_HelpInfos; }
        }
        public static List<CommandInfo> SortedHelpInfo
        {
            get { return m_SortedHelpInfo; }
        }
        ...
        public static void Initialize()
        {
            CommandSystem.Register(
                "HelpInfo",
                AccessLevel.Player,
                new CommandEventHandler(HelpInfo_OnCommand)
            );
        ...
        public static void FillTable()
        {
            ...
            for (int i = 0; i < commands.Count; ++i)
            {
                ...
                if (aliases == null)
                    list.Add(
                        new CommandInfo(e.AccessLevel, e.Command, null, usage.Usage, descString)
                    );
                else
                {
                    list.Add(
            ...
            for (int i = 0; i < TargetCommands.AllCommands.Count; ++i)
            {
                ...
                for (int j = 0; j < aliases.Length; j++)
                {
                    ...
                    list.Add(
                        new CommandInfo(command.AccessLevel, aliases[j], newAliases, usage, desc)
                    );
                }
            }
                ...
                )
                {
                    ...
                    if (from.AccessLevel >= c.AccessLevel)
                    {
                        if ((int)c.AccessLevel != last)
                        {
                            ...
                            AddEntryHtml(
                                20 + OffsetSize + 160,
                                Color(c.AccessLevel.ToString(), 0xFF0000)
                            );
                            AddEntryHeader(20);
                            line++;
                        }
