    ...
    public class CommandHandlers
    {
        public static void Initialize()
        {
            ...
            Register(
                ...
                new CommandEventHandler(DropHolding_OnCommand)
            );

            Register(
                "GetFollowers",
                AccessLevel.GameMaster,
                new CommandEventHandler(GetFollowers_OnCommand)
            );

            Register(
                "ClearFacet",
                AccessLevel.Administrator,
                new CommandEventHandler(ClearFacet_OnCommand)
            );
            ...
            Register(
                "AutoPageNotify",
                AccessLevel.Counselor,
                new CommandEventHandler(APN_OnCommand)
            );
            ...
            Register(
                "BackgroundSave",
                AccessLevel.Administrator,
                new CommandEventHandler(BackgroundSave_OnCommand)
            );
            Register(
                "BGSave",
                AccessLevel.Administrator,
            ...
            Register(
                "SMsg",
                AccessLevel.Counselor,
                new CommandEventHandler(StaffMessage_OnCommand)
            );
            ...
            Register(
                "BCast",
                AccessLevel.GameMaster,
                new CommandEventHandler(BroadcastMessage_OnCommand)
            );
            Register(
                "BC",
                AccessLevel.GameMaster,
            ...
            Register(
                "ViewEquip",
                AccessLevel.GameMaster,
                new CommandEventHandler(ViewEquip_OnCommand)
            );
            ...
            Register(
                "ReplaceBankers",
                AccessLevel.Administrator,
                new CommandEventHandler(ReplaceBankers_OnCommand)
            );

            Register(
                "SpeedBoost",
                AccessLevel.Counselor,
                new CommandEventHandler(SpeedBoost_OnCommand)
            );
        }
        ...
        public static void DropHolding_OnCommand(CommandEventArgs e)
        {
            e.Mobile.BeginTarget(
                ...
                false,
                TargetFlags.None,
                new TargetCallback(DropHolding_OnTarget)
            );
            ...
            if (list.Count > 0)
            {
                ...
                e.Mobile.SendGump(
                    new WarningGump(
                        ...
                        30720,
                        String.Format(
                            "You are about to delete {0} object{1} from this facet.  Do you really wish to continue?",
                            list.Count,
                            list.Count == 1 ? "" : "s"
                        ),
                        0xFFC000,
                        360,
                        260,
                        new WarningGumpCallback(DeleteList_Callback),
                        list
                    )
                );
            }
        ...
        public static void GetFollowers_OnCommand(CommandEventArgs e)
        {
            e.Mobile.BeginTarget(
                ...
                false,
                TargetFlags.None,
                new TargetCallback(GetFollowers_OnTarget)
            );
            ...
            else if (obj is Mobile && ((Mobile)obj).Player)
            {
                ...
                foreach (Mobile m in World.Mobiles.Values)
                {
                    if (m is BaseCreature)
                    {
                        ...
                        if (
                            (bc.Controlled && bc.ControlMaster == master)
                            || (bc.Summoned && bc.SummonMaster == master)
                        )
                            pets.Add(bc);
                    }
                }
        ...
        private class ViewEqTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is Mobile)
                    from.SendMenu(
                        new EquipMenu(from, (Mobile)targeted, GetEquip((Mobile)targeted))
                    );
            }
            ...
            private static ItemListEntry[] GetEquip(Mobile m)
            {
                ...
                for (int i = 0; i < m.Items.Count; ++i)
                {
                    ...
                    entries[i] = new ItemListEntry(
                        String.Format("{0}: {1}", item.Layer, item.GetType().Name),
                        item.ItemID,
                        item.Hue
                    );
                }
            ...
            private class EquipMenu : ItemListMenu
            {
                ...
                private class EquipDetailsMenu : QuestionMenu
                {
                    ...
                    {
                        state.Mobile.SendMenu(
                            new EquipMenu(state.Mobile, m_Mobile, ViewEqTarget.GetEquip(m_Mobile))
                        );
                    }
        ...
        private static void Go_OnCommand(CommandEventArgs e)
        {
            ...
            else if (e.Length == 6)
            {
                ...
                if (map != null)
                {
                    Point3D p = Sextant.ReverseLookup(
                        map,
                        e.GetInt32(3),
                        e.GetInt32(0),
                        e.GetInt32(4),
            ...
            {
                from.SendMessage(
                    "Format: Go [name | serial | (x y [z]) | (deg min (N | S) deg min (E | W)]"
                );
            }
        }
        ...
        public static void StaffMessage_OnCommand(CommandEventArgs e)
        {
            BroadcastMessage(
                AccessLevel.Counselor,
                e.Mobile.SpeechHue,
                String.Format("[{0}] {1}", e.Mobile.Name, e.ArgString)
            );
        }
        ...
        public static void BroadcastMessage_OnCommand(CommandEventArgs e)
        {
            BroadcastMessage(
                AccessLevel.Player,
                0x482,
                String.Format("Staff message from {0}:", e.Mobile.Name)
            );
        ...
        public static void APN_OnCommand(CommandEventArgs e)
        {
            ...
            m.SendMessage(
                "Your auto-page-notify has been turned {0}.",
                m.AutoPageNotify ? "on" : "off"
            );
        }
        ...
        public static void Animate_OnCommand(CommandEventArgs e)
        {
            ...
            else
            {
                e.Mobile.SendMessage(
                    "Format: Animate <action> <frameCount> <repeatCount> <forward> <repeat> <delay>"
                );
            }
        }
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is Mobile)
                {
                    if (((Mobile)targeted).AccessLevel >= from.AccessLevel && targeted != from)
                        from.SendMessage(
                            "You can't do that to someone with higher Accesslevel than you!"
                        );
                    else
