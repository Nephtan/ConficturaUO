    ...
    class ClassicPoison
    {
        public static void Initialize()
        {
            CommandSystem.Register(
                "poisons",
                AccessLevel.Player,
                new CommandEventHandler(OnTogglePlayOriental)
            );
        }
        ...
        private static void OnTogglePlayOriental(CommandEventArgs e)
        {
            ...
            if (((PlayerMobile)m).ClassicPoisoning == 1)
            {
                m.SendMessage(
                    38,
                    "Poisons are now set for precision with special weapon infectious strikes."
                );
            ...
            else
            {
                m.SendMessage(
                    68,
                    "Poisons are now set with hits from one-handed slashing or piercing weapons."
                );
