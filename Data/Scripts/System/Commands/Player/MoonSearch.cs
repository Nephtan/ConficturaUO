...
namespace Server.Misc
{
    class MoonSearch
    {
        public static void Initialize()
        {
            CommandSystem.Register(
                "magicgate",
                AccessLevel.Player,
                new CommandEventHandler(Moon_OnCommand)
            );
        }
        ...
        public static void Moon_OnCommand(CommandEventArgs e)
        {
            ...
            {
                Mobile theMouse = (Mobile)mice[m];
                theMouse.Delete();
            }
            if (distchk == 0)
            {
                from.SendMessage("There is no nearby magical gate in this area!");
            }
        }
    ...
    public class MoonArrow : QuestArrow
    {
        ...
        public MoonArrow(Mobile from, Mobile target, int range)
            : base(from, target)
        {
            m_From = from;
            m_Target = target;
    ...
    public class MoonTimer : Timer
    {
        ...
        public MoonTimer(Mobile from, Mobile target, int range, QuestArrow arrow)
        ...
        protected override void OnTick()
        {
            if (!m_Arrow.Running)
            {
                ...
                || MoonSearch.SameArea(m_From, m_Target) == false
            )
            {
                m_Arrow.Stop();
                Stop();
...
namespace Server.Mobiles
{
    ...
    public class MoonCritter : BaseCreature
    {
        ...
        public MoonCritter(Serial serial)
            : base(serial) { }
