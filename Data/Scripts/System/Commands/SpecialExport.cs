    ...
    public class SpecialExporters
    {
        ...
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(SpecialExport_OnCommand)
            );
            CommandSystem.Register(
                "SpcEx",
                AccessLevel.Administrator,
        ...
        public static void BeginSpcEx(Mobile mob, string file)
        {
            BoundingBoxPicker.Begin(
                mob,
                new BoundingBoxCallback(SpcExBox_Callback),
                new object[] { file }
            );
        }

        private static void SpcExBox_Callback(
            Mobile mob,
            Map map,
            Point3D start,
            Point3D end,
        ...
        )
        {
            ...
            Export(
                mob,
                file,
                new Rectangle2D(new Point2D(start.X, start.Y), new Point2D(end.X + 1, end.Y + 1))
            );
        }
