    ...
    public class Statics
    {
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(Freeze_OnCommand)
            );
            CommandSystem.Register(
                "FreezeMap",
                AccessLevel.Administrator,
                new CommandEventHandler(FreezeMap_OnCommand)
            );
            CommandSystem.Register(
                "FreezeWorld",
                AccessLevel.Administrator,
                new CommandEventHandler(FreezeWorld_OnCommand)
            );

            CommandSystem.Register(
                "Unfreeze",
                AccessLevel.Administrator,
                new CommandEventHandler(Unfreeze_OnCommand)
            );
            CommandSystem.Register(
                "UnfreezeMap",
                AccessLevel.Administrator,
                new CommandEventHandler(UnfreezeMap_OnCommand)
            );
            CommandSystem.Register(
                "UnfreezeWorld",
                AccessLevel.Administrator,
                new CommandEventHandler(UnfreezeWorld_OnCommand)
            );
        }
        ...
        public static void FreezeMap_OnCommand(CommandEventArgs e)
        {
            ...
            if (map != null && map != Map.Internal)
                SendWarning(
                    ...
                    NullP3D,
                    NullP3D,
                    new WarningGumpCallback(FreezeWarning_Callback)
                );
        }
        ...
        private const string BaseFreezeWarning =
            "{0}  "
        ...
        private static void FreezeBox_Callback(
            Mobile from,
            Map map,
            Point3D start,
            Point3D end,
        ...
        public static void Freeze(Mobile from, Map targetMap, Point3D start3d, Point3D end3d)
        {
            ...
            else if (targetMap != null)
            {
                Point2D start = targetMap.Bound(new Point2D(start3d)),
                    end = targetMap.Bound(new Point2D(end3d));
                ...
                IPooledEnumerable eable = targetMap.GetItemsInBounds(
                    new Rectangle2D(start.X, start.Y, end.X - start.X + 1, end.Y - start.Y + 1)
                );
            ...
            foreach (DictionaryEntry de in mapTable)
            {
                ...
                using (FileStream idxStream = OpenWrite(matrix.IndexStream))
                {
                    using (FileStream mulStream = OpenWrite(matrix.DataStream))
                    {
                        ...
                        {
                            int oldTileCount;
                            StaticTile[] oldTiles = ReadStaticBlock(
                                idxReader,
                                mulStream,
                                state.m_X,
                                state.m_Y,
            ...
            if (totalFrozen == 0 && badDataFile)
                ...
                from.SendGump(
                    new NoticeGump(
                        ...
                        30720,
                        String.Format(
                            "Freeze operation completed successfully.<br><br>{0} item{1} frozen.<br><br>You must restart your client and update it's data files to see the changes.",
                            totalFrozen,
                            totalFrozen != 1 ? "s were" : " was"
                        ),
                        0xFFC000,
                        320,
                        240,
        ...
        private const string BaseUnfreezeWarning =
            "{0}  "
        ...
        public static void UnfreezeMap_OnCommand(CommandEventArgs e)
        {
            ...
            if (map != null && map != Map.Internal)
                SendWarning(
                    ...
                    NullP3D,
                    NullP3D,
                    new WarningGumpCallback(UnfreezeWarning_Callback)
                );
        }
        ...
        private static void UnfreezeBox_Callback(
            Mobile from,
            Map map,
            Point3D start,
            Point3D end,
        ...
        private static void DoUnfreeze(
            Map map,
            Point2D start,
            Point2D end,
            ref bool badDataFile,
            ref int totalUnfrozen
        )
        {
            ...
            int xTileStart = start.X,
                yTileStart = start.Y;
            int xTileWidth = end.X - start.X + 1,
                yTileHeight = end.Y - start.Y + 1;
                ...
                using (FileStream mulStream = OpenWrite(matrix.DataStream))
                {
                    ...
                    for (int x = xStartBlock; x <= xEndBlock; ++x)
                    {
                        for (int y = yStartBlock; y <= yEndBlock; ++y)
                        {
                            ...
                            int baseX = (x << 3) - xTileStart,
                                baseY = (y << 3) - yTileStart;
                            ...
                            for (int i = 0; i < oldTileCount; ++i)
                            {
                                ...
                                else
                                {
                                    ...
                                    item.MoveToWorld(
                                        new Point3D(px + xTileStart, py + yTileStart, oldTile.Z),
                                        map
                                    );
                                }
                            }
            ...
            if (totalUnfrozen == 0 && badDataFile)
                ...
                from.SendGump(
                    new NoticeGump(
                        ...
                        30720,
                        String.Format(
                            "Unfreeze operation completed successfully.<br><br>{0} item{1} unfrozen.<br><br>You must restart your client and update it's data files to see the changes.",
                            totalUnfrozen,
                            totalUnfrozen != 1 ? "s were" : " was"
                        ),
                        0xFFC000,
                        320,
                        240,
