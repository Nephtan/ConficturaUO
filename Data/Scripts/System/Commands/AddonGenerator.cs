	...
	public class {name}AddonDeed : BaseAddonDeed
	{
        ...
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(OnAddonGen)
            );
        }
        ...
        )
        {
            ...
            foreach (Item item in target)
            {
                ...
                if (
                    ((item.ItemData.Flags & TileFlag.LightSource) == TileFlag.LightSource)
                    || (item.Hue != 0)
                    || (item.Name != null)
                    || item.Amount > 1
                ) // Use old method
                {
                    if (item.Name != null || item.Amount != 0) // Have to do this one the old method
                    {
                        ...
                        nc.AppendFormat(
                            "\t\t\tAddComplexComponent( (BaseAddon) this, {0}, {1}, {2}, {3}, {4}, {5}, \"{6}\", {7});// {8}\n",
                            id,
                            xOffset,
                            yOffset,
                            zOffset,
            ...
            if (m_CustomOutputDirectory != null)
                path = Path.Combine(
                    ...
                    string.Format(@"TheBox\{0}Addon.cs", name)
                );
            else
            ...
            public override void OnResponse(Server.Network.NetState sender, RelayInfo info)
            {
                ...
                if (Verify(sender.Mobile, m_State))
                {
                    BoundingBoxPicker.Begin(
                        sender.Mobile,
                        new BoundingBoxCallback(AddonGenerator.PickerCallback),
                        m_State
                    );
                }
                else
