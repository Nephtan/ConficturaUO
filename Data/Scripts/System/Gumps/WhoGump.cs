    ...
    public class WhoGump : Gump
    {
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(WhoList_OnCommand)
            );
            CommandSystem.Register(
                "WhoList",
                AccessLevel.Counselor,
            ...
            for (int i = 0; i < states.Count; ++i)
            {
                ...
                if (
                    ...
                    && (
                        m == owner
                        || !m.Hidden
                        || owner.AccessLevel > m.AccessLevel
                        || (m is PlayerMobile && ((PlayerMobile)m).VisibilityList.Contains(owner))
                    )
                )
                {
        ...
        public void Initialize(int page)
        {
            ...
            AddLabel(
                ...
                String.Format(
                    "Page {0} of {1} ({2})",
                    page + 1,
                    (m_Mobiles.Count + EntryCount - 1) / EntryCount,
                    m_Mobiles.Count
                )
            );
            ...
            for (
                ...
                i < EntryCount && index < m_Mobiles.Count;
                ++i, ++index
            )
            {
                ...
                default:
                {
                    ...
                    if (index >= 0 && index < m_Mobiles.Count)
                    {
                        ...
                        else if (m.NetState == null)
                        {
                            ...
                            || (
                                m is PlayerMobile && ((PlayerMobile)m).VisibilityList.Contains(from)
                            )
                        )
                        {
