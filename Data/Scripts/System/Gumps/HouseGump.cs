                ...
                for (int i = 0; i < list.Count; ++i)
                {
                    ...
                    AddLabel(
                        ...
                        accountOf && m.Player && m.Account != null
                            ? String.Format("Account of {0}", name)
                            : name
                    );
                }
            }
    ...
    public class HouseGump : Gump
    {
            ...
            : base(20, 30)
        {
            ...
            if (m_House.Sign != null)
            {
                ...
                {
                    for (
                        int i = 0, y = (101 - (lines.Count * 14)) / 2;
                        i < lines.Count;
                        ++i, y += 14
                    )
                    {
            ...
            switch (info.ButtonID)
            {
                ...
                case 4: // Remove co-owner
                {
                    if (isOwner)
                    {
                        ...
                        from.CloseGump(typeof(HouseRemoveGump));
                        from.SendGump(
                            new HouseRemoveGump(1011274, m_House.CoOwners, m_House, false)
                        );
                    }
                    else
                    {
                ...
                case 8: // Remove friend
                {
                    if (isCoOwner)
                    {
                        ...
                        from.CloseGump(typeof(HouseRemoveGump));
                        from.SendGump(
                            new HouseRemoveGump(1011272, m_House.Friends, m_House, false)
                        );
                    }
                    else
                    {
