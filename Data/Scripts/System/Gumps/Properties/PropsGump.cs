        ...
        private void Initialize(int page)
        {
            ...
            if (OldStyle)
                AddImageTiled(
                    x,
                    y,
                    TotalWidth - (OffsetSize * 3) - SetWidth,
                    EntryHeight,
                    HeaderGumpID
                );
            else
            ...
            )
            {
                ...
                else if (o is PropertyInfo)
                {
                    ...
                    if (
                        prop.CanWrite
                        && cpa != null
                        && m_Mobile.AccessLevel >= cpa.WriteLevel
                        && !cpa.ReadOnly
                    )
                        AddButton(
                            x + SetOffsetX,
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                case 1: // Previous
                {
                    if (m_Page > 0)
                        from.SendGump(
                            new PropertiesGump(from, m_Object, m_Stack, m_List, m_Page - 1)
                        );
                ...
                default:
                {
                    ...
                    if (index >= 0 && index < m_List.Count)
                    {
                        ...
                        if (
                            !prop.CanWrite
                            || attr == null
                            || from.AccessLevel < attr.WriteLevel
                            || attr.ReadOnly
                        )
                            return;
                        ...
                        if (IsType(type, typeofMobile) || IsType(type, typeofItem))
                            ...
                            from.Target = new SetObjectTarget(
                                prop,
                                from,
                                m_Object,
                                m_Stack,
                            ...
                            from.SendGump(
                                new SetPoint3DGump(prop, from, m_Object, m_Stack, m_Page, m_List)
                            );
                        else if (IsType(type, typeofPoint2D))
                            from.SendGump(
                                new SetPoint2DGump(prop, from, m_Object, m_Stack, m_Page, m_List)
                            );
                        else if (IsType(type, typeofTimeSpan))
                            from.SendGump(
                                new SetTimeSpanGump(prop, from, m_Object, m_Stack, m_Page, m_List)
                            );
                            ...
                            from.SendGump(
                                new SetGump(prop, from, m_Object, m_Stack, m_Page, m_List)
                            );
