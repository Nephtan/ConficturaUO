...
namespace Server.Gumps
{
    public abstract class BaseVendorRentalGump : Gump
    {
            ...
            : base(100, 100)
        {
            ...
            else
            {
                ...
                if (
                    type == GumpType.LockedContract
                    || type == GumpType.UnlockedContract
                    || type == GumpType.VendorLandlord
                )
    ...
    public class VendorRentalOfferGump : BaseVendorRentalGump
    {
        ...
        protected override bool IsValidResponse(Mobile from)
        {
            return m_Contract.IsUsableBy(m_Landlord, true, false, false, false)
                && from.CheckAlive()
                && m_Contract.Offeree == from;
        }
    ...
    public class VendorRentalRefundGump : Gump
    {
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            if (
                !m_Vendor.CanInteractWith(from, true)
                || !m_Vendor.CanInteractWith(m_Landlord, false)
