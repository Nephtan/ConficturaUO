...
namespace Server.Guilds
{
    public class GuildRosterGump : BaseGuildListGump<PlayerMobile>
    {
        ...
        private static InfoField<PlayerMobile>[] m_Fields = new InfoField<PlayerMobile>[]
        {
        ...
        public GuildRosterGump(
            PlayerMobile pm,
            Guild g,
            IComparer<PlayerMobile> currentComparer,
            bool ascending,
            string filter,
            int startNumber
        )
        ...
        public override Gump GetResentGump(
            PlayerMobile pm,
            Guild g,
            IComparer<PlayerMobile> comparer,
            bool ascending,
            string filter,
            int startNumber
        )
            ...
            if (info.ButtonID == 8)
            {
                if (pm.GuildRank.GetFlag(RankFlags.CanInvitePlayer))
                {
                    ...
                    pm.BeginTarget(
                        ...
                        new TargetStateCallback(InvitePlayer_Callback),
                        guild
                    );
                }
                else
        ...
        public void InvitePlayer_Callback(Mobile from, object targeted, object state)
        {
            ...
            if (
                pm == null
                || !IsMember(pm, guild)
                || !pm.GuildRank.GetFlag(RankFlags.CanInvitePlayer)
            )
            {
                pm.SendLocalizedMessage(503301); // You don't have permission to do that.
            }
