    ...
    public class GuildMemberInfoGump : BaseGuildGump
    {
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                {
                    if (
                        playerRank.GetFlag(RankFlags.CanPromoteDemote)
                        && (
                            (playerRank.Rank - 1) > targetRank.Rank
                            || (
                                playerRank == RankDefinition.Leader
                                && playerRank.Rank > targetRank.Rank
                            )
                        )
                    )
                    {
                        ...
                        if (targetRank == RankDefinition.Leader)
                        {
                            ...
                            else
                            {
                                ...
                                pm.SendGump(
                                    new GuildMemberInfoGump(player, guild, m_Member, false, true)
                                );
                            }
                        }
                        else
                    ...
                    )
                    {
                        if (targetRank == RankDefinition.Lowest)
                        {
                            if (RankDefinition.Lowest.Name.Number == 1062963)
                                ...
                                pm.SendMessage(
                                    "You can't demote a {0}.",
                                    RankDefinition.Lowest.Name
                                );
                        }
                        else
                        {
                ...
                {
                    if (
                        playerRank.GetFlag(RankFlags.CanSetGuildTitle)
                ...
                case 5: //Kick
                {
                    if (
                        (
                            playerRank.GetFlag(RankFlags.RemovePlayers)
                            && playerRank.Rank > targetRank.Rank
                        )
                        || (
                            playerRank.GetFlag(RankFlags.RemoveLowestRank)
                            && targetRank == RankDefinition.Lowest
                        )
                    )
                    {
                        if (m_toKick)
                        ...
                        else
                        {
                            ...
                            pm.SendGump(
                                new GuildMemberInfoGump(player, guild, m_Member, true, false)
                            );
                        }
                    }
                    else
        ...
        public void SetTitle_Callback(Mobile from, string text)
        {
            ...
            if (
                ...
                || !(
                    ...
                    && (pm.GuildRank.Rank > targ.GuildRank.Rank || pm == targ)
                )
            )
            {
