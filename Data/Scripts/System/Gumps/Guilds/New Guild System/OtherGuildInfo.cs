    ...
    public class OtherGuildInfo : BaseGuildGump
    {
        ...
        public override void PopulateGump()
        {
            ...
            if (ActiveWar)
            {
                ...
                if (
                    activeWar.WarLength != TimeSpan.Zero
                    && (activeWar.WarBeginning + activeWar.WarLength) > DateTime.Now
                )
                ...
                time = String.Format(
                    "{0:D2}:{1:mm}",
                    timeRemaining.Hours,
                    DateTime.MinValue + timeRemaining
                );
            ...
            else if (PendingWar)
            {
                ...
                time = Color(
                    String.Format(
                        "{0:D2}:{1:mm}",
                        war.WarLength.Hours,
                        DateTime.MinValue + war.WarLength
                    ),
                    0x990000
                );
                ...
                if (otherWar != null)
                    otherKills = Color(
                        String.Format("{0}/{1}", otherWar.Kills, otherWar.MaxKills),
                        0x990000
                    );
            }
            ...
            switch (info.ButtonID)
            {
                    ...
                    if (war != null && !war.WarRequester && activeWar == null)
                    {
                        ...
                        else if (alliance != null && alliance.Leader != guild)
                        {
                            pm.SendLocalizedMessage(
                                1063239,
                                String.Format("{0}\t{1}", guild.Name, alliance.Name)
                        ...
                        else
                        {
                            ...
                            if (alliance != null && alliance.IsMember(guild))
                            {
                                alliance.AllianceMessage(
                                    1070769,
                                    ((otherAlliance != null) ? otherAlliance.Name : otherGuild.Name)
                            ...
                            if (otherAlliance != null && m_Other.Alliance.IsMember(m_Other))
                            {
                                otherAlliance.AllianceMessage(
                                    1070769,
                                    ((alliance != null) ? alliance.Name : guild.Name)
                                ); // Guild Message: Your guild is now at war with ~1_GUILDNAME~
                    ...
                    else
                    {
                        if (activeWar != null)
                        {
                            ...
                            if (otherAlliance != null && otherAlliance.IsMember(otherGuild))
                            {
                                otherAlliance.AllianceMessage(
                                    1070739,
                                    ((guild.Alliance != null) ? guild.Alliance.Name : guild.Name)
                ...
                case 1: //Declare War
                {
                    if (war == null && activeWar == null)
                    {
                        ...
                        else if (otherAlliance != null && otherAlliance.Leader != m_Other)
                        {
                            pm.SendLocalizedMessage(
                                1063239,
                                String.Format("{0}\t{1}", m_Other.Name, otherAlliance.Name)
                            ); // ~1_val~ is not the leader of the ~2_val~ alliance.
                ...
                case 13: //Promote to Alliance leader
                {
                    ...
                    )
                    {
                        pm.SendLocalizedMessage(
                            1063434,
                            String.Format("{0}\t{1}", m_Other.Name, alliance.Name)
                ...
                case 14: //Withdraw Request
                {
                    if (!playerRank.GetFlag(RankFlags.AllianceControl))
                    {
                        ...
                        && alliance.IsPendingMember(m_Other)
                    )
                    {
                        m_Other.Alliance = null;
                        pm.SendLocalizedMessage(1070752); // The proposal has been updated.
