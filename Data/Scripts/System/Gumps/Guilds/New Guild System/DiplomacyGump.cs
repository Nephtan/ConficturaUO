    ...
    public class GuildDiplomacyGump : BaseGuildListGump<Guild>
    {
        ...
        public GuildDiplomacyGump(PlayerMobile pm, Guild g)
            : this(
                pm,
                g,
                GuildDiplomacyGump.NameComparer.Instance,
                true,
                "",
                0,
                GuildDisplayType.All,
                Utility.CastConvertList<BaseGuild, Guild>(new List<BaseGuild>(Guild.List.Values)),
        ...
        public override bool HasRelationship(Guild g)
        {
            ...
            if (alliance != null)
            {
                ...
                {
                    if (
                        guild == leader && alliance.IsPendingMember(g)
                        || g == leader && alliance.IsPendingMember(guild)
                    )
                        return true;
                }
        ...
        protected override bool IsFiltered(Guild g, string filter)
        {
            ...
            return !(
                Insensitive.Contains(g.Name, filter) || Insensitive.Contains(g.Abbreviation, filter)
            );
        }
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            if (AllowAdvancedSearch && info.ButtonID == 8)
                pm.SendGump(
                    new GuildAdvancedSearchGump(
                        pm,
                        guild,
                        m_Display,
                        new SearchSelectionCallback(AdvancedSearch_Callback)
                    )
                );
        }
