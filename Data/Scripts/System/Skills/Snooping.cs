...
namespace Server.SkillHandlers
{
    public class Snooping
    {
        ...
        public static bool CheckSnoopAllowed(Mobile from, Mobile to)
        {
            ...
            if (
                to.Body.IsHuman
                && (cret == null || (!cret.AlwaysAttackable && !cret.AlwaysMurderer))
            )
        ...
        public static void Container_Snoop(Container cont, Mobile from)
        {
            if (from.AccessLevel > AccessLevel.Player || from.InRange(cont.GetWorldLocation(), 1))
            {
                ...
                if (
                    root != null
                    && root.AccessLevel > AccessLevel.Player
                ...
                )
                {
                    ...
                    if (map != null)
                    {
                        string message = String.Format(
                            "You notice {0} attempting to peek into {1}'s belongings.",
                            from.Name,
                            root.Name
                        );
                ...
                if (
                    from.AccessLevel > AccessLevel.Player
                    || from.CheckTargetSkill(SkillName.Snooping, cont, 0.0, 125.0)
                )
                {
