            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is BaseCreature && from.CanBeHarmful((Mobile)targeted, true))
                {
                    ...
                    else if (creature.Controlled)
                    {
                        ...
                        creature.IsParagon && BaseInstrument.GetBaseDifficulty(creature) >= 160.0
                    )
                    {
                ...
                if (targeted is BaseCreature)
                {
                    ...
                    else if (m_Creature != creature)
                    {
                        ...
                        double diff =
                            (
                                (
                                    m_Instrument.GetDifficultyFor(m_Creature)
                                    + m_Instrument.GetDifficultyFor(creature)
                                ) * 0.5
                            ) - 5.0;
                        ...
                        if (
                            from.CanBeHarmful(m_Creature, true) && from.CanBeHarmful(creature, true)
                        )
                        {
