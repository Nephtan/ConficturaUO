...
namespace Server.SkillHandlers
{
    public class Searching
    {
                ...
                if (range > 0)
                {
                    ...
                    foreach (Mobile trg in inRange)
                    {
                        if (trg.Hidden && src != trg)
                        {
                            ...
                            double ts =
                                trg.Skills[SkillName.Hiding].Value + Utility.Random(21) - 10;

                            if (
                                src.AccessLevel >= trg.AccessLevel
                                && (ss >= ts || (inHouse && house.IsInside(trg)))
                            )
                            {
                                trg.RevealingAction();
        ...
        public static bool DetectSomething(Item item, Mobile m, bool skillCheck)
        {
            ...
            else if (!foundIt && SpotInTheDark(m))
                foundIt = true;
                ...
                else if (item is HiddenChest)
                {
                    ...
                    if (Utility.RandomMinMax(1, 3) > 1)
                    {
                        ...
                        if (Server.Misc.Worlds.IsOnSpaceship(item.Location, item.Map))
                        {
                            coins.Delete();
                            coins = new DDXormite();
                            coins.Amount = (int)((money * level) / 3);
                        }
                        else if (
                            Server.Misc.Worlds.GetMyWorld(item.Map, item.Location, item.X, item.Y)
                            == "the Underworld"
                        )
                        {
                            coins.Delete();
                        ...
                        {
                            coins.Delete();
                            coins = new DDGemstones();
                            coins.Amount = (int)((money * level) / 2);
                        }
                        else if (Utility.RandomMinMax(1, 100) > 95)
                        {
                            coins.Delete();
                            coins = new DDGoldNuggets();
                            coins.Amount = (int)((money * level));
                        }
                        else if (Utility.RandomMinMax(1, 100) > 80)
                        {
                            coins.Delete();
                            coins = new DDSilver();
                            coins.Amount = (int)((money * level) * 5);
                        }
                        else if (Utility.RandomMinMax(1, 100) > 60)
                        {
                            coins.Delete();
                            coins = new DDCopper();
                            coins.Amount = (int)((money * level) * 10);
                        }
