    ...
    public class Forensics
    {
        ...
        public class ForensicTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object target)
            {
                if (target is Mobile)
                {
                    if (from.CheckTargetSkill(SkillName.Forensics, target, 40.0, 125.0))
                    {
                        if (
                            ...
                            && ((PlayerMobile)target).NpcGuild == NpcGuild.ThievesGuild
                        )
                ...
                else if (target is Coffer)
                {
                    ...
                    if (coffer.CofferRobbed > 0)
                    {
                        from.SendMessage(
                            "It seems that "
                                + coffer.CofferRobber
                                + " has robbed this coffer of it's gold!"
                        );
                    }
                    else
                    {
                ...
                {
                    from.SendMessage(
                        "This adventurer looks to have been slain by some wild animal."
                    );
                }
                else if (target is LandChest && !LandChest.isBody(((Item)target).ItemID))
                {
                ...
                {
                    if (from.CheckTargetSkill(SkillName.Forensics, target, 0.0, 125.0))
                    {
