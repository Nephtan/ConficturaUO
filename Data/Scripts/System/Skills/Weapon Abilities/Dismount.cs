...
namespace Server.Items
{
    ...
    public class Dismount : WeaponAbility
    {
        ...
        public override void OnHit(Mobile attacker, Mobile defender, int damage)
        {
            ...
            if (
                defender is HenchmanArcher
                || defender is HenchmanFighter
                || defender is HenchmanWizard
            )
            {
            ...
            if (
                Core.ML
                && attacker is BaseCreature
                && ((BaseCreature)attacker).ControlMaster != null
            )
            {
                BaseMount.SetMountPrevention(
                    ((BaseCreature)attacker).ControlMaster,
                    BlockMountType.DismountRecovery,
                    AttackerRemountDelay
                );
            }
            else
            {
