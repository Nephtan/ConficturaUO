        ...
        public override void OnHit(Mobile attacker, Mobile defender, int damage)
        {
            ...
            if (ClassicPoisons > 0)
            {
                attacker.SendMessage(
                    "You cannot use this attack with your current poison settings!"
                );
                return;
            }
            if (
                (p == null || weapon.PoisonCharges <= 0)
                && ((skill != null && skill.Value < 80.0) || skill == null)
            )
            {
                attacker.SendMessage(
                    "Your stealth is not sufficient, and the weapon is out of poison!"
                );
                return;
            }
