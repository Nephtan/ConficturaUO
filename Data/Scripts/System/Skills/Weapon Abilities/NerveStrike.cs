        ...
        public override void OnHit(Mobile attacker, Mobile defender, int damage)
        {
            ...
            if (Core.ML)
            {
                AOS.Damage(
                    ...
                    attacker,
                    (int)(
                        15.0 * (attacker.Skills[SkillName.Tactics].Value - 50.0) / 70.0
                        + Utility.Random(10)
                    ),
                    true,
                    100,
                ...
                if (
                    ((150.0 / 7.0 + (4.0 * attacker.Skills[SkillName.Tactics].Value) / 7.0) / 100.0)
                    > Utility.RandomDouble()
                )
                {
                    defender.Paralyze(TimeSpan.FromSeconds(2.0));
                    Server.Items.ParalyzingBlow.BeginImmunity(
                        defender,
                        Server.Items.ParalyzingBlow.FreezeDelayDuration
                    );
                }
            }
            else
            {
                AOS.Damage(
                    defender,
                    attacker,
                    (int)(15.0 * (attacker.Skills[SkillName.Tactics].Value - 50.0) / 70.0 + 10),
                    true,
                    100,
                    0,
                    0,
