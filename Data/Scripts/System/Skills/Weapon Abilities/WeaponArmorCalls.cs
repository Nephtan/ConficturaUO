...
namespace Server.Items
{
    ...
    {
        public static SkillName GetSkill(Mobile from)
        {
            return GetSkill(from, 0);
        }
        ...
        public static SkillName GetSkill(Mobile from, int spot)
        {
            ...
            skilllist.Sort(
                delegate(LGSkillLists p1, LGSkillLists p2)
                {
                    return p2.LGSkillVal.CompareTo(p1.LGSkillVal);
                }
            );
            if (spot2 > (skilllist.Count - 1))
                spot2 = skilllist.Count - 1;
            sk = skilllist[spot2].LGSkill;
            return sk;
        }

        public static void GetSkillTop5(
            Mobile from,
            ref SkillName skill1,
            ref SkillName skill2,
            ref SkillName skill3,
            ref SkillName skill4,
            ref SkillName skill5
        )
        {
            List<LGSkillLists> skilllist = new List<LGSkillLists>();
            skilllist.Sort(
        ...
        public static void GetSkillCRTop5(
            Mobile from,
            ref SkillName skill1,
            ref SkillName skill2,
            ref SkillName skill3,
        ...
        )
        {
            ...
            skilllist.Add(
                new LGSkillLists(SkillName.Lumberjacking, from.Skills.Lumberjacking.Value)
            );
        ...
        public static void GetSkillCRBot5(
            Mobile from,
            ref SkillName skill1,
            ref SkillName skill2,
            ref SkillName skill3,
        ...
        )
        {
            ...
            skilllist.Sort(
                delegate(LGSkillLists p1, LGSkillLists p2)
                {
                    return p1.LGSkillVal.CompareTo(p2.LGSkillVal);
                }
            );
            skill1 = skilllist[0].LGSkill;
            skill2 = skilllist[1].LGSkill;
        ...
        public static void GetSkillFITop5(
            Mobile from,
            ref SkillName skill1,
            ref SkillName skill2,
            ref SkillName skill3,
    ...
    public class ManaRedux
    {
        public static int CalculateSpecialMana(Mobile from, int BaseMana)
        {
            int mana = BaseMana;
            double skillTotal =
                GetReduxSkill(from, SkillName.Swords)
                + GetReduxSkill(from, SkillName.Bludgeoning)
                + GetReduxSkill(from, SkillName.Fencing)
                + GetReduxSkill(from, SkillName.Marksmanship)
                + GetReduxSkill(from, SkillName.Parry)
                + GetReduxSkill(from, SkillName.Lumberjacking)
                + GetReduxSkill(from, SkillName.Stealth)
                + GetReduxSkill(from, SkillName.Poisoning)
                + GetReduxSkill(from, SkillName.Bushido)
                + GetReduxSkill(from, SkillName.Ninjitsu);
    ...
    public class WeaponStrikes
    {
        ...
        private class CantWalkTimer : Timer
        {
            ...
            public CantWalkTimer(Mobile m, TimeSpan duration)
                : base(duration)
            {
    ...
    public class GetSpawnRegion
    {
        public static bool IsChampSpawn(Mobile from)
        {
            if (
                Region
                    .Find(from.Location, from.Map)
                    .IsPartOf(typeof(Engines.CannedEvil.ChampionSpawnRegion))
            )
                return true;
            return false;
        }
