...
namespace Server.SkillHandlers
{
    public class Stealth
    {
        ...
        private static int[,] m_ArmorTable = new int[,]
        {
            ...
            /* Horned	*/{ 1, 1, 0, 2, 3, 5, 0 },
            /* Barbed	*/{ 1, 1, 0, 2, 3, 5, 0 },
            /* Necrotic	*/{ 1, 1, 0, 2, 3, 5, 0 },
            /* Volcanic	*/{ 1, 1, 0, 2, 3, 5, 0 },
            /* Frozen	*/{ 1, 1, 0, 2, 3, 5, 0 },
        ...
        public static int GetArmorRating(Mobile m)
        {
            ...
            for (int i = 0; i < m.Items.Count; i++)
            {
                ...
                if (
                    materialType >= m_ArmorTable.GetLength(0)
                    || bodyPosition >= m_ArmorTable.GetLength(1)
                )
                    continue;
        ...
        public static TimeSpan OnUse(Mobile m)
        {
            if (!m.Hidden)
            {
                ...
                m.Skills[SkillName.Hiding].Value
                < (
                    (Core.ML)
                        ? 30.0
                        : (Core.SE)
