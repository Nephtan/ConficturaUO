...
namespace Knives.Chat3
{
    ...
    public abstract class GumpPlus : Gump
    {
        ...
        {
            get { return GumpInfo.GetInfo(c_Owner, this.GetType()); }
        }
        public bool Override
        {
            get { return c_Override; }
            set { c_Override = value; }
        }

        public GumpPlus(Mobile m, int x, int y)
            : base(x, y)
        {
            c_Owner = m;
        ...
        private void ModifyGump()
        {
            try
            {
                ...
                foreach (GumpEntry entry in new ArrayList(Entries))
                {
                    ...
                    else if (entry is HtmlPlus)
                    {
                        ...
                        while (
                            (num = text.ToLower().IndexOf("<basefont")) != -1
                            || (num = text.ToLower().IndexOf("</font")) != -1
                        )
                        {
                            length = 0;
                            ...
                            text =
                                ...
                                + text.Substring(num + length, text.Length - num - length);
                        }
            ...
            {
                Errors.Report("GumpPlus-> ModifyGump-> " + GetType());
            }
        }
