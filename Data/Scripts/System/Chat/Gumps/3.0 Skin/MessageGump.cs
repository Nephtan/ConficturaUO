...
namespace Knives.Chat3
{
    public class MessageGump : GumpPlus
    {
        ...
        public MessageGump(Mobile m, Message msg)
            : base(m, 200, 400)
        {
            Override = true;
        ...
        protected override void BuildGump()
        {
            ...
            if (Data.GetData(Owner).Messages.Contains(c_Message))
            {
                ...
                {
                    if (
                        Message.CanMessage(Owner, c_Message.From)
                        && !Message.StaffTimeout(c_Message)
                    )
                    {
        ...
        private void Ignore()
        {
            ...
            Owner.SendMessage(
                ...
                General.Local(68) + " " + c_Message.From.RawName
            );
        ...
        private void Accept()
        {
            c_Message.From.SendMessage(
                Data.GetData(c_Message.From).SystemC,
                Owner.RawName + " " + General.Local(87)
            );
        ...
        private void Deny()
        {
            c_Message.From.SendMessage(
                Data.GetData(c_Message.From).SystemC,
                Owner.RawName + " " + General.Local(88)
            );
            Owner.SendMessage(
                Data.GetData(Owner).SystemC,
                General.Local(89) + " " + c_Message.From.RawName
            );
