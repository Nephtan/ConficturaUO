    ...
    public class ProfileGump : GumpPlus
    {
        ...
        protected override void BuildGump()
        {
            ...
            else
            {
                if (c_Target.Guild != null)
                    AddHtml(
                        ...
                        y += 20,
                        width - 95,
                        "[" + c_Target.Guild.Abbreviation + "] " + c_Target.GuildTitle
                    );
                if (General.IsInFaction(c_Target))
                    AddHtml(
                        ...
                        y += 20,
                        width - 95,
                        General.FactionName(c_Target) + " " + General.FactionTitle(c_Target)
                    );
            }
            ...
            AddHtml(
                ...
                y += 20,
                width,
                "<CENTER>" + General.Local(253) + " " + Data.GetData(c_Target).Karma
            );
            ...
            if (Owner == c_Target)
            {
                ...
                AddTextField(
                    ...
                    0xBBC,
                    "Signature",
                    Data.GetData(c_Target).Signature
                );
        ...
        private void ViewOptions(int x)
        {
            ...
            AddButton(
                x + width / 2 - 60,
                y,
                Data.GetData(Owner).Friends.Contains(c_Target) ? 0x2343 : 0x2342,
                "Friend",
                new GumpCallback(Friend)
            );
            AddButton(
                x + width / 2 + 40,
                y,
            ...
            AddButton(
                x + width / 2 - 60,
                y,
                Data.GetData(Owner).Ignores.Contains(c_Target) ? 0x2343 : 0x2342,
                "Ignore",
                new GumpCallback(Ignore)
            );
            AddButton(
                x + width / 2 + 40,
                y,
            ...
            if (Owner.AccessLevel >= AccessLevel.Administrator)
            {
                if (Owner.AccessLevel > c_Target.AccessLevel)
                {
                    AddHtml(
                        ...
                        y += 20,
                        width,
                        HTML.LightPurple + "<CENTER>" + General.Local(14),
                        false
                    );
                    AddButton(
                        x + width / 2 - 60,
                ...
                if (
                    c_Target.AccessLevel < AccessLevel.Administrator
                    && c_Target.AccessLevel != AccessLevel.Player
                )
                {
                    AddHtml(
                        x,
                        y += 20,
                        width,
                        HTML.LightPurple + "<CENTER>" + General.Local(4),
                        false
                    );
                    AddButton(
                        x + width / 2 - 60,
                        y,
                        Data.GetData(c_Target).GlobalAccess ? 0x2343 : 0x2342,
                        "Global Access",
                        new GumpCallback(GlobalAccess)
                    );
                    AddButton(
                        x + width / 2 + 40,
                        y,
            ...
            )
            {
                ...
                AddButton(
                    x + width / 2 - 60,
                    y,
                    Data.GetData(c_Target).Banned ? 0x2343 : 0x2342,
                    "Ban",
                    new GumpCallback(Ban)
                );
                AddButton(
                    x + width / 2 + 40,
                    y,
            ...
            if (Data.GetData(Owner).GlobalAccess)
            {
                ...
                if (Data.GetData(Owner).Global)
                {
                    ...
                    AddButton(
                        x + width / 2 - 60,
                        y,
                        Data.GetData(Owner).GIgnores.Contains(c_Target) ? 0x2343 : 0x2342,
                        "Global Ignore",
                        new GumpCallback(GIgnore)
                    );
                    AddButton(
                        x + width / 2 + 40,
                        y,
                ...
                else
                {
                    ...
                    AddButton(
                        x + width / 2 - 60,
                        y,
                        Data.GetData(Owner).GListens.Contains(c_Target) ? 0x2343 : 0x2342,
                        "Global Listen",
                        new GumpCallback(GListen)
                    );
                    AddButton(
                        x + width / 2 + 40,
                        y,
            ...
            if (Data.GetData(Owner).MsgSound)
            {
                ...
                AddTextField(
                    ...
                    0xBBA,
                    "Sound",
                    Data.GetData(Owner).GetSound(c_Target).ToString()
                );
                AddButton(
                    x + width / 2 + 30,
                    y + 3,
        ...
        private void Friend()
        {
            if (Data.GetData(c_Target).ByRequest && !Data.GetData(Owner).Friends.Contains(c_Target))
            {
                ...
                Data.GetData(c_Target)
                    .AddMessage(
                        new Message(Owner, General.Local(84), General.Local(85), MsgType.Invite)
                    );

                Owner.SendMessage(
                    Data.GetData(Owner).SystemC,
                    General.Local(86) + " " + c_Target.Name
                );
        ...
        private void GlobalAccess()
        {
            ...
            if (Data.GetData(c_Target).GlobalAccess)
                Owner.SendMessage(
                    Data.GetData(Owner).SystemC,
                    c_Target.Name + " " + General.Local(75)
                );
            else
                Owner.SendMessage(
                    Data.GetData(Owner).SystemC,
                    c_Target.Name + " " + General.Local(76)
                );
        ...
        private void Ban()
        {
            if (Data.GetData(c_Target).Banned)
            {
                ...
                Owner.SendMessage(
                    Data.GetData(Owner).SystemC,
                    General.Local(78) + " " + c_Target.Name
                );
                NewGump();
            }
            else
        ...
        private void Client()
        {
            ...
            if (c_Target.NetState == null)
                Owner.SendMessage(
                    Data.GetData(Owner).SystemC,
                    c_Target.Name + " " + General.Local(83)
                );
            else
        ...
        private class BanGump : GumpPlus
        {
            ...
            private void BanTime(object o)
            {
                ...
                Owner.SendMessage(
                    Data.GetData(Owner).SystemC,
                    General.Local(77) + " " + c_Target.Name
                );
