...
namespace Knives.Chat3
{
    public class SendMessageGump : GumpPlus
    {
        ...
        protected override void BuildGump()
        {
            ...
            else
            {
                ...
                AddButton(
                    50,
                    y,
                    Data.GetData(Owner).ExtraPm ? 0x25E4 : 0x25E8,
                    Data.GetData(Owner).ExtraPm ? 0x25E5 : 0x25E9,
                    "ExtraPm",
                    new GumpCallback(ExtraPm)
                );
            }
        ...
        private void Send()
        {
            ...
            {
                Data.GetData(c_To)
                    .AddMessage(new Message(Owner, c_Subject, c_Text, MsgType.Normal));
                General.PmNotify(c_To);

                Owner.SendMessage(
                    Data.GetData(Owner).SystemC,
                    General.Local(67) + " " + c_To.RawName
                );
                if (Data.GetData(c_To).Status != OnlineStatus.Online)
                    Owner.SendMessage(
                        Data.GetData(Owner).SystemC,
                        c_To.RawName + ": " + Data.GetData(c_To).AwayMsg
                    );
            }
            ...
            if (Data.LogPms)
                Logging.LogPm(
                    String.Format(
                        DateTime.Now + " <Mail> {0} to {1}: {2}",
                        Owner.RawName,
                        (c_To == null ? "All" : c_To.RawName),
                        c_Text
                    )
                );
            ...
            foreach (Data data in Data.Datas.Values)
                if (
                    data.Mobile.AccessLevel >= c_From.AccessLevel
                    && (
                        (data.GlobalM && !data.GIgnores.Contains(c_From))
                        || data.GListens.Contains(c_From)
                    )
                )
                    data.Mobile.SendMessage(
                        data.GlobalMC,
                        String.Format(
                            "(Global) <Mail> {0} to {1}: {2}",
                            Owner.RawName,
                            (c_To == null ? "All" : c_To.RawName),
                            c_Text
                        )
