    ...
    public class FilterGump : GumpPlus
    {
        ...
        protected override void BuildGump()
        {
            ...
            AddButton(
                width / 2 - 120,
                y,
                Data.FilterSpeech ? 0x2343 : 0x2342,
                "Filter Speech",
                new GumpCallback(FilterSpeech)
            );
            AddButton(
                width / 2 + 100,
                y,
            ...
            AddButton(
                width / 2 - 120,
                y,
                Data.FilterMsg ? 0x2343 : 0x2342,
                "Filter Messages",
                new GumpCallback(FilterMsg)
            );
            AddButton(
                width / 2 + 100,
                y,
            ...
            AddHtml(
                ...
                "<CENTER>"
                    + General.Local(154)
                    + ": "
                    + General.Local(155 + (int)Data.FilterPenalty)
            );
            AddButton(
                width / 2 - 80,
                y + 4,
                0x2716,
                "Filter Penalty",
                new GumpCallback(FilterPenalty)
            );
            AddButton(
                width / 2 + 70,
                y + 4,
        ...
        private void AddFilter()
        {
            ...
            if (Data.Filters.Contains(GetTextField("Add/Remove").ToLower()))
            {
                ...
                Owner.SendMessage(
                    ...
                    General.Local(149) + " " + GetTextField("Add/Remove").ToLower()
                );
            }
            else
            {
                ...
                Owner.SendMessage(
                    Data.GetData(Owner).SystemC,
                    General.Local(150) + " " + GetTextField("Add/Remove").ToLower()
                );
            }
