...
namespace Knives.Chat3
{
    ...
    public class Filter
    {
        ...
        public static string FilterText(Mobile m, string s, bool punish)
        {
                    ...
                    if (index >= 0)
                    {
                        if (m.AccessLevel == AccessLevel.Player && punish)
                        {
                            if (++Data.GetData(m).Warnings <= Data.FilterWarnings)
                                m.SendMessage(
                                    Data.GetData(m).SystemC,
                                    General.Local(111) + " " + filter
                                );
                            else
                            {
                        ...
                        subThree = s.Substring(
                            index + filter.Length,
                            s.Length - filter.Length - index
                        );
            ...
            {
                Errors.Report(General.Local(176));
            }
