...
namespace Knives.Chat3
{
    ...
    public class IrcConnection
    {
        ...
        {
            get { return c_Connecting; }
        }
        public bool Connected
        {
            get { return c_Connected; }
        }
        public bool HasMoreAttempts
        {
            get { return c_Attempts <= Data.IrcMaxAttempts; }
        }
        ...
        public string Status
        {
            get
            {
                return "Server: RunUO 2.0   Creatures: "
                    + World.Mobiles.Count
                    + "   Items: "
                    + World.Items.Count
                    + "   Guilds: "
                    + BaseGuild.List.Count
                    + "   Players: "
        ...
        private void ConnectTcp()
        {
            try
            {
                c_Tcp = new TcpClient(Data.IrcServer, Data.IrcPort);
            }
            catch
            {
        ...
        private void ConnectStream()
        {
            ...
            {
                c_Connecting = true;
                c_ConnectTimer = Server.Timer.DelayCall(
                    TimeSpan.FromSeconds(30),
                    new Server.TimerCallback(TimerFail)
                );
        ...
        private void HandleInput(string str)
        {
            try
            {
                ...
                if (str.IndexOf("353") != -1)
                {
                    ...
                    int index =
                        str.ToLower().IndexOf(Data.IrcRoom.ToLower()) + Data.IrcRoom.Length + 2;
                ...
                if (str.IndexOf("PRIVMSG") != -1)
                {
                    ...
                    string parThree = str.Substring(
                        str.IndexOf("!") + 1,
                        str.Length - str.IndexOf("!") - (str.Length - str.IndexOf("PRIVMSG")) - 1
                    );
                    ...
                    {
                        c_NextStatus = DateTime.Now + TimeSpan.FromSeconds(15);
                        s_Connection.SendMessage(
                            String.Format("PRIVMSG {0} : {1}", Data.IrcRoom, Status)
                        );
                        BroadcastSystem(Status);
                    }
                }
