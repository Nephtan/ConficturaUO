...
namespace Knives.Chat3
{
    ...
    public class Notification
    {
        ...
        {
            get { return c_AntiMacro; }
            set { c_AntiMacro = value; }
        }
        public TimeSpan Recur
        {
            get { return c_Recur; }
            set
            {
        ...
        private void StartNotify()
        {
            ...
            if (c_Recur != TimeSpan.Zero && Data.Notifications.Contains(this))
                c_Timer = Timer.DelayCall(
                    c_Recur + TimeSpan.FromSeconds(Utility.Random(20)),
                    new TimerCallback(Notify)
                );
        }
        ...
        private class NotAlertGump : GumpPlus
        {
                ...
                : base(m, 300, 50)
            {
                ...
                if (c_Not.AntiMacro)
                    c_Timer = Timer.DelayCall(
                        TimeSpan.FromSeconds(Data.AntiMacroDelay),
                        new TimerCallback(AntiMacro)
                    );
            }
                    ...
                    if (data.Mobile.AccessLevel >= AccessLevel.GameMaster)
                        data.AddMessage(
                            new Message(
                                data.Mobile,
                                General.Local(277),
                                Owner.RawName + General.Local(278),
                                MsgType.System
                            )
                        );
            }
