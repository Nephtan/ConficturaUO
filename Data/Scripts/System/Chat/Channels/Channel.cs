    ...
    public class Channel
    {
        ...
        public static void Register(Channel c)
        {
            foreach (string str in c.Commands)
                RUOVersion.AddCommand(
                    str,
                    AccessLevel.Player,
                    new ChatCommandHandler(ChannelCommand)
                );
        }
        ...
        public static void Save()
        {
            try
            {
                ...
                GenericWriter writer = new BinaryFileWriter(
                    Path.Combine(General.SavePath, "Channels.bin"),
                    true
                );
                    ...
                    for (int i = 0; i < count; ++i)
                    {
                        c =
                            Activator.CreateInstance(
                                ScriptCompiler.FindTypeByFullName(reader.ReadString())
                            ) as Channel;
                        if (c == null)
                        {
                            c = new Channel();
        ...
        {
            get { return c_Commands; }
        }
        public ArrayList History
        {
            get { return c_History; }
        }
        public int DefaultC
        {
            get { return c_DefaultC; }
        ...
        public bool NewChars
        {
            get { return c_NewChars; }
        ...
        {
            get { return c_ShowStaff; }
            set { c_ShowStaff = value; }
        }
        public bool Enabled
        {
            get { return c_Enabled; }
            set { c_Enabled = value; }
        }
        ...
        public virtual bool CanChat(Mobile m, bool say)
        {
            ...
            {
                if (say)
                    m.SendMessage(Data.GetData(m).SystemC, General.Local(213));
                return false;
            }
            ...
            {
                if (say)
                    m.SendMessage(Data.GetData(m).SystemC, General.Local(260));
                return false;
            }
            ...
            {
                if (say)
                    m.SendMessage(Data.GetData(m).SystemC, General.Local(33));
                return false;
            }

            if (
                c_Style == ChatStyle.Regional
                && (m.Region == null || Server.Misc.Worlds.GetRegionName(m.Map, m.Location) == "")
            )
            {
                if (say)
                    m.SendMessage(Data.GetData(m).SystemC, General.Local(35));
                return false;
            }
            ...
            if (Data.LogChat)
                Logging.LogChat(
                    String.Format(
                        ...
                        c_Name,
                        (
                            c_Style == ChatStyle.Regional && m.Region != null
                                ? "-" + Server.Misc.Worlds.GetRegionName(m.Map, m.Location)
                                : ""
                        ),
                        m.RawName,
                ...
                if (c_Mobiles.Contains(data.Mobile) && !data.Ignores.Contains(m))
                {
                    ...
                    data.Mobile.SendMessage(
                        m.AccessLevel == AccessLevel.Player
                            ? ColorFor(data.Mobile)
                            : Data.GetData(m).StaffC,
                        String.Format(
                            "<{0}{1}> {2}: {3}",
                            NameFor(m),
                            (
                    ...
                    data.Mobile.AccessLevel >= m.AccessLevel
                    && ((data.GlobalC && !data.GIgnores.Contains(m)) || data.GListens.Contains(m))
                )
                    data.Mobile.SendMessage(
                        data.GlobalCC,
                        String.Format(
        ...
        public virtual ArrayList BuildList(Mobile m)
        {
            ...
            foreach (Mobile tolist in new ArrayList(c_Mobiles))
            {
                ...
                if (
                    Data.GetData(tolist).Status == OnlineStatus.Hidden
                    && tolist.AccessLevel >= m.AccessLevel
                )
                    continue;
