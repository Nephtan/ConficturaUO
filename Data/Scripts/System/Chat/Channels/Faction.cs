...
namespace Knives.Chat3
{
    public class Faction : Channel
    {
        ...
        public override bool CanChat(Mobile m, bool say)
        {
            ...
            {
                if (say)
                    m.SendMessage(Data.GetData(m).SystemC, General.Local(37));
                return false;
            }
        ...
        protected override void Broadcast(Mobile m, string msg)
        {
            foreach (Data data in Data.Datas.Values)
            {
                if (
                    ...
                    && General.FactionName(data.Mobile) == General.FactionName(m)
                )
                    ...
                    && ((data.GlobalF && !data.GIgnores.Contains(m)) || data.GListens.Contains(m))
                )
                    data.Mobile.SendMessage(
                        data.GlobalFC,
