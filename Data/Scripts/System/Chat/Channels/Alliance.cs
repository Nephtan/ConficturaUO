...
namespace Knives.Chat3
{
    public class Alliance : Channel
    {
        ...
        public override bool CanChat(Mobile m, bool say)
        {
            ...
            {
                if (say)
                    m.SendMessage(Data.GetData(m).SystemC, General.Local(36));
                return false;
            }
        ...
        protected override void Broadcast(Mobile m, string msg)
        {
            foreach (Data data in Data.Datas.Values)
            {
                ...
                if (
                    ...
                    && ((data.GlobalG && !data.GIgnores.Contains(m)) || data.GListens.Contains(m))
                )
                    data.Mobile.SendMessage(
                        data.GlobalGC,
                        String.Format("(Alliance) <{0}> {1}: {2}", NameFor(m), m.RawName, msg)
                    );
                else if (
                    IsIn(data.Mobile)
                    ...
                    && (
                        ...
                        || ((Server.Guilds.Guild)data.Mobile.Guild).Allies.Contains(
                            (Server.Guilds.Guild)m.Guild
                        )
                    )
                )
                    data.Mobile.SendMessage(
                        ...
                        String.Format(
                            ...
                            NameFor(m),
                            (
                                Style == ChatStyle.Regional && m.Region != null
        ...
        public override ArrayList BuildList(Mobile m)
        {
            ...
            foreach (Mobile mob in new ArrayList(list))
                if (
                    ...
                    || (
                        mob.Guild != m.Guild
                        && !((Server.Guilds.Guild)mob.Guild).Allies.Contains(
                            (Server.Guilds.Guild)m.Guild
                        )
                    )
                )
