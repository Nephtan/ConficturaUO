...
namespace Joeku.MOTD
{
    public class MOTD_Gump : Gump
    {
        ...
        public MOTD_Gump(Mobile user, bool help, int index, int origin)
            : base(50, 50)
        {
            m_Origin = origin;
            string color = "#ddbc4b";
            ...
            AddHtml(
                ...
                291,
                20,
                @"<BODY><BASEFONT Color=" + color + ">MESSAGE OF THE DAY</BASEFONT></BODY>",
                (bool)false,
                (bool)false
            );
            ...
            AddHtml(
                ...
                141,
                20,
                @"<BODY><BASEFONT Color=" + color + ">SHOW AT LOGIN</BASEFONT></BODY>",
                (bool)false,
                (bool)false
            );
        }
...
namespace Joeku.MOTD
{
    public class MOTD_Main
    {
        ...
        public static MOTD_Info[] Info = new MOTD_Info[] { new MOTD_Info("News"), };
        ...
        public static void Initialize()
        {
            ...
            CommandSystem.Register(
                "MOTD",
                AccessLevel.Player,
                new CommandEventHandler(MOTD_Utility.EventSink_OnCommand)
            );
...
namespace Joeku.MOTD
{
    public class MOTD_Utility
    {
        ...
        public static void SendGump(Mobile mob)
        {
            SendGump(mob, false, 0, 0);
        }

        public static void SendGump(Mobile mob, bool help)
        {
            SendGump(mob, help, 0, 0);
        }
        ...
        public static void CheckFiles(bool checkTime)
        {
            ...
            for (int i = 0; i < MOTD_Main.Info.Length; i++)
            {
                path = Path.Combine(
                    MOTD_Main.FilePath,
                    String.Format("{0}.txt", MOTD_Main.Info[i].Name)
                );
                if (
                    !checkTime
                    || (checkTime && File.GetLastWriteTime(path) > MOTD_Main.Info[i].LastWriteTime)
                )
                {
