    ...
    public class Reports
    {
        ...
        public static void Initialize()
        {
            ...
            Timer.DelayCall(
                ...
                TimeSpan.FromMinutes(0.5),
                new TimerCallback(CheckRegenerate)
            );
        }
        ...
        public static PersistableObject[] CompileSkillReports()
        {
            ...
            {
                CompileSkillChart(distribs),
                CompileSkillReport(distribs)
            };
        }
        ...
        public static Report CompileFactionTownReport()
        {
            ...
            for (int i = 0; i < towns.Count; ++i)
            {
                ...
                item.Values.Add(
                    town.Owner == null ? "Neutral" : town.Owner.Definition.FriendlyName
                );
...
namespace Server.Engines.Reports
{
    ...
    public class BarGraphRenderer : ChartRenderer
    {
        ...
        public void CollectDataPoints(string[] labels, string[] values)
        {
            if (labels.Length == values.Length)
            {
                for (int i = 0; i < labels.Length; i++)
                {
                    ...
                    DataPoints.Add(
                        new DataItem(shortLbl, labels[i], temp, 0.0f, 0.0f, GetColor(i))
                    );
                    ...
                    if (_displayLegend)
                    {
                        ...
                        float currentWidth = CalculateImgFontWidth(
                            currentLbl,
                            _legendFontSize,
                            FontFamily
                        );
                            ...
                            using (Pen lightPen = new Pen(Color.FromArgb(128, _fontColor)))
                            {
                                ...
                                for (int i = 0; i < _regions.Length; ++i)
                                {
                                    ...
                                    RectangleF rc = new RectangleF(
                                        _xOrigin + (reg.m_RangeFrom * labelWidth),
                                        _yOrigin,
                                        (reg.m_RangeTo - reg.m_RangeFrom + 1) * labelWidth,
                                        _graphHeight
                                    );
                                    ...
                                    using (
                                        SolidBrush brsh = new SolidBrush(
                                            Color.FromArgb(48, GetColor(i))
                                        )
                                    )
                    ...
                    using (SolidBrush whiteBrsh = new SolidBrush(Color.FromArgb(128, Color.White)))
                    {
                        foreach (DataItem item in DataPoints)
                        {
                            using (SolidBrush barBrush = new SolidBrush(item.ItemColor))
                            {
                                ...
                                if (_renderMode == BarGraphRenderMode.Lines)
                                {
                                    linePoints[pointIndex++] = new PointF(
                                        _xOrigin + item.StartPos + (_barWidth / 2),
                                        itemY
                                    );
                                }
                                else if (_renderMode == BarGraphRenderMode.Bars)
                                {
                                    ...
                                    graph.FillPolygon(
                                        barBrush,
                                        new PointF[] { pts[2], pts[3], pts[6], pts[5] }
                                    );

                                    using (
                                        SolidBrush ltBrsh = new SolidBrush(
                                        ...
                                        graph.FillPolygon(
                                            ltBrsh,
                                            new PointF[] { pts[0], pts[2], pts[5], pts[4] }
                                        );

                                    using (
                                        SolidBrush drkBrush = new SolidBrush(
                                            System.Windows.Forms.ControlPaint.Dark(
                                                item.ItemColor,
                                                0.05f
                                            )
                                        )
                                        ...
                                        graph.FillPolygon(
                                            drkBrush,
                                            new PointF[] { pts[0], pts[1], pts[3], pts[2] }
                                        );
                                    ...
                                    if (_displayBarData && (i % _interval) == 0)
                                    {
                                        ...
                                        float startX =
                                            _xOrigin + (i * sectionWidth) + (sectionWidth / 2); // This draws the value on center of the bar
                                        float startY = itemY - 2f - valFont.Height; // Positioned on top of each bar by 2 pixels
                                        RectangleF recVal = new RectangleF(
                                            startX - ((sectionWidth * _interval) / 2),
                                            startY,
                                            sectionWidth * _interval,
                                            valFont.Height
                                        );
                                        SizeF sz = graph.MeasureString(
                                            item.Value.ToString("#,###.##"),
                                            valFont,
                                            recVal.Size,
                                            sfFormat
                            ...
                            using (Pen linePen = new Pen(Color.FromArgb(40, _fontColor), 0.8f))
                            {
                                for (int j = 0; j < linePoints.Length; ++j)
                                {
                                    ...
                                    if (_displayBarData && (j % _interval) == 0)
                                    {
                                        ...
                                        float sectionWidth = (_barWidth + _spaceBtwBars);
                                        float startX =
                                            _xOrigin + (j * sectionWidth) + (sectionWidth / 2); // This draws the value on center of the bar
                                        float startY = itemY - 2f - valFont.Height; // Positioned on top of each bar by 2 pixels
                                        RectangleF recVal = new RectangleF(
                                            startX - ((sectionWidth * _interval) / 2),
                                            startY,
            ...
            try
            {
                ...
                if (_yTitle != null)
                {
                    ...
                    graph.DrawString(
                        ...
                        lblFont,
                        brs,
                        new RectangleF(0.0f, _yOrigin, 20.0f, _graphHeight),
                        sfVLabel
                    );
                    lblFont.Dispose();
                }
                ...
                RectangleF recVLabel = new RectangleF(
                    0,
                    _yOrigin - 2 * _spacer - lblFont.Height,
                    _xOrigin * 2,
                    lblFont.Height
                );
                sfVLabel.Alignment = StringAlignment.Center;
                ...
                using (Pen smallPen = new Pen(Color.FromArgb(96, _fontColor), 0.8f))
                {
                    for (int i = 0; i < _yTickCount; i++)
                    {
                        ...
                        RectangleF lblRec = new RectangleF(
                            _spacer + fo - 6,
                            labelY,
                            _maxTickValueWidth,
                            lblFont.Height
            ...
            try
            {
                    ...
                    graph.DrawString(
                        ...
                        new RectangleF(
                            ...
                            _yOrigin
                                + _graphHeight
                                + 14.0f
                                + (_renderMode == BarGraphRenderMode.Bars ? 10.0f : 0.0f)
                                + ((DataPoints.Count / _interval) > 24 ? 16.0f : 0.0f),
                            _graphWidth,
                            20.0f
                        ),
                        lblFormat
                ...
                foreach (DataItem item in DataPoints)
                {
                    if ((i % _interval) == 0)
                    {
                        ...
                        RectangleF recLbl = new RectangleF(
                            currentX - ((labelWidth * _interval) / 2),
                            currentY + of,
                            labelWidth * _interval,
                            lblFont.Height * 2
                        );
        ...
        private void CalculateGraphDimension()
        {
            ...
            _graphHeight =
                _totalHeight - _topBuffer - _bottomBuffer - (_xTitle == null ? 0.0f : 20.0f); // Buffer spaces are used to print labels
        ...
        private void CalculateTickAndMax()
        {
            ...
            if (_maxValue != 0.0f)
            {
                ...
                tempMax = Convert.ToSingle(
                    Math.Ceiling(_maxValue / Math.Pow(10, exp)) * Math.Pow(10, exp)
                );
            }
            else
                tempMax = 1.0f;
            ...
            _yTickValue = Convert.ToSingle(
                Math.Ceiling(_yTickValue / Math.Pow(10, expTick)) * Math.Pow(10, expTick)
            );
    ...
    public class HtmlRenderer
    {
        ...
        public void Upload()
        {
            ...
            using (StreamWriter op = new StreamWriter(filePath))
            {
                ...
                op.WriteLine(
                    "cd \"{0}\"",
                    (m_Type == "staff" ? FtpStaffDirectory : FtpStatsDirectory)
                );
        ...
        public void RenderFull(HtmlTextWriter html)
        {
            ...
            html.Write(
                "Snapshot taken at {0:d} {0:t}. All times are {1}.",
                m_TimeStamp,
                tz.StandardName
            );
            html.RenderEndTag();
        ...
        public void RenderSingle(PersistableObject obj)
        {
            string filePath = Path.Combine(
                m_OutputDirectory,
                SafeFileName(FindNameFrom(obj)) + ".html"
            );
        ...
        private void RenderPieChart(PieChart chart, HtmlTextWriter html)
        {
            ...
            html.AddAttribute(
                HtmlAttr.Onclick,
                String.Format(
                    "javascript:window.open('{0}.html','ChildWindow','width={1},height={2},resizable=no,status=no,toolbar=no')",
                    SafeFileName(FindNameFrom(chart)),
                    bmp.Width + 30,
                    bmp.Height + 80
                )
            );
        ...
        private void RenderBarGraph(BarGraph graph, HtmlTextWriter html)
        {
            ...
            html.AddAttribute(
                ...
                String.Format(
                    "javascript:window.open('{0}.html','ChildWindow','width={1},height={2},resizable=no,status=no,toolbar=no')",
                    SafeFileName(FindNameFrom(graph)),
                    bmp.Width + 30,
                    bmp.Height + 80
                )
            );
    ...
    public class PieChartRenderer : ChartRenderer
    {
        ...
        public void CollectDataPoints(string[] xValues, string[] yValues)
        {
            ...
            for (int i = 0; i < xValues.Length; i++)
            {
                ...
                _chartItems.Add(
                    new DataItem(xValues[i], xValues.ToString(), ftemp, 0, 0, Color.AliceBlue)
                );
                _total += ftemp;
            }
            ...
            try
            {
                    ...
                    try
                    {
                        ...
                        if (_showPercents && item.SweepSize > 10)
                        {
                            ...
                            grp.DrawString(
                                ...
                                fnt,
                                Brushes.Gray,
                                new RectangleF(px - 30 - 1, py - 20, 60, 40),
                                sfp
                            );
                            ...
                            grp.DrawString(
                                ...
                                fnt,
                                Brushes.Gray,
                                new RectangleF(px - 30, py - 20 - 1, 60, 40),
                                sfp
                            );
                            ...
                            grp.DrawString(
                                ...
                                fnt,
                                Brushes.White,
                                new RectangleF(px - 30, py - 20, 60, 40),
                                sfp
                            );
                        }
                        ...
                        grp.FillRectangle(
                            brs,
                            perimeter + _bufferSpace,
                            i * _legendFontHeight + 15,
                            10,
                            10
                        );
                        grp.DrawRectangle(
                ...
                using (Font fntb = new Font(_legendFontStyle, _legendFontSize, FontStyle.Bold))
                {
                    grp.DrawString(
                        ...
                        fntb,
                        Brushes.Black,
                        perimeter + _bufferSpace + 30,
                        (_chartItems.Count + 1) * _legendFontHeight,
                        sf
                    );
                    grp.DrawString(
                        _total.ToString("#,###.##"),
...
namespace Server.Engines.Reports
{
    ...
    public class ObjectCollection : System.Collections.CollectionBase
    {
        ...
        {
            get { return ((Server.Engines.Reports.PersistableObject)(this.List[index])); }
    ...
    public sealed class XmlPersistanceWriter : PersistanceWriter
    {
        ...
        public override void SetDateTime(string key, DateTime value)
        {
            if (value != DateTime.MinValue)
                m_Xml.WriteAttributeString(
                    key,
                    XmlConvert.ToString(value, XmlDateTimeSerializationMode.Local)
                );
        }
        ...
        public override void Close()
        {
            ...
            try
            {
                ...
                if (File.Exists(m_RealFilePath))
                {
                    ...
                    {
                        // If the renamed file already exists, delete it
                        File.Delete(renamed);
                    }
                ...
                // Move the temporary file to the real file path
                File.Move(m_TempFilePath, m_RealFilePath);

                // If a file was renamed, delete it
...
namespace Server.Engines.Reports
{
    ...
    public class BarGraph : Chart
    {
        ...
        public static readonly PersistableType ThisTypeID = new PersistableType(
            "bg",
            new ConstructCallback(Construct)
        );
        ...
        public static BarGraph DailyAverage(
        ...
        )
        {
            ...
            BarGraph barGraph = new BarGraph(
                "Hourly average " + valueName,
                "graphs_" + valueName.ToLower() + "_avg",
                10,
                "Time",
                valueName,
            ...
            for (int i = 7; i <= totals.Length + 7; ++i)
            {
                ...
                if (counts[i % totals.Length] == 0)
                    ...
                    val =
                        (totals[i % totals.Length] + (counts[i % totals.Length] / 2))
                        / counts[i % totals.Length];
        ...
        {
            BarGraph barGraph = new BarGraph(
                "Growth of " + valueName + " over time",
                "graphs_" + valueName.ToLower() + "_growth",
                10,
                "Time",
                valueName,
                BarGraphRenderMode.Lines
            ...
            for (int i = 0; i < history.Snapshots.Count; ++i)
            {
                ...
                if (curDate != thisDate && curValu >= 0)
                {
                    ...
                    {
                        regions.Add(
                            new BarRegion(barGraph.Items.Count, barGraph.Items.Count, mnthName)
                        );
                    }
                    else
                    {
        ...
        public static BarGraph OverTime(
            SnapshotHistory history,
            string reportName,
            string valueName,
            int step,
            ...
            for (int i = 0; i < history.Snapshots.Count; ++i)
            {
                ...
                else
                {
                    ...
                    if (region.m_Name == dayName)
                        ...
                        regions.Add(
                            new BarRegion(barGraph.Items.Count, barGraph.Items.Count, dayName)
                        );
                }
...
namespace Server.Engines.Reports
{
    ...
    public class ChartItemCollection : System.Collections.CollectionBase
    {
        ...
        {
            get { return ((Server.Engines.Reports.ChartItem)(this.List[index])); }
            set { this.List[index] = value; }
        }
                ...
                if (m_Account != null)
                {
                    ...
                    if (acct != null)
                    {
                        ...
                        for (int i = 0; i < acct.Length; ++i)
                        {
                            ...
                            if (
                                check != null
                                && (mob == null || check.AccessLevel > mob.AccessLevel)
                            )
                                mob = check;
                        }
...
namespace Server.Engines.Reports
{
    ...
    public class ItemValueCollection : System.Collections.CollectionBase
    {
        ...
        {
            get { return ((Server.Engines.Reports.ItemValue)(this.List[index])); }
            set { this.List[index] = value; }
        }
...
namespace Server.Engines.Reports
{
    ...
    public class PageInfoCollection : System.Collections.CollectionBase
    {
        ...
        {
            get { return ((Server.Engines.Reports.PageInfo)(this.List[index])); }
            set { this.List[index] = value; }
        }
...
namespace Server.Engines.Reports
{
    ...
    public class QueueStatusCollection : System.Collections.CollectionBase
    {
        ...
        {
            get { return ((Server.Engines.Reports.QueueStatus)(this.List[index])); }
            set { this.List[index] = value; }
        }
...
namespace Server.Engines.Reports
{
    ...
    public class ReportColumnCollection : System.Collections.CollectionBase
    {
        ...
        {
            get { return ((Server.Engines.Reports.ReportColumn)(this.List[index])); }
            set { this.List[index] = value; }
        }
...
namespace Server.Engines.Reports
{
    ...
    public class ReportItemCollection : System.Collections.CollectionBase
    {
        ...
        {
            get { return ((Server.Engines.Reports.ReportItem)(this.List[index])); }
            set { this.List[index] = value; }
        }
...
namespace Server.Engines.Reports
{
    ...
    public class ResponseInfoCollection : System.Collections.CollectionBase
    {
        ...
        {
            get { return ((Server.Engines.Reports.ResponseInfo)(this.List[index])); }
            set { this.List[index] = value; }
        }
...
namespace Server.Engines.Reports
{
    ...
    public class SnapshotCollection : System.Collections.CollectionBase
    {
        ...
        {
            get { return ((Server.Engines.Reports.Snapshot)(this.List[index])); }
            set { this.List[index] = value; }
        }
...
namespace Server.Engines.Reports
{
    public class StaffHistory : PersistableObject
    {
        ...
        public void Render(ObjectCollection objects)
        {
            lock (RenderLock)
            {
                ...
                objects.Add(
                    GraphHourlyPages(
                        m_Pages,
                        PageResolution.Handled,
                        "Handled pages by hour",
                ...
                objects.Add(
                    GraphHourlyPages(
                        m_Pages,
                        PageResolution.Deleted,
                        "Deleted pages by hour",
                ...
                objects.Add(
                    GraphHourlyPages(
                        m_Pages,
                        PageResolution.Canceled,
                        "Canceled pages by hour",
                ...
                objects.Add(
                    GraphHourlyPages(
                        m_Pages,
                        PageResolution.Logged,
                        "Logged-out pages by hour",
                ...
                objects.AddRange(
                    (PersistableObject[])ChartTotalPages(
                        staff,
        ...
        {
            return GraphHourlyPages(
                staff.Pages,
                PageResolution.Handled,
                "Average pages handled by " + staff.Display,
                "graphs_" + staff.Account.ToLower() + "_avg"
            );
        }

        private BarGraph GraphHourlyPages(
            PageInfoCollection pages,
            PageResolution res,
            string title,
            string fname
        ...
        private PieChart[] ChartTotalPages(
            StaffInfo[] staff,
            TimeSpan ts,
            string title,
            string fname
        )
        {
            ...
            int countUnres =
                countTotal - (countHandled + countDeleted + countCanceled + countLogged);
