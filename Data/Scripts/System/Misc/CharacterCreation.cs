...
namespace Server.Misc
{
    public class CharacterCreation
    {
        ...
        public static void Initialize()
        {
            ...
            EventSink.CharacterCreated += new CharacterCreatedEventHandler(
                EventSink_CharacterCreated
            );
        }
        ...
        private static void EventSink_CharacterCreated(CharacterCreatedEventArgs args)
        {
            ...
            switch (Utility.RandomMinMax(1, 3))
            {
                case 1:
                    ...
                    newChar.AddItem(torch);
                    torch.OnDoubleClick(newChar);
                    break;
                case 2:
                    Item lamp = new Lantern();
                    newChar.AddItem(lamp);
                    lamp.OnDoubleClick(newChar);
                    break;
                case 3:
                    Item candle = new Candle();
                    newChar.AddItem(candle);
                    candle.OnDoubleClick(newChar);
                    break;
            }
        ...
        public static bool CheckDupe(Mobile m, string name)
        {
            ...
            foreach (Mobile wm in World.Mobiles.Values)
            {
                if (
                    ...
                    && !wm.Deleted
                    && wm is PlayerMobile
                    && Insensitive.Equals(wm.RawName, name)
                ) //Filter Mobiles by PlayerMobile type and do the name check in one go, no need for another list.
        ...
        private static void SetSkills(Mobile m, SkillNameValue[] skills, int prof)
        {
            ...
            for (int i = 0; i < skills.Length; ++i)
            {
                ...
                if (
                    ...
                    && snv.Name != SkillName.Elementalism
                )
                {
