...
namespace Server.Misc
{
    public class ShardPoller : Item
    {
        ...
        private static void EventSink_Login(LoginEventArgs e)
        {
            ...
            Timer.DelayCall(
                ...
                new TimerStateCallback(EventSink_Login_Callback),
                e.Mobile
            );
        }
    ...
    public class ShardPollOption
    {
        ...
        {
            get { return m_LineBreaks; }
        }

        public int Votes
        {
            get { return m_Voters.Length; }
        }
    ...
    public class ShardPollGump : Gump
    {
        ...
        public ShardPollGump(
            Mobile from,
            ShardPoller poller,
            bool editing,
            Queue<ShardPoller> polls
        )
            : base(50, 50)
        {
            m_From = from;
            ...
            if (editing)
            {
                AddHtml(
                    ...
                    294,
                    20,
                    Color(String.Format("{0} total", totalVotes), LabelColor32),
                    false,
                    false
                );
            ...
            for (int i = 0; i < poller.Options.Length; ++i)
            {
                ...
                AddHtml(
                    60,
                    y - (9 * option.LineBreaks),
                    250,
                    18 * option.LineBreaks,
                    Color(text, LabelColor32),
                    false,
                    false
                );
            ...
            if (editing && !isViewingResults)
            {
                ...
                AddHtml(
                    ...
                    250,
                    18,
                    Color("Create new option.", 0x99CC66),
                    false,
                    false
                );
            }
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            if (m_Polls != null && m_Polls.Count > 0)
            {
                ...
                if (poller != null)
                    Timer.DelayCall(
                        TimeSpan.FromSeconds(1.0),
                        new TimerStateCallback(poller.SendQueuedPoll_Callback),
    ...
    public class ShardPollPrompt : Prompt
    {
        ...
        private static Regex m_UrlRegex = new Regex(
            @"\[url(?:=(.*?))?\](.*?)\[/url\]",
