...
namespace Server.Misc
{
    ...
    public class LanguageStatistics
    {
        struct InternationalCode
        {
            ...
            {
                get { return m_Language_LocalName; }
            }
            public string Country_LocalName
            {
                get { return m_Country_LocalName; }
            }
            ...
            public string GetName()
            {
                ...
                if (m_HasLocalInfo)
                {
                    s = String.Format(
                        "{0}‎ - {1}",
                        DefaultLocalNames ? m_Language_LocalName : m_Language,
                        DefaultLocalNames ? m_Country_LocalName : m_Country
                    );

                    if (ShowAlternatives)
                        s += String.Format(
                            "‎ 【{0}‎ - {1}‎】",
                            DefaultLocalNames ? m_Language : m_Language_LocalName,
                            DefaultLocalNames ? m_Country : m_Country_LocalName
                        );
                }
                else
                {
        ...
        private static InternationalCode[] InternationalCodes =
        {
            ...
            new InternationalCode(
                "ESP",
                "Spanish",
                "Spain (Traditional Sort)",
                "Español",
                "España (tipo tradicional)"
            ),
        ...
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(LanguageStatistics_OnCommand)
            );
        }
        ...
        {
            Dictionary<string, InternationalCodeCounter> ht =
                new Dictionary<string, InternationalCodeCounter>();
            ...
            using (StreamWriter writer = new StreamWriter("languages.txt"))
            {
                ...
                writer.WriteLine(
                    String.Format(
                        "Language statistics. Numbers show how many {0} use the specified language.",
                        CountAccounts ? "accounts" : "playermobile"
                    )
                );
                writer.WriteLine(
                    "===================================================================================================="
                ...
                foreach (InternationalCodeCounter c in list)
                    writer.WriteLine(
                        String.Format("{0}‎ : {1}", GetFormattedInfo(c.Code), c.Count)
                    );
        ...
        {
            public static readonly InternationalCodeComparer Instance =
                new InternationalCodeComparer();
