...
namespace Server.Misc
{
    ...
    {
        private static Dictionary<Map, List<Weather>> m_WeatherByFacet =
            new Dictionary<Map, List<Weather>>();
        ...
        public static void Initialize()
        {
            ...
            AddWeather(
                ...
                100,
                0,
                new Rectangle2D(298, 3461, 283, 239),
                new Rectangle2D(322, 3689, 244, 114),
                new Rectangle2D(6466, 3044, 553, 878)
            );
            AddWeather(
                Map.Lodor,
                -15,
                100,
                0,
                new Rectangle2D(1965, 700, 177, 376),
                new Rectangle2D(2035, 233, 1130, 676),
                new Rectangle2D(2942, 311, 304, 650),
                new Rectangle2D(3103, 943, 162, 123),
                new Rectangle2D(3229, 187, 666, 989),
                new Rectangle2D(3885, 343, 575, 844),
                new Rectangle2D(4235, 1145, 249, 154)
            );
            AddWeather(
                Map.Lodor,
                +15,
                25,
                0,
                new Rectangle2D(5154, 1099, 144, 310),
                new Rectangle2D(6851, 115, 209, 214)
            );
            ...
            AddWeather(
                ...
                100,
                0,
                new Rectangle2D(4514, 812, 333, 273),
                new Rectangle2D(4233, 1076, 880, 339)
            );
            AddWeather(
                Map.IslesDread,
                -15,
                100,
                0,
                new Rectangle2D(10, 10, 544, 551),
                new Rectangle2D(255, 520, 288, 162)
            );
            AddWeather(
                Map.SavagedEmpire,
                -15,
                100,
                0,
                new Rectangle2D(134, 4, 283, 128),
                new Rectangle2D(411, 9, 206, 138),
                new Rectangle2D(752, 4, 205, 110),
                new Rectangle2D(908, 8, 188, 147),
                new Rectangle2D(1075, 3, 92, 291)
            );
            AddWeather(
                Map.SavagedEmpire,
                +15,
                25,
                0,
                new Rectangle2D(1000, 1866, 195, 135),
                new Rectangle2D(0, 2048, 182, 416)
            );
            ...
            for (int i = 0; i < 15; ++i)
                AddDynamicWeather(
                    ...
                    100,
                    100,
                    new Rectangle2D(130, 0, 1032, 1794)
                );
        }
        ...
        public static void AddDynamicWeather(
            Map map,
            int temperature,
            int chanceOfPercipitation,
            int chanceOfExtremeTemperature,
            int moveSpeed,
            int width,
            int height,
            Rectangle2D bounds
        ...
        {
            get { return m_MoveAngleX; }
            set { m_MoveAngleX = value; }
        }
        public int MoveAngleY
        {
            get { return m_MoveAngleY; }
        ...
        )
        {
            ...
            Timer.DelayCall(
                TimeSpan.FromSeconds(
                    (0.2 + (Utility.RandomDouble() * 0.8)) * interval.TotalSeconds
                ),
                interval,
        ...
        public virtual void Reposition()
        {
            ...
            {
                area = new Rectangle2D(
                    m_Bounds.X + Utility.Random(m_Bounds.Width - width),
                    m_Bounds.Y + Utility.Random(m_Bounds.Height - height),
                    width,
                    height
                );
        ...
        public virtual void MoveForward()
        {
            ...
            for (int i = 0; i < 5; ++i) // try 5 times to find a valid spot
            {
                ...
                Rectangle2D newArea = new Rectangle2D(
                    oldArea.X + xOffset,
                    oldArea.Y + yOffset,
                    oldArea.Width,
                    oldArea.Height
                ...
                if (
                    !CheckWeatherConflict(m_Facet, this, newArea)
                    && CheckContains(m_Bounds, newArea)
                )
                {
                    m_Area[0] = newArea;
                    break;
            ...
            if (m_Active)
            {
                ...
                for (int i = 0; i < states.Count; ++i)
                {
                    ...
                    if (weatherPacket == null)
                        weatherPacket = Packet.Acquire(
                            new Server.Network.Weather(type, density, temperature)
                        );
