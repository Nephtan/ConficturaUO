...
namespace Server
{
    public class AssemblyEmitter
    {
        ...
        public void Save()
        {
            m_AssemblyBuilder.Save(String.Format("{0}.dll", m_AssemblyName.ToLower()));
        }
    }
    ...
    public class MethodEmitter
    {
...
#else
                Type[] ifaces = active.FindInterfaces(
                    delegate(Type type, object obj)
                    {
                        return (type.IsGenericType)
                            && (type.GetGenericTypeDefinition() == typeof(IComparable<>))
        ...
        public void FinishCall()
        {
            ...
            if (
                (call.type.IsValueType || call.type.IsByRef)
                && call.method.DeclaringType != call.type
            )
