        ...
        public static string ComputeTitle(Mobile beholder, Mobile beheld)
        {
            ...
            if (beheld is PlayerMobile && ((PlayerMobile)beheld).DisplayChampionTitle)
            {
                ...
                if (info.Harrower > 0)
                    title.AppendFormat(
                        ": {0} of Evil",
                        HarrowerTitles[Math.Min(HarrowerTitles.Length, info.Harrower) - 1]
                    );
                else
                {
                    int highestValue = 0,
                    ...
                    if (offset > 0)
                    {
                        ChampionSpawnInfo champInfo = ChampionSpawnInfo.GetInfo(
                            (ChampionSpawnType)highestType
                        );
                        title.AppendFormat(
                            ": {0} of the {1}",
