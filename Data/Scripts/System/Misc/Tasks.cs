...
namespace Server.Items
{
    public class TaskManager150Min : Item
    {
        ...
        public TaskManager150Min(Serial serial)
            : base(serial) { }
        ...
        public class TaskTimer : Timer
        {
            ...
            public TaskTimer(Item task)
                : base(TimeSpan.FromMinutes(150.0))
            {
...
namespace Server.Items
{
    public class TaskManager200Min : Item
    {
        ...
        public TaskManager200Min(Serial serial)
            : base(serial) { }
        ...
        public class TaskTimer : Timer
        {
            ...
            public TaskTimer(Item task)
                : base(TimeSpan.FromMinutes(200.0))
            {
                Priority = TimerPriority.OneMinute;
                i_item = task;
            }
                ...
                if (item is PremiumSpawner)
                {
                    ...
                    if (spawner.SpawnID == 8888)
                    {
                        ...
                        foreach (NetState state in NetState.Instances)
                        {
                            ...
                            if (
                                m is PlayerMobile
                                && m.InRange(spawner.Location, (spawner.HomeRange + 20))
                            )
                            {
                                reconfigure = false;
                            }
...
namespace Server.Items
{
    public class TaskManager250Min : Item
    {
        ...
        public TaskManager250Min(Serial serial)
            : base(serial) { }
        ...
        public class TaskTimer : Timer
        {
            ...
            public TaskTimer(Item task)
                : base(TimeSpan.FromMinutes(250.0))
            {
                Priority = TimerPriority.OneMinute;
                i_item = task;
            }
...
namespace Server.Items
{
    public class TaskManagerDaily : Item
    {
        ...
        public TaskManagerDaily(Serial serial)
            : base(serial) { }
        ...
        public static void RunThis(Item item)
        {
            ...
            foreach (Mobile creature in World.Mobiles.Values)
            {
                ...
                else if (creature.WhisperHue == 999 || creature.WhisperHue == 911)
                {
                    ...
                    {
                        if (
                            creature is Adventurers
                            || creature is WanderingHealer
                            || creature is Courier
                            || creature is Syth
                            || creature is Jedi
                        )
            ...
            for (int i = 0; i < healers.Count; ++i)
            {
                ...
                if (!(healer is Citizens))
                {
                    Effects.SendLocationParticles(
                        EffectItem.Create(healer.Location, healer.Map, EffectItem.DefaultDuration),
                        0x3728,
                        10,
                        10,
                        2023
            ...
            foreach (Mobile drunk in World.Mobiles.Values)
                if (
                    drunk is AdventurerWest
                    || drunk is AdventurerSouth
                    || drunk is AdventurerNorth
                    || drunk is AdventurerEast
                    || drunk is TavernPatronWest
                    || drunk is TavernPatronSouth
                    || drunk is TavernPatronNorth
                    || drunk is TavernPatronEast
                )
                {
                    if (drunk != null)
                    {
...
namespace Server.Items
{
    public class TaskManager : Item
    {
        ...
        public TaskManager(Serial serial)
            : base(serial) { }
        ...
        public static void RunThis()
        {
            ...
            {
                if (
                    vendor is BaseVendor
                    && vendor.WhisperHue != 911
                    && vendor.WhisperHue != 999
                    && !(vendor is PlayerVendor)
                    && !(vendor is PlayerBarkeeper)
                )
                {
                    vendors.Add(vendor);
                }
                ...
                else if (vendor is Citizens && vendor.Fame > 0)
                {
                    ...
                    vendor is BaseCreature
                    && (
                        ((BaseCreature)vendor).WhisperHue == 999
                        || ((BaseCreature)vendor).WhisperHue == 999
                    )
                    && vendor.Location == ((BaseCreature)vendor).Home
                )
                {
            ...
            foreach (Item item in World.Items.Values)
                if (
                    ...
                    || item is HiddenTrap
                    || item is DungeonChest
                    || item is HiddenChest
                    || item is AltarGodsEast
                    || item is AltarGodsSouth
                    || item is AltarShrineEast
                    || item is AltarShrineSouth
                    || item is AltarStatue
                    || item is AltarSea
                    || item is AltarDryad
                    || item is AltarEvil
            ...
            for (int i = 0; i < targets.Count; ++i)
            {
                ...
                if (item is MushroomTrap)
                {
                    ...
                    {
                        case 1:
                            item.Name = "strange mushroom";
                            break;
                        case 2:
                            item.Name = "weird mushroom";
                            break;
                        case 3:
                            item.Name = "odd mushroom";
                            break;
                        case 4:
                            item.Name = "curious mushroom";
                            break;
                        case 5:
                            item.Name = "peculiar mushroom";
                            break;
                        case 6:
                            item.Name = "bizarre mushroom";
                            break;
                    }
                }
