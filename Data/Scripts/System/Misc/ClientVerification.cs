    ...
    public class ClientVerification
    {
        ...
        public static void Initialize()
        {
            EventSink.ClientVersionReceived += new ClientVersionReceivedHandler(
                EventSink_ClientVersionReceived
            );
        ...
        private static void EventSink_ClientVersionReceived(ClientVersionReceivedArgs e)
        {
            ...
            )
            {
                kickMessage = String.Format(
                    "This server requires your client version be at least {0}.",
                    Required
                );
            }
        ...
        private static void SendAnnoyGump(Mobile m)
        {
            if (m.NetState != null && m.NetState.Version < Required)
            {
                Gump g = new WarningGump(
                    ...
                    30720,
                    String.Format(
                        "Your client is out of date. Please update your client.<br>This server recommends that your client version be at least {0}.<br> <br>You are currently using version {1}.<br> <br>To patch, run UOPatch.exe inside your client folder.",
                        Required,
                        m.NetState.Version
                    ),
                    0xFFC000,
                    ...
                    delegate(Mobile mob, bool selection, object o)
                    {
                        ...
                        Timer.DelayCall(
                            TimeSpan.FromMinutes(Utility.Random(5, 15)),
                            delegate
                            {
                                SendAnnoyGump(m);
                            }
