...
namespace Server.PathAlgorithms.FastAStar
{
    ...
    public class FastAStarAlgorithm : PathAlgorithm
    {
        ...
        public override Direction[] Find(Mobile m, Map map, Point3D start, Point3D goal)
        {
            ...
            m_Nodes[m_OpenList].total = Heuristic(
                start.X - m_xOffset,
                start.Y - m_yOffset,
                start.Z
            );
...
namespace Server.Movement
{
    public class MovementImpl : IMovementImpl
    {
        ...
        )
        {
            ...
            for (int i = 0; i < items.Count; ++i)
            {
                ...
                if ((flags & ImpassableSurface) != 0) // Impassable || Surface
                {
                    ...
                    if (
                        ignoreSpellFields
                        && (itemID == 0x82 || itemID == 0x3946 || itemID == 0x3956)
                    )
                        continue;
        ...
        )
        {
            ...
            bool landBlocks =
                (
                    TileData.LandTable[landTile.ID & TileData.MaxLandValue].Flags
                    & TileFlag.Impassable
                ) != 0;
            ...
            if (
                landBlocks
                && canSwim
                && (TileData.LandTable[landTile.ID & TileData.MaxLandValue].Flags & TileFlag.Wet)
                    != 0
            ...
            bool ignoreDoors = (
                m_AlwaysIgnoreDoors || !m.Alive || m.Body.BodyID == 0x3DB || m.IsDeadBondedPet
            );
            ...
            if (considerLand && !landBlocks && stepTop >= landZ)
            {
                ...
                if (
                    shouldCheck && IsOk(ignoreDoors, ignoreSpellFields, ourZ, testTop, tiles, items)
                )
                {
                    newZ = ourZ;
                    moveIsOk = true;
            ...
            if (checkDiagonals)
            {
                ...
                for (int i = 0; i < sectors.Count; ++i)
                {
                    ...
                    for (int j = 0; j < sector.Items.Count; ++j)
                    {
                        ...
                        if (
                            sector == sectorStart
                            && item.AtWorldPoint(xStart, yStart)
                            && !(item is BaseMulti)
                            && item.ItemID <= TileData.MaxItemValue
                        )
                            itemsStart.Add(item);
                        else if (
                            sector == sectorForward
                            && item.AtWorldPoint(xForward, yForward)
                            && !(item is BaseMulti)
                            && item.ItemID <= TileData.MaxItemValue
                        )
                            ...
                            sector == sectorRight
                            && item.AtWorldPoint(xRight, yRight)
                            && !(item is BaseMulti)
                            && item.ItemID <= TileData.MaxItemValue
                        )
                            itemsRight.Add(item);
        ...
        )
        {
            ...
            for (int i = 0; i < staticTiles.Length; ++i)
            {
                ...
                if (
                    (!isSet || calcTop >= zCenter)
                    && (
                        (id.Flags & TileFlag.Surface) != 0
    ...
    public class SlowAStarAlgorithm : PathAlgorithm
    {
        ...
        public override Direction[] Find(Mobile m, Map map, Point3D start, Point3D goal)
        {
            ...
            while (openCount > 0)
            {
                ...
                if (
                    curNode.x == goalNode.x
                    && curNode.y == goalNode.y
                    && Math.Abs(curNode.z - goalNode.z) < 16
                )
                {
                    ...
                    while (
                        xBacktrack != startNode.x
                        || yBacktrack != startNode.y
                        || zBacktrack != startNode.z
                    )
                    {
                        bool found = false;
                        ...
                        for (int j = 0; !found && j < closedCount; ++j)
                        {
                            if (
                                ...
                                && closed[j].y == yBacktrack
                                && closed[j].z == zBacktrack
                            )
                            {
                                if (pathCount == MaxNodes)
                                    break;
...
namespace Server
{
    public sealed class MovementPath
    {
        ...
        public bool Success
        {
            get { return (m_Directions != null && m_Directions.Length > 0); }
        }
        ...
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(Path_OnCommand)
            );
        }
        ...
        public MovementPath(Mobile m, Point3D goal)
        {
            ...
            {
                Console.WriteLine(
                    "Warning: {0}: Pathing error from {1} to {2}",
                    e.GetType().Name,
                    start,
                    goal
                );
    ...
    public class PathFollower
    {
        ...
        public bool CheckPath()
        {
            ...
            if (m_Path == null)
                ...
                (!m_Path.Success || goal != m_LastGoalLoc)
                && (m_LastPathTime + RepathDelay) <= DateTime.Now
            )
                repath = true;
