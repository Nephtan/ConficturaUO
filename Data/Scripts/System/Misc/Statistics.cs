...
namespace Server
{
    class Statistics
    {
        ...
        public static DateTime LastRestart
        {
            get { return m_LastRestart; }
        }
        public static DateTime LastStatsUpdate
        {
            get { return m_LastStatsUpdate; }
        }
        public static int ActiveAccounts
        {
            get { return m_ActiveAccounts; }
        }
        public static int ActiveStaffMembers
        {
            get { return m_ActiveStaffMembers; }
        }
        public static int ActiveGuilds
        {
            get { return m_ActiveGuilds; }
        }
        public static int ActiveParties
        {
            get { return m_ActiveParties; }
        }
        public static int PlayersInParty
        {
            get { return m_PlayersInParty; }
        }
        public static int PlayerHouses
        {
            get { return m_PlayerHouses; }
        }
        ...
        {
            get { return m_PlayersOnline; }
        }
        public static int StaffOnline
        {
            get { return m_StaffOnline; }
        }
        ...
        public static void Initialize()
        {
            if (Config.Enabled)
            {
                CommandSystem.Register(
                    "Statistics",
                    Config.CanSeeStats,
                    new CommandEventHandler(SeeStatistics_OnCommand)
                );
                CommandSystem.Register(
                    "UpdateStatistics",
                    Config.CanUpdateStats,
                    new CommandEventHandler(UpdateStatistics_OnCommand)
                );
        ...
        private static void UpdateStatistics_OnCommand(CommandEventArgs e)
        {
            Mobile m = e.Mobile;
            Console.WriteLine(
                "Statistics: {0} ({1}) has updated statistics in-game.",
                m.RawName,
        ...
        private static void CollectStats()
        {
            ...
            StatsList.Add(String.Format(Server.Misc.MyServerSettings.Version()));
            StatsList.Add(String.Format(statUpdate));
            StatsList.Add(
                String.Format(
                    "World Age: {0:n0} days, {1:n0} hours and {2:n0} minutes",
                    m_ShardAge.Days,
                    m_ShardAge.Hours,
                    m_ShardAge.Minutes
                )
            ...
            StatsList.Add(
                String.Format(
                    "<BR>Total Game Time: {0:n0} hours and {1:n0} minutes",
                    m_TotalGameTime.TotalHours,
                    m_TotalGameTime.Minutes
                )
            );
            StatsList.Add(String.Format("<BR>Last Restart: {0}", m_LastRestart));
            StatsList.Add(
                String.Format(
                    "<BR>Uptime: {0:n0} days, {1:n0} hours and {2:n0} minutes",
                    m_Uptime.Days,
                    m_Uptime.Hours,
                    m_Uptime.Minutes
                )
            ...
            StatsList.Add(
                String.Format(
                    "<BR>Active Accounts: {0:n0} [{1:n0} Players Online]",
                    m_ActiveAccounts,
                    m_PlayersOnline
                )
            );
            StatsList.Add(
                String.Format(
                    "Active Staff Members: {0:n0} [{1:n0} Online]",
                    m_ActiveStaffMembers,
                    m_StaffOnline
                )
            );
            StatsList.Add(
                String.Format(
                    "Active Parties: {0:n0} [{1:n0} Players]",
                    m_ActiveParties,
                    m_PlayersInParty
                )
            );
        ...
        public class StatisticsGump : Gump
        {
            ...
            public StatisticsGump(Mobile from, int origin)
                : base(50, 50)
            {
                m_Origin = origin;
                ...
                AddHtml(
                    ...
                    @"<BODY><BASEFONT Color="
                        + color
                        + ">"
                        + Server.Misc.MyServerSettings.ServerName()
                        + "</BASEFONT></BODY>",
                    (bool)false,
                    (bool)false
                );
                AddHtml(
                    ...
                    @"<BODY><BASEFONT Color="
                        + color
                        + ">"
                        + String.Join("<br>", StatsList.ToArray())
                        + "</BASEFONT></BODY>",
                    (bool)false,
                    (bool)false
                );
