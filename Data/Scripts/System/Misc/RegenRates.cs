    ...
    public class RegenRates
    {
        ...
        private static TimeSpan Mobile_ManaRegenRate(Mobile from)
        {
            ...
            if (Core.AOS)
            {
                ...
                double totalPoints =
                    focusPoints
                    + medPoints
                    + (from.Meditating ? (medPoints > 13.0 ? 13.0 : medPoints) : 0.0);
                ...
                if (Core.ML && from is PlayerMobile)
                    cappedPoints = Math.Min(
                        cappedPoints,
                        Server.Misc.MyServerSettings.PlayerLevelMod(36, from)
                    ); // 18
        ...
        public static double GetArmorOffset(Mobile from)
        {
            ...
            if (from.FindItemOnLayer(Layer.OuterTorso) is BaseArmor)
                rating += GetArmorMeditationValue(
                    (BaseArmor)(from.FindItemOnLayer(Layer.OuterTorso))
                );
