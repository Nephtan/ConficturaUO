        ...
        public static void RegisterInvalidAccess(NetState ns)
        {
            ...
            if (accessLog.Counts >= 3)
            {
                try
                {
                    ...
                    {
                        op.WriteLine("{0}\t{1}\t{2}", DateTime.Now, ns, accessLog.Counts);
                    }
                }
                catch { }
            }
...
namespace Server.Accounting
{
    public class AccountComment
    {
        ...
        public AccountComment(XmlElement node)
        {
            ...
            m_LastModified = Utility.GetXMLDateTime(
                Utility.GetAttribute(node, "lastModified"),
                DateTime.Now
            );
        ...
        public void Save(XmlTextWriter xml)
        {
            ...
            xml.WriteAttributeString(
                "lastModified",
                XmlConvert.ToString(m_LastModified, XmlDateTimeSerializationMode.Local)
            );
...
namespace Server.Misc
{
    ...
    public class AccountHandler
    {
        ...
        private static TimeSpan DeleteDelay = TimeSpan.FromDays(
            Server.Misc.MyServerSettings.DeleteDelay()
        );
        ...
        public static void Initialize()
        {
            ...
            if (PasswordCommandEnabled)
                CommandSystem.Register(
                    "Password",
                    AccessLevel.Player,
                    new CommandEventHandler(Password_OnCommand)
                );
        }
        ...
        public static void Password_OnCommand(CommandEventArgs e)
        {
            ...
            {
                from.SendMessage(
                    "To prevent potential typing mistakes, you must type the password twice. Use the format:"
                );
        ...
        private static void EventSink_DeleteRequest(DeleteRequestEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Console.WriteLine(
                        "Client: {0}: Deleting character {1} (0x{2:X})",
                        state,
                        index,
                        m.Serial.Value
                    );
                    ...
                    acct.Comments.Add(
                        new AccountComment(
                            "System",
                            String.Format("Character #{0} {1} deleted by {2}", index + 1, m, state)
                        )
                    );
        ...
        {
            return true; // Unlimited Accounts per IP. - Nephtan
        ...
        public static void EventSink_AccountLogin(AccountLoginEventArgs e)
        {
            ...
            else if (!acct.HasAccess(e.State))
            {
                ...
                e.RejectReason = (
                    m_LockdownLevel > AccessLevel.Player ? ALRReason.BadComm : ALRReason.BadPass
                );
            }
...
namespace Server.Accounting
{
    public class Account : IAccount, IComparable, IComparable<Account>
    {
        ...
        public bool Inactive
        {
            ...
            {
                return (
                    (m_LastLogin + InactiveDuration) <= DateTime.Now
        ...
        public static string HashMD5(string phrase)
        {
            ...
            int length = Encoding.ASCII.GetBytes(
                phrase,
                0,
                phrase.Length > 256 ? 256 : phrase.Length,
                m_HashBuffer,
                0
            );
        ...
        public Account(XmlElement node)
        {
            ...
            m_Created = Utility.GetXMLDateTime(
                Utility.GetText(node["created"], null),
                DateTime.Now
            );
            m_LastLogin = Utility.GetXMLDateTime(
                Utility.GetText(node["lastLogin"], null),
                DateTime.Now
            );
            ...
            TimeSpan totalGameTime = Utility.GetXMLTimeSpan(
                Utility.GetText(node["totalGameTime"], null),
                TimeSpan.Zero
            );
        ...
        public class CIDRFirewallEntry : IFirewallEntry
        {
            ...
            public override bool Equals(object obj)
            {
                    ...
                    if (str.Length == 2)
                    {
                        ...
                        if (IPAddress.TryParse(str[0], out cidrPrefix))
                        {
                            ...
                            if (int.TryParse(str[1], out cidrLength))
                                return m_CIDRPrefix.Equals(cidrPrefix)
                                    && m_CIDRLength.Equals(cidrLength);
                        }
                    }
                }
                else if (obj is CIDRFirewallEntry)
                {
                    ...
                    return m_CIDRPrefix.Equals(entry.m_CIDRPrefix)
                        && m_CIDRLength.Equals(entry.m_CIDRLength);
                }
