...
namespace Server.Guilds
{
    ...
    public class RankDefinition
    {
        public static RankDefinition[] Ranks = new RankDefinition[]
        {
            ...
            new RankDefinition(
                ...
                2,
                RankFlags.Member
                    | RankFlags.RemovePlayers
                    | RankFlags.CanInvitePlayer
                    | RankFlags.CanSetGuildTitle
    ...
    {
        private static Dictionary<string, AllianceInfo> m_Alliances =
            new Dictionary<string, AllianceInfo>();
        ...
        public class AllianceRosterGump : GuildDiplomacyGump
        {
            ...
            public override Gump GetResentGump(
                PlayerMobile pm,
                Guild g,
                IComparer<Guild> comparer,
                bool ascending,
                string filter,
                int startNumber
            )
            {
                return new AllianceRosterGump(
                    pm,
                    g,
                    m_Alliance,
                    comparer,
    ...
    public class WarDeclaration
    {
        ...
        public WarStatus Status
        {
            get
            {
                ...
                if (
                    m_Opponent.FindPendingWar(m_Guild) != null
                    && m_Guild.FindPendingWar(m_Opponent) != null
                )
    ...
    public class Guild : BaseGuild
    {
        public static void Configure()
        {
            ...
            CommandSystem.Register(
                ...
                new CommandEventHandler(GuildProps_OnCommand)
            );
        }
        ...
        private class GuildPropsTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object o)
            {
                ...
                if (o is Guildstone)
                {
                    ...
                    {
                        from.SendMessage(
                            "The guild associated with that Guildstone no longer exists"
                        );
                        return;
                    }
                    else
                    ...
                    if (g != null)
                    {
                        ...
                        if (otherInAlliance)
                        {
                            otherAlliance.AllianceMessage(
                                1070739 + (int)status,
                                (inAlliance ? this.Alliance.Name : this.Name)
                            );
        ...
        private List<Guild> m_AllyDeclarations,
            m_AllyInvitations;

        private List<Guild> m_WarDeclarations,
            m_WarInvitations;
        private List<Mobile> m_Candidates,
            m_Accepted;

        private List<WarDeclaration> m_PendingWars,
            m_AcceptedWars;
        ...
        {
            get { return (m_Leader == null || m_Leader.Deleted); }
        }
        ...
        public void CalculateGuildmaster()
        {
            ...
            if (
                NewGuildSystem
                && (highVotes * 100) / Math.Max(votingMembers, 1) < MajorityPercentage
                && m_Leader != null
                && winner != m_Leader
                && !m_Leader.Deleted
                && m_Leader.Guild == this
    ...
    public class Guildstone : Item, IAddon, IChopable
    {
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
            ...
            if (m_Guild != null && !m_Guild.Disbanded)
            {
                ...
                list.Add(
                    1060802,
                    String.Format("{0} [{1}]", Utility.FixHtml(name), Utility.FixHtml(abbr))
                );
            }
            else
            {
                list.Add(
                    ...
                    String.Format(
                        "{0} [{1}]",
                        Utility.FixHtml(m_GuildName),
                        Utility.FixHtml(m_GuildAbbrev)
                    )
                );
            }
        }
        ...
        public void OnChop(Mobile from)
        {
            ...
            if (
                (house == null && m_BeforeChangeover)
                || (house != null && house.IsOwner(from) && house.Addons.Contains(this))
            )
            {
            ...
            if (IsChildOf(from.Backpack))
            {
                ...
                if (house != null && house.IsOwner(from))
                {
                    ...
                    from.BeginTarget(
                        ...
                        new TargetStateCallback(Placement_OnTarget),
                        null
                    );
                }
                else
