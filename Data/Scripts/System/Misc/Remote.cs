    ...
    public class AdminNetwork
    {
...
#if !MONO
            Core.MultiConsoleOut.Add(
                new EventTextWriter(
                    new EventTextWriter.OnConsoleChar(OnConsoleChar),
                    new EventTextWriter.OnConsoleLine(OnConsoleLine),
                    new EventTextWriter.OnConsoleStr(OnConsoleString)
                )
            );
#endif
            Timer.DelayCall(
        ...
        public static void OnReceive(NetState state, PacketReader pvSrc)
        {
            ...
            else if (cmd == 0xFF)
            {
                string statStr = String.Format(
                    ", Name={0}, Age={1}, Clients={2}, Items={3}, Chars={4}, Mem={5}K",
                    Server.Misc.ServerList.ServerName,
                    (int)(DateTime.Now - Server.Items.Clock.ServerStart).TotalHours,
        ...
        public static void Authenticate(NetState state, PacketReader pvSrc)
        {
            ...
            else if (!a.CheckPassword(pw))
            {
                ...
                Console.WriteLine(
                    "ADMIN: Invalid password '{0}' for user '{1}' from {2}",
                    pw,
                    user,
                    state
                );
            ...
            {
                Console.WriteLine(
                    "ADMIN: Account '{0}' does not have admin access. Connection Denied.",
                    user
                );
        ...
        public static Packet Compress(Packet p)
        {
            ...
            if (length > 100 && length < 60000)
            {
                ...
                ZLibError error = Compression.Pack(
                    dest,
                    ref destSize,
                    source,
                    length,
                ...
                {
                    Console.WriteLine(
                        "WARNING: Unable to compress admin packet, zlib error: {0}",
                        error
                    );
                    return p;
                }
...
namespace Server.RemoteAdmin
{
    public class RemoteAdminHandlers
    {
        ...
        public static bool Handle(byte command, NetState state, PacketReader pvSrc)
        {
            ...
            {
                Console.WriteLine(
                    "ADMIN: Invalid packet 0x{0:X2} from {1}, disconnecting",
                    command,
                    state
                );
                return false;
        ...
        private static void RemoveAccount(NetState state, PacketReader pvSrc)
        {
            ...
            if (a == null)
            {
                state.Send(
                    new MessageBoxMessage(
                        "The account could not be found (and thus was not deleted).",
                        "Account Not Found"
                    )
                );
            }
            ...
            {
                state.Send(
                    new MessageBoxMessage("You may not delete your own account.", "Not Allowed")
                );
            }
            else
            {
                ...
                state.Send(
                    new MessageBoxMessage(
                        "The requested account (and all it's characters) has been deleted.",
                        "Account Deleted"
                    )
                );
            }
        ...
        private static void UpdateAccount(NetState state, PacketReader pvSrc)
        {
            ...
            else
            {
                ...
                state.Send(
                    new MessageBoxMessage(
                        "Warning: When editing your own account, account status and accesslevel cannot be changed.",
                        "Editing Own Account"
                    )
                );
            }
