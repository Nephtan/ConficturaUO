...
namespace Server.Items
{
    public class QuestSouvenir : Item
    {
        ...
        public Mobile Souvenir_Mobile
        {
            get { return SouvenirMobile; }
            set { SouvenirMobile = value; }
        }

        public string SouvenirOwner;

        [CommandProperty(AccessLevel.Owner)]
        public string Souvenir_Owner
        {
            get { return SouvenirOwner; }
            set
            {
                SouvenirOwner = value;
        ...
        public QuestSouvenir(Serial serial)
            : base(serial) { }
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            if (Name == "Candle of Love")
            {
                Light = LightType.Circle150;
            }
            else if (Name == "Lantern of Discipline")
            {
                Light = LightType.Circle150;
            }
            else if (Name == "Orb of Logic")
            {
                Light = LightType.Circle150;
            }
            else if (ItemID > 19512 && ItemID < 19539)
            {
                Light = LightType.Circle150;
            }
        }
        ...
        public static void GiveReward(Mobile from, string name, int hue, int id)
        {
            if (from is PlayerMobile)
            {
                ...
                if (name == "Candle of Love")
                {
                    reward.Light = LightType.Circle150;
                }
                else if (name == "Lantern of Discipline")
                {
                    reward.Light = LightType.Circle150;
                ...
                else if (id > 19512 && id < 19539)
                {
                    reward.Light = LightType.Circle150;
                }
                else if (id >= 0x530C && id <= 0x531B)
                {
                    reward.Light = LightType.Circle150;
                }
