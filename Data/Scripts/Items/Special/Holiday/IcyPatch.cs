    ...
    public class IcyPatch : Item
    {
        ...
        public virtual void RunSequence(Mobile m, int message, bool freeze)
        {
            ...
            if (freeze)
            {
                m.Frozen = freeze;
                Timer.DelayCall(
                    TimeSpan.FromSeconds((message == 1095162) ? 2.0 : 1.25),
                    new TimerStateCallback(EndFall_Callback),
                    m
                );
            }
            ...
            if (message == 1095162)
            {
                ...
                if (SpellHelper.FindValidSpawnLocation(Map, ref p, true))
                {
                    Timer.DelayCall(
                        ...
                        new TimerStateCallback(Relocate_Callback),
                        new object[] { m, p }
                    );
                }
            ...
            if (arg != null)
            {
                Timer.DelayCall(
                    ...
                    new TimerStateCallback(BeginFall_Callback),
                    arg
                );
            }
        }
