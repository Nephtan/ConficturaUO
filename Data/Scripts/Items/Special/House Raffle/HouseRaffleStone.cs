    ...
    public class HouseRaffleStone : Item
    {
        ...
        public static void Initialize()
        {
            ...
            Timer.DelayCall(
                ...
                TimeSpan.FromMinutes(1.0),
                new TimerCallback(CheckEnd_OnTick)
            );
        }
        ...
        {
            return (
                m_Bounds.Start != Point2D.Zero
                && m_Bounds.End != Point2D.Zero
                && m_Facet != null
            ...
            else
            {
                from.SendGump(
                    new WarningGump(
                        ...
                        0x7F00,
                        String.Format(
                            "You are about to purchase a raffle ticket for the house plot located at {0}.  The ticket price is {1}.  Tickets are non-refundable and you can only purchase one ticket per account.  Do you wish to continue?",
                            FormatLocation(),
                            FormatPrice()
                        ),
                        0xFFFFFF,
                        420,
                        280,
                        new WarningGumpCallback(Purchase_Callback),
                        null
                    )
        ...
        public void Purchase_Callback(Mobile from, bool okay, object state)
        {
            ...
            if (okay)
            {
                ...
                if (
                    ...
                    || (
                        from.Backpack != null
                        && from.Backpack.ConsumeTotal(typeof(Gold), m_TicketPrice)
                    )
                ...
                )
                {
                    ...
                    from.SendMessage(
                        MessageHue,
                        "You have successfully entered the plot's raffle."
                    );
                }
                else
                {
                    from.SendMessage(
                        MessageHue,
                        "You do not have the {0} required to enter the raffle.",
                        FormatPrice()
                    );
                }
            }
            ...
            if (m_Region != null && m_Entries.Count != 0)
            {
                ...
                if (m_Winner != null)
                {
                    ...
                    if (m_Winner.AddToBackpack(m_Deed))
                    {
                        m_Winner.SendMessage(
                            MessageHue,
                            "The writ of lease has been placed in your backpack."
                        );
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            m_Winner.SendMessage(
                                MessageHue,
                                "As both your backpack and bank box are full, the writ of lease has been placed at your feet."
                            );
                        }
                    }
                }
        ...
        private class ActivateEntry : RaffleContextMenuEntry
        {
            ...
            public override void OnClick()
            {
                if (
                    m_Stone.Deleted
                    || m_From.AccessLevel < AccessLevel.Seer
                    || m_Stone.Active
                    || !m_Stone.ValidLocation()
                )
                    return;
