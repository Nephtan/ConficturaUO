...
{
    public class BraceletOfBinding : GoldBracelet
    {
...
namespace Server.Items
{
    public class BraceletOfBinding : BaseBracelet, TranslocationItem
    {
        ...
        private bool CheckUse( Mobile from, bool successMessage )
        {
            ...
            {
                from.SendMessage( "This bracelet does not seem to work in this place." );
                return false;
            }
            else if ( Worlds.RegionAllowedTeleport( boundRoot.Map, boundRoot.Location, boundRoot.X, boundRoot.Y ) == false )
            {
                from.SendMessage( "The destination seems magically unreachable." );
                return false;
            }
            ...
            else if ( boundRoot.Map == Map.Lodor && from is PlayerMobile && ((PlayerMobile)from).Young )
            {
                from.SendLocalizedMessage( 1049543 ); // You decide against traveling to Lodor while you are still young.
                return false;
            }
            else if ( from.Kills >= 5 && boundRoot.Map != Map.Lodor )
            {
