...
namespace Server.Items
{
    public class BallOfSummoning : Item, TranslocationItem
    {
        ...
        {
            get { return "crystal ball of pet summoning"; }
        }
        ...
        public override void AddNameProperties(ObjectPropertyList list)
        {
            ...
            if (m_PetName != "" && m_PetName != null)
            {
                list.Add(1049644, "Pet: " + m_PetName);
            }
        }
        ...
        public void CastSummonPet(Mobile from)
        {
            ...
            else if (pet is BaseMount && ((BaseMount)pet).Rider == from)
            {
                ...
                && (!pet.IsStabled || (from.Followers + pet.ControlSlots) > from.FollowersMax)
            )
            {
            ...
            else if (!pet.IsBonded)
            {
                ...
                || from.Region.IsPartOf(typeof(Jail))
            )
            {
                from.Send(
                    new AsciiMessage(
                ...
                && DateTime.Now < ((PlayerMobile)from).LastPetBallTime.AddSeconds(15.0)
            )
            {
