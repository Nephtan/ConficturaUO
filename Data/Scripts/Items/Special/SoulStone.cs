    ...
    public class SoulStone : Item, ISecurable
    {
        ...
        protected virtual bool CheckUse(Mobile from)
        {
            ...
            else if (from.Map != this.Map || !from.InRange(GetWorldLocation(), 2))
            {
                ...
                this.Account != null
                && (!(from.Account is Account) || from.Account.Username != this.Account)
            )
            {
        ...
        private class ConfirmSkillGump : Gump
        {
            ...
            public override void OnResponse(NetState sender, RelayInfo info)
            {
                ...
                Effects.SendMovingParticles(
                    new Entity(
                        Server.Serial.Zero,
                        new Point3D(from.X - 6, from.Y - 6, from.Z + 15),
                        from.Map
                    ),
                    from,
                    0x36D4,
        ...
        private class ConfirmTransferGump : Gump
        {
            ...
            public override void OnResponse(NetState sender, RelayInfo info)
            {
                ...
                int requiredAmount =
                    (int)(skillValue * 10)
                    - fromSkill.BaseFixedPoint
                    - (from.SkillsCap - from.SkillsTotal);
