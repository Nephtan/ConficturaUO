...
namespace Server.Items
{
    public class SlaversNet : Item
    {
        ...
        private class SlaveTarget : Target
        {
            ...
            public SlaveTarget(SlaversNet net)
                ...
                if (targeted is Mobile)
                {
                    ...
                    if (o_Net is BaseCreature)
                    {
                        ...
                        else if (i_Net.Tamable)
                        {
                            ...
                            if (i_Net.MinTameSkill > 29.0)
                            {
                                i_Net.MinTameSkill = 29.1;
                            }
                            i_Net.SetControlMaster(from);
                            i_Net.ControlTarget = from;
                            i_Net.IsBonded = true;
                            // Below fixes invisible pets - Nephtan
                            i_Net.CanHearGhosts = false;
                            i_Net.CantWalk = false;
                            i_Net.Hidden = false;
                            // Above fixes invisible pets
        ...
        public SlaversNet(Serial serial)
            : base(serial) { }
