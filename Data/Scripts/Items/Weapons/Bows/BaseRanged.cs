    ...
    public abstract class BaseRanged : BaseMeleeWeapon
    {
        ...
        public override void OnHit(Mobile attacker, Mobile defender, double damageBonus)
        {
            if (
                ...
                && !(Ammo is HarpoonRope)
                && !(Ammo is MageEye)
                && !(Ammo is ThrowingWeapon)
                && attacker.Player
                && !defender.Player
                && (defender.Body.IsAnimal || defender.Body.IsMonster)
            ...
            if (defender is BaseCreature && Ammo is ThrowingWeapon && attacker.Player)
            {
                ...
                if (attacker.FindItemOnLayer(Layer.OneHanded) != null)
                {
                    ...
                    {
                        ThrowingGloves glove = (ThrowingGloves)(
                            attacker.FindItemOnLayer(Layer.OneHanded)
                        );
                        ThrowingWeapon knife = new ThrowingWeapon();

                        if (glove.GloveType == "Stones")
                        {
                            knife.ammo = "Throwing Stones";
                            knife.ItemID = 0x10B6;
                            knife.Name = "throwing stone";
                        }
                        else if (glove.GloveType == "Axes")
                        {
                            knife.ammo = "Throwing Axes";
                            knife.ItemID = 0x10B3;
                            knife.Name = "throwing axe";
                        }
                        else if (glove.GloveType == "Daggers")
                        {
                            knife.ammo = "Throwing Daggers";
                            knife.ItemID = 0x10B7;
                            knife.Name = "throwing dagger";
                        }
                        else if (glove.GloveType == "Darts")
                        {
                            knife.ammo = "Throwing Darts";
                            knife.ItemID = 0x10B5;
                            knife.Name = "throwing dart";
                        }
                        else if (
                            glove.GloveType == "Cards"
                            && Server.Misc.GetPlayerInfo.isJester(attacker)
                        )
                        {
                            knife.ammo = "Throwing Cards";
                            knife.ItemID = 0x4C29;
                            knife.Name = "throwing card";
                        }
                        else if (
                            glove.GloveType == "Tomatoes"
                            && Server.Misc.GetPlayerInfo.isJester(attacker)
                        )
                        {
                            knife.ammo = "Throwing Tomatoes";
                            knife.ItemID = 0x4C28;
                            knife.Name = "throwing tomato";
                        }
                        else
                        {
                            knife.ammo = "Throwing Stars";
                            knife.ItemID = 0x10B2;
                            knife.Name = "throwing star";
                        }
                    ...
                    if (p != null)
                    {
                        ...
                        if (!p.Warmode)
                        {
                            if (m_RecoveryTimer == null)
                                m_RecoveryTimer = Timer.DelayCall(
                                    TimeSpan.FromSeconds(10),
                                    new TimerCallback(p.RecoverAmmo)
                                );
        ...
        public virtual bool OnFired(Mobile attacker, Mobile defender)
        {
            if (this is ThrowingGloves && attacker.Player)
            {
                ...
                else if (glove.GloveType == "Cards" && Server.Misc.GetPlayerInfo.isJester(attacker))
                {
                    ...
                    glove.GloveType == "Tomatoes" && Server.Misc.GetPlayerInfo.isJester(attacker)
                )
                {
                ...
                foreach (Item i in attacker.Backpack.FindItemsByType(typeof(ThrowingWeapon), true))
                {
                    ...
                    {
                        ((ThrowingWeapon)i).ammo = "Throwing Stones";
                        i.ItemID = 0x10B6;
                        i.Name = "throwing stone";
                    }
                    else if (ammoType == "Throwing Axes")
                    {
                        ((ThrowingWeapon)i).ammo = "Throwing Axes";
                        i.ItemID = 0x10B3;
                        i.Name = "throwing axe";
                    }
                    else if (ammoType == "Throwing Daggers")
                    {
                        ((ThrowingWeapon)i).ammo = "Throwing Daggers";
                        i.ItemID = 0x10B7;
                        i.Name = "throwing dagger";
                    }
                    else if (ammoType == "Throwing Darts")
                    {
                        ((ThrowingWeapon)i).ammo = "Throwing Darts";
                        i.ItemID = 0x10B5;
                        i.Name = "throwing dart";
                    }
                    else if (ammoType == "Throwing Cards")
                    {
                        ((ThrowingWeapon)i).ammo = "Throwing Cards";
                        i.ItemID = 0x4C29;
                        i.Name = "throwing card";
                    }
                    else if (ammoType == "Throwing Tomatoes")
                    {
                        ((ThrowingWeapon)i).ammo = "Throwing Tomatoes";
                        i.ItemID = 0x4C28;
                        i.Name = "throwing tomato";
                    }
                    else
                    {
                        ((ThrowingWeapon)i).ammo = "Throwing Stars";
                        i.ItemID = 0x10B2;
                        i.Name = "throwing star";
                    }
