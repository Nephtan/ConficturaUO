...
namespace Server.Items
{
    public class WizardStaff : BaseWizardStaff
    {
        ...
        public override WeaponAbility PrimaryAbility
        {
            get
            {
                if (damageType == 1)
                {
                    return WeaponAbility.ZapStamStrike;
                } // Fire
                else if (damageType == 2)
                {
                    return WeaponAbility.ZapDexStrike;
                } // Cold
                else if (damageType == 3)
                {
                    return WeaponAbility.ZapIntStrike;
                } // Energy
                else if (damageType == 4)
                {
                    return WeaponAbility.ZapStrStrike;
                } // Poison
        ...
        {
            get { return Core.ML ? 15 : 16; }
        }
        public override int AosMaxDamage
        {
            get { return Core.ML ? 19 : 18; }
        }
        ...
        {
            get { return WeaponAnimation.Pierce2H; }
        }
            ...
            : base(0x0908)
        {
            ...
            Hue = Server.Misc.MaterialInfo.PlainIronColor(0);
        }
    ...
    public class WizardStick : BaseWizardStaff
    {
        ...
        public WizardStick(Serial serial)
            : base(serial) { }
    ...
    public abstract class BaseWizardStaff : BaseMeleeWeapon
    {
        ...
        public BaseWizardStaff(int itemID)
            : base(itemID)
        {
            damageType = 0;
        }
        ...
        public override bool OnEquip(Mobile from)
        {
            ...
            from.SendMessage(
                "You need mage eye crystals to power this item, and you can turn common gems into that with this."
            );
        ...
        private class GemTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                )
                {
                    ...
                    {
                        int amount = 4;
                        if (iGem is StarSapphire)
                        {
                            amount = iGem.Amount * 62;
                        }
                        else if (iGem is Emerald)
                        {
                            amount = iGem.Amount * 50;
                        }
                        else if (iGem is Sapphire)
                        {
                            amount = iGem.Amount * 50;
                        }
                        else if (iGem is Ruby)
                        {
                            amount = iGem.Amount * 37;
                        }
                        else if (iGem is Citrine)
                        {
                            amount = iGem.Amount * 25;
                        }
                        else if (iGem is Amethyst)
                        {
                            amount = iGem.Amount * 50;
                        }
                        else if (iGem is Tourmaline)
                        {
                            amount = iGem.Amount * 47;
                        }
                        else if (iGem is Amber)
                        {
                            amount = iGem.Amount * 25;
                        }
                        else if (iGem is Diamond)
                        {
                            amount = iGem.Amount * 100;
                        }
                        amount = (int)(amount / 4);
        ...
        public void EnergyType()
        {
            int physical =
                100
                - AosElementDamages.Fire
                - AosElementDamages.Cold
                - AosElementDamages.Energy
                - AosElementDamages.Poison;
            damageType = 0;
            if (
                AosElementDamages.Fire > AosElementDamages.Cold
                && AosElementDamages.Fire > AosElementDamages.Poison
                && AosElementDamages.Fire > AosElementDamages.Energy
                && AosElementDamages.Fire > physical
            )
            {
                damageType = 1;
            }
            else if (
                AosElementDamages.Cold > AosElementDamages.Fire
                && AosElementDamages.Cold > AosElementDamages.Poison
                && AosElementDamages.Cold > AosElementDamages.Energy
                && AosElementDamages.Cold > physical
            )
            {
                damageType = 2;
            }
            else if (
                AosElementDamages.Energy > AosElementDamages.Cold
                && AosElementDamages.Energy > AosElementDamages.Fire
                && AosElementDamages.Energy > AosElementDamages.Poison
                && AosElementDamages.Energy > physical
            )
            {
                damageType = 3;
            }
            else if (
                AosElementDamages.Poison > AosElementDamages.Fire
                && AosElementDamages.Poison > AosElementDamages.Cold
                && AosElementDamages.Poison > AosElementDamages.Energy
                && AosElementDamages.Poison > physical
            )
            {
                damageType = 4;
            }
        ...
        public BaseWizardStaff(Serial serial)
            : base(serial) { }
            ...
            if (
                DateTime.Now
                    > (
                        ...
                        + TimeSpan.FromSeconds(Core.SE ? 0.25 : (Core.AOS ? 0.5 : 1.0))
                    )
        ...
        public static bool HasStaff(Mobile from)
        {
            if (from.FindItemOnLayer(Layer.OneHanded) != null)
            {
                ...
                if (
                    oneHand is BaseWizardStaff
                    || oneHand is BaseLevelStave
                    || oneHand is BaseGiftStave
                )
                {
                    return true;
                }
            ...
            if (from.FindItemOnLayer(Layer.TwoHanded) != null)
            {
                ...
                if (
                    twoHand is BaseWizardStaff
                    || twoHand is BaseLevelStave
                    || twoHand is BaseGiftStave
                )
                {
                    return true;
                }
        ...
        public virtual bool OnFired(Mobile attacker, Mobile defender)
        {
            ...
            if (attacker.Player)
            {
                if (
                    ...
                    || quiver.LowerAmmoCost > Utility.Random(100)
                )
                {
