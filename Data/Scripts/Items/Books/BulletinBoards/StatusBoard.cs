...
namespace Server.Items
{
    ...
    public class StatusBoard : Item
    {
        ...
        public StatusBoard(Serial serial)
            : base(serial) { }
...
namespace Server.Gumps
{
    public class StatusGump : Gump
    {
        ...
        public StatusGump(Mobile from, int listPage, ArrayList list, ArrayList count)
            : base(140, 80)
        {
            ...
            AddHtml(
                ...
                770,
                17,
                "<div align=\"center\" color=\"2100\">" + ServerList.ServerName + "</div>",
                false,
                false
            );
                    ...
                    else
                    {
                        if (
                            (
                                ...
                                && (
                                    (mk.AccessLevel == AccessLevel.Counselor && mk.Hidden)
                                    || mk.AccessLevel >= AccessLevel.GameMaster
                                )
                            )
                            || (
                                from.AccessLevel >= AccessLevel.Player
                                && mk.AccessLevel > from.AccessLevel
                            )
                        )
                        {
                            --t;
            ...
            AddHtml(
                ...
                30,
                17,
                "<div align=\"right\" color=\"2100\">" + t.ToString() + "</div>",
                false,
                false
            );
            ...
            AddHtml(
                ...
                30,
                17,
                "<div align=\"right\" color=\"2100\">" + Accounts.Count.ToString() + "</div>",
                false,
                false
            );
            ...
            for (
                int i = 0, j = 0, index = ((listPage * 25) + k);
                i < 25 && index >= 0 && index < m_List.Count && j >= 0;
                ++i, ++j, ++index
            )
            {
                ...
                else
                {
                    if (
                        (
                            from.AccessLevel == AccessLevel.Player
                            && (
                                (m.AccessLevel == AccessLevel.Counselor && m.Hidden)
                                || m.AccessLevel >= AccessLevel.GameMaster
                            )
                        )
                        || (
                            from.AccessLevel >= AccessLevel.Player
                            && m.AccessLevel > from.AccessLevel
                        )
                    )
                    {
                        --i;
                    ...
                    else
                    {
                        ...
                        else if (GetPlayerInfo.GetStatusGuild(m) != "")
                        {
                            AddLabelCropped(
                                ...
                                20,
                                2100,
                                GetPlayerInfo.GetStatusGuild(m)
                            );
                        }
