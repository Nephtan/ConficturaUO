...
namespace Server.Misc
{
    class RelicItems
    {
        public static string IdentifyRelicValue(Mobile examiner, Mobile customer, Item item)
        {
            ...
            {
                gold = ((DDRelicBearRugsAddonDeed)item).RelicGoldValue;
                buyer = "a fur trader";
            }
        ...
        {
            if (
                item is DDRelicAlchemy
                || item is DDRelicArmor
                || item is DDRelicArts
                || item is DDRelicBanner
                || item is DDRelicBearRugsAddonDeed
                || item is DDRelicBook
        ...
        public static int RelicValue(Item relics, Mobile m)
        {
            ...
            {
                DDRelicVase relic = (DDRelicVase)relics;
            ...
            {
                EmptyCanopicJar relic = (EmptyCanopicJar)relics;
                RelicValue = relic.RelicGoldValue;
            } // ART COLLECTOR
            else if (relics is DDRelicBanner && (IsHenchman || m is VarietyDealer))
            {
                DDRelicBanner relic = (DDRelicBanner)relics;
                RelicValue = relic.RelicGoldValue;
            } // ART COLLECTOR
            else if (
                relics is DDRelicGrave
                && (
                    IsHenchman
                    || m is Necromancer
            ...
            else if (relics is DDRelicPainting && m is Artist)
            {
                ...
                relics is DDRelicBanner
                && relics.Name.Contains("painting")
                && (IsHenchman || m is Artist)
            )
            {
                DDRelicBanner relic = (DDRelicBanner)relics;
                RelicValue = relic.RelicGoldValue * 2;
            ...
            {
                StatueGygaxAddonDeed relic = (StatueGygaxAddonDeed)relics;
                RelicValue = relic.RelicGoldValue;
            } // ART COLLECTOR
