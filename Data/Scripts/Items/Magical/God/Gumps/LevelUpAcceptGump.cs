    ...
    public class LevelUpAcceptGump : Gump
    {
            ...
            : base(0, 0)
        {
            ...
            if (LevelItems.RewardBlacksmith && LevelItems.BlacksmithRewardAmt > 0)
                PaymentMsg =
                    "<BR><BR>If you accept and the process is successful, you will be given additional compensation of "
                    + LevelItems.BlacksmithRewardAmt
                    + ".";
            ...
            AddHtml(
                ...
                271,
                116,
                @"<CENTER><U>Max Level Increase Request</U><BR><BR>Someone has requested your expert services in increasing the max levels of a levelable item."
                    + PaymentMsg
                    + "<BR><BR>Do you accept their offer?",
                (bool)false,
                (bool)true
            );
            ...
            if (info.ButtonID == 1)
            {
                if (m_From != null && m_Scroll != null)
                {
                    ...
                    if (smith != null) //Accepted... send message to smith and pay them bonus reward
                    {
                        ...
                        if (
                            smith != m_From
                            && LevelItems.RewardBlacksmith
                            && LevelItems.BlacksmithRewardAmt > 0
                        )
                        {
