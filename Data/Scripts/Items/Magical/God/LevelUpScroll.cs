    ...
    public class LevelUpScroll : Item
    {
        ...
        public override void AddNameProperties(ObjectPropertyList list)
        {
            ...
            if (IsBlacksmithOnly)
            {
                if (!m_BlacksmithValidated)
                    BlacksmithMsg =
                        "(Must be validated by player with "
                        + LevelItems.BlacksmithSkillRequired
                        + "+ Blacksmithy skill)";
                else
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else
            {
                ...
                {
                    if (
                        m_BlacksmithValidated
                        || (
                            from.Skills[SkillName.Blacksmith].Value
                            >= LevelItems.BlacksmithSkillRequired
                        )
                    )
                    {
                        from.SendMessage(
                            "Which legendary artefact item would you like to enhance?"
                        );
                    ...
                    {
                        from.SendMessage(
                            "Please target one with a base Blacksmith skill of "
                                + LevelItems.BlacksmithSkillRequired
                                + " or higher."
                        );
                ...
                else if (target is Item)
                {
                    ...
                    else
                    {
                        if (target is ILevelable)
                        {
                            ...
                            {
                                from.SendMessage(
                                    "The level on this legendary artefact is already too high to use this rune!"
                                );
                            }
                            else
                            {
                                ...
                                from.SendMessage(
                                    "Your legendary artefact has been enhanced by "
                                        + m_Scroll.Value
                                        + " levels."
                                );
                                m_Scroll.Delete();
        ...
        private class BlacksmithTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object target)
            {
                if (target is PlayerMobile)
                {
                    ...
                    if (
                        smith.Skills[SkillName.Blacksmith].Value
                        < LevelItems.BlacksmithSkillRequired
                    )
                    {
                        from.SendMessage(
                            "This one's blacksmith skill is not high enough to enhance legendary artefacts."
                        );
                    }
                    else
                    {
