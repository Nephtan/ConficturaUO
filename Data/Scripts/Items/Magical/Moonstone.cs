    ...
    public class MoonStone : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            if (!IsChildOf(from.Backpack))
            {
                ...
                && !(Server.Misc.PlayerSettings.GetKeys(from, "VordoKey"))
            )
            {
                from.SendMessage("This magical gate doesn't seem to do anything.");
            }
            else if (
                Worlds.AllowEscape(from, from.Map, from.Location, from.X, from.Y) == false
            ...
            )
            {
                ...
                Worlds.RegionAllowedRecall(from.Map, from.Location, from.X, from.Y) == false
                && Worlds.GetMyWorld(from.Map, from.Location, from.X, from.Y)
                    != "the Land of Ambrosia"
                && Worlds.GetMyWorld(from.Map, from.Location, from.X, from.Y)
                    != "the Bottle World of Kuldar"
            )
            {
