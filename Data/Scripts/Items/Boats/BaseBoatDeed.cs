    ...
    public abstract class BaseBoatDeed : Item
    {
        ...
        public void OnPlacement(Mobile from, Point3D p, int hue)
        {
            ...
            else
            {
                ...
                {
                    phrase_a =
                        "You may not place the carpet while on a ship or carpet, or inside a house.";
                }
                ...
                if (
                    from.Region.IsPartOf(typeof(HouseRegion))
                    || BaseBoat.FindBoatAt(from, from.Map) != null
                )
                {
                ...
                if (!DockSearch.NearDock(from) && !BaseBoat.isCarpet(boat))
                {
                    ...
                    BaseBoat.IsValidLocation(p, map)
                    && CanBuild == true
                    && boat.CanFit(p, map, boat.ItemID)
                )
                {
                    Delete();
                    ...
                    if (from.Skills[SkillName.Seafaring].Base >= 90 && boat.m_BoatDoor != null)
                    {
            ...
            protected override void OnTarget(Mobile from, object o)
            {
                ...
                if (ip != null)
                {
                    ...
                    if (region.IsPartOf(typeof(DungeonRegion)))
                        ...
                        region.IsPartOf(typeof(HouseRegion))
                        || region.IsPartOf(typeof(ChampionSpawnRegion))
                    )
