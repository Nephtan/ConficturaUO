...
namespace Server.Items
{
    ...
    public class Plank : Item, ILockable
    {
        ...
        {
            get { return (m_Side == PlankSide.Starboard); }
        }
        ...
        public void SetFacing(Direction dir)
        {
            if (BaseBoat.isCarpet(m_Boat))
            {
                if (IsOpen)
                {
                    switch (dir)
                    {
                        case Direction.North:
                            ItemID = Starboard ? 0x5437 : 0x5435;
                            break;
                        case Direction.East:
                            ItemID = Starboard ? 0x5438 : 0x5436;
                            break;
                        case Direction.South:
                            ItemID = Starboard ? 0x5435 : 0x5437;
                            break;
                        case Direction.West:
                            ItemID = Starboard ? 0x5436 : 0x5438;
                            break;
                    }
                }
                else
                {
                    ...
                    {
                        case Direction.North:
                            ItemID = Starboard ? 0x5433 : 0x5431;
                            break;
                        case Direction.East:
                            ItemID = Starboard ? 0x5434 : 0x5432;
                            break;
                        case Direction.South:
                            ItemID = Starboard ? 0x5431 : 0x5433;
                            break;
                        case Direction.West:
                            ItemID = Starboard ? 0x5432 : 0x5434;
                            break;
                    }
                }
            }
        ...
        public override bool OnMoveOver(Mobile from)
        {
            ...
            {
                if (
                    (from.Direction & Direction.Running) != 0
                    || (m_Boat != null && !m_Boat.Contains(from))
                )
                    return true;
                ...
                for (int i = 1; i <= 6; ++i)
                {
                    ...
                    for (int j = -16; j <= 16; ++j)
                    {
                        ...
                        if (
                            map.CanFit(x, y, z, 16, false, false)
                            && !Server.Spells.SpellHelper.CheckMulti(new Point3D(x, y, z), map)
                            && !Region
                                .Find(new Point3D(x, y, z), map)
                                .IsPartOf(typeof(Factions.StrongholdRegion))
                        )
                        {
                            if (i == 1 && j >= -2 && j <= 2)
                                return true;
