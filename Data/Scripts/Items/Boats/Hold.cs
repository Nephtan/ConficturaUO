...
namespace Server.Items
{
    public class Hold : Container
    {
            ...
            : base(0x3EAE)
        {
            ...
            if (boat is LargeDragonBoat)
                m_MaxWeightDefault = 3200;
            else if (boat is MediumDragonBoat)
                m_MaxWeightDefault = 2200;
            else if (boat is SmallDragonBoat)
                m_MaxWeightDefault = 1400;
            else if (boat is LargeBoat)
                m_MaxWeightDefault = 2600;
            else if (boat is MediumBoat)
                m_MaxWeightDefault = 1800;
        }

        public override int DefaultMaxWeight
        {
            get { return m_MaxWeightDefault; }
        }
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            switch (version)
            {
                case 0:
                {
                    ...
                    {
                        if (m_Boat is LargeDragonBoat)
                            m_MaxWeightDefault = 3200;
                        else if (m_Boat is MediumDragonBoat)
                            m_MaxWeightDefault = 2200;
                        else if (m_Boat is SmallDragonBoat)
                            m_MaxWeightDefault = 1400;
                        else if (m_Boat is LargeBoat)
                            m_MaxWeightDefault = 2600;
                        else if (m_Boat is MediumBoat)
                            m_MaxWeightDefault = 1800;
                        else
                        {
                            m_MaxWeightDefault = 1000;
