...
namespace Server.Items
{
    public class SHTeleComponent : AddonComponent
    {
        ...
        public Point3D TelePoint
        {
            ...
            {
                return new Point3D(
                    Location.X + TeleOffset.X,
                    Location.Y + TeleOffset.Y,
                    Location.Z + TeleOffset.Z
                );
            }
            set
            {
                m_TeleOffset = new Point3D(
                    value.X - Location.X,
                    value.Y - Location.Y,
                    value.Z - Location.Z
                );
    ...
    public class SHTeleporter : BaseAddon
    {
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(SHTelGen_OnCommand)
            );
        }
        ...
        public static void SHTelGen_OnCommand(CommandEventArgs e)
        {
            World.Broadcast(
                0x35,
                true,
                "Solen hives teleporters are being generated, please wait."
            );
