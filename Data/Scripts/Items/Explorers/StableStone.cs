...
namespace Server.Items
{
    public class StableStone : Item
    {
        ...
        public StableStone(Serial serial)
            : base(serial) { }
        ...
        private class StableEntry : ContextMenuEntry
        {
            ...
            public StableEntry(StableStone trainer, Mobile from)
                : base(6126, 2)
            {
                m_Trainer = trainer;
                m_From = from;
        ...
        public class ClaimingGumpEntry : ContextMenuEntry
        {
            ...
            public ClaimingGumpEntry(StableStone trainer, Mobile from)
                : base(6165, 3)
            {
                m_Trainer = trainer;
                m_From = from;
        ...
        private class StableTarget : Target
        {
            ...
            public StableTarget(StableStone trainer)
        ...
        public void BeginStable(Mobile from)
        {
            ...
            {
                from.SendMessage(
                    "You can't stable that! You have too many pets stabled, you have reached your max amount"
                );
            }
            else
            {
                from.SendMessage(
                    "The hitching post requires 30 gold per pet for each real world week for maintenance!"
                );
        ...
        public void EndStable(Mobile from, BaseCreature pet)
        {
            ...
            {
                from.SendMessage(
                    "Your pet seems to be busy at the moment, try again when its not!"
                );
            }
            else if (from.Stabled.Count >= Server.Mobiles.AnimalTrainer.GetMaxStabled(from))
            {
            ...
            else
            {
                ...
                if (bank != null && bank.ConsumeTotal(typeof(Gold), 30))
                {
                    ...
                    from.SendMessage(
                        "Your pet is stabled. You may recover it by saying 'claim'. In one real world week,"
                    );
                    from.SendMessage(
                        "if your pet is not claimed by then, it will vanish if it is not claimed!"
                    );
                }
                else
                {
        ...
        private class ClaimListGump : Gump
        {
            ...
            public ClaimListGump(StableStone trainer, Mobile from, ArrayList list)
                : base(50, 50)
            {
        ...
        public void EndClaimList(Mobile from, BaseCreature pet)
        {
            ...
            {
                from.SendMessage(
                    "That Pet remained in the stables because you have too many followers"
                );
            }
        }
        ...
        private class ClaimAllEntry : ContextMenuEntry
        {
            ...
            public ClaimAllEntry(StableStone trainer, Mobile from)
                : base(6127, 12)
            {
                m_Trainer = trainer;
                m_From = from;
