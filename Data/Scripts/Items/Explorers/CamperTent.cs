...
namespace Server.Items
{
    ...
    public class CampersTent : Item
    {
                    ...
                    if (!mobile.HasGump(typeof(SpeechGump)))
                    {
                        mobile.SendGump(
                            new SpeechGump(
                                mobile,
                                "Camping Tent",
                                SpeechFunctions.SpeechText(m_Mobile, m_Mobile, "CampTent")
                            )
                        );
                    }
                }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            bool inCombat = (
                from.Combatant != null
                && from.InRange(from.Combatant.Location, 20)
                && from.Combatant.InLOS(from)
            );
            ...
            else if (inCombat)
            {
                ...
                && from.Skills[SkillName.Camping].Value >= 90
            )
            {
                CanUseTent = 1;
            }
            ...
            {
                from.SendMessage(
                    "Your tent is a bit more worn out as you fail to set it up properly."
                );
                Server.Items.Kindling.RaiseCamping(from);
                ConsumeCharge(from);
        ...
        public static void TentTeleport(
            Mobile m,
            Point3D loc,
            Map map,
            int sound,
        ...
        public CampersTent(Serial serial)
            : base(serial) { }
