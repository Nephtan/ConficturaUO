...
namespace Server.Items
{
    public class PlasmaTorch : Item
    {
        ...
        private class UnlockTarget : Target
        {
            ...
            public UnlockTarget(PlasmaTorch key)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                {
                    from.SendMessage(
                        "This torch can never penetrate the magic of this cursed box."
                    );
                }
                else if (
                ...
                else if (targeted is ILockable)
                {
                    ...
                    if ((o.Locked) || (cont3.TrapType != TrapType.None))
                    {
                        ...
                        {
                            from.SendMessage(
                                "This torch is to melt locks and traps on most chests."
                            );
                        }
                        else if (targeted is TreasureMapChest)
                        {
                            from.SendMessage(
                                "The torch seems to have done nothing to the mechanism inside."
                            );
                            m_Key.Consume();
                        }
                        else if (100 >= cont2.RequiredSkill)
                        {
                            ...
                            if (targeted is Item)
                            {
                                ...
                                from.SendMessage(
                                    "The torch seems to have melted the mechanism inside."
                                );
                            }
        ...
        public PlasmaTorch(Serial serial)
            : base(serial) { }
