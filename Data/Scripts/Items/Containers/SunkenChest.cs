...
namespace Server.Items
{
    public class SunkenChest : LockableContainer
    {
        ...
        [CommandProperty(AccessLevel.Owner)]
        public string Container_Digger
        {
            get { return ContainerDigger; }
            set
            {
                ContainerDigger = value;
                InvalidateProperties();
            }
        }

        [Constructable]
        public SunkenChest(int level, Mobile digger, int ancient)
            : base(0x455)
        {
            level = level + 4;
            if (level > 10)
            ...
            if (Utility.RandomMinMax(0, 100) < ((level * 10) + artychance))
            {
                ...
                {
                    case 1:
                        arty = ArtifactBuilder.CreateArtifact("driftwood");
                        break;
                    case 2:
                        arty = ArtifactBuilder.CreateArtifact("kelp");
                        break;
                    case 3:
                        arty = ArtifactBuilder.CreateArtifact("barnacle");
                        break;
                    case 4:
                        arty = ArtifactBuilder.CreateArtifact("neptune");
                        break;
                    case 5:
                        arty = ArtifactBuilder.CreateArtifact("bronzed");
                        break;
                }
                DropItem(arty);
            }
            ...
            while (giveRelics > 0)
            {
                ...
                if (Utility.RandomMinMax(1, 100) > 94 && GetPlayerInfo.LuckyPlayer(digger.Luck))
                {
                    relic = Loot.RandomSea();
                }
        ...
        public SunkenChest(Serial serial)
            : base(serial) { }
