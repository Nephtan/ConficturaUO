...
namespace Server.Items
{
    public class SkeletonsKey : Item
    {
        ...
        private class UnlockTarget : Target
        {
            ...
            public UnlockTarget(SkeletonsKey key)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                else if (targeted is BookBox) // cursed box of books
                {
                    ...
                    targeted is UnidentifiedArtifact
                    || targeted is UnidentifiedItem
                    || targeted is CurseItem
                )
                {
                ...
                {
                    if (
                        Server.Items.DoorType.IsSpaceshipDoor((BaseDoor)targeted)
                        && m_Key.ItemID != 0x3A75
                    )
                    {
                        from.SendMessage(
                            "This doesn't have a key hole, but it does have a card slot."
                        );
                    }
                    else if (
                        !(Server.Items.DoorType.IsSpaceshipDoor((BaseDoor)targeted))
                    ...
                    {
                        from.SendMessage(
                            "This doesn't have a card slot, but it does have a key hole."
                        );
                    }
                    else if (
                        Server.Items.DoorType.IsSpaceshipDoor((BaseDoor)targeted)
                    ...
                    )
                    {
                        ...
                        Server.Items.DoorType.IsDungeonDoor((BaseDoor)targeted)
                        && m_Key.ItemID != 0x3A75
                    )
                    {
                        if (((BaseDoor)targeted).Locked == false)
                ...
                else if (targeted is ILockable)
                {
                    ...
                    if (Multis.BaseHouse.CheckSecured(cont2))
                        ...
                        Server.Misc.ContainerFunctions.IsSpaceCrate(cont2.ItemID)
                        && o.Locked
                        && m_Key.ItemID != 0x3A75
                    )
                    {
                    ...
                    {
                        int neededMod =
                            Server.Misc.Worlds.GetDifficultyLevel(from.Location, from.Map) * 5;
                        if (neededMod < 1)
                        {
                            neededMod = 0;
                        }
                        ...
                        if (cont2.RequiredSkill < neededSkill)
                        {
                            ...
                            if (targeted is Item)
                            {
                                ...
                                from.SendMessage(
                                    "You swipe the key card to open the lock, but also wearing it out from further use."
                                );
                            }
                    ...
                    else if (o.Locked && m_Key.ItemID != 0x3A75)
                    {
                        if (o is BaseDoor && !((BaseDoor)o).UseLocks()) // this seems to check house doors also
                        {
                            ...
                            (cont2.RequiredSkill < 51)
                            && !(targeted is TreasureMapChest)
                            && !(targeted is PirateChest)
                            && !(targeted is ParagonChest)
                        )
                        {
                            o.Locked = false;
                            ...
                            if (targeted is Item)
                            {
                                Item item = (Item)targeted;
                                from.SendMessage(
                                    "The key opens the lock, wearing the key out from further use."
                                );
                            }
        ...
        public SkeletonsKey(Serial serial)
            : base(serial) { }
