...
namespace Server.Items
{
    public class DungeonChest : LockableContainer
    {
        ...
        [CommandProperty(AccessLevel.Owner)]
        public int Container_Gump
        {
            get { return ContainerGump; }
            set
            {
                ContainerGump = value;
                InvalidateProperties();
        ...
        [CommandProperty(AccessLevel.Owner)]
        public int Container_Hue
        {
            get { return ContainerHue; }
            set
            {
                ContainerHue = value;
                InvalidateProperties();
        ...
        [CommandProperty(AccessLevel.Owner)]
        public int Container_Flip
        {
            get { return ContainerFlip; }
            set
            {
                ContainerFlip = value;
                InvalidateProperties();
        ...
        [CommandProperty(AccessLevel.Owner)]
        public double Container_Weight
        {
        ...
        [CommandProperty(AccessLevel.Owner)]
        public int Container_Touched
        {
            get { return ContainerTouched; }
            set
            {
                ContainerTouched = value;
                InvalidateProperties();
            }
        }

        [CommandProperty(AccessLevel.Owner)]
        public int Container_NoSpawn
        {
            get { return ContainerNoSpawn; }
            set
            {
                ContainerNoSpawn = value;
                InvalidateProperties();
            }
        }

        [CommandProperty(AccessLevel.Owner)]
        public int Container_Lockable
        {
            get { return ContainerLockable; }
            set
            {
                ContainerLockable = value;
                InvalidateProperties();
            }
        }
        ...
        public static void SetupChest(Item box)
        {
            ...
            else if (reg.IsPartOf("the Temple of Osirus"))
            {
                ...
                reg.IsPartOf("the Lodoria Sewers")
                || reg.IsPartOf("the Montor Sewers")
            ...
            else if (Server.Misc.Worlds.IsFireDungeon(box.Location, box.Map))
            {
                ...
                reg.IsPartOf("Dungeon Hythloth")
                || reg.IsPartOf("the Ancient Pyramid")
                || reg.IsPartOf("the Tomb of the Fallen Wizard")
            )
            {
                design = 7;
            ...
            else if (Server.Misc.Worlds.IsCrypt(box.Location, box.Map))
            {
                ...
                Server.Misc.Worlds.IsSeaDungeon(box.Location, box.Map)
            ...
            int nContainerLockable = ContainerFunctions.BuildContainer(
            ...
            int LockWatch = ContainerFunctions.LockTheContainer(
                chest.ContainerLevel,
                ((LockableContainer)box),
                nContainerLockable
            );
            ...
            else if (box.ItemID == 0x5718)
            {
                chest.ContainerFlip = 0x5719;
            }
            else if (box.ItemID == 0x571A)
            {
                chest.ContainerFlip = 0x571B;
            }
            else if (box.ItemID == 0x5752)
            {
                chest.ContainerFlip = 0x5753;
            }
            else if (box.ItemID == 0x5719)
            {
                chest.ContainerFlip = 0x5718;
            }
            else if (box.ItemID == 0x571B)
            {
                chest.ContainerFlip = 0x571A;
            }
            else if (box.ItemID == 0x5753)
            {
                chest.ContainerFlip = 0x5752;
            }
            else
            {
        ...
        public void RemoveDungeonChest()
        {
            ...
            {
                Item spawnBox = new DungeonChestSpawner(
                    ContainerLevel,
                    (double)(Utility.RandomMinMax(45, 105))
                );
        ...
        public override void Open(Mobile from)
        {
            if (from.Blessed)
            {
                ...
                && from.Skills[SkillName.Hiding].Value < Utility.RandomMinMax(1, 125)
            )
            {
        ...
        public static void OpenCoffin(Mobile from, int item, int level)
        {
            if (Utility.RandomMinMax(1, 10) == 1) // 10% CHANCE FOR RAISING DEAD IN COFFINS
            {
                ...
                )
                {
                    ...
                    if (
                        !Server.Misc.GetPlayerInfo.LuckyPlayer(from.Luck)
                        && Utility.RandomMinMax(1, 100) > seance
                    )
                    {
                        if (level > 6)
                        {
                        ...
                        switch (Utility.Random(level))
                        {
                            ...
                            case 6:
                                ...
                                break;
                            case 7:
                                spawned = new SkeletalWizard();
                                break;
                            case 8:
                                spawned = new BoneKnight();
                                break;
                            case 9:
                                spawned = new BoneMagi();
                                break;
                            case 10:
                                spawned = new SkeletalKnight();
                                break;
                            case 11:
                                spawned = new SkeletalMage();
                                break;
                            case 12:
                                spawned = new Mummy();
                                break;
                            ...
                            case 15:
                                ...
                                break;
                            case 16:
                                spawned = new LichLord();
                                break;
                            case 17:
                                spawned = new RottingCorpse();
                                break;
                        }
                        ...
                        {
                            case 0:
                                sSaying = "Who has disturbed me!";
                                break;
                            case 1:
                                sSaying = "You dare steal from my grave?";
                                break;
                            case 2:
                                sSaying = "Those that take from me will join me!";
                                break;
                            case 3:
                                sSaying = "Your soul is now mine for the taking!";
                                break;
                            case 4:
                                sSaying = "Who dares waken me?";
                                break;
                            case 5:
                                sSaying = "Your life will be extinguished!";
                                break;
                            case 6:
                                sSaying = "Do you have no respect for the dead?";
                                break;
                            case 7:
                                sSaying = "I have been waiting to feast off the living!";
                                break;
                            case 8:
                                sSaying = "Soon you will join my legion of the dead!";
                                break;
                        }
        ...
        public DungeonChest(Serial serial)
            : base(serial) { }
    ...
    public class DungeonChestSpawner : Item
    {
        ...
        [Constructable]
        public DungeonChestSpawner()
            : this(Utility.RandomMinMax(1, 3), 1.0) { }

        [Constructable]
        public DungeonChestSpawner(int level, double respawn)
            : base(0x51e)
        {
            SpawnerLevel = level;
            Name = "chest spawner";
        ...
        public DungeonChestSpawner(Serial serial)
            : base(serial) { }
