    ...
    public class SalvageBag : Bag
    {
        ...
        public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> list)
        {
            ...
            if (from.Alive)
            {
                ...
                list.Add(
                    new SalvageAllEntry(
                        this,
                        IsChildOf(from.Backpack) && Resmeltables() && Scissorables()
                    )
                );
            }
        }
        ...
        private bool Resmeltables() //Where context menu checks for metal items and dragon barding deeds
        {
            foreach (Item i in Items)
            {
                if (i != null && !i.Deleted)
                {
                    ...
                    {
                        if (
                            CraftResources.GetType(((BaseWeapon)i).Resource)
                            == CraftResourceType.Metal
                        )
                            return true;
                    }
                    if (i is BaseArmor)
                    {
                        if (
                            CraftResources.GetType(((BaseArmor)i).Resource)
                            == CraftResourceType.Metal
                        )
                            return true;
                    }
        ...
        private bool Resmelt(Mobile from, Item item, CraftResource resource)
        {
            try
            {
                ...
                CraftItem craftItem = DefBlacksmithy.CraftSystem.CraftItems.SearchFor(
                    item.GetType()
                );
        ...
        private void SalvageIngots(Mobile from)
        {
            ...
            if (m_Failure)
            {
                ...
                from.SendLocalizedMessage(
                    1079973,
                    String.Format("{0}\t{1}", salvaged, salvaged + notSalvaged)
