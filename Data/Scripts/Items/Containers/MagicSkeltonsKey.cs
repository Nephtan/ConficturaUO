...
namespace Server.Items
{
    public class MagicSkeltonsKey : Item
    {
        ...
        private class UnlockTarget : Target
        {
            ...
            public UnlockTarget(MagicSkeltonsKey key)
                ...
                else if (targeted is ILockable)
                {
                    ...
                    else if (o.Locked)
                    {
                        if (o is BaseDoor && !((BaseDoor)o).UseLocks()) // this seems to check house doors also
                        {
                            ...
                            Server.Misc.ContainerFunctions.IsSpaceCrate(((Item)o).ItemID)
                            && ((ILockpickable)targeted).Locked
                            && m_Key.ItemID != 0x3A75
                        )
                        {
                            from.SendMessage(
        ...
        public MagicSkeltonsKey(Serial serial)
            : base(serial) { }
