    ...
    public class Runebook : Item, ISecurable, ICraftable
    {
        ...
        public override void OnAfterDuped(Item newItem)
        {
            ...
            for (int i = 0; i < m_Entries.Count; i++)
            {
                ...
                book.m_Entries.Add(
                    new RunebookEntry(entry.Location, entry.Map, entry.Description, entry.House)
                );
            }
        }
        ...
        public override bool OnDragDrop(Mobile from, Item dropped)
        {
            if (dropped is RecallRune)
            {
                ...
                dropped is Elemental_Warp_Scroll
                || dropped is Elemental_Gate_Scroll
                || dropped is RecallScroll
                || dropped is GateTravelScroll
                || dropped is GraveyardGatewayScroll
                || dropped is HellsGateScroll
                || dropped is AstralTravelScroll
                || dropped is NaturesPassagePotion
                || dropped is MushroomGatewayPotion
            )
            {
                if (m_CurCharges < m_MaxCharges)
                {
                    ...
                    if (amount > (m_MaxCharges - m_CurCharges))
                    {
                        ...
                        if (jars)
                        {
                            from.AddToBackpack(new Jar((m_MaxCharges - m_CurCharges)));
                        }
                    }
                    else
                    {
                        ...
                        if (jars)
                        {
                            from.AddToBackpack(new Jar(amount));
                        }
