...
namespace Server.Items
{
    public class DisguiseKit : Item
    {
        ...
        public bool ValidateUse(Mobile from)
        {
            ...
            if (from.RaceID != 0)
            {
                from.SendMessage("This isn't useful to someone like you.");
            }
            ...
            {
                from.SendMessage(
                    "You cannot disguise yourself since you already are using deception."
                );
            }
        ...
        public static void RemoveDisguise(Mobile from)
        {
            if (
                TransformationSpellHelper.UnderTransformation(
                    from,
                    typeof(Spells.Necromancy.VampiricEmbraceSpell)
                )
            )
            { /* IGNORE */
            }
                ...
                TransformationSpellHelper.UnderTransformation(
                    from,
                    typeof(Spells.Necromancy.WraithFormSpell)
                )
            )
            { /* IGNORE */
            }
                ...
                TransformationSpellHelper.UnderTransformation(
                    from,
                    typeof(Spells.Necromancy.LichFormSpell)
                )
            )
            { /* IGNORE */
            }
                ...
                TransformationSpellHelper.UnderTransformation(
                    from,
                    typeof(Spells.Necromancy.HorrificBeastSpell)
                )
            )
            { /* IGNORE */
            }
                ...
                IsDisguised(from)
                || !from.CanBeginAction(typeof(PolymorphSpell))
                || !from.CanBeginAction(typeof(IncognitoSpell))
                || !from.CanBeginAction(typeof(Deception))
            )
            {
                from.HueMod = -1;
                from.NameMod = null;
