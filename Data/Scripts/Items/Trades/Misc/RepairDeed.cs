...
namespace Server.Items
{
    public class RepairDeed : Item
    {
        private class RepairSkillInfo
        {
            ...
            private TextDefinition m_NotNearbyMessage,
                m_Name;

            public TextDefinition NotNearbyMessage
            {
                get { return m_NotNearbyMessage; }
            }
            ...
            {
                get { return m_NearbyTypes; }
            }

            public RepairSkillInfo(
                CraftSystem system,
                Type[] nearbyTypes,
                TextDefinition notNearbyMessage,
                TextDefinition name
            )
            {
                m_System = system;
        ...
        public override void AddNameProperty(ObjectPropertyList list)
        {
            list.Add(
                ...
                String.Format(
                    "{0}\t{1}",
                    GetSkillTitle(m_SkillLevel).ToString(),
                    RepairSkillInfo.GetInfo(m_Skill).Name
                )
        ...
        public bool Check(Mobile from)
        {
            if (!IsChildOf(from.Backpack))
                ...
                TextDefinition.SendMessageTo(
                    from,
                    RepairSkillInfo.GetInfo(m_Skill).NotNearbyMessage
                );
            else
                return true;
        ...
        public bool VerifyRegion(Mobile m)
        {
            ...
            return Server.Factions.Faction.IsNearType(
                m,
                RepairSkillInfo.GetInfo(m_Skill).NearbyTypes,
                6
            );
        }
