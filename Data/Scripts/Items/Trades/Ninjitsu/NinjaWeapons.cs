...
namespace Server.Items
{
    ...
    public class NinjaWeapon
    {
        ...
        private static void Unload(Mobile from, INinjaWeapon weapon)
        {
            ...
            {
                INinjaAmmo ammo =
                    Activator.CreateInstance(weapon.AmmoType, new object[] { weapon.UsesRemaining })
                    as INinjaAmmo;
        ...
        private static void Reload(PlayerMobile from, INinjaWeapon weapon, INinjaAmmo ammo)
        {
            if (weapon.UsesRemaining < MaxUses)
            {
                ...
                if (need > 0)
                {
                    if (
                        weapon.Poison != null
                        && (ammo.Poison == null || weapon.Poison.Level > ammo.Poison.Level)
                    )
                    {
                    ...
                    else
                    {
                        ...
                        int poisonneeded = Math.Min(
                            (MaxUses - weapon.PoisonCharges),
                            ammo.PoisonCharges
                        );
        ...
        private static bool CombatCheck(Mobile attacker, Mobile defender) /* mod'd from baseweapon */
        {
            ...
            if (
                ...
                || AnimalForm.UnderTransformation(attacker, typeof(MysticalFox))
            )
            {
                attackValue += 20;
            }
