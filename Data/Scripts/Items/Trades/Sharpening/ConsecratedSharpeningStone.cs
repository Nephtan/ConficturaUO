...
{
    public class ConsecratedSharpeningStone : Item
    {
        private int i_Uses;
        ...
        [Constructable]
        public ConsecratedSharpeningStone()
            : this(5) { }

        [Constructable]
        public ConsecratedSharpeningStone(int uses)
            : base(0x1F14)
        {
            Weight = 1.0;
            i_Uses = uses;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (IsChildOf(from.Backpack))
            {
                ...
                from.Target = new ConsecratedSharpeningStoneTarget(this);
            }
            else
        ...
        public override void AddNameProperties(ObjectPropertyList list)
        {
            ...
            list.Add(1070722, "Can Increase A Sword Weapon's Damage");
            ...
            if (o is Item)
            {
                ...
                else if (o is BaseSword && ((BaseSword)o).IsChildOf(from.Backpack))
                {
                    ...
                    if (i_DI >= 70)
                    {
                        ...
                        from.SendMessage(
                            32,
                            "You need at least 100.0 blacksmith and 80.0 knightship to sharpen weapons with this stone"
                        );
                    else if (from.Skills[SkillName.Knightship].Value < 80.0)
                        from.SendMessage(
                            32,
                            "You need at least 100.0 blacksmith and 80 knightship to sharpen weapons with this stone"
                        );
                    else if (!Deleted)
                    {
                        int bonus = Utility.Random(
                            (int)(from.Skills[SkillName.Blacksmith].Value / 10)
                        );
                        if (bonus > 0)
                        {
                            if (70 < i_DI + bonus)
                            ...
                            from.SendMessage(
                                88,
                                "You sharpened the weapon with {0} damange increase",
                                bonus
                            );
                        }
                        else
                ...
                else if (o is BasePoleArm && ((BasePoleArm)o).IsChildOf(from.Backpack))
                {
                    BasePoleArm weap = o as BasePoleArm;
                    int i_DI = weap.Attributes.WeaponDamage;
                    if (weap.Quality == WeaponQuality.Exceptional)
                        i_DI += 15;
                    if (i_DI >= 70)
        ...
        public ConsecratedSharpeningStone(Serial serial)
            : base(serial) { }
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            if (version == 0)
            {
                Serial sr_Owner = reader.ReadInt();
            }
        }
    }

    public class ConsecratedSharpeningStoneTarget : Target
    {
        private ConsecratedSharpeningStone sb_Blade;

        public ConsecratedSharpeningStoneTarget(ConsecratedSharpeningStone blade)
