            ...
            IWearableDurability
    {
        ...
        public static void CheckMetalColor(BaseArmor armor)
        {
            ...
            )
            {
                ...
                if (armor.Hue == 0)
                {
                    armor.Hue = Server.Misc.MaterialInfo.PlainLeatherColor();
                }
            }
            else if (
            ...
            )
            {
                ...
                if (armor.Hue == 0)
                {
                    if (armor.Resource == CraftResource.Iron && armor is RingmailSkirt)
                    {
                        armor.Hue = 0xABF;
                    }
                    else if (armor.Resource == CraftResource.Iron)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.PlainIronColor(armor.ItemID);
                    }
                    else if (armor.Resource == CraftResource.DullCopper)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("dull copper", "", 0);
                    }
                    else if (armor.Resource == CraftResource.ShadowIron)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("shadow iron", "", 0);
                    }
                    else if (armor.Resource == CraftResource.Copper)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("copper", "", 0);
                    }
                    else if (armor.Resource == CraftResource.Bronze)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("bronze", "", 0);
                    }
                    else if (armor.Resource == CraftResource.Gold)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("gold", "", 0);
                    }
                    else if (armor.Resource == CraftResource.Agapite)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("agapite", "", 0);
                    }
                    else if (armor.Resource == CraftResource.Verite)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("verite", "", 0);
                    }
                    else if (armor.Resource == CraftResource.Valorite)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("valorite", "", 0);
                    }
                    else if (armor.Resource == CraftResource.Steel)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("steel", "", 0);
                    }
                    else if (armor.Resource == CraftResource.Brass)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("brass", "", 0);
                    }
                    else if (armor.Resource == CraftResource.Mithril)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("mithril", "", 0);
                    }
                    else if (armor.Resource == CraftResource.Xormite)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("xormite", "", 0);
                    }
                    else if (armor.Resource == CraftResource.Obsidian)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("obsidian", "", 0);
                    }
                    else if (armor.Resource == CraftResource.Nepturite)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("nepturite", "", 0);
                    }
                    else if (armor.Resource == CraftResource.Dwarven)
                    {
                        armor.Hue = Server.Misc.MaterialInfo.GetMaterialColor("dwarven", "", 0);
                    }
                }
            }
        }
        ...
        public override int PhysicalResistance
        {
            get
            {
                return BasePhysicalResistance
                    + GetProtOffset()
                    + GetResourceAttrs().ArmorPhysicalResist
                    + m_PhysicalBonus;
            }
        }
        public override int FireResistance
        {
            get
            {
                return BaseFireResistance
                    + GetProtOffset()
                    + GetResourceAttrs().ArmorFireResist
                    + m_FireBonus;
            }
        }
        public override int ColdResistance
        {
            get
            {
                return BaseColdResistance
                    + GetProtOffset()
                    + GetResourceAttrs().ArmorColdResist
                    + m_ColdBonus;
            }
        }
        public override int PoisonResistance
        {
            get
            {
                return BasePoisonResistance
                    + GetProtOffset()
                    + GetResourceAttrs().ArmorPoisonResist
                    + m_PoisonBonus;
            }
        }
        public override int EnergyResistance
        {
            get
            {
                return BaseEnergyResistance
                    + GetProtOffset()
                    + GetResourceAttrs().ArmorEnergyResist
                    + m_EnergyBonus;
            }
        }
                ...
                try
                {
                    Item res = (Item)
                        Activator.CreateInstance(
                            CraftResources.GetInfo(m_Resource).ResourceTypes[0]
                        );

                    ScissorHelper(
                        from,
                        res,
                        m_PlayerConstructed ? (item.Resources.GetAt(0).Amount / 2) : 1
                    );
                    return true;
                }
                catch { }
        ...
        public override bool CanEquip(Mobile from)
        {
            ...
            if (from.AccessLevel < AccessLevel.GameMaster)
            {
                ...
                {
                    int strBonus = ComputeStatBonus(StatType.Str),
                        strReq = ComputeStatReq(StatType.Str);
                    int dexBonus = ComputeStatBonus(StatType.Dex),
                        dexReq = ComputeStatReq(StatType.Dex);
                    int intBonus = ComputeStatBonus(StatType.Int),
                        intReq = ComputeStatReq(StatType.Int);
        ...
        public override bool OnEquip(Mobile from)
        {
            ...
            if (this.Layer == Layer.Gloves)
            {
                ...
                )
                {
                    ...
                    from.Backpack.DropItem(oneHand);
                }
                else if (
                    (from.FindItemOnLayer(Layer.FirstValid) is PugilistGloves)
                    || (from.FindItemOnLayer(Layer.FirstValid) is PugilistGlove)
                    || (from.FindItemOnLayer(Layer.FirstValid) is Artifact_GlovesOfThePugilist)
                    || (from.FindItemOnLayer(Layer.FirstValid) is GlovesOfThePugilist)
                )
                {
                    Item firstValid = from.FindItemOnLayer(Layer.FirstValid);
                    from.Backpack.DropItem(firstValid);
                }
            }
        ...
        private class HatTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is Item)
                {
                    ...
                    if (from.FindItemOnLayer(Layer.Helm) != m_Hats)
                    {
                        from.SendMessage("You must be wearing this to change the color.");
                    }
                    else if (from.FindItemOnLayer(Layer.Waist) == iColorHat)
                    {
                        color = iColorHat.Hue;
                    }
                    else if (from.FindItemOnLayer(Layer.OuterTorso) == iColorHat)
                    {
                        color = iColorHat.Hue;
                    }
                    else if (from.FindItemOnLayer(Layer.Arms) == iColorHat)
                    {
                        color = iColorHat.Hue;
                    }
                    else if (from.FindItemOnLayer(Layer.OuterLegs) == iColorHat)
                    {
                        color = iColorHat.Hue;
                    }
                    else if (from.FindItemOnLayer(Layer.Neck) == iColorHat)
                    {
                        color = iColorHat.Hue;
                    }
                    else if (from.FindItemOnLayer(Layer.Gloves) == iColorHat)
                    {
                        color = iColorHat.Hue;
                    }
                    else if (from.FindItemOnLayer(Layer.Shoes) == iColorHat)
                    {
                        color = iColorHat.Hue;
                    }
                    else if (from.FindItemOnLayer(Layer.Cloak) == iColorHat)
                    {
                        color = iColorHat.Hue;
                    }
                    else if (from.FindItemOnLayer(Layer.FirstValid) == iColorHat)
                    {
                        color = iColorHat.Hue;
                    }
                    else if (from.FindItemOnLayer(Layer.InnerLegs) == iColorHat)
                    {
                        color = iColorHat.Hue;
                    }
                    else if (from.FindItemOnLayer(Layer.InnerTorso) == iColorHat)
                    {
                        color = iColorHat.Hue;
                    }
                    else if (from.FindItemOnLayer(Layer.Pants) == iColorHat)
                    {
                        color = iColorHat.Hue;
                    }
                    else if (from.FindItemOnLayer(Layer.Shirt) == iColorHat)
                    {
                        color = iColorHat.Hue;
                    }
                    else
        ...
        public virtual int OnHit(BaseWeapon weapon, int damageTaken)
        {
            ...
            if (Server.Misc.MaterialInfo.IsAnyKindOfMetalItem((Item)this))
                ruin = 85; // Metal armor is just more durable and it gives life back to metal armor types
            ...
            if (ruin < Utility.Random(100)) // chance to lower durability
            {
                ...
                else if (Core.AOS && m_AosArmorAttributes.SelfRepair > Utility.Random(10))
                {
                    HitPoints += 2;
                }
                else
