    ...
    public class RaceTeleporter : Item
    {
        ...
        public void UseTeleporter(Mobile m)
        {
            ...
            if (
                this.Name == "kraken"
                && m is PlayerMobile
                && Server.Items.BaseRace.IsEvilSeaCreature(m)
                && m.RaceHomeLand == 2
            )
            {
            ...
            )
            {
                ...
                this.Name == "gargoyle"
                && m is PlayerMobile
                && Server.Items.BaseRace.IsEvilDemonCreature(m)
            )
            {
            ...
            )
            {
                ...
                && m is PlayerMobile
                && Server.Items.BaseRace.IsEvilDemonCreature(m)
                && PlayerSettings.GetDiscovered(m, "the Serpent Island")
            )
            {
                loc = new Point3D(cur.X + 2, cur.Y, cur.Z + 5);
                map = Map.SerpentIsland;
            ...
            )
            {
                ...
                this.Name == "dead"
                && m is PlayerMobile
                && Server.Items.BaseRace.IsEvilDeadCreature(m)
                && m.RaceHomeLand == 2
            )
            {
