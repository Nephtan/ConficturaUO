...
namespace Server.Items
{
    ...
    public class MagicMirror : Item
    {
        ...
        public int Mirror_Destination
        {
            get { return MirrorDestination; }
            set
            {
                MirrorDestination = value;
        ...
        [CommandProperty(AccessLevel.Owner)]
        public Map Mirror_Map
        {
            get { return MirrorMap; }
            set
            {
                MirrorMap = value;
                InvalidateProperties();
        ...
        public MagicMirror(Serial serial)
            : base(serial) { }
        ...
        public static void SetMirrors()
        {
            ...
            for (int i = 0; i < mirrors.Count; ++i)
            {
                ...
                {
                    if (world == "the Isles of Dread" && MirrorIslesDread < 2)
                    {
                        MirrorIslesDread++;
                        item.Visible = true;
                    }
                    else if (world == "the Savaged Empire" && MirrorSavagedEmpire < 2)
                    {
                        MirrorSavagedEmpire++;
                        item.Visible = true;
                    }
                    else if (world == "the Land of Lodoria" && MirrorLodor < 2)
                    {
                        MirrorLodor++;
                        item.Visible = true;
                    }
                    else if (world == "the Serpent Island" && MirrorSerpentIsland < 2)
                    {
                        MirrorSerpentIsland++;
                        item.Visible = true;
                    }
        ...
        public virtual void DoTeleport(Mobile m)
        {
            ...
            if (ItemID == 0x2145)
            {
                p = new Point3D((this.X + 1), this.Y, this.Z);
            }
            ...
            if (MirrorDestination < 1)
            {
                foreach (Item item in World.Items.Values)
                    if (
                        ...
                        && item != this
                        && item.Visible
                        && Worlds.GetMyWorld(item.Map, item.Location, item.X, item.Y)
                            == Worlds.GetMyWorld(this.Map, this.Location, this.X, this.Y)
                    )
                    {
                        p = new Point3D(item.X, (item.Y + 1), item.Z);
                        if (item.ItemID == 0x2145)
                        {
                            p = new Point3D((item.X + 1), item.Y, item.Z);
                        }
