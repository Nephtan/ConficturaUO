...
namespace Server.Items
{
    ...
    public abstract class BaseDoor : Item, ILockable, ITelekinesisable
    {
        ...
        {
            EventSink.OpenDoorMacroUsed += new OpenDoorMacroEventHandler(
                EventSink_OpenDoorMacroUsed
            );

            CommandSystem.Register(
                "Link",
                AccessLevel.GameMaster,
                new CommandEventHandler(Link_OnCommand)
            );
            CommandSystem.Register(
                "ChainLink",
                AccessLevel.GameMaster,
                new CommandEventHandler(ChainLink_OnCommand)
            );
        }
        ...
        private static void Link_OnCommand(CommandEventArgs e)
        {
            e.Mobile.BeginTarget(
                ...
                false,
                TargetFlags.None,
                new TargetCallback(Link_OnFirstTarget)
            );
        ...
        private static void Link_OnFirstTarget(Mobile from, object targeted)
        {
            ...
            else
            {
                from.BeginTarget(
                    ...
                    false,
                    TargetFlags.None,
                    new TargetStateCallback(Link_OnSecondTarget),
                    door
                );
        ...
        private static void ChainLink_OnCommand(CommandEventArgs e)
        {
            e.Mobile.BeginTarget(
                ...
                false,
                TargetFlags.None,
                new TargetStateCallback(ChainLink_OnTarget),
                new List<BaseDoor>()
            );
            ...
            else
            {
                ...
                if (list.Count > 0 && list[0] == door)
                {
                    ...
                    else
                    {
                        ...
                        from.SendMessage(
                            "You have not yet targeted two unique doors. Target the second door to link."
                        );
                    }
                }
            ...
            if (m.Map != null)
            {
                ...
                foreach (Item item in sector.Items)
                {
                    if (
                        item.Location.X == x
                        && item.Location.Y == y
                        && (item.Z + item.ItemData.Height) > m.Z
                        && (m.Z + 16) > item.Z
                        && item is BaseDoor
                        && m.CanSee(item)
                        && m.InLOS(item)
