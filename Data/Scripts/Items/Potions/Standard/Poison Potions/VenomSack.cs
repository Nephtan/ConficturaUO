...
namespace Server.Items
{
    public class VenomSack : Item
    {
        ...
        public VenomSack(Serial serial)
            : base(serial) { }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else if (this.Name == "venom sack")
            {
                nSkill = 15;
            }
            else if (this.Name == "greater venom sack")
            {
                nSkill = 35;
            }
            else if (this.Name == "deadly venom sack")
            {
                nSkill = 55;
            }
            else
            ...
            if (from.CheckSkill(SkillName.Poisoning, nSkill, 125))
            {
                ...
                else
                {
                    ...
                    else if (this.Name == "deadly venom sack")
                    {
                        ...
                        sPotion = "deadly poison";
                    }
                    else
                    {
                        from.AddToBackpack(new LethalPoisonPotion());
                        sPotion = "lethal poison";
                    }

                    if (from.CheckSkill(SkillName.Poisoning, nSkill, 125))
                    {
                        from.SendMessage(
                            "You get a bottle of " + sPotion + " from some of the sack."
                        );
                    }
                    else
                    {
                        from.SendMessage(
                            "You get a bottle of " + sPotion + " from the rest of the sack."
                        );
                        this.Consume();
                    }
                    return;
