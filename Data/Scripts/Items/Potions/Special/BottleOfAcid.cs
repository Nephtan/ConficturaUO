...
namespace Server.Items
{
    public class BottleOfAcid : Item
    {
        ...
        private class UnlockTarget : Target
        {
            ...
            public UnlockTarget(BottleOfAcid key)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                else if (targeted is Head)
                {
                    if (((Item)targeted).ItemID == 7584 || ((Item)targeted).ItemID == 7393)
                    {
                        ...
                        {
                            (((Item)targeted).Name) = (((Item)targeted).Name).Replace(
                                " head ",
                                " skull "
                            );
                        }
                ...
                else if (targeted is ILockable)
                {
                    ...
                    if ((o.Locked) || (cont3.TrapType != TrapType.None))
                    {
                        ...
                        {
                            from.SendMessage(
                                "This acid is to dissolve locks and traps on most chests."
                            );
                        }
                        else if (targeted is TreasureMapChest)
                        {
                            from.SendMessage(
                                "The acid seems to have done nothing to the mechanism inside."
                            );
                            m_Key.Consume();
                        }
                        else if (100 >= cont2.RequiredSkill)
                        {
                            ...
                            if (targeted is Item)
                            {
                                ...
                                from.SendMessage(
                                    "The acid seems to have eaten away at the mechanism inside."
                                );
                            }
        ...
        public BottleOfAcid(Serial serial)
            : base(serial) { }
