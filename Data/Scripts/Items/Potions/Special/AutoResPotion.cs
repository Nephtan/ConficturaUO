...
namespace Server.Items
{
    public class AutoResPotion : Item
    {
        ...
        [Constructable]
        public AutoResPotion()
            : this(1) { }

        [Constructable]
        public AutoResPotion(int charges)
            : base(0x0E0F)
        {
            m_Charges = charges;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else if (from != null && this != null && !m_ResList.ContainsKey(from))
            {
                if (!m_ResList.ContainsValue(this))
                {
                    ...
                    from.SendMessage(
                        "You feel the spirits watching you, awaiting to send you back to your body."
                    );
                }
                else
        ...
        private static void Resurrect_OnTick(object state)
        {
            ...
            if (owner != null && !owner.Deleted && arp != null && !arp.Deleted)
            {
                ...
                owner.SendMessage(
                    "You died under the watch of the spirits, they have offered you another chance at life."
                );
                owner.Resurrect();
