...
namespace Server.Items
{
    public class SuperPotion : BasePotion
    {
        [Constructable]
        public SuperPotion()
            : base(0x180F, PotionEffect.SuperPotion)
        {
            Hue = 0xBA4;
            Name = "superior potion";
        }

        public SuperPotion(Serial serial)
            : base(serial) { }
        ...
        public override void Drink(Mobile m)
        {
            if (m.InRange(this.GetWorldLocation(), 1))
            {
                ...
                {
                    m.AddStatMod(
                        new StatMod(StatType.Int, modName + "Int", 10, TimeSpan.FromMinutes(5))
                    );
                    m.AddStatMod(
                        new StatMod(StatType.Dex, modName + "Dex", 10, TimeSpan.FromMinutes(5))
                    );
                    m.AddStatMod(
                        new StatMod(StatType.Str, modName + "Str", 10, TimeSpan.FromMinutes(5))
                    );
                    m.Hits = m.HitsMax;
                    m.Mana = m.ManaMax;
                    m.Stam = m.StamMax;
