...
namespace Server.Items
{
    public abstract class BaseManaRefreshPotion : BasePotion
    {
        ...
        public BaseManaRefreshPotion(PotionEffect effect)
            : base(0x180F, effect) { }

        public BaseManaRefreshPotion(Serial serial)
            : base(serial) { }
            ...
            if (from.Mana < from.ManaMax)
            {
                if (from.BeginAction(typeof(BaseManaRefreshPotion)))
                {
                    ...
                    Timer.DelayCall(
                        ...
                        new TimerStateCallback(ReleaseManaLock),
                        from
                    );
                }
                else
                    from.LocalOverheadMessage(
                        ...
                        0x22,
                        true,
                        "You must wait 10 seconds before using another mana potion."
                    );
            }
            else
                from.SendMessage(
                    "You decide against drinking this potion, as you are already at full mana."
                );
        }
