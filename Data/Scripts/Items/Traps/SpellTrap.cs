...
namespace Server.Items
{
    public class SpellTrap : Item
    {
        ...
        } // HOW LONG UNTIL THE TRAP DECAYS IN SECONDS

        [Constructable]
        public SpellTrap(Mobile source, int level)
            : base(0x0702)
        {
        ...
        public SpellTrap(Serial serial)
            : base(serial) { }
            ...
            if (owner != m)
            {
                ...
                if (m is PlayerMobile && Spells.Research.ResearchAirWalk.UnderEffect(m))
                {
                    ...
                    (
                        ...
                        && Server.Misc.SeeIfGemInBag.GemInPocket(m) == false
                ...
                )
                {
                        ...
                        if (m.CheckSkill(SkillName.Psychology, 0, 125))
                        {
                            if (m is PlayerMobile)
                            {
                                m.LocalOverheadMessage(
                                    ...
                                    0x3B2,
                                    false,
                                    "You got near a magical trap, but you were too intelligent to suffer the effects."
                                );
                            }
                            Sprung = 0;
                        }
                    ...
                    if (Sprung > 0)
                    {
                        if (this.Hue == 0x48F) // POISON TRAP
                        {
                            ...
                            if (m is PlayerMobile)
                            {
                                m.LocalOverheadMessage(
                                    ...
                                    0xB1F,
                                    true,
                                    "You triggered a magical trap!"
                                );
                            }
                            itHurts = (int)(
                                (Utility.RandomMinMax(StrMin, StrMax) * (100 - m.PoisonResistance))
                                / 100
                            );
                            m.Damage(itHurts, m);
                        }
                        else if (this.Hue == 0x489) // FLAME TRAP
                        {
                            ...
                            int itHurts = (int)(
                                (Utility.RandomMinMax(StrMin, StrMax) * (100 - m.FireResistance))
                                / 100
                            );
                            m.Damage(itHurts, m);
                        }
                        else if (this.Hue == 0x48E) // EXPLOSION TRAP
                        {
                            ...
                            int itHurts = (int)(
                                (
                                    Utility.RandomMinMax(StrMin, StrMax)
                                    * (100 - m.PhysicalResistance)
                                ) / 100
                            );
                            m.Damage(itHurts, m);
                        }
                        else if (this.Hue == 0x490) // ELECTRICAL TRAP
                        {
                            ...
                            int itHurts = (int)(
                                (Utility.RandomMinMax(StrMin, StrMax) * (100 - m.EnergyResistance))
                                / 100
                            );
                            m.Damage(itHurts, m);
                        }
                        else if (this.Hue == 0x480) // BLIZZARD TRAP
                        {
                            ...
                            int itHurts = (int)(
                                (Utility.RandomMinMax(StrMin, StrMax) * (100 - m.ColdResistance))
                                / 100
                            );
                            m.Damage(itHurts, m);
                        }
