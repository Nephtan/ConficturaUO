...
namespace Server.Multis
{
    public class HouseSign : Item
    {
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
            ...
            list.Add(
                1061640,
                (m_Owner == null || m_Owner.Owner == null) ? "nobody" : m_Owner.Owner.Name
            ); // Owner: ~1_OWNER~
        ...
        public void ClaimGump_Callback(Mobile from, bool okay, object state)
        {
            if (
                ...
                && m_Owner != null
                && m_Owner.Owner == null
                && m_Owner.DecayLevel != DecayLevel.DemolitionPending
            )
            {
                bool canClaim = false;
                ...
                if (canClaim && !BaseHouse.HasAccountHouse(m))
                {
                    ...
                    m.SendGump(
                        new WarningGump(
                            ...
                            420,
                            280,
                            new WarningGumpCallback(ClaimGump_Callback),
                            null
                        )
                    );
                }
        ...
        private class ReclaimVendorInventoryEntry : ContextMenuEntry
        {
            ...
            public override void OnClick()
            {
                ...
                if (
                    m_Sign.Deleted
                    || m_Sign.Owner == null
                    || m_Sign.Owner.VendorInventories.Count == 0
                    || !from.CheckAlive()
                )
                    return;
