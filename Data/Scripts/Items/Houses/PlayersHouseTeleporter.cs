...
namespace Server.Items
{
    public class PlayersHouseTeleporter : Item, IDyable, ISecurable
    {
        ...
        [Constructable]
        public PlayersHouseTeleporter()
            : this(new Point3D(0, 0, 0), null, false) { }

        [Constructable]
        public PlayersHouseTeleporter(Point3D pointDest, Map mapDest)
            : this(pointDest, mapDest, false) { }

        [Constructable]
        public PlayersHouseTeleporter(Point3D pointDest, Map mapDest, bool creatures)
            : base(0x181D)
        {
            Movable = true;
            Visible = true;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.SendMessage(
                    "This does not seem to have any magical properties in this place!"
                );
                m_Owner = null;
                m_MapDest = null;
            ...
            {
                from.SendMessage(
                    "You change the appearance of the teleporter but it must be secured in a house to mark it. This teleporter can be dyed as well."
                );
            ...
            {
                from.SendMessage(
                    "The destination has been cleared. This must be secured in a house to mark it or in your backpack to change the appearance! This teleporter can be dyed as well."
                );
                m_Owner = null;
                m_MapDest = null;
                m_PointDest = new Point3D(0, 0, 0);
        ...
        {
            if (
                Worlds.RegionAllowedTeleport(m_MapDest, m_PointDest, m_PointDest.X, m_PointDest.Y)
                == false
            )
            {
        ...
        public PlayersHouseTeleporter(Serial serial)
            : base(serial) { }
