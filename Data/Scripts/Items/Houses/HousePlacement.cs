...
namespace Server.Multis
{
    ...
    public class HousePlacement
    {
        ...
        )
        {
            ...
            NoHousingRegion noHousingRegion = (NoHousingRegion)
                Region.Find(center, map).GetRegion(typeof(NoHousingRegion));
            ...
            List<Point2D> yard = new List<Point2D>(),
                borders = new List<Point2D>();
            ...
            for (int x = 0; x < mcl.Width; ++x)
            {
                for (int y = 0; y < mcl.Height; ++y)
                {
                    ...
                    for (int i = 0; i < addTiles.Length; ++i)
                    {
                        ...
                        for (int j = 0; j < oldTiles.Length; ++j)
                        {
                            ...
                            if (
                                (
                                    id.Impassable
                                    || (id.Surface && (id.Flags & TileFlag.Background) == 0)
                                )
                                && addTileTop > oldTile.Z
                                && (oldTile.Z + id.CalcHeight) > addTileZ
                            )
                    ...
                    if (
                        in_reg.IsPartOf(typeof(NecromancerRegion))
                        || in_reg.IsPartOf(typeof(LunaRegion))
                        || in_reg.IsPartOf(typeof(UnderHouseRegion))
                        || (
            ...
            for (int i = 0; i < borders.Count; ++i)
            {
                ...
                for (int j = 0; j < tiles.Length; ++j)
                {
                    ...
                    if (
                        ...
                        || (
                            id.Surface
                            && (id.Flags & TileFlag.Background) == 0
