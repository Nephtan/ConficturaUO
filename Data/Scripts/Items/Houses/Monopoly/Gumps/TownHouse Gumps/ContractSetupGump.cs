    ...
    public class ContractSetupGump : GumpPlusLight
    {
        ...
        private void BlocksPage(int width, ref int y)
        {
            ...
            if (!General.HasOtherContract(c_Contract.ParentHouse, c_Contract))
            {
                ...
                AddButton(
                    30,
                    y,
                    c_Contract.EntireHouse ? 0xD3 : 0xD2,
                    "Entire House",
                    new GumpCallback(EntireHouse)
                );
            }
        ...
        private void PricePage(int width, ref int y)
        {
            ...
            AddButton(
                width / 2 - 80,
                y,
                c_Contract.Free ? 0xD3 : 0xD2,
                "Free",
                new GumpCallback(Free)
            );
            AddButton(
                width / 2 + 60,
                y,
            ...
            if (!c_Contract.Free)
            {
                AddHtml(
                    ...
                    y += 25,
                    width / 2 - 20,
                    "<DIV ALIGN=RIGHT>Per " + c_Contract.PriceTypeShort
                );
                AddTextField(
                    width / 2 + 20,
                    y,
            ...
            if (c_Contract.Price != 0)
            {
                ...
                AddButton(
                    ...
                    0x15E5,
                    "Finalize",
                    new GumpCallback(FinalizeSetup)
                );
            }
        }
        ...
        private void EntireHouse()
        {
            ...
            if (c_Contract.EntireHouse)
            {
                ...
                foreach (Rectangle3D rect in RUOVersion.RegionArea(c_Contract.ParentHouse.Region))
                {
                    list.Add(
                        new Rectangle2D(
                            new Point2D(rect.Start.X, rect.Start.Y),
        ...
        private void FinalizeSetup()
        {
            ...
            Owner.SendMessage(
                "You have finalized this rental contract.  Now find someone to sign it!"
            );
        }
            ...
            protected override void OnTarget(Mobile m, object o)
            {
                ...
                switch (c_Type)
                {
                    ...
                    case TargetType.MinZ:
                        if (
                            !c_Contract.ParentHouse.Region.Contains(
                    ...
                    case TargetType.BlockTwo:
                        Rectangle2D rect = TownHouseSetupGump.FixRect(
                            new Rectangle2D(
                                c_BoundOne,
