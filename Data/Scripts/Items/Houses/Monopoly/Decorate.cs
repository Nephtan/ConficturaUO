    ...
    public class Monopoly
    {
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(Monopoly_OnCommand)
            );
        }
        ...
        public static void Monopoly_OnCommand(CommandEventArgs e)
        {
            ...
            m_Mobile.SendMessage(
                "Special home items generation complete. {0} items were generated.",
                m_Count
            );
        }
    ...
    public class MonopoliesList
    {
        ...
        public Item Construct()
        {
            ...
            else if (item is FillableContainer)
            {
                ...
                for (int i = 0; i < m_Params.Length; ++i)
                {
                    if (m_Params[i].StartsWith("ContentType"))
                    {
                        ...
                        if (indexOf >= 0)
                            cont.ContentType = (FillableContentType)
                                Enum.Parse(
            ...
            for (int i = 0; i < m_Params.Length; ++i)
            {
                if (m_Params[i].StartsWith("Light"))
                {
                    ...
                    if (indexOf >= 0)
                        item.Light = (LightType)
                            Enum.Parse(typeof(LightType), m_Params[i].Substring(++indexOf), true);
                }
        ...
        private static bool FindItem(int x, int y, int z, Map map, Item srcItem)
        {
            ...
            if (srcItem is BaseDoor)
            {
                ...
                (TileData.ItemTable[itemID & TileData.MaxItemValue].Flags & TileFlag.LightSource)
                != 0
            )
            {
                ...
                foreach (Item item in eable)
                {
                    if (item.Z == z)
                    {
                        if (item.ItemID == itemID)
                        {
                            ...
                            (item.ItemData.Flags & TileFlag.LightSource) != 0
                            && item.ItemData.Name == srcName
                        )
                        {
                            m_DeleteQueue.Enqueue(item);
                        }
        ...
        public static MonopoliesList Read(StreamReader ip)
        {
            ...
            if (list.m_Type == null)
                throw new ArgumentException(
                    String.Format("Type not found for header: '{0}'", line)
                );
