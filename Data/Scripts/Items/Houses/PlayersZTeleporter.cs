...
namespace Server.Items
{
    public class PlayersZTeleporter : Item, IDyable, ISecurable
    {
        ...
        [Constructable]
        public PlayersZTeleporter()
            : this(new Point3D(0, 0, 0), null, false) { }

        [Constructable]
        public PlayersZTeleporter(Point3D pointDest, Map mapDest)
            : this(pointDest, mapDest, false) { }

        [Constructable]
        public PlayersZTeleporter(Point3D pointDest, Map mapDest, bool creatures)
            : base(0x181D)
        {
            Movable = true;
            Visible = true;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.SendMessage(
                    "You change the appearance of the teleporter but it must be secured in a house to set it. This teleporter can be dyed as well."
                );
            ...
            {
                from.SendMessage(
                    "The ownership has been cleared. This must be secured in a house to set it or in your backpack to change the appearance! This teleporter can be dyed as well."
                );
                m_Owner = null;
                m_MapDest = null;
            ...
            else
            {
                ...
                else
                {
                    ...
                    from.SendMessage(
                        "You have set the teleporter.  It must be locked down in that house to use it."
                    );
                }
            }
            return;
        ...
        public PlayersZTeleporter(Serial serial)
            : base(serial) { }
