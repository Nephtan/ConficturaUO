...
namespace Server.Multis
{
    ...
    public class HouseFoundation : BaseHouse
    {
        ...
        {
            get { return m_Fixtures; }
        }
        public Item SignHanger
        {
            get { return m_SignHanger; }
        }
        ...
        {
            get { return Customizer == null; }
        }

        public virtual int CustomizationCost
        {
            get { return (Core.AOS ? 0 : 10000); }
        }
        ...
        public override void OnLocationChange(Point3D oldLocation)
        {
            ...
            if (m_SignHanger != null)
                m_SignHanger.MoveToWorld(
                    new Point3D(m_SignHanger.X + x, m_SignHanger.Y + y, m_SignHanger.Z + z),
                    Map
                );

            if (m_Signpost != null)
                m_Signpost.MoveToWorld(
                    new Point3D(m_Signpost.X + x, m_Signpost.Y + y, m_Signpost.Z + z),
                    Map
                );
        ...
        public void AddFixtures(Mobile from, MultiTileEntry[] list)
        {
            ...
            for (int i = 0; i < list.Length; ++i)
            {
                ...
                else
                {
                    ...
                    else if (itemID >= 0x2A05 && itemID < 0x2A1D)
                    {
                        ...
                        door = new GenericHouseDoor(
                            facing,
                            0x29F5 + (8 * ((itemID - 0x2A05) / 8)),
                            sound,
                            sound
                        );
                    }
            ...
            for (int i = 0; i < m_Fixtures.Count; ++i)
            {
                ...
                if (fixture is HouseTeleporter)
                {
                    ...
                    {
                        HouseTeleporter check =
                            m_Fixtures[(i + j) % m_Fixtures.Count] as HouseTeleporter;
                ...
                else if (fixture is BaseHouseDoor)
                {
                    ...
                    for (int j = i + 1; j < m_Fixtures.Count; ++j)
                    {
                        ...
                        if (
                            check != null
                            && check.Link == null
                            && check.Facing == linkFacing
                            && (check.X - door.X) == xOffset
                            && (check.Y - door.Y) == yOffset
                            && (check.Z == door.Z)
                        )
                        {
                            check.Link = door;
        ...
        public void AddFixture(Item item, MultiTileEntry mte)
        {
            m_Fixtures.Add(item);
            item.MoveToWorld(
                new Point3D(X + mte.m_OffsetX, Y + mte.m_OffsetY, Z + mte.m_OffsetZ),
                Map
            );
        }
        ...
        public override Rectangle2D[] Area
        {
            get
            {
                ...
                // exclude the outermost tile used for the exterior walkway.  The
                // original implementation returned a rectangle covering the entire
                // multi which meant the walkway was treated as part of the house
                // region.  Players standing on the south or east edges were therefore
                // considered inside by the region system even though IsInside() would
                // return false, causing inconsistent behavior such as being
                // dismounted while still being unable to interact with the interior.
                // Shrink the region by one tile on each side so only the true house
                // interior is considered part of the house region.
        ...
        public bool IsHiddenToCustomizer(Item item)
        {
            return (
                ...
                || item == m_SignHanger
                || item == Sign
                || (m_Fixtures != null && m_Fixtures.Contains(item))
            );
        }
        ...
        public static void Initialize()
        {
            ...
            PacketHandlers.RegisterEncoded(
                0x03,
                true,
                new OnEncodedPacketReceive(Designer_Restore)
            );
            ...
            PacketHandlers.RegisterEncoded(
                0x14,
                true,
                new OnEncodedPacketReceive(Designer_RoofDelete)
            ); // Samurai Empire roof
            ...
            CommandSystem.Register(
                ...
                new CommandEventHandler(DesignInsert_OnCommand)
            );
        ...
        public static void Designer_Clear(NetState state, IEntity e, EncodedReader pvSrc)
        {
            ...
            if (context != null)
            {
                ...
                DesignState newDesign = new DesignState(
                    context.Foundation,
                    context.Foundation.GetEmptyFoundation()
                );
            ...
            int newPrice =
                ...
                + (
                    (
                        ...
                        - (CurrentState.Components.List.Length + CurrentState.Fixtures.Length)
                    ) * 500
                );
        ...
        public static void Designer_Commit(NetState state, IEntity e, EncodedReader pvSrc)
        {
            ...
            if (context != null)
            {
                ...
                from.SendGump(
                    new ConfirmCommitGump(from, context.Foundation, bankBalance, oldPrice, newPrice)
                );
            }
        }
            ...
            protected override void OnTarget(Mobile from, object obj)
            {
                ...
                else
                {
                    ...
                    if (house == null)
                    {
                        from.Target = new DesignInsertTarget(m_Foundation);
                        from.SendMessage(
                            "That item is not inside a customizable house. Try again."
                        );
                    }
                    else if (m_Foundation != null && house != m_Foundation)
                    {
                        from.Target = new DesignInsertTarget(m_Foundation);
                        from.SendMessage(
                            "That item is not inside the current house; all targeted items must reside in the same house. You may cancel this target and repeat the command."
                        );
                    }
                    else
                    {
        ...
        public static void Designer_Level(NetState state, IEntity e, EncodedReader pvSrc)
        {
            ...
            if (context != null)
            {
                ...
                // Teleport mobile to new level
                from.Location = new Point3D(
                    from.X,
                    from.Y,
                    context.Foundation.Z + GetLevelZ(newLevel, context.Foundation)
                );
            ...
            if (context != null && (Core.SE || from.AccessLevel >= AccessLevel.GameMaster))
            {
                ...
                for (int i = 0; i < list.Length; i++)
                {
                    ...
                    if (
                        ...
                        && (
                            TileData.ItemTable[mte.m_ItemID & TileData.MaxItemValue].Flags
                            & TileFlag.Roof
                        ) != 0
                    )
    ...
    public class ConfirmCommitGump : Gump
    {
            ...
            : base(50, 50)
        {
            ...
            AddHtmlLocalized(
                ...
                300,
                140,
                (newPrice - oldPrice) <= bankBalance ? 1061898 : 1061903,
                1023,
                false,
                true
            );
    ...
    public class DesignContext
    {
        ...
        public int MaxLevels
        {
            get { return m_Foundation.MaxLevels; }
        }
        ...
        private static Dictionary<Mobile, DesignContext> m_Table =
            new Dictionary<Mobile, DesignContext>();
    ...
    public sealed class DesignStateDetailed : Packet
    {
            ...
            : base(0xD8)
        {
            ...
            int totalStairBuffersUsed =
                (totalStairsUsed + (MaxItemsPerStairBuffer - 1)) / MaxItemsPerStairBuffer;
            ...
            while (!Core.Closing)
            {
                ...
                while (count > 0)
                {
                    ...
                    try
                    {
                        ...
                        Timer.DelayCall(
                            TimeSpan.Zero,
                            new TimerStateCallback(SendPacket_Sandbox),
                            new object[] { sqe.m_NetState, p }
                        );
                    }
                    catch (Exception e)
                    {
