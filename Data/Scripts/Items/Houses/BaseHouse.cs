...
namespace Server.Multis
{
    public abstract class BaseHouse : BaseMulti
    {
        ...
        {
            get { return TimeSpan.FromDays(MyServerSettings.HomeDecay()); }
        }
        ...
        {
            DateTime checkTime = (
                check.LastTraded > check.BuiltOn ? check.LastTraded : check.BuiltOn
            );
            DateTime houseTime = (
                house.LastTraded > house.BuiltOn ? house.LastTraded : house.BuiltOn
            );
        ...
        public virtual DecayLevel DecayLevel
        {
            get
            {
                ...
                if (percent >= 1000) // 100.0%
                    return (HasRentedVendors || VendorInventories.Count > 0)
                        ? DecayLevel.DemolitionPending
                        : DecayLevel.Collapsed;
        ...
        {
            get { return TimeSpan.FromHours(0.0); }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public virtual double BonusStorageScalar
        {
            get { return (Core.ML ? 1.2 : 1.0); }
        }
        ...
        private static Dictionary<Mobile, List<BaseHouse>> m_Table =
            new Dictionary<Mobile, List<BaseHouse>>();
        ...
        public bool InRange(IPoint2D from, int range)
        {
            ...
            {
                if (
                    from.X >= rect.Start.X - range
                    && from.Y >= rect.Start.Y - range
                    && from.X < rect.End.X + range
        ...
        public static void IsThereVendor(
            Point3D location,
            Map map,
            out bool vendor,
            out bool rentalContract
        )
        {
            vendor = false;
            ...
            foreach (IEntity entity in eable)
            {
                ...
                {
                    if (
                        entity is PlayerVendor
                        || entity is PlayerBarkeeper
                        || entity is PlayerVendorPlaceholder
                    )
                    {
                        vendor = true;
                        break;
                ...
                if (entity is Item)
                {
                    ...
                    if (!item.Deleted)
                    {
                        if (item is IAddon)
                        {
                            ...
                            {
                                BaseAddonContainer bac = (BaseAddonContainer)item;
        ...
        public static void Configure()
        {
            ...
            Timer.DelayCall(
                ...
                TimeSpan.FromMinutes(1.0),
                new TimerCallback(Decay_OnTick)
            );
        }
        ...
        public bool CheckAccessibility(Item item, Mobile from)
        {
            ...
            if (!IsLockedDown(item))
                ...
                !(item.Stackable)
                && Region.Find(from.Location, from.Map) == Region.Find(item.Location, item.Map)
            )
            {
            ...
            for (int i = 0; i < m_Secures.Count; ++i)
            {
                ...
                if (info.Item == item)
                    return HasSecureAccess(m, info.Level)
                        ? SecureAccessResult.Accessible
                        : SecureAccessResult.Inaccessible;
            }
        ...
        public virtual void ChangeSignType(int itemID)
        {
            if (m_Sign != null)
            {
                if (IsSouthFacingSight(m_Sign.ItemID))
                {
                    FlipSignType(itemID);
                }
                else
        ...
        public BaseDoor AddHouseDoor(int x, int y, int z, int id)
        {
            ...
            {
                door = MakeDoor(false, DoorFacing.NorthCCW);
            }
            else if (id == 1653)
            {
                door = MakeDoor(false, DoorFacing.EastCCW);
            }
            else if (id == 1655)
            {
                door = MakeDoor(false, DoorFacing.WestCW);
            }
            else if (id == 1711)
            {
                door = MakeDoor(true, DoorFacing.NorthCCW);
            }
            else if (id == 1709)
            {
                door = MakeDoor(true, DoorFacing.SouthCW);
            }
            else if (id == 1701)
            {
                door = MakeDoor(true, DoorFacing.WestCW);
            }
            else if (id == 1703)
            {
                door = MakeDoor(true, DoorFacing.EastCCW);
            }
        ...
        public bool LockDown(Mobile m, Item item, bool checkIsInside)
        {
            ...
            if (item.Movable && !IsSecure(item))
            {
                ...
                else if (parentItem != null && !IsLockedDown(parentItem))
                {
                    ...
                    !(item is VendorRentalContract)
                    && (
                        IsAosRules
                            ? (!CheckAosLockdowns(amt) || !CheckAosStorage(amt))
                            : (this.LockDownCount + amt) > m_MaxLockDowns
                    )
                )
                {
                    ...
                    if (HasRentedVendors)
                    {
                        ...
                        to.SendGump(
                            new WarningGump(
                                ...
                                420,
                                280,
                                new WarningGumpCallback(ConfirmTransfer_Callback),
                                from
                            )
                        );
                    }
        ...
        public void AddSecure(Mobile m, Item item)
        {
            ...
            else
            {
                ...
                else if (!IsAosRules && SecureCount >= MaxSecures)
                {
                    ...
                    IsAosRules ? !CheckAosLockdowns(1) : ((LockDownCount + 125) >= MaxLockDowns)
                )
                {
                    m.SendLocalizedMessage(1005379); // That would exceed the maximum lock down limit for this house
                }
        ...
        public virtual bool IsCombatRestricted(Mobile m)
        {
            ...
            for (int i = 0; i < m.Aggressed.Count; ++i)
            {
                ...
                if (
                    ...
                    && info.Defender.Alive
                    && (DateTime.Now - info.LastCombatTime) < HouseRegion.CombatHeatDelay
                    && (
                        attackerGuild == null
                        || defenderGuild == null
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            foreach (RelocatedEntity relEntity in m_RelocatedEntities)
            {
                ...
                if (
                    (relEntity.Entity is Item && ((Item)relEntity.Entity).Deleted)
                    || (relEntity.Entity is Mobile && ((Mobile)relEntity.Entity).Deleted)
                )
        ...
        public Point3D BanLocation
        {
            ...
            set
            {
                this.RelativeBanLocation = new Point3D(
                    value.X - this.X,
                    value.Y - this.Y,
                    value.Z - this.Z
                );
        ...
        {
            get { return m_RelativeBanLocation; }
            set
            {
                m_RelativeBanLocation = value;
                m_Region.GoLocation = new Point3D(
                    this.X + value.X,
                    this.Y + value.Y,
                    this.Z + value.Z
                );
        ...
        public bool IsOwner(Mobile m)
        {
            ...
            if (m == m_Owner || m.AccessLevel >= AccessLevel.GameMaster)
                return true;
            else if (m_CoOwners.Contains(m) && Server.Misc.MyServerSettings.HouseOwners())
                return true;
        ...
        public bool HasAccess(Mobile m)
        {
            ...
            if (
                ...
                || IsFriend(m)
                || (m_Access != null && m_Access.Contains(m))
            )
                return true;
    ...
    public class SetSecureLevelEntry : ContextMenuEntry
    {
        ...
        public static ISecurable GetSecurable(Mobile from, Item item)
        {
            ...
            if (item is ISecurable)
            {
                ...
                if (!isOwned)
                    isOwned = (
                        house is HouseFoundation && ((HouseFoundation)house).IsFixture(item)
                    );
        ...
        public override void OnClick()
        {
            ...
            Owner.From.SendGump(
                new SetSecureLevelGump(Owner.From, sec, BaseHouse.FindHouseAt(m_Item))
            );
        }
    }
