...
namespace Server.Items
{
    public class TastyHeart : Item
    {
        ...
        [Constructable]
        public TastyHeart(string sName)
            : base(0x1CED)
        {
            if (sName != null)
            {
                HeartName = "the heart of " + sName;
            }
            else
            {
                HeartName = "heart";
                ...
                if (from.Hunger < 20)
                {
                    ...
                    if (Server.Items.BaseRace.BloodDrinker(from.RaceID))
                    {
                        ...
                        if (iHunger < 5)
                            ...
                            from.SendMessage(
                                "You eat the heart, but could still induldge in blood."
                            );
                        else
                            from.SendMessage(
                                "You eat the heart, but have indulged in enough blood."
                            );
                    }
                    else if (Server.Items.BaseRace.BrainEater(from.RaceID))
                    {
                        ...
                        if (iHunger < 5)
                            ...
                            from.SendMessage(
                                "You eat the heart, but could still induldge in some brains."
                            );
                        else
                            from.SendMessage(
                                "You eat the heart, and you no longer hunger for brains."
                            );
                    }
                    else
                    {
        ...
        public TastyHeart(Serial serial)
            : base(serial) { }
