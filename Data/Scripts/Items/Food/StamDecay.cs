...
namespace Server.Misc
{
    ...
    public class StamDecayTimer : Timer
    {
        ...
        public StamDecayTimer()
            : base(TimeSpan.FromSeconds(7), TimeSpan.FromSeconds(7))
        {
        ...
        public static void StamDecaying(Mobile m)
        {
            if (m != null)
            {
                ...
                if (
                    m.RaceID != 0
                    && (
                        Server.Items.BaseRace.NoFoodOrDrink(m.RaceID)
                        || Server.Items.BaseRace.BrainEater(m.RaceID)
                    )
                )
                {
