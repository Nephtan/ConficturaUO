...
namespace Server.LiarsDice
{
    ...
    public class DiceState
    {
        ...
        private int[] Dice1Values = new int[]
        {
            2,
            6,
            5,
        ...
        private int[] Dice2Values = new int[]
        {
            1,
            6,
            5,
        ...
        )
        {
            ...
            statusGumpTimer = Timer.DelayCall(
                TimeSpan.FromSeconds(15),
                new TimerCallback(StatusTimerCheck)
            );
        ...
        */
        public MobileDiceStstatus AddPlayer(Mobile m, int tableBalance)
        {
            ...
            {
                m.SendMessage(
                    "Liars Dice is currently at it maximum capacity of "
        ...
        public void RemovePlayer(Mobile m, bool exchangeBalance)
        {
            ...
            if (this.playerCnt > 1 && exchangeBalance == true)
            {
                if (exitMobileIdx == updatedMobileIdx)
                {
                    ...
                    dicePlayers[prevExitPlayerIdx].SetTableBalance(
                        exitPrevPlayerBal + this.goldPerGame
                    );
                    SendMessageAllPlayers(
                        "Player "
                            + mds.getMobile().Name
                            + " Left on his turn, so "
                            + dicePlayers[prevPlayerIdx].getMobile().Name
                            + " wins "
                            + this.goldPerGame
                            + " gp. from "
                            + mds.getMobile().Name
                ...
                else if (exitMobileIdx == prevPlayerIdx)
                {
                    ...
                    mds.SetTableBalance(exitPlayerBal - this.goldPerGame);
                    dicePlayers[updatedMobileIdx].SetTableBalance(
                        exitPrevPlayerBal + this.goldPerGame
                    );
                    SendMessageAllPlayers(
                        "Player "
                            ...
                            + dicePlayers[updatedMobileIdx].getMobile().Name
                            + " could make his decision, and so "
                            + dicePlayers[updatedMobileIdx].getMobile().Name
                            + " wins "
                            + this.goldPerGame
                            + " gp. from "
        ...
        */
        public void PlayBluffDecisionTurn(
...
*/
        public void UpdateGameChannel(Mobile m, int buttonId)
        {
            ...
            {
                PlayerTurn(
                    dicePlayers[updatedMobileIdx],
                    dicePlayers[prevPlayerIdx].GetPrevRollOrBluff()
                );
                AddPlayerWaiting(updatedMobileIdx);
            }
            else if (buttonId == 2)
                ...
                if (CheckBluff() == true)
                {
                    ...
                    if (prevPlayerBal < this.goldPerGame)
                    {
                        ...
                        SendMessageAllPlayers(
                            "Player "
                                + dicePlayers[prevPlayerIdx].getMobile().Name
                                + " does not have enough balance to continue playing, and so has been kicked out."
                        );
        ...
        */
        public void SetTimerAction(MobileDiceStstatus prevPlayer, MobileDiceStstatus currPlayer)
        {
            ...
            Timer timer = Timer.DelayCall(
                TimeSpan.FromSeconds(this.playerToActSeconds - 5),
                new TimerCallback(
                    delegate()
                    {
            ...
            Timer timer2 = Timer.DelayCall(
                TimeSpan.FromSeconds(this.playerToActSeconds),
                new TimerCallback(
                    delegate()
                    {
                        TimeSpan timeDiff = (DateTime.Now - currPlayer.GetTimeStamp());
