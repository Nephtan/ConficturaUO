...
namespace Server.Gumps
{
    public class NewDiceGameGump : Gump
    {
        ...
        public NewDiceGameGump(DiceState _ds, int _bankBalance)
            : base(325, 345)
        {
            this.ds = _ds;
            ...
            if (info.ButtonID == 1)
            {
                ...
                try
                {
                    ...
                    if (num > maxGameBal)
                    {
                        num = maxGameBal;
                        m.SendMessage(
                            "You entered more than the max of "
                                + maxGameBal
                                + " gp. on this table, you are buying in with the max instead."
                        );
                    }
                    ...
                    else
                    {
                        ...
                        m.SendMessage(
                            "You did not enter a sufficient minimum amount to play,try again."
                        );
                    }
                }
                catch
