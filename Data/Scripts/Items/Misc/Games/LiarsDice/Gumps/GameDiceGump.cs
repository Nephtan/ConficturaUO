...
namespace Server.Gumps
{
    public class GameDiceGump : Gump
    {
        ...
        */
        public GameDiceGump(DiceState _ds)
            : base(0, 30)
        {
            this.ds = _ds;
            this.currentRoll = 10;
        ...
        */
        public GameDiceGump(DiceState _ds, int _currentRoll, int _diceToBeat)
            : base(0, 30)
        {
            this.ds = _ds;
        ...
        private void AddRollGump()
        {
            ...
            for (int i = 0; i < 11; ++i)
            {
                if (i <= this.diceToBeat)
                {
                    ...
                    this.DisplayDiceCombo(
                        LEFT_BAR_SIDE + RADIO_WIDTH,
                        70 + (i * 25),
                        this.Dice1Values[i],
                        this.Dice2Values[i]
                    );
            ...
            for (int i = 11; i < 21; ++i)
            {
                if (i <= this.diceToBeat)
                {
                    ...
                    this.DisplayDiceCombo(
                        LEFT_BAR_SIDE + LEFT_SIDE_WIDTH + RADIO_WIDTH,
                        70 + ((i - 11) * 25),
                        this.Dice1Values[i],
                        this.Dice2Values[i]
                    );
        ...
        private void DisplayRollDice()
        {
            if (this.currentRoll >= 0 && this.currentRoll <= 20)
            {
                this.DisplayDiceCombo(
                    125,
                    20,
                    Dice1Values[this.currentRoll],
                    Dice2Values[this.currentRoll]
                );
            }
        }
