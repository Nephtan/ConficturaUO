...
namespace Server.Items
{
    ...
    public class CEOBlackJack : Item
    {
            ...
            set
            {
                if (m_InUseBy != null)
                {
                    this.PublicOverheadMessage(
                        0,
                        (this.Hue == 907 ? 0 : this.Hue),
                        false,
                        "Can not alter Test Mode while in use."
                    );
                    return;
                }
                m_TestMode = value;
            ...
            set
            {
                if (!m_Active && value)
                {
                    ...
                    Effects.SendLocationEffect(
                        new Point3D(this.X, this.Y + 1, this.Z),
                        this.Map,
                        0x373A,
                        15,
                        this.Hue - 1,
                    ...
                    Effects.SendLocationEffect(
                        new Point3D(this.X + 1, this.Y, this.Z),
                        this.Map,
                        0x373A,
                        15,
                        this.Hue - 1,
        ...
        private void BlackJackOffline(int error)
        {
            if (m_InUseBy != null)
            {
                m_InUseBy.SendMessage(
                    "A critical error has forced this game to close, notify the gods."
                );
                m_InUseBy = null;
            }
        ...
        {
            get { return (m_InUseBy != null && m_Active); }
        ...
        {
            if (
                !from.InRange(GetWorldLocation(), 2) && (from.AccessLevel >= AccessLevel.GameMaster)
            )
            {
#if XMLSPAWNER
            ...
            else
            {
                if (m_IdleTimer < m_TimeOut)
                {
                    ...
                    from.SendMessage(
                        "{0} has left this table too long, it is yours to play.",
                        tempName
                    );
                    InUseBy = from;
                    m_BJInfo.status = GameStatus.Waiting;
        ...
        private void PlayBlackJack(Mobile from, int cost)
        {
            ...
            if (!m_BJInfo.askInsurance)
            {
                ...
                else if (CheckForBlackJack(m_BJInfo.HandInfo[1]))
                {
                    ...
                    Credit(
                        (int)(
                            m_BJInfo.HandInfo[1].bet * BlackJackMultiplier(m_BlackJackPays, false)
                        ),
                        (int)(
                            m_BJInfo.HandInfo[1].bet * BlackJackMultiplier(m_BlackJackPays, true)
                        ),
        ...
        private bool CashCheck(Mobile m, out int checkamount)
        {
            ...
            for (int i = 0; i < packlist.Count; ++i)
            {
                ...
                if (item != null && !item.Deleted && item is BankCheck)
                {
                    ...
                    {
                        m.SendMessage(
                            "There's a problem trying to cash a check in your backpack, this game is closed. Seek help from the gods."
                        );
        ...
        private void DoCashOut(Mobile from)
        {
            ...
            else if (from == null)
            {
                ...
                !m_Active && (m_ErrorCode == 9500 || m_ErrorCode == 9501 || m_ErrorCode == 9502)
            ) // Prevent a loop cashing out
                return;
            else if (m_OnCredit < 0) // This should never happen but protect against some kind of overflow and a wild payout
            {
                ...
                from.SendMessage(
                    "There's a problem with this table's gold amount, this game is closed. Seek the gods for help."
                );
            ...
            if (m_OnCredit < 1000)
            {
                ...
                {
                    from.SendMessage(
                        "There's a problem returning your gold, this game is closed. Seek the gods for help."
                    );
            ...
            else
            {
                try
                {
                    ...
                    from.SendMessage(
                        "A bank check for {0} gold has been placed in your pack.",
                        m_OnCredit
                    );
                }
                catch
        ...
        public void Hit(Mobile from, bool gump)
        {
            ...
            m_BJInfo.HandInfo[m_BJInfo.activehand].card[
            ...
            m_BJInfo.HandInfo[m_BJInfo.activehand] = EvalHand(
                m_BJInfo.HandInfo[m_BJInfo.activehand]
            );
            if (m_BJInfo.HandInfo[m_BJInfo.activehand].bestscore > 21)
            {
                ...
                message =
                    m_BJInfo.totalhands > 2
                        ? string.Format("Hand {0} busted.", m_BJInfo.activehand)
                        : "You busted.";
                DoLosingSound(from);
                Stand(from, message);
                return;
            ...
            if (m_BJInfo.HandInfo[m_BJInfo.activehand].totalcards > 10)
            {
                ...
                message =
                    (
                        m_BJInfo.totalhands > 2
                            ? string.Format("Hand {0} ", m_BJInfo.activehand)
                            : "You "
                    ) + "must stand at eleven cards.";
                Stand(from, message);
                return;
            }
            if (gump)
            ...
            else
            {
                if (m_BJInfo.HandInfo[m_BJInfo.activehand].status == HandStatus.Split)
                {
                    ...
                    if (m_BJInfo.HandInfo[m_BJInfo.activehand].card[0] % 13 == 0)
                    {
                        ...
                        if (
                            !m_BJSplitAces21
                            && CheckForBlackJack(m_BJInfo.HandInfo[m_BJInfo.activehand])
                        )
                        {
                            if (PlayerCardsFaceUp)
                            {
                                m_BJInfo.HandInfo[m_BJInfo.activehand].status =
                            ...
                            {
                                Credit(
                                    m_BJInfo.HandInfo[m_BJInfo.activehand].bet * 2,
                            ...
                            else
                            {
                                Credit(
                                    (int)(
                                        m_BJInfo.HandInfo[1].bet
                                        * BlackJackMultiplier(m_BlackJackPays, false)
                                    ),
                                    (int)(
                                        m_BJInfo.HandInfo[1].bet
                                        * BlackJackMultiplier(m_BlackJackPays, true)
                                    ),
                                    (int)(
                                        m_BJInfo.HandInfo[1].bet
                                        * BlackJackMultiplier(m_BlackJackPays, true)
        ...
        public void Double(Mobile from)
        {
            ...
            {
                from.SendGump(
                    new BlackJackCardGump(from, this, "Insufficient funds to double down.")
                );
                return;
            }
        ...
        private BJStruct Double(Mobile from, int bet, BJStruct bj)
        {
            ...
            bj.HandInfo[bj.activehand].card[bj.HandInfo[bj.activehand].totalcards] =
                carddeck.GetOneCard();
        ...
        private static bool OkToDouble(BJStruct bj, DoubleDown ddRule)
        {
            ...
            {
                if (
                    bj.HandInfo[bj.activehand].bestscore == 9
        ...
        {
            if (
                (bj.HandInfo[bj.activehand].card[0] % 13) == 0
                && (bj.HandInfo[bj.activehand].card[1] % 13) == 0
            )
                return AcesX2(bj);
            if (!m_Resplits && m_SplitCount != 0)
                return false;
            if (
                bj.HandInfo[bj.activehand].card[0] % 13 == bj.HandInfo[bj.activehand].card[1] % 13
                && bj.totalhands < 5
            )
                return true;
            return false;
        ...
        public void ProcessInsurance(Mobile from, int choice)
        {
            ...
            {
                from.SendGump(
                    new BlackJackCardGump(from, this, "Insufficient funds for insurance.")
                );
                return;
            }
            ...
            else if (CheckForBlackJack(m_BJInfo.HandInfo[1]))
            {
                m_BJInfo.HandInfo[1].status = HandStatus.BlackJack;
                Credit(
                    (int)(m_BJInfo.HandInfo[1].bet * BlackJackMultiplier(m_BlackJackPays, false)),
                    (int)(m_BJInfo.HandInfo[1].bet * BlackJackMultiplier(m_BlackJackPays, true)),
                    (int)(m_BJInfo.HandInfo[1].bet * BlackJackMultiplier(m_BlackJackPays, true))
                );
                DoBlackJackSound(from, this);
        ...
        {
            return (CardValue(hand.card[0], true) + CardValue(hand.card[1], true)) == 21
                ? true
                : false;
        }
        ...
        private static bool DealerMustHit(
            HandStruct hand,
        ...
        private void DealerStands(Mobile from, string message)
        {
            ...
            {
                if (
                    m_BJInfo.HandInfo[h].status == HandStatus.SplitAces
                    && m_BJInfo.HandInfo[h].bestscore == 21
                )
                {
                    if (m_BJSplitAces21)
        ...
        private void SecurityCamera(int chatter, string text)
        {
            ...
            if (m_SecurityCamMobile.Player)
                ...
                m_SecurityCamMobile.PublicOverheadMessage(
                    0,
                    (this.Hue == 907 ? 0 : this.Hue),
                    false,
                    text
        ...
        public static string HelpText()
        {
            ...
            return Color("Objective: ", HC, false)
                + Color(
                    "The basic premise of Blackjack is that you want to have a hand value that is closer to 21 than that of the dealer, without going over 21. Your hand is strictly played out against the hand of the dealer. The rules of play for the dealer are strictly dictated, leaving no decisions up to the dealer. If you're playing at a shoe game(greater then 2 decks), the player cards are usually dealt face up.",
                    TC,
                    false
                )
                + "<p>"
                + Color("Values of the cards: ", HC, false)
...
namespace Server.Gumps
{
    public class BlackJackCardGump : Gump
    {
        ...
        private string[] CardFace = new string[]
        {
            "A",
            "2",
            "3",
            ...
            : base(20, 20)
        {
            ...
            m_ySize =
                (m_CEOBlackJack.m_BJInfo.totalhands == 0)
                    ? 360
                    : 115 + (m_CEOBlackJack.m_BJInfo.totalhands * 80);
            ...
            {
                if (
                    m_CEOBlackJack.m_BJInfo.activehand != h
                    || m_CEOBlackJack.m_BJInfo.status == CEOBlackJack.GameStatus.Waiting
                )
                    labelcolor = 912;
                else
                    labelcolor = 57;
                AddLabel(
                    ...
                    h == 0
                        ? "Dealer"
                        : (m_CEOBlackJack.m_BJInfo.totalhands > 2 ? "Hand " + h.ToString() : "You")
                );
                if (h == 0)
                {
                    if (
                        m_CEOBlackJack.m_BJInfo.HandInfo[h].status
                        == CEOBlackJack.HandStatus.BlackJack
                    )
                    {
                    ...
                    else if (m_CEOBlackJack.DealerCardsFaceUp)
                    {
                        score = String.Format(
                            "{0} {1}",
                            m_CEOBlackJack.m_BJInfo.HandInfo[h].bestscore,
                            (
                                m_CEOBlackJack.m_BJInfo.HandInfo[h].altscore == 0
                                || (
                                    m_CEOBlackJack.m_BJInfo.status
                                    == CEOBlackJack.GameStatus.Waiting
                                )
                                || (
                                    m_CEOBlackJack.m_BJInfo.HandInfo[h].status
                                    == CEOBlackJack.HandStatus.Waiting
                                )
                            )
                                ? null
                                : "or " + m_CEOBlackJack.m_BJInfo.HandInfo[h].altscore.ToString()
                        );
                ...
                else
                {
                    ...
                    else if (m_CEOBlackJack.PlayerCardsFaceUp)
                    {
                        ...
                        if (
                            m_CEOBlackJack.m_BJInfo.HandInfo[h].status
                            == CEOBlackJack.HandStatus.Bust
                        )
                            AddLabel(15, 75 + h * 80, 37, "Bust");
                    }
                    else if (
                        m_CEOBlackJack.m_BJInfo.HandInfo[h].status
                        == CEOBlackJack.HandStatus.SplitAces
                    )
                    {
                        ...
                        h > 0
                        && m_CEOBlackJack.m_BJInfo.HandInfo[h].status
                            == CEOBlackJack.HandStatus.Double
                    )
                    {
                    ...
                    AddLabel(
                        ...
                        27 + h * 80,
                        labelcolor,
                        String.Format("Bet: {0}", m_CEOBlackJack.m_BJInfo.HandInfo[h].bet)
                    );
                }
            ...
            if (m_CEOBlackJack.m_BJInfo.status != CEOBlackJack.GameStatus.DealerTurn)
                AddButton(
                    m_xSize - 45,
                    20,
                    m_HelpGump ? 4014 : 4005,
                    m_HelpGump ? 4016 : 4007,
                    m_Base + 401,
                    GumpButtonType.Reply,
                    0
                ); //Help
        ...
        private void DisplayRuleInfo()
        {
            ...
            text = string.Format(
                "{0}/{1}",
                m_CEOBlackJack.m_BetTable[(int)m_CEOBlackJack.MinBet],
                m_CEOBlackJack.m_BetTable[(int)m_CEOBlackJack.MaxBet]
            );
            ...
            AddHtml(
                ...
                m_xSize - 80,
                25,
                Center(string.Format("RuleSet: {0}", m_CEOBlackJack.CasinoName)),
                false,
                false
            );
            ...
            DisplayRule(
                155,
                "Split ace Blackjack counts as 21: ",
                YesNo(m_CEOBlackJack.BJSplitAces21)
            );
            DisplayRule(
                180,
                "Otherwise split ace Blackjack pays: ",
                (m_CEOBlackJack.BJSplitAPaysEven ? "Even money" : "Same as other Blackjacks")
            );
                ...
                for (int c = 0; c < m_CEOBlackJack.m_BJInfo.HandInfo[h].totalcards; c++)
                {
                    if (m_CEOBlackJack.m_BJInfo.HandInfo[h].card[c] < 0)
                        ...
                        DrawCard(
                            cardbasex + (c * 55),
                            cardbasey + h * 80,
                            m_CEOBlackJack.m_BJInfo.HandInfo[h].card[c]
                        );
                    else if (m_CEOBlackJack.m_BJInfo.status == CEOBlackJack.GameStatus.Waiting)
                        DrawCard(
                            cardbasex + (c * 55),
                        ...
                        h == 0
                        && c == 0
                        && (m_CEOBlackJack.m_BJInfo.status != CEOBlackJack.GameStatus.DealerTurn)
                    )
                        DrawBlankCard(cardbasex + (c * 55), cardbasey + h * 80);
                    else
                        DrawCard(
            ...
            else if (m_CEOBlackJack.m_BJInfo.status == CEOBlackJack.GameStatus.Waiting)
            {
                ...
                else if (info.ButtonID == m_Base + 302) // ATM
                {
                    ...
                    else
                    {
                        int amount =
                            (
                                m_CEOBlackJack.CreditATMLimit - m_CEOBlackJack.OnCredit()
                                < m_CEOBlackJack.CreditATMIncrements
                            )
                                ? m_CEOBlackJack.CreditATMLimit - m_CEOBlackJack.OnCredit()
                                : m_CEOBlackJack.CreditATMIncrements;
                        if (from.BankBox.ConsumeTotal(typeof(Gold), amount))
                        {
                            ...
                            Effects.PlaySound(
                                new Point3D(m_CEOBlackJack.X, m_CEOBlackJack.Y, m_CEOBlackJack.Z),
                                m_CEOBlackJack.Map,
                                501
                            );
