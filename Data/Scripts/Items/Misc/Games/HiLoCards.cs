...
namespace Server.Items
{
    ...
    public class HiLoCards : Item
    {
        ...
        public double[] m_WinMult = new double[9]
        {
            1.5,
            2.25,
            3.25,
        ...
        private void HiLoOffline(int error)
        {
            if (m_InUseBy != null)
            {
                m_InUseBy.SendMessage(
                    "A critical error has forced this game offline, notify a GameMaster please."
                );
                m_InUseBy = null;
            }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else
            {
                if (m_IdleTimer < m_TimeOut)
                {
                    ...
                    from.SendMessage(
                        "{0} has left this game too long, it is yours to play.",
                        tempName
                    );
                    InUseBy = from;
                }
            ...
            from.SendGump(
                new HiloCardGump(
                    ...
                    this,
                    carddeck.BackDesign,
                    Focus3(m_Hand, m_Count - 1, m_GuessCount),
                    -2,
                    false,
                    null
                )
        ...
        public bool CashCheck(Mobile m, out int amount)
        {
            ...
            for (int i = 0; i < packlist.Count; ++i)
            {
                ...
                if (item != null && !item.Deleted && item is BankCheck)
                {
                    ...
                    {
                        m.SendMessage(
                            "There's a problem trying to cash a check in your backpack, this game is offline."
                        );
                        HiLoOffline(9503);
                        return false;
                    }
        ...
        public void DoCashOut(Mobile from)
        {
            ...
            if (m_OnCredit < 0) // This should never happen but protect against some kind of overflow and a wild payout
            {
                ...
                from.SendMessage(
                    "There's a problem with this game's gold amount, this game is offline."
                );
                HiLoOffline(9502);
                return;
            }
            if (m_OnCredit < 1000)
            {
                ...
                {
                    from.SendMessage(
                        "There's a problem returning your gold, this game is offline."
                    );
                    HiLoOffline(9500);
                    return;
                }
                ...
                if (m_Count > m_GuessCount)
                {
                    ...
                    from.SendGump(
                        new HiloCardGump(
                            ...
                            this,
                            carddeck.BackDesign,
                            Focus3(m_Hand, m_Count - 2, m_GuessCount),
                            m_Progress,
                            false,
                            null
                        )
...
namespace Server.Gumps
{
    public class HiloCardGump : Gump
    {
        ...
        private void DisplayHelpGump()
        {
            ...
            string text =
                Color("Objective: ", HC, false)
                + Color(
                    "Guess the next card in the sequence, the more you choose to guess the higher the pay out!\n",
                    TC,
                    false
                )
                + "<p>"
                + Color("Rules: ", HC, false)
                + Color("Aces are high. A tie goes to the house(IE you lose).", TC, false)
                + Color(
                    " Do not sit idle at the game (over 5 mins) or someone else can take it away from you.",
                    TC,
                    false
                )
                + "</p><p>"
                + Color("Controls: ", HC, false)
                + Color(
                    "Use the small arrows to change your bet, the blue buttons to select how many cards in a row you want to try and guess.",
                    TC,
                    false
                )
                + Color(
                    " Press 'Play' to start, then use the large red arrows to select high/low for your guess.",
                    TC,
                    false
                )
                + Color(
                    " Need more cash? Use the ATM button to withdraw gold direct from your bank.",
                    TC,
                    false
                )
                ...
                + "</p><p>"
                + Color(Center("High-Low Card Game"), 0x696969, false)
                + Color(
                    "This is a simple card game you can enjoy while sitting in a tavern.",
                    CR,
                    false
                )
                + "</p></BASEFONT>";
                        ...
                        AddHtml(
                            ...
                            Color(
                                Right(
                                    string.Format(
                                        "{0}",
                                        (int)(m_HiLoCards.m_WinMult[x - 2] * m_HiLoCards.CurrentBet)
                                    )
                                ),
                                GREEN
                            ),
                                ...
                                Color(
                                    Right(
                                        string.Format(
                                            "{0}",
                                            (int)(
                                                m_HiLoCards.m_WinMult[x - 2]
                                                * m_HiLoCards.CurrentBet
                                            )
                                        )
                                    ),
                                    GREEN
            ...
            else
            {
                ...
                else if (info.ButtonID == m_Base + 302) // ATM
                {
                    ...
                    else
                    {
                        int amount =
                            (
                                m_HiLoCards.CreditATMLimit - m_HiLoCards.OnCredit()
                                < m_HiLoCards.CreditATMIncrements
                            )
                                ? m_HiLoCards.CreditATMLimit - m_HiLoCards.OnCredit()
                                : m_HiLoCards.CreditATMIncrements;
                        if (from.BankBox.ConsumeTotal(typeof(Gold), amount))
                        {
                            ...
                            Effects.PlaySound(
                                new Point3D(m_HiLoCards.X, m_HiLoCards.Y, m_HiLoCards.Z),
                                m_HiLoCards.Map,
                                501
                            );
                ...
                else if ((info.ButtonID == m_Base + 102) && (!m_HiLoCards.FixedBets))
                {
                    ...
                    (info.ButtonID > m_Base + 200 && info.ButtonID < m_Base + 211)
                    && (!m_HiLoCards.FixedGuesses)
                )
                {
                    m_HiLoCards.GuessCount = info.ButtonID - 200 - m_Base;
                }
                from.SendGump(
                    new HiloCardGump(from, m_HiLoCards, m_BackDesign, m_Card, -2, false, null)
                );
            }
        }
    }
