...
{
    public delegate void OnMahjongPacketReceive(
        MahjongGame game,
        NetState state,
        PacketReader pvSrc
    );
    ...
    {
        private static OnMahjongPacketReceive[] m_SubCommandDelegates = new OnMahjongPacketReceive[
            0x100
        ];
        ...
        public static void MoveWallBreakIndicator(
            MahjongGame game,
            NetState state,
            PacketReader pvSrc
        )
        ...
        public static void MoveTile(MahjongGame game, NetState state, PacketReader pvSrc)
        {
            ...
            game.Tiles[number].Move(
                new Point2D(x, y),
                direction,
                flip,
                game.Players.GetPlayerIndex(state.Mobile)
            );
        }
