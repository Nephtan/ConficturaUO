...
namespace Server.Engines.Mahjong
{
    public class MahjongPlayers
    {
        ...
        {
            get { return m_Players[m_DealerPosition]; }
        }
        public int DealerPosition
        {
            get { return m_DealerPosition; }
        }
        ...
        public void CheckPlayers()
        {
                ...
                if (player != null)
                {
                    ...
                    else if (m_InGame[i])
                    {
                        if (player.NetState == null)
                        {
                            ...
                            !m_Game.IsAccessibleTo(player)
                            || player.Map != m_Game.Map
                            || !player.InRange(m_Game.GetWorldLocation(), 5)
                        )
                        {
                            m_InGame[i] = false;
            ...
            for (int i = 0; i < m_Spectators.Count; )
            {
                ...
                if (mobile.NetState == null || mobile.Deleted)
                {
                    ...
                    !m_Game.IsAccessibleTo(mobile)
                    || mobile.Map != m_Game.Map
                    || !mobile.InRange(m_Game.GetWorldLocation(), 5)
                )
                {
                    m_Spectators.RemoveAt(i);
        ...
        public void TransferScore(Mobile from, int toPosition, int amount)
        {
            ...
            SendLocalizedMessage(
                1062774,
                string.Format("{0}\t{1}\t{2}", from.Name, to.Name, amount)
