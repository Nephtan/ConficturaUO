        ...
        private class InternalTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is Item)
                {
                    ...
                    {
                        if (
                            !from.InRange(m_Tub.GetWorldLocation(), 1)
                            || !from.InRange(item.GetWorldLocation(), 1)
                        )
                            from.SendLocalizedMessage(500446); // That is too far away.
                    ...
                    else if (item is DDRelicRugAddonDeed)
                    {
                        ...
                        item is HenchmanFamiliarItem
                        || item is WaxPainting
                        || item is WaxSculptors
                        || item is WaxSculptorsD
                        || item is WaxSculptorsE
                        || item is ColorCandleShort
                        || item is ColorCandleLong
                    )
                    {
                        if (
                            !from.InRange(m_Tub.GetWorldLocation(), 1)
                    ...
                    else if ((item is MyTentEastAddonDeed) || (item is MyTentSouthAddonDeed))
                    {
                        ...
                        (item is MyCircusTentEastAddonDeed) || (item is MyCircusTentSouthAddonDeed)
                    )
                    {
                        if (
                            !from.InRange(m_Tub.GetWorldLocation(), 1)
