    ...
    public class Corpse : Container, ICarvable
    {
        ...
        public override bool IsChildVisibleTo(Mobile m, Item child)
        {
            ...
            if (m_InstancedItems != null)
            {
                ...
                if (
                    m_InstancedItems.TryGetValue(child, out info)
        ...
        private void AssignInstancedLoot()
        {
            ...
            for (int i = 0; i < m_Stackables.Count; i++)
            {
                ...
                if (item.Amount >= attackers.Count)
                {
                    ...
                    for (
                        int j = 0;
                        j < ((remainder == 0) ? attackers.Count - 1 : attackers.Count);
                        j++
                    )
                    {
                        ...
                        m_InstancedItems.Add(
                            splitItem,
                            new InstancedItemInfo(splitItem, attackers[j])
                        );
                    ...
                    if (remainder == 0)
                    {
                        m_InstancedItems.Add(
                            item,
                            new InstancedItemInfo(item, attackers[attackers.Count - 1])
                        );
        ...
        private static TimeSpan m_DefaultDecayTime = TimeSpan.FromMinutes(
            MyServerSettings.CorpseDecay()
        );
        private static TimeSpan m_BoneDecayTime = TimeSpan.FromMinutes(
            MyServerSettings.BoneDecay()
        );
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            List<KeyValuePair<Item, Point3D>> list = (
                m_RestoreTable == null
                    ? null
        ...
        {
            if (
                IsCriminalAction(from)
                && this.Map != null
                && (this.Map.Rules & MapRules.HarmfulRestrictions) != 0
            )
            {
                if (m_Owner == null || !m_Owner.Player)
                    from.SendLocalizedMessage(1005035); // You did not earn the right to loot this creature!
            ...
            if (GetFlag(CorpseFlag.Carved) || dead == null)
            {
                ...
                ((Body)Amount).IsHuman
                && ItemID == 0x2006
                && ( // DON'T WANT TO CARVE ORKS AND ELVES
                    dead is EvilMage
                    || dead is EvilMageLord
                    || dead is Brigand
                    || dead is Executioner
                    || dead is Savage
                    || dead is SavageRider
                    ...
                    || dead is Jedi
                    || dead is ZuluuArcher
                    || dead is NativeWitchDoctor
                    || dead is NativeArcher
                    || dead is Native
                    || dead is Rogue
                    || dead is ZuluuWitchDoctor
                    || dead is PirateLand
                    || dead is PirateCrew
                    || dead is Monks
                    || dead is PirateCaptain
                    || dead is BoatPirateArcher
                    || dead is BoatPirateMage
                    || dead is Minstrel
                    || dead is BoatPirateBard
                    || dead is BoatSailorArcher
                    || dead is BoatSailorMage
                    || dead is Berserker
                    || dead is BoatSailorBard
                    || dead is BlackKnight
                    || dead is BloodAssassin
                    || dead is Archmage
                    || dead is BombWorshipper
                    || dead is Psionicist
                    || dead is SavageAlien
                    || (HumanLands == true && dead is BaseVendor)
                    || (HumanLands == true && dead is BaseHealer)
                    || (HumanLands == true && dead is BasePerson)
                )
            )
            {
                ...
                if (m_CorpseName != null && m_CorpseName != "")
                {
                    ...
                    {
                        myWork = "Assassin";
                        dead.Name = (dead.Name).Replace(" the assassin", "");
                        CriminalCarve = false;
                    }
                    else if (m_CorpseName.Contains(" the thief"))
                    {
                        myWork = "Thief";
                        dead.Name = (dead.Name).Replace(" the thief", "");
                        CriminalCarve = false;
                    }
                    else if (m_CorpseName.Contains(" the pirate"))
                    {
                        myWork = "Pirate";
                        dead.Name = (dead.Name).Replace(" the pirate", "");
                        CriminalCarve = false;
                    }
                    else if (m_CorpseName.Contains(" the bandit"))
                    {
                        myWork = "Bandit";
                        dead.Name = (dead.Name).Replace(" the bandit", "");
                        CriminalCarve = false;
                    }
                    else if (m_CorpseName.Contains(" the brigand"))
                    {
                        myWork = "Brigand";
                        dead.Name = (dead.Name).Replace(" the brigand", "");
                        CriminalCarve = false;
                    }
                }
                ...
                dead is ElfBoatPirateBard
                || dead is ElfBoatPirateArcher
                || dead is ElfBoatPirateMage
                || dead is ElfPirateCaptain
                || dead is ElfPirateCrew
                || dead is OrkRogue
                || dead is ElfRogue
            )
            {
