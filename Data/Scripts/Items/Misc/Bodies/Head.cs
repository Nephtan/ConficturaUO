...
namespace Server.Items
{
    ...
    public class Head : Item
    {
                ...
                switch (m_HeadType)
                {
                    ...
                    case HeadType.Tournament:
                        return String.Format(
                            "the head of {0}, taken in a tournament",
                            m_PlayerName
                        );
                }
            }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else if (this.ItemID == 0x1AE0)
            {
                this.ItemID = 0x1AE1;
            }
            else if (this.ItemID == 0x1AE1)
            {
                this.ItemID = 0x1AE2;
            }
            else if (this.ItemID == 0x1AE2)
            {
                this.ItemID = 0x1AE3;
            }
            else if (this.ItemID == 0x1AE3)
            {
                this.ItemID = 0x1AE0;
            }
            else if (this.ItemID == 7584)
            {
                this.ItemID = 7393;
            }
            else
                    ...
                    if (format != null)
                    {
                        ...
                        if (format.EndsWith(", taken in a duel"))
                        {
                            format = format.Substring(
                                0,
                                format.Length - ", taken in a duel".Length
                            );
                        ...
                        else if (format.EndsWith(", taken in a tournament"))
                        {
                            format = format.Substring(
                                0,
                                format.Length - ", taken in a tournament".Length
                            );
