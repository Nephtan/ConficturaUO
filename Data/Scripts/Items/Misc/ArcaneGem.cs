        ...
        public void OnTarget(Mobile from, object obj)
        {
            ...
            if (obj is IArcaneEquip && obj is Item)
            {
                ...
                else if (from.Skills[SkillName.Tailoring].Value >= 80.0)
                {
                    ...
                    if (item is BaseClothing)
                        isExceptional = (
                            ((BaseClothing)item).Quality == ClothingQuality.Exceptional
                        );
                    else if (item is BaseArmor)
                    ...
                    if (isExceptional)
                    {
                        ...
                        else if (item is BaseArmor)
                        {
                            ...
                            ((BaseArmor)item).PhysicalBonus =
                                ((BaseArmor)item).FireBonus =
                                ((BaseArmor)item).ColdBonus =
                                ((BaseArmor)item).PoisonBonus =
                                ((BaseArmor)item).EnergyBonus =
                                    0; // Is there a method to remove bonuses?
                        }
                        else if (item is BaseWeapon) // Sanity, weapons cannot recieve gems...
                        {
                ...
                {
                    from.SendMessage(
                        "You do not have enough skill in tailoring to enhance the item."
                    );
                }
            }
            else
