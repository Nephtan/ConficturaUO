...
namespace Server.Spells.Jedi
{
    public class MindsEye : JediSpell
    {
        ...
        {
            get { return Int32.Parse(Server.Spells.Jedi.JediSpell.SpellInfo(spellIndex, 10)); }
        }
        ...
        public override double RequiredSkill
        {
            get
            {
                return (double)(Int32.Parse(Server.Spells.Jedi.JediSpell.SpellInfo(spellIndex, 2)));
            }
        }
        public override int RequiredMana
        {
            get { return Int32.Parse(Server.Spells.Jedi.JediSpell.SpellInfo(spellIndex, 3)); }
        }
        ...
        public MindsEye(Mobile caster, Item scroll)
        ...
        public void Target(IPoint3D p)
        {
            ...
            else if (CheckSequence())
            {
                ...
                IPooledEnumerable TitemsInRange = Caster.Map.GetItemsInRange(
                    new Point3D(p),
                    1 + (int)(Caster.Skills[SkillName.Psychology].Value / 20.0)
                );
                string sTrap;
                foreach (Item item in TitemsInRange)
                {
                    ...
                    {
                        string where = Server.Misc.Worlds.GetRegionName(
                            Caster.Map,
                            Caster.Location
                        );
                ...
                if (map != null)
                {
                    IPooledEnumerable eable = map.GetMobilesInRange(
                        new Point3D(p),
                        1 + (int)(GetJediDamage(Caster) / 20.0)
                    );
        ...
        private class InternalTarget : Target
        {
            ...
            public InternalTarget(MindsEye owner)
