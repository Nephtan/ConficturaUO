...
namespace Server.Spells.Chivalry
{
    ...
    {
        private static SpellInfo m_Info = new SpellInfo("Dispel Evil", "Dispiro Malum", -1, 9002);
                ...
                for (int i = 0; i < targets.Count; ++i)
                {
                    ...
                    if (bc != null)
                    {
                        ...
                        if (dispellable)
                        {
                            double dispelChance =
                                (
                                    50.0
                                    + ((100 * (chiv - bc.DispelDifficulty)) / (bc.DispelFocus * 2))
                                ) / 100;
                    ...
                    if (context != null && context.Spell is NecromancerSpell) //Trees are not evil!	TODO: OSI confirm?
                    {
                        ...
                        double drainChance =
                            0.5
                            * (
                                Caster.Skills.Knightship.Value
                                / Math.Max(m.Skills.Necromancy.Value, 1)
                            );
