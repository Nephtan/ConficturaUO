...
namespace Server.Spells.Herbalist
{
    public class WoodlandProtectionSpell : HerbalistSpell
    {
        ...
        public WoodlandProtectionSpell(Mobile caster, Item scroll)
        ...
        public override void OnCast()
        {
            ...
            {
                int MyResist =
                    ((int)(Caster.Skills[DamageSkill].Value) / 5)
                    + ((int)(Server.Items.BasePotion.EnhancePotions(Caster) / 5));
        ...
        private class InternalTimer : Timer
        {
            ...
            public InternalTimer(Mobile owner, TimeSpan duration)
                : base(TimeSpan.Zero, TimeSpan.FromMinutes(0.1))
            {
                m_Owner = owner;
            ...
            protected override void OnTick()
            {
                if (DateTime.Now >= m_Expire)
                {
                    m_Owner.PrivateOverheadMessage(
                        ...
                        0x14C,
                        false,
                        "Your woodland protection has worn off",
                        m_Owner.NetState
                    );
