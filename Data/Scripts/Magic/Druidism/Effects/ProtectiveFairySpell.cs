...
namespace Server.Spells.Herbalist
{
    public class ProtectiveFairySpell : HerbalistSpell
    {
        ...
        public ProtectiveFairySpell(Mobile caster, Item scroll)
        ...
        public override bool CheckCast()
        {
            ...
            else if (Caster.Skills[CastSkill].Value < RequiredSkill)
            {
                Caster.PrivateOverheadMessage(
                    ...
                    0x14C,
                    false,
                    "You lack the understanding to even command a fairy.",
                    Caster.NetState
                );
                return false;
            }
        ...
        public override void OnCast()
        {
            if (CheckSequence())
            {
                TimeSpan duration = TimeSpan.FromSeconds(
                    (Caster.Skills[CastSkill].Value + Caster.Skills[DamageSkill].Value) * 9
                );
...
namespace Server.Mobiles
{
    ...
    public class DruidFairy : BaseCreature
    {
        ...
        public DruidFairy(Serial serial)
            : base(serial) { }
