...
namespace Server.Spells.Shinobi
{
    public class Deception : ShinobiSpell
    {
        ...
        private static SpellInfo m_Info = new SpellInfo("Deception", "Azamuku Koto", -1, 0);
        ...
        public Deception(Mobile caster, Item scroll)
        ...
        public override void OnCast()
        {
            ...
            else if (DisguiseTimers.IsDisguised(Caster))
            {
                ...
                !Caster.CanBeginAction(typeof(PolymorphSpell))
                || (Caster.IsBodyMod && Caster.RaceID != Caster.BodyMod)
            )
            {
                DoFizzle();
            }
            else if (CheckSequence())
            {
                if (Caster.BeginAction(typeof(Deception)))
                {
                    ...
                    else
                    {
                        ...
                        {
                            pm.SetHairMods(
                                pm.Race.RandomHair(pm.Female),
                                pm.Race.RandomFacialHair(pm.Female)
                            );
