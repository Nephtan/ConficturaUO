...
namespace Server.Spells.Jester
{
    public abstract class JesterSpell : Spell
    {
        ...
        public JesterSpell(Mobile caster, Item scroll, SpellInfo info)
            ...
            if (Server.Items.BagOfTricks.GetPranks(Caster) < RequiredTithing)
            {
                Caster.SendMessage(
                    "You must have at least "
                        + RequiredTithing.ToString()
                        + " pranks to use this ability."
                );
                return false;
            }
        ...
        public static int Buff(Mobile m, string category)
        {
            int value = 10;
            double var = 2.0;
            if (m.Skills[SkillName.Psychology].Value >= Utility.RandomMinMax(1, 400))
            {
                var = 1.5;
            }
            else if (m.Skills[SkillName.Psychology].Value >= Utility.RandomMinMax(1, 200))
            {
                var = 1.8;
            }
            ...
            if (category == "time")
            {
                value = (int)(TotalTime / var);
            }
            else if (category == "strength")
            {
                value = (int)(TotalBuff / var);
            }
            else if (category == "skills")
            {
                value = (int)(TotalSkill / var);
            }
            else if (category == "damage")
            {
                value = (int)(TotalDamage / var);
            }
            else if (category == "poison")
            {
                value = (int)(TotalPoison / var);
            }
            else if (category == "hurts")
            {
                value = TotalBuff;
            }
