...
namespace Server.Spells.Elementalism
{
    ...
    {
        private static SpellInfo m_Info = new SpellInfo("Elemental Sanctuary", "Invata", 239, 9031);
        ...
        {
            get { return TimeSpan.FromSeconds(2.25); }
        }

        public Elemental_Sanctuary_Spell(Mobile caster, Item scroll)
        ...
        public override void OnCast()
        {
            bool inCombat = (
                ...
                && Caster.InRange(Caster.Combatant.Location, 20)
                && Caster.Combatant.InLOS(Caster)
            );
            ...
            {
                Caster.SendMessage(
                    "The metal walls of this place seems to be blocking this spell."
                );
            }
            else if (inCombat)
            {
                ...
                && Caster.Skills[SkillName.Elementalism].Value >= 90
            )
            {
                CanCast = true;
            }
                ...
                && !Server.Misc.Worlds.IsMainRegion(
                    Server.Misc.Worlds.GetRegionName(Caster.Map, Caster.Location)
                )
