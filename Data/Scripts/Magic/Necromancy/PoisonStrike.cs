...
namespace Server.Spells.Necromancy
{
    public class PoisonStrikeSpell : NecromancerSpell
    {
        ...
        {
            get { return TimeSpan.FromSeconds((Core.ML ? 1.75 : 1.5)); }
        }
        ...
        public void Target(Mobile m)
        {
            if (CheckHSequence(m))
            {
                ...
                double damage =
                    Utility.RandomMinMax((Core.ML ? 32 : 36), 40)
                        * ((300 + (GetDamageSkill(Caster) * 9)) / 1000)
                    + nBenefit;

                double sdiBonusPVM =
                    (double)AosAttributes.GetValue(Caster, AosAttribute.SpellDamage) / 100;
                if (
