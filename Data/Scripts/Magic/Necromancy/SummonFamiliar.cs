...
namespace Server.Spells.Necromancy
{
    ...
    public class SummonFamiliarEntry
    {
        ...
        {
            get { return m_ReqNecromancy; }
        }
        public double ReqSpiritualism
        {
            get { return m_ReqSpiritualism; }
        }

        public SummonFamiliarEntry(
            Type type,
            object name,
            double reqNecromancy,
            double reqSpiritualism
        )
        {
            m_Type = type;
            m_Name = name;
    ...
    public class SummonFamiliarGump : Gump
    {
        ...
        public SummonFamiliarGump(
            Mobile from,
            SummonFamiliarEntry[] entries,
            SummonFamiliarSpell spell
        )
            : base(200, 100)
        {
            m_From = from;
            ...
            double spirit = from.Skills[SkillName.Spiritualism].Value;
            ...
            for (int i = 0; i < entries.Length; ++i)
            {
                ...
                bool enabled = (
                    necro >= entries[i].ReqNecromancy && spirit >= entries[i].ReqSpiritualism
                );
            ...
            if (index >= 0 && index < m_Entries.Length)
            {
                ...
                else if (necro < entry.ReqNecromancy || spirit < entry.ReqSpiritualism)
                {
                    ...
                    m_From.SendLocalizedMessage(
                        1061606,
                        String.Format("{0:F1}\t{1:F1}", entry.ReqNecromancy, entry.ReqSpiritualism)
                    );

                    m_From.CloseGump(typeof(SummonFamiliarGump));
                    m_From.SendGump(
                        new SummonFamiliarGump(m_From, SummonFamiliarSpell.Entries, m_Spell)
                    );
                }
