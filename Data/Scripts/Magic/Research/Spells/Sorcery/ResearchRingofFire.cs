    ...
    public class ResearchRingofFire : ResearchSpell
    {
        ...
        public void Target(IPoint3D p)
        {
            ...
            else if (SpellHelper.CheckTown(p, Caster) && CheckSequence())
            {
                ...
                RingOfFire ring = new RingOfFire(duration);
                ring.MoveToWorld(new Point3D(p.X, p.Y - 3, p.Z), Caster.Map);
                ring = new RingOfFire(duration);
                ring.MoveToWorld(new Point3D(p.X - 1, p.Y - 2, p.Z), Caster.Map);
                ring = new RingOfFire(duration);
                ring.MoveToWorld(new Point3D(p.X - 2, p.Y - 1, p.Z), Caster.Map);
                ring = new RingOfFire(duration);
                ring.MoveToWorld(new Point3D(p.X - 2, p.Y, p.Z), Caster.Map);
                ring = new RingOfFire(duration);
                ring.MoveToWorld(new Point3D(p.X - 2, p.Y + 1, p.Z), Caster.Map);
                ring = new RingOfFire(duration);
                ring.MoveToWorld(new Point3D(p.X - 1, p.Y + 2, p.Z), Caster.Map);
                ...
                ring.MoveToWorld(new Point3D(p.X + 1, p.Y - 2, p.Z), Caster.Map);
                ring = new RingOfFire(duration);
                ring.MoveToWorld(new Point3D(p.X, p.Y - 4, p.Z), Caster.Map);
                ring = new RingOfFire(duration);
                ring.MoveToWorld(new Point3D(p.X - 1, p.Y - 3, p.Z), Caster.Map);
                ring = new RingOfFire(duration);
        ...
        private class InternalTarget : Target
        {
            ...
            public InternalTarget(ResearchRingofFire owner)
...
namespace Server.Items
{
    public class RingOfFire : Item
    {
        ...
        public RingOfFire(Serial serial)
            : base(serial) { }
        ...
        public class ItemRemovalTimer : Timer
        {
            ...
            public ItemRemovalTimer(Item item, Double lasts)
                : base(TimeSpan.FromSeconds(lasts))
            {
