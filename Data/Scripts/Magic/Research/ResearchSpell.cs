...
namespace Server.Spells.Research
{
    public abstract class ResearchSpell : Spell
    {
        ...
        public static double CastingSkill(Mobile from)
        {
            ...
            if (from.Skills[SkillName.Magery].Value > skill)
            {
                skill = from.Skills[SkillName.Magery].Value;
            }
        ...
        public static double DamagingSkill(Mobile from)
        {
            return (
                    ...
                    + from.Skills[SkillName.Spiritualism].Value
                    + from.Skills[SkillName.Psychology].Value
                ) / 2;
        }
        ...
        public static void KarmaMod(Mobile from, int mod)
        {
            ...
            if (karma != 0)
            {
                Misc.Titles.AwardKarma(from, karma, true);
            }
        }
        ...
        public ResearchSpell(Mobile caster, Item scroll, SpellInfo info)
        ...
        public override bool CheckCast()
        {
            ...
            if (CastingSkill(Caster) < RequiredSkill)
            {
                Caster.SendMessage(
                    "You must have at least "
                        + RequiredSkill
                        + " in magery or necromancy to cast this spell."
                );
                return false;
            }
            ...
            {
                Caster.SendMessage(
                    "You must have at least " + GetMana() + " mana to cast this spell."
                );
                return false;
            }
        ...
        public static int ComputePowerValue(Mobile from, int div)
        {
            ...
            int v = (int)
                Math.Sqrt(
                    (from.Karma * -1)
                        + 20000
                        + ((from.Skills.Necromancy.Fixed + from.Skills.Magery.Fixed) * 10)
                );
