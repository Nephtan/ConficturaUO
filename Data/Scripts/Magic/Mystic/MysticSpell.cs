...
namespace Server.Spells.Mystic
{
    public abstract class MysticSpell : Spell
    {
        ...
        {
            get { return SkillName.FistFighting; }
        }
        ...
        public override bool CheckCast()
        {
            ...
            {
                Caster.SendMessage(
                    "Your equipment or skills are not commensurate to that of a true monk."
                );
                return false;
            }
            else if (
        ...
        public static bool MonkNotIllegal(Mobile from)
        {
            if (from.FindItemOnLayer(Layer.OneHanded) != null)
            {
                ...
                if (
                    oneHand is Artifact_GlovesOfThePugilist
                    || oneHand is GlovesOfThePugilist
                    || oneHand is GiftPugilistGloves
                    || oneHand is LevelPugilistGloves
                    || oneHand is PugilistGloves
                    || oneHand is PugilistGlove
                ) { }
            ...
            if (
                from.Skills[SkillName.Focus].Base < 100
                || from.Skills[SkillName.Meditation].Base < 100
            )
            {
                return false;
            }
        ...
        public virtual double GetResistPercentForCircle(Mobile target)
        {
            ...
            double secondPercent =
                ...
                - (
                    ((Caster.Skills[CastSkill].Value - 20.0) / 5.0)
                    + (1 + (int)MysticSpellCircle) * 5.0
                );
