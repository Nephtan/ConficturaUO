...
namespace Server.Spells
{
    ...
    public class SpellHelper
    {
        ...
        public static bool CheckMulti(Point3D p, Map map, bool houses, int housingrange)
        {
            ...
            for (int i = 0; i < sector.Multis.Count; ++i)
            {
                ...
                if (multi is BaseHouse)
                {
                    ...
                    if (
                        (houses && bh.IsInside(p, 16))
                        || (housingrange > 0 && bh.InRange(p, housingrange))
                    )
                        return true;
                }
            ...
            if (Core.Expansion == Expansion.AOS)
            {
                for (int i = 0; i < m.Aggressors.Count; ++i)
                {
                    ...
                    if (
                        ...
                        && (DateTime.Now - info.LastCombatTime) < CombatHeatDelay
                    )
                        return true;
                }
            }
        ...
        {
            if (Core.AOS)
                return TimeSpan.FromSeconds(((6 * caster.Skills.Psychology.Fixed) / 50) + 1);
        ...
        public static double GetOffsetScalar(Mobile caster, Mobile target, bool curse)
        {
            ...
            if (curse)
                percent =
                    8
                    + (caster.Skills.Psychology.Fixed / 100)
                    - (target.Skills.MagicResist.Fixed / 100);
            else
                percent = 1 + (caster.Skills.Psychology.Fixed / 100);
        ...
        {
            new TravelValidator(IsLodorT2A),
            new TravelValidator(IsUnderworld),
            new TravelValidator(IsSosariaWind),
            new TravelValidator(IsLodorWind),
            new TravelValidator(IsLodorDungeon),
            new TravelValidator(IsSosariaSolenHive),
            new TravelValidator(IsLodorSolenHive),
            ...
            new TravelValidator(IsMinesMorinia),
        ...
        {
            /*T2A(Fel)		Underworld		Wind(Tram),	Wind(Fel),	Dungeons(Fel),	Solen(Tram),	Solen(Fel), CrystalCave(SerpentIsland),	Gauntlet(SerpentIsland),	Gauntlet(Ferry),	Stronghold,		ChampionSpawn, Dungeons(IslesDread[SerpentIsland]), LampRoom(Doom),	GuardianRoom(Doom),	Heartwood */
            /* Recall From */{
                true,
                true,
                true,
        ...
        public static bool CheckTravel(Mobile caster, Map map, Point3D loc, TravelCheckType type)
        {
            ...
            if (
                caster != null
                && caster.AccessLevel == AccessLevel.Player
                && caster.Region.IsPartOf(typeof(Regions.Jail))
            )
            {
        ...
        public static bool IsLodorWind(Map map, Point3D loc)
        {
            return (map == Map.Lodor && IsWindLoc(loc));
        }

        public static bool IsSosariaWind(Map map, Point3D loc)
        {
            return (map == Map.Sosaria && IsWindLoc(loc));
        }

        public static bool IsUnderworld(Map map, Point3D loc)
        {
            return (map == Map.Underworld);
        }
        ...
        public static bool IsSosariaSolenHive(Map map, Point3D loc)
        {
            return (map == Map.Sosaria && IsSolenHiveLoc(loc));
        }

        public static bool IsLodorSolenHive(Map map, Point3D loc)
        {
            return (map == Map.Lodor && IsSolenHiveLoc(loc));
        }

        public static bool IsLodorT2A(Map map, Point3D loc)
        {
            int x = loc.X,
                y = loc.Y;

            return (map == Map.Lodor && x >= 5120 && y >= 2304 && x < 6144 && y < 4096);
        }
        ...
        public static bool IsLodorDungeon(Map map, Point3D loc)
        {
            Region region = Region.Find(loc, map);
            return (region.IsPartOf(typeof(DungeonRegion)) && region.Map == Map.Lodor);
        }
        ...
        public static bool IsMinesMorinia(Map map, Point3D loc)
        {
        ...
        public static bool IsHeartwood(Map map, Point3D loc)
        {
            ...
            return (map == Map.Sosaria || map == Map.Lodor)
                && (x >= 6911 && y >= 254 && x < 7167 && y < 511);
        }
    ...
    public class TransformationSpellHelper
    {
        ...
        private static Dictionary<Mobile, TransformContext> m_Table =
            new Dictionary<Mobile, TransformContext>();
        ...
        public static bool OnCast(Mobile caster, Spell spell)
        {
            ...
            else if (AnimalForm.UnderTransformation(caster))
            {
                ...
                !caster.CanBeginAction(typeof(Deception))
                || !caster.CanBeginAction(typeof(IncognitoSpell))
                || (
                    caster.IsBodyMod
                    && GetContext(caster) == null
                    && caster.RaceID != caster.BodyMod
                )
            )
            {
                spell.DoFizzle();
                    ...
                    if (transformSpell.PhysResistOffset != 0)
                        mods.Add(
                            new ResistanceMod(
                                ...
                                transformSpell.PhysResistOffset
                            )
                        );

                    if (transformSpell.FireResistOffset != 0)
                        mods.Add(
                            new ResistanceMod(ResistanceType.Fire, transformSpell.FireResistOffset)
                        );

                    if (transformSpell.ColdResistOffset != 0)
                        mods.Add(
                            new ResistanceMod(ResistanceType.Cold, transformSpell.ColdResistOffset)
                        );
                    ...
                    if (transformSpell.PoisResistOffset != 0)
                        mods.Add(
                            new ResistanceMod(
                                ...
                                transformSpell.PoisResistOffset
                            )
                        );
                    ...
                    if (transformSpell.NrgyResistOffset != 0)
                        mods.Add(
                            new ResistanceMod(
                                ...
                                transformSpell.NrgyResistOffset
                            )
                        );
    ...
    public class TransformContext
    {
        ...
        public TransformContext(
            Timer timer,
            List<ResistanceMod> mods,
            Type type,
            ITransformationSpell spell
        )
        {
            m_Timer = timer;
            m_Mods = mods;
    ...
    public class TransformTimer : Timer
    {
        ...
        protected override void OnTick()
        {
            if (
                m_Mobile.Deleted
                || !m_Mobile.Alive
                || m_Mobile.Body != m_Spell.Body
                || m_Mobile.Hue != m_Spell.Hue
            )
            {
