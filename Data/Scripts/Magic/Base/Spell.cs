    ...
    public abstract class Spell : ISpell
    {
        ...
        )
        {
            ...
            if (
                ...
                && sdiBonus > Server.Misc.MyServerSettings.SpellDamageIncreaseVsMonsters()
            )
            {
                sdiBonus = Server.Misc.MyServerSettings.SpellDamageIncreaseVsMonsters();
            }
            ...
            if (
                playerVsPlayer
                && Server.Misc.MyServerSettings.SpellDamageIncreaseVsPlayers() > 0
        ...
        public virtual double GetDamageScalar(Mobile target)
        {
            ...
            if (!Core.AOS) //EvalInt stuff for AoS is handled elsewhere
            {
                ...
                */
                ...
                if (
                    !target.Player && !target.Body.IsHuman /*&& !Core.AOS*/
                )
        ...
        public virtual double GetSlayerDamageScalar(Mobile defender)
        {
            ...
            if (atkBook != null)
            {
                ...
                if (
                    (atkBook.Slayer == SlayerName.Silver || atkBook.Slayer2 == SlayerName.Silver)
                    && context != null
            ...
            if (defISlayer != null)
            {
                ...
                if (
                    defSlayer != null && defSlayer.Group.OppositionSuperSlays(m_Caster)
        ...
        public virtual void OnDisturb(DisturbType type, bool message)
        {
            if (message)
            {
                ...
                {
                    m_Caster.SendMessage(
                        "Your concentration is disturbed, thus ruining thy prayer."
                    );
                }
                else if (this is MysticSpell || this is JesterSpell)
                {
                    m_Caster.SendMessage(
                        "Your concentration is disturbed, thus ruining thy attempt."
                    );
                }
                else
                {
        ...
        public virtual void SayMantra()
        {
            if (m_Info.Mantra != null && m_Info.Mantra.Length > 0 && m_Caster.Player)
                m_Caster.PublicOverheadMessage(
                    MessageType.Spell,
                    m_Caster.SpeechHue,
                    true,
                    m_Info.Mantra,
        ...
        public bool Cast()
        {
            ...
            if (
                Core.AOS
                && m_Caster.Spell is Spell
                && ((Spell)m_Caster.Spell).State == SpellState.Sequencing
            )
            ...
            {
                m_Caster.SendMessage(
                    "The darkness of the Underworld seems to be affecting this spell."
                );
                return false;
            }
            ...
            {
                m_Caster.SendMessage(
                    "Elementalism, with magery or necromancy, are affecting your magic."
                );
                return false;
            }
            ...
            else if (m_Caster.Spell != null && m_Caster.Spell.IsCasting)
            {
                ...
                BlockedByHorrificBeast
                    && TransformationSpellHelper.UnderTransformation(
                        m_Caster,
                        typeof(HorrificBeastSpell)
                    )
            ...
            else if (m_Caster.Mana >= ScaleMana(GetMana()))
            {
                ...
                )
                {
                    ...
                    {
                        int count = (int)
                            Math.Ceiling(castDelay.TotalSeconds / AnimateDelay.TotalSeconds);
                        ...
                        if (m_Info.LeftHandEffect > 0)
                            Caster.FixedParticles(
                                ...
                                10,
                                5,
                                m_Info.LeftHandEffect,
                                EffectLayer.LeftHand
                            );
        ...
        public static bool CanCastSpell(Mobile m, ISpell s)
        {
            ...
            {
                if (
                    Worlds.GetMyWorld(m.Map, m.Location, m.X, m.Y) == "the Underworld"
                    && (s is MagerySpell || s is ResearchSpell || s is ElementalSpell)
                ...
                )
                {
                    ...
                    if (relic1 is OrbOfTheAbyss)
                    {
                        fizzle = false;
                    }
                    else if (relic1 is CodexWisdom)
                    {
                        fizzle = false;
                    }
                    else if (relic2 is OrbOfTheAbyss)
                    {
                        fizzle = false;
                    }
                    else if (relic3 is OrbOfTheAbyss)
                    {
                        fizzle = false;
                    }
                    else if (relic4 is OrbOfTheAbyss)
                    {
                        fizzle = false;
                    }
                    else if (relic5 is OrbOfTheAbyss)
                    {
                        fizzle = false;
                    }
                    else if (relic1 is Artifact_PyrosGrimoire)
                    {
                        fizzle = false;
                    }
                    else if (relic1 is Artifact_StratosManual)
                    {
                        fizzle = false;
                    }
                    else if (relic1 is Artifact_HydrosLexicon)
                    {
                        fizzle = false;
                    }
                    else if (relic1 is Artifact_LithosTome)
                    {
                        fizzle = false;
                    }
        ...
        public static bool CantMixSpell(Mobile m, ISpell s)
        {
            ...
            {
                if (
                    m.Skills[SkillName.Necromancy].Value >= 1
                    && m.Skills[SkillName.Elementalism].Value >= 1
                    && (s is ElementalSpell || s is NecromancerSpell)
                )
                {
                    canCast = false;
                }
                if (
                    m.Skills[SkillName.Magery].Value >= 1
                    && m.Skills[SkillName.Elementalism].Value >= 1
                    && (s is ElementalSpell || s is MagerySpell)
                )
                {
                    canCast = false;
                }
        ...
        {
            if (MagicCastingItem.CastNoSkill(m_Scroll))
                return true;
        ...
        public virtual TimeSpan GetDisturbRecovery()
        {
            ...
            double delay =
                1.0
                - Math.Sqrt(
                    (DateTime.Now - m_StartCastTime).TotalSeconds / GetCastDelay().TotalSeconds
                );
        ...
        public virtual TimeSpan GetCastDelay()
        {
            ...
            if (
                CastSkill == SkillName.Elementalism
                || CastSkill == SkillName.Magery
                || CastSkill == SkillName.Necromancy
                || (
                    CastSkill == SkillName.Knightship
                    && m_Caster.Skills[SkillName.Magery].Value >= 70.0
                )
            )
                fcMax = 2;
            ...
            TimeSpan fcDelay = TimeSpan.FromSeconds(
                -(CastDelayFastScalar * fc * CastDelaySecondsPerTick)
            );
        ...
        public virtual bool CheckSequence()
        {
            ...
            )
            {
                ...
                m_Scroll != null
                && !(m_Scroll is RuneBag)
                && !(m_Scroll is BaseMagicStaff)
                && !(m_Scroll is Runebook)
            ...
            {
                m_Caster.Mana -= mana;
                if (this is ElementalSpell)
                {
                    m_Caster.Stam -= ((ElementalSpell)this).GetStam();
                }

                if (
                    m_Scroll is BloodPactScroll
                    || m_Scroll is GhostlyImagesScroll
                    || m_Scroll is GhostPhaseScroll
                    || m_Scroll is GraveyardGatewayScroll
                    || m_Scroll is HellsBrandScroll
                    || m_Scroll is HellsGateScroll
                    || m_Scroll is ManaLeechScroll
                    || m_Scroll is NecroCurePoisonScroll
                    || m_Scroll is NecroPoisonScroll
                    || m_Scroll is NecroUnlockScroll
                    || m_Scroll is PhantasmScroll
                    || m_Scroll is RetchedAirScroll
                    || m_Scroll is SpectreShadowScroll
                    || m_Scroll is UndeadEyesScroll
                    || m_Scroll is VampireGiftScroll
                ...
                )
                {
                    ...
                    m_Scroll is ShieldOfEarthPotion
                    || m_Scroll is WoodlandProtectionPotion
                    || m_Scroll is ProtectiveFairyPotion
                    || m_Scroll is HerbalHealingPotion
                    || m_Scroll is GraspingRootsPotion
                    || m_Scroll is BlendWithForestPotion
                    || m_Scroll is SwarmOfInsectsPotion
                    || m_Scroll is VolcanicEruptionPotion
                    || m_Scroll is TreefellowPotion
                    || m_Scroll is StoneCirclePotion
                    || m_Scroll is DruidicRunePotion
                    || m_Scroll is LureStonePotion
                    || m_Scroll is NaturesPassagePotion
                    || m_Scroll is MushroomGatewayPotion
                    || m_Scroll is RestorativeSoilPotion
                    || m_Scroll is FireflyPotion
                )
                {
                    m_Scroll.Consume();
                    m_Caster.AddToBackpack(new Jar());
                ...
                if (MagicCastingItem.CastNoSkill(m_Scroll)) // DO NOT REMOVE WAND FROM HANDS
                { }
                else
                {
                    if (ClearHandsOnCast)
        ...
        private class AnimTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                if (
                    !m_Spell.Caster.Mounted
                    && m_Spell.Caster.Body.IsHuman
                    && m_Spell.m_Info.Action >= 0
                )
