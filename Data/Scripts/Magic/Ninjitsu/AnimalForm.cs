...
namespace Server.Spells.Ninjitsu
{
    public class AnimalForm : NinjaSpell
    {
        ...
        {
            get { return (Core.ML ? 10 : base.CastRecoveryBase); }
        }
        ...
        public override void OnCast()
        {
            ...
            else if (CheckSequence())
            {
                ...
                {
                    if (
                        GetLastAnimalForm(Caster) == -1
                        || DateTime.Now - Caster.LastMoveTime
                            > Caster.ComputeMovementSpeed(Caster.Direction)
                    )
                    {
        ...
        public static MorphResult Morph(Mobile m, int entryID)
        {
            ...
            AddContext(
                m,
                new AnimalFormContext(timer, mod, entry.SpeedBoost, entry.Type, stealingMod)
            );
            m.CheckStatTimers();
        ...
        private static AnimalFormEntry[] m_Entries = new AnimalFormEntry[]
        {
            ...
            new AnimalFormEntry(
                ...
                1028496,
                8496,
                Utility.RandomList(0x5AC, 0x5A3, 0x59A, 0x591, 0x588, 0x57F),
                1070807,
                50.0,
                81,
                false,
        ...
        public class AnimalFormGump : Gump
        {
                ...
                : base(50, 50)
            {
                ...
                {
                    bool enabled = (
                        ninjitsu >= entries[i].ReqSkill
                        && BaseFormTalisman.EntryEnabled(caster, entries[i].Type)
                    );
    ...
    public class AnimalFormContext
    {
        ...
        {
            get { return m_StealingMod; }
        }

        public AnimalFormContext(
            Timer timer,
            SkillMod mod,
            bool speedBoost,
            Type type,
            SkillMod stealingMod
        )
        {
            m_Timer = timer;
            m_Mod = mod;
    ...
    public class AnimalFormTimer : Timer
    {
        ...
        protected override void OnTick()
        {
            if (
                m_Mobile.Deleted
                || !m_Mobile.Alive
                || m_Mobile.Body != m_Body
                || (m_Hue != 0 && m_Mobile.Hue != m_Hue)
            )
            {
            ...
            else
            {
                if (m_Body == 0x115) // Cu Sidhe
                {
                    if (m_Counter++ >= 8)
                    {
                        ...
                        {
                            Bandage b =
                                m_Mobile.Backpack.FindItemByType(typeof(Bandage)) as Bandage;
                ...
                else if (m_Body == 0x114) // Reptalon
                {
                    ...
                    if (
                        ...
                        && m_LastTarget != null
                        && m_LastTarget.Alive
                        && !m_LastTarget.Deleted
                        && m_Counter-- <= 0
                    )
                    {
                        if (
                            m_Mobile.CanBeHarmful(m_LastTarget)
                            && m_LastTarget.Map == m_Mobile.Map
                            && m_LastTarget.InRange(
                                m_Mobile.Location,
                                BaseCreature.DefaultRangePerception
                            )
                        ...
                        )
                        {
                            ...
                            Timer.DelayCall<Mobile>(
                                ...
                                new TimerStateCallback<Mobile>(BreathEffect_Callback),
                                m_LastTarget
                            );
                        }
        ...
        public void BreathEffect_Callback(Mobile target)
        {
            if (m_Mobile.CanBeHarmful(target))
            {
                ...
                Timer.DelayCall<Mobile>(
                    ...
                    new TimerStateCallback<Mobile>(BreathDamage_Callback),
                    target
                );
            }
        }
