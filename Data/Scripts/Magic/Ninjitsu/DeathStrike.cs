...
namespace Server.Spells.Ninjitsu
{
    public class DeathStrike : NinjaMove
    {
        ...
        {
            get { return new TextDefinition(1063091); }
        ...
        public override void OnHit(Mobile attacker, Mobile defender, int damage)
        {
            ...
            info.m_Timer = Timer.DelayCall(
                ...
                new TimerStateCallback(ProcessDeathStrike),
                defender
            );
        ...
        private static void ProcessDeathStrike(object state)
        {
            ...
            if (Core.ML)
            {
                double scalar =
                    (
                        info.m_Attacker.Skills[SkillName.Hiding].Value
                        + info.m_Attacker.Skills[SkillName.Stealth].Value
                    ) / 220;
                ...
                if (info.m_Steps >= 5)
                    damage = (int)
                        Math.Floor(
                            Math.Min(60, (ninjitsu / 3) * (0.3 + 0.7 * scalar) + stalkingBonus)
                        );
                else
                    damage = (int)
                        Math.Floor(
                            Math.Min(30, (ninjitsu / 9) * (0.3 + 0.7 * scalar) + stalkingBonus)
                        );
            ...
            else
            {
                ...
                damage =
                    Math.Max(0, Math.Min(maxDamage, (int)(baseDamage + stalkingBonus)))
                    + info.m_DamageBonus;
            }
