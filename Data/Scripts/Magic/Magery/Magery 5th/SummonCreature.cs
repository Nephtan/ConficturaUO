    ...
    public class SummonCreatureSpell : MagerySpell
    {
        ...
        public override void OnCast()
        {
            if (CheckSequence())
            {
                try
                {
                    BaseCreature creature = (BaseCreature)
                        Activator.CreateInstance(m_Types[Utility.Random(m_Types.Length)]);
                    ...
                    if (Core.AOS)
                        duration = TimeSpan.FromSeconds(
                            ((2 * Caster.Skills.Magery.Fixed) / 5) + nBenefit
                        );
                    else
                        duration = TimeSpan.FromSeconds(
                            (4.0 * Caster.Skills[SkillName.Magery].Value) + nBenefit
                        );
