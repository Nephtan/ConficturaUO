    ...
    public class PlantSystem
    {
        ...
        public static void GrowAll()
        {
            ...
            for (int i = plants.Count - 1; i >= 0; --i)
            {
                ...
                if (
                    plant.IsGrowable
                    && (plant.RootParent as Mobile) == null
                    && now >= plant.PlantSystem.NextGrowth
                )
                    plant.PlantSystem.DoGrowthCheck();
            }
        }
        ...
        private void Grow()
        {
            if (Health < PlantHealth.Healthy)
            {
                ...
                m_FertileDirt
                && m_Plant.PlantStatus <= PlantStatus.Stage5
            ...
            else
            {
                ...
                if (
                    LeftResources > 0
                    && PlantResourceInfo.GetInfo(m_Plant.PlantType, m_Plant.PlantHue) != null
                )
                {
                    LeftResources--;
                    AvailableResources++;
