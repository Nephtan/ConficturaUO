    ...
    public class Seed : Item
    {
        ...
        public static Seed RandomPeculiarSeed(int group)
        {
            switch (group)
            {
                ...
                case 3:
                    return new Seed(
                        PlantTypeInfo.RandomPeculiarGroupThree(),
                        PlantHue.Plain,
                        false
                    );
                default:
        ...
        public Seed()
            : this(
                PlantTypeInfo.RandomFirstGeneration(),
                PlantHueInfo.RandomFirstGeneration(),
                false
            ) { }
        ...
        public override void AddNameProperty(ObjectPropertyList list)
        {
            ...
            if (m_ShowType)
            {
                ...
                list.Add(
                    hueInfo.IsBright() ? 1061918 : 1061917,
                    String.Concat("#", title.ToString(), "\t#", typeInfo.Name.ToString())
                ); // [bright] ~1_COLOR~ ~2_TYPE~ seed
            }
            else
            {
                list.Add(
                    hueInfo.IsBright() ? 1060839 : 1060838,
                    String.Concat("#", title.ToString())
                ); // [bright] ~1_val~ seed
            }
        }
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            if (m_ShowType)
            {
                ...
                LabelTo(
                    from,
                    hueInfo.IsBright() ? 1061918 : 1061917,
                    String.Concat("#", hueInfo.Name.ToString(), "\t#", typeInfo.Name.ToString())
                ); // [bright] ~1_COLOR~ ~2_TYPE~ seed
            }
            else
            {
                LabelTo(
                    from,
                    hueInfo.IsBright() ? 1060839 : 1060838,
                    String.Concat("#", hueInfo.Name.ToString())
                ); // [bright] ~1_val~ seed
            }
        }
