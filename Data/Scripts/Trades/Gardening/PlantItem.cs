...
namespace Server.Engines.Plants
{
    ...
    public class PlantItem : Item, ISecurable
    {
        ...
        public bool IsGrowable
        {
            get
            {
                return m_PlantStatus >= PlantStatus.BowlOfDirt
                    && m_PlantStatus <= PlantStatus.Stage9;
            }
        }
        ...
        public bool IsCrossable
        {
            get
            {
                return PlantHueInfo.IsCrossable(this.PlantHue)
                    && PlantTypeInfo.IsCrossable(this.PlantType);
            }
        }
        ...
        public override void AddNameProperty(ObjectPropertyList list)
        {
            ...
            else if (m_PlantStatus >= PlantStatus.FullGrownPlant)
            {
                ...
                if (m_PlantStatus < PlantStatus.DecorativePlant)
                {
                    ...
                    else
                    {
                        string args = string.Format(
                            "#{0}\t#{1}\t#{2}",
                            m_PlantSystem.GetLocalizedHealth(),
                            title,
                            typeInfo.Name
                        );
                ...
                else
                {
                    if (m_PlantType == PlantType.SugarCanes)
                        ...
                        list.Add(
                            hueInfo.IsBright() ? 1074267 : 1070973,
                            string.Format("#{0}\t#{1}", title, typeInfo.Name)
                        );
                }
            }
            else if (m_PlantStatus >= PlantStatus.Seed)
            {
                ...
                string args = string.Format(
                    "#{0}\t#{1}\t#{2}",
                    m_PlantSystem.GetLocalizedDirtStatus(),
                    m_PlantSystem.GetLocalizedHealth(),
                    title
                );
                ...
                if (m_ShowType)
                {
                    ...
                    else
                    {
                        ...
                        list.Add(
                            hueInfo.IsBright() ? 1061887 : 1061888,
                            args + "\t#" + GetLocalizedPlantStatus().ToString()
                        );
                    }
                }
                else
        ...
        public bool IsUsableBy(Mobile from)
        {
            ...
            return IsChildOf(from.Backpack)
                || IsChildOf(from.FindBankNoCreate())
