...
namespace Server.Items
{
    public class GuildCraftingProcess
    {
        ...
        public void BeginProcess()
        {
            ...
            if (
                !(
                    ItemToUpgrade is BaseShield
                    || ItemToUpgrade is BaseClothing
                    || ItemToUpgrade is BaseArmor
                    || ItemToUpgrade is BaseWeapon
                    || ItemToUpgrade is BaseJewel
                    || ItemToUpgrade is Spellbook
                )
            )
            {
        ...
        public void BeginUpgrade(AttributeHandler handler)
        {
            ...
            {
                Owner.SendMessage(
                    "This piece of equipment cannot be enhanced with that any further."
                );
            }
        ...
        private bool SpendGold(int amount)
        {
            ...
            if (bought)
            {
                if (Owner.AccessLevel >= AccessLevel.GameMaster)
                    Owner.SendMessage(
                        "{0} gold would have been withdrawn from your bank if you were not an admin.",
                        amount
                    );
                else if (fromBank)
                    Owner.SendMessage(
                        "The total of your purchase is {0} gold, which has been withdrawn from your bank account.",
                        amount
                    );
                else
            ...
            {
                Owner.PlaySound(0x248);
            }
            else if (pc.NpcGuild == NpcGuild.CarpentersGuild)
            {
                Owner.PlaySound(0x23D);
            }
            else if (pc.NpcGuild == NpcGuild.ArchersGuild)
            {
                Owner.PlaySound(0x55);
            }
            else if (pc.NpcGuild == NpcGuild.TinkersGuild)
            {
                Owner.PlaySound(0x542);
            }
            else if (pc.NpcGuild == NpcGuild.BlacksmithsGuild)
            {
                Owner.PlaySound(0x541);
            }
