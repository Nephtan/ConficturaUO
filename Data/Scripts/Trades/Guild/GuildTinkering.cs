...
namespace Server.Items
{
    public class GuildTinkering : Item
    {
        ...
        public GuildTinkering(Serial serial)
            : base(serial) { }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (from is PlayerMobile)
            {
                ...
                {
                    from.SendMessage(
                        "You need to be near a tinker guildmaster, or a tinker shoppe you own, to use this!"
                    );
                }
                else
                {
                    ...
                    if (!mobile.HasGump(typeof(SpeechGump)))
                    {
                        mobile.SendGump(
                            new SpeechGump(
                                mobile,
                                "Enhancing Items",
                                SpeechFunctions.SpeechText(m_Mobile, m_Mobile, "EnhanceJewels")
                            )
                        );
                    }
                }
