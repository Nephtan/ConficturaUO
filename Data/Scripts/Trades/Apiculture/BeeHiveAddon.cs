...
namespace Server.Engines.Apiculture
{
    public class apiBeeHive : BaseAddon
    {
        ...
        public HiveStatus HiveStage
        {
            get { return m_Status; }
            set { m_Status = value; }
        }

        public HiveGrowthIndicator LastGrowth
        {
            get { return m_LastGrowth; }
        ...
        public apiBeeHiveComponent BeeHiveComponent
        {
            get { return m_Comp; }
        }
        ...
        public bool IsUsableBy(Mobile from)
        {
            ...
            return IsChildOf(from.Backpack)
                || IsChildOf(from.BankBox)
                || IsLockedDown && IsAccessibleTo(from)
        ...
        public bool ApplyPotion(PotionEffect effect, bool testOnly, out string message)
        {
            ...
            else if (effect == PotionEffect.AgilityGreater)
            {
                ...
                effect == PotionEffect.PoisonLesser
                || effect == PotionEffect.Poison
                || effect == PotionEffect.CureLesser
                || effect == PotionEffect.Cure
                || effect == PotionEffect.HealLesser
        ...
        public void FindFlowersInRange()
        {
            ...
            foreach (Item item in eable)
            {
                ...
                if (
                    iName.IndexOf("FLOWER") != -1
                    || iName.IndexOf("SNOWDROP") != -1
                    || iName.IndexOf("POPPIE") != -1
                )
                    FlowersInRange++;
            }
        ...
        public void Grow()
        {
            ...
            else
            {
                ...
                if (
                    Population < 10
                    && !(ScaleFlower() < ResourceStatus.Normal)
        ...
        public void UpdateMaladies()
        {
            ...
            double parasiteChance =
                0.30 - (potStrength * 0.075) + (((int)ScaleWater() - 3) * 0.10) + (HiveAge * 0.01); //Older hives are more susceptible to infestation
            ...
            double diseaseChance =
                0.30 - (potStrength * 0.075) + (((int)ScaleFlower() - 3) * 0.10) + (HiveAge * 0.01); //Older hives are more susceptible to disease
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
            ...
            if (m_Hive.HiveStage != HiveStatus.Empty)
                list.Add(
                    ...
                    "{0}\t{1}",
                    "Age",
                    m_Hive.HiveAge + (m_Hive.HiveAge == 1 ? " day" : " days")
                );
            if (m_Hive.HiveStage >= HiveStatus.Producing)
