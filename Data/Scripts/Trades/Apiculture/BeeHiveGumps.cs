...
namespace Server.Engines.Apiculture
{
    public class apiBeeHiveMainGump : Gump
    {
        ...
        public static Item GetPotion(Mobile from, PotionEffect[] effects)
        {
            ...
            Item[] items = from.Backpack.FindItemsByType(
                new Type[] { typeof(BasePotion), typeof(PotionKeg) }
            );
                ...
                case (int)Buttons.butHoney: //Honey
                {
                    ...
                    if (NeedHiveTool)
                    {
                        if (hivetool == null || !(hivetool is HiveTool))
                        {
                            m_hive.LabelTo(
                                from,
                                "You need a hive tool to extract the excess honey!"
                            );
                            from.SendGump(new apiBeeHiveProductionGump(from, m_hive));
                            return;
                        }
                    ...
                    if (m_hive.Honey < 3)
                    {
                        m_hive.LabelTo(
                            from,
                            "There isn't enough honey in the hive to fill a bottle!"
                        );
                        from.SendGump(new apiBeeHiveProductionGump(from, m_hive));
                        break;
                    }
                    ...
                    if (pack != null && pack.ConsumeTotal(typeof(Bottle), 1))
                    {
                        ...
                        if (!from.PlaceInBackpack(honey))
                        {
                            ...
                            m_hive.LabelTo(
                                from,
                                "There is not enough room in your backpack for the honey!"
                            );
                            from.SendGump(new apiBeeHiveProductionGump(from, m_hive));
                            break;
                        }
                        ...
                        m_hive.LabelTo(
                            from,
                            "You fill a bottle with golden honey and place it in your pack."
                        );
                        from.SendGump(new apiBeeHiveProductionGump(from, m_hive));
                        break;
                    }
                ...
                case (int)Buttons.butWax: //Wax
                {
                    ...
                    if (NeedHiveTool)
                    {
                        if (hivetool == null || !(hivetool is HiveTool))
                        {
                            m_hive.LabelTo(
                                from,
                                "You need a hive tool to scrape the excess beeswax!"
                            );
                            from.SendGump(new apiBeeHiveProductionGump(from, m_hive));
                            return;
                        }
                    ...
                    if (m_hive.Wax < 1)
                    {
                        m_hive.LabelTo(
                            from,
                            "There isn't enough excess wax in the hive to harvest!"
                        );
                        return;
                    }
                    ...
                    m_hive.LabelTo(
                        from,
                        "You collect the excess beeswax and place it in your pack."
                    );
                    from.SendGump(new apiBeeHiveProductionGump(from, m_hive));
                    break;
                }
