    ...
    class Customers
    {
        public static void AddCustomers(Mobile from, BaseShoppe shoppe, int customer)
        {
            ...
            if (from != null && exist == 0)
            {
                ...
                int fame = Utility.RandomMinMax(
                    ((int)(gold / 20)),
                    ((int)(gold / 20) + Utility.RandomMinMax(0, 3))
                );
                if (fame < 5)
                {
                    fame = 5;
                ...
                int skill = Utility.RandomMinMax(
                    ((int)((gold / 125) + 35)),
                    ((int)((gold / 125) + 35) + Utility.RandomMinMax(0, 5))
                );
                if (skill < 30)
                {
                    skill = 30;
                ...
                int tools = Utility.RandomMinMax(
                    ((int)(gold / 100)),
                    ((int)(gold / 100) + Utility.RandomMinMax(0, 2))
                );
                if (tools < 1)
                {
                    tools = 1;
                ...
                int resource = Utility.RandomMinMax(
                    ((int)(gold / 20)),
                    ((int)(gold / 20) + Utility.RandomMinMax(0, 5))
                );
                if (resource < 5)
                {
                    resource = 5;
                ...
                string finalCustomer =
                    customerTask + "#" + customerName + " " + customerTitle + "#";
                finalCustomer =
                    finalCustomer
                    + "1"
                    + "#"
        ...
        public static void FillOrder(Mobile from, BaseShoppe shoppe, int customer)
        {
            ...
            int difficulty = Server.Misc.Customers.GetChance(
                Server.Items.BaseShoppe.GetSkillValue(shoppe.ShelfSkill, from),
                Convert.ToInt32(Server.Misc.Customers.GetDataElement(client, 7))
            );
            ...
            if (difficulty < 1 || shoppe.ShoppeTools < tools || shoppe.ShoppeResources < resources)
            {
                // DO NOTHING
            }
            else
            {
                if (difficulty >= Utility.RandomMinMax(1, 100))
                {
                    from.SendSound(0x2E6);
                    from.SendSound(shoppe.ShelfSound);

                    int gold = Convert.ToInt32(GetDataElement(client, 4));

                    shoppe.ShoppeReputation = shoppe.ShoppeReputation + fame;
                    if (shoppe.ShoppeReputation > 10000)
                    {
                        shoppe.ShoppeReputation = 10000;
                    }

                    shoppe.ShoppeGold = shoppe.ShoppeGold + gold;
                    if (shoppe.ShoppeGold > 500000)
                    {
                        shoppe.ShoppeGold = 500000;
                    }
                }
                else
                {
                    from.SendSound(from.Female ? 812 : 1086);
                    from.SendSound(shoppe.ShelfSound);

                    shoppe.ShoppeReputation = shoppe.ShoppeReputation - fame;
                    if (shoppe.ShoppeReputation < 0)
                    {
                        shoppe.ShoppeReputation = 0;
                    }
                }

                shoppe.ShoppeTools = shoppe.ShoppeTools - tools;
                if (shoppe.ShoppeTools < 0)
                {
                    shoppe.ShoppeTools = 0;
                }

                shoppe.ShoppeResources = shoppe.ShoppeResources - resources;
                if (shoppe.ShoppeResources < 0)
                {
                ...
                Server.Items.BaseShoppe.ProgressSkill(from, shoppe);
                Server.Items.BaseShoppe.ProgressSkill(from, shoppe);
            }
        }
        ...
        public static string GetDataElement(string customer, int section)
        {
            ...
            if (customer.Length > 0)
            {
                ...
                {
                    if (element == 1 && element == section)
                    {
                        value = info;
                    } // TASK
                    else if (element == 2 && element == section)
                    {
                        value = info;
                    } // FOR WHO
                    else if (element == 3 && element == section)
                    {
                        value = info;
                    } // TASK STATUS
                    else if (element == 4 && element == section)
                    {
                        value = info;
                    } // GOLD
                    else if (element == 5 && element == section)
                    {
                        value = info;
                    } // TOOLS
                    else if (element == 6 && element == section)
                    {
                        value = info;
                    } // RESOURCES
                    else if (element == 7 && element == section)
                    {
                        value = info;
                    } // DIFFICULTY
                    else if (element == 8 && element == section)
                    {
                        value = info;
                    } // REPUTATION
