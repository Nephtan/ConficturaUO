...
namespace Server.Engines.Harvest
{
    public class Mining : HarvestSystem
    {
        ...
        private Mining()
        {
            ...
            oreAndStone.ConsumedPerHarvest = 1 * Server.Misc.MyServerSettings.Resources();
            oreAndStone.ConsumedPerIslesDreadHarvest =
                oreAndStone.ConsumedPerHarvest + (int)(oreAndStone.ConsumedPerHarvest / 2) + 3;
            ...
            sand.ConsumedPerHarvest = 1 * Server.Misc.MyServerSettings.Resources();
            sand.ConsumedPerIslesDreadHarvest =
                sand.ConsumedPerHarvest + (int)(sand.ConsumedPerHarvest / 2) + 2;
        ...
        )
        {
            if (def == m_OreAndStone)
            {
                ...
                if (
                    ...
                    && from.Skills[SkillName.Mining].Base >= 100.0
        ...
        )
        {
            ...
            else if (tool is OreShovel && def == m_OreAndStone)
            {
                int veinIndex = Array.IndexOf(def.Veins, vein);
                return def.Veins[0];
            }
                ...
                if (res == resource && res.Types.Length >= 3)
                {
                    try
                    {
                        ...
                        BaseCreature spawned =
                            Activator.CreateInstance(res.Types[2], new object[] { 25 })
                            as BaseCreature;
