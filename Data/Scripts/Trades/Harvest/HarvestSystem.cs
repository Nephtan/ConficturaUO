...
namespace Server.Engines.Harvest
{
    public abstract class HarvestSystem
    {
        ...
        public virtual void OnHarvestStarted(
            Mobile from,
            Item tool,
            HarvestDefinition def,
            object toHarvest
        ...
        public virtual bool BeginHarvesting(Mobile from, Item tool)
        {
            ...
            else
            {
                CaptchaGump.sendCaptcha(
                    from,
                    HarvestSystem.SendHarvestTarget,
                    new object[] { tool, this }
                );
            }
        ...
        public virtual void FinishHarvesting(
            Mobile from,
            Item tool,
            HarvestDefinition def,
            object toHarvest,
        ...
        )
        {
            ...
            HarvestResource resource = MutateResource(
                from,
                tool,
                def,
                map,
            ...
            {
                skillBase = Server.Engines.Harvest.Fishing.FishSkills(from, false);
                skillValue = Server.Engines.Harvest.Fishing.FishSkills(from, true);

                if (!Worlds.IsOnBoat(from) && from.Skills[def.Skill].Base >= 50)
                {
                    from.SendMessage(
                        "You will only get better at seafaring if you fish from a boat."
                    );
                    if (Utility.RandomMinMax(0, 200) > skillValue)
                        testSkill = true;
                    else
                        testSkill = false;
                }
            ...
            if (skillBase >= resource.ReqSkill && testSkill)
            {
                ...
                if (type != null)
                {
                    ...
                    else
                    {
                        if (item.Stackable)
                        {
                            ...
                            if (
                                tool is FishingPole
                                && !Worlds.IsOnBoat(from)
                                && from.Skills[def.Skill].Base >= 50
                            )
                            {
                                addUp = false;
                            }
                            ...
                            if (addUp)
                            {
                                int skillCycle = Server.Misc.MyServerSettings.Resources() - 1;
                                int extra = 0;

                                while (skillCycle > 0)
                                {
                                    extra++;
                                    if (extra > Server.Misc.MyServerSettings.StatGainDelayNum())
                                    {
                                        extra = 1;
                                    }
                            ...
                            if (item is BlankScroll)
                            {
                                amount = Utility.RandomMinMax(
                                    amount,
                                    (int)(amount + (from.Skills[SkillName.Inscribe].Value / 10))
                                );
                            ...
                            if (
                                ...
                                reg.IsPartOf("the Mines of Morinia")
                                && item is BaseOre
                            ...
                            if (
                                (item is AgapiteOre || item is VeriteOre || item is ValoriteOre)
                                && Utility.RandomMinMax(1, 2) == 1
                            )
                                FindSpecialOre = true;
                            ...
                            if (
                                (
                                    Worlds.IsExploringSeaAreas(from)
                                    || reg.IsPartOf("Shipwreck Grotto")
                                    || reg.IsPartOf("Barnacled Cavern")
                                )
                                && FindSeaLog
                                && item is BaseLog
                            )
                            ...
                            if (
                                ...
                                && Worlds.GetMyWorld(from.Map, from.Location, from.X, from.Y)
                                    == "the Savaged Empire"
                                && from.Skills[SkillName.Mining].Value
                                    > Utility.RandomMinMax(1, 500)
                            )
                            {
                            ...
                            if (
                                tool is FishingPole
                                && Server.Engines.Harvest.Fishing.IsNearHugeShipWreck(from)
                                && Server.Engines.Harvest.Fishing.FishSkills(from, true)
                                    >= Utility.RandomMinMax(1, 250)
                            )
                            {
                                ...
                                tool is FishingPole
                                && Server.Engines.Harvest.Fishing.IsNearSpaceCrash(from)
                                && Server.Engines.Harvest.Fishing.FishSkills(from, true)
                                    >= Utility.RandomMinMax(1, 250)
                            )
                            {
                                ...
                                tool is FishingPole
                                && Server.Engines.Harvest.Fishing.IsNearUnderwaterRuins(from)
                                && Server.Engines.Harvest.Fishing.FishSkills(from, true)
                                    >= Utility.RandomMinMax(1, 250)
                            )
                            {
                        ...
                        )
                        {
                            ...
                            {
                                ((DDRelicBook)item).RelicGoldValue =
                                    ((DDRelicBook)item).RelicGoldValue
                                    + Utility.RandomMinMax(
                                        1,
                                        (int)(from.Skills[SkillName.Inscribe].Value * 2)
                                    );
                            }
                        ...
                        )
                        {
                            ...
                            {
                                ((DDRelicScrolls)item).RelicGoldValue =
                                    ((DDRelicScrolls)item).RelicGoldValue
                                    + Utility.RandomMinMax(
                                        1,
                                        (int)(from.Skills[SkillName.Inscribe].Value * 2)
        ...
        public virtual bool SpecialHarvest(
            Mobile from,
            Item tool,
            HarvestDefinition def,
            Map map,
        ...
        public virtual void SendPackFullTo(
            Mobile from,
            Item item,
            HarvestDefinition def,
            HarvestResource resource
        ...
        public virtual void DoHarvestingSound(
            Mobile from,
            Item tool,
            HarvestDefinition def,
            object toHarvest
        ...
        public virtual void DoHarvestingEffect(
            Mobile from,
            Item tool,
            HarvestDefinition def,
            Map map,
