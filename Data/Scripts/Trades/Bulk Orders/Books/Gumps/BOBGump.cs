        ...
        public bool CheckFilter(object obj)
        {
            if (obj is BOBLargeEntry)
            {
                ...
                return CheckFilter(
                    ...
                    e.AmountMax,
                    true,
                    e.RequireExceptional,
                    e.DeedType,
                    (e.Entries.Length > 0 ? e.Entries[0].ItemType : null)
                );
            }
            else if (obj is BOBSmallEntry)
            {
            ...
            switch (type)
            {
                ...
                case BODType.Tailor:
                {
                    switch (mat)
                    {
                        ...
                        {
                            if (
                                itemType.IsSubclassOf(typeof(BaseArmor))
                                || itemType.IsSubclassOf(typeof(BaseShoes))
                            )
                                return 1062235;
                ...
                default:
                {
                    ...
                    if (type == 0) // Drop
                    {
                        if (m_Book.IsChildOf(m_From.Backpack))
                        {
                                ...
                                else
                                {
                                    if (m_Book.IsChildOf(m_From.Backpack))
                                    {
                                        ...
                                        if (m_Book.Entries.Count > 0)
                                        {
                                            ...
                                            m_From.SendGump(
                                                new BOBGump(m_From, m_Book, m_Page, null)
                                            );
                                        }
                                        else
                            ...
                            {
                                m_From.SendMessage(
                                    "Internal error. The bulk order deed could not be reconstructed."
                                );
                            }
                        }
                    }
                            ...
                            if (vi != null && !vi.IsForSale)
                            {
                                ...
                                else
                                {
                                    if (m_Book.Entries.Count > 0)
                                    {
                                        m_Page = GetPageForIndex(index, sizeOfDroppedBod);
                                        m_From.SendGump(
                                            new BODBuyGump(m_From, m_Book, obj, m_Page, price)
                                        );
                                    }
                                    else
            ...
            if (canPrice)
            {
                ...
                else
                {
                    ...
                    AddHtml(
                        ...
                        120,
                        20,
                        "<BASEFONT Color=#FBFBFB>Price all</FONT>",
                        false,
                        false
                    );
                }
                ...
                if (obj is BOBLargeEntry)
                {
                    ...
                    for (int j = 0; j < e.Entries.Length; ++j)
                    {
                        ...
                        AddLabel(
                            ...
                            y,
                            1152,
                            String.Format("{0} / {1}", sub.AmountCur, e.AmountMax)
                        );
