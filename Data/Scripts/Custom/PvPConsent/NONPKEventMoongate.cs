...
namespace Server.Items
{
    ...
    public class NONPKEventMoongate : Item
    {
        ...
        public virtual bool ShowLodorWarning
        {
            get { return false; }
        }

        [Constructable]
        public NONPKEventMoongate()
        ...
        [Constructable]
        public NONPKEventMoongate(bool bDispellable)
            : this(Point3D.Zero, null)
        {
        ...
        [Constructable]
        public NONPKEventMoongate(Point3D target, Map targetMap)
            : base(0xF6C)
        {
            Movable = false;
            Light = LightType.Circle300;
            Name = "NONPK Event Moongate";
            m_Target = target;
            m_TargetMap = targetMap;
        }

        public NONPKEventMoongate(Serial serial)
            : base(serial) { }
        ...
        {
            if (m is PlayerMobile && ((PlayerMobile)m).NONPK == NONPK.NONPK)
            {
                ((PlayerMobile)m).NONPK = NONPK.NONPKinEvent;
            }
            else if (m is PlayerMobile && ((PlayerMobile)m).NONPK == NONPK.NONPKinEvent)
            {
                ((PlayerMobile)m).NONPK = NONPK.NONPK;
                m.Combatant = null;
            }
        }
        ...
        public virtual void UseGate(Mobile m)
        {
            ...
            else if (m_TargetMap == Map.Lodor && m is PlayerMobile && ((PlayerMobile)m).Young)
            {
                ...
                (m.Kills >= 1 && m_TargetMap != Map.Lodor)
                || (m_TargetMap == Map.IslesDread && (flags & ClientFlags.IslesDread) == 0)
                || (m_TargetMap == Map.SerpentIsland && (flags & ClientFlags.SerpentIsland) == 0)
                || (m_TargetMap == Map.Underworld && (flags & ClientFlags.Underworld) == 0)
            )
            {
        ...
        {
            if (
                IsInTown(from.Location, from.Map) && !IsInTown(m_Target, m_TargetMap)
                || (from.Map != Map.Lodor && TargetMap == Map.Lodor && ShowLodorWarning)
            )
            {
                if (from.AccessLevel == AccessLevel.Player || !from.Hidden)
                    from.Send(new PlaySound(0x20E, from.Location));
                from.CloseGump(typeof(NONPKEventMoongateConfirmGump));
                from.SendGump(new NONPKEventMoongateConfirmGump(from, this));
            }
            else
            {
        ...
        private class DelayTimer : Timer
        {
            ...
            private NONPKEventMoongate m_Gate;
            private int m_Range;

            public DelayTimer(Mobile from, NONPKEventMoongate gate, int range)
    ...
    public class NONPKEventConfirmationMoongate : Moongate
    {
        ...
        [Constructable]
        public NONPKEventConfirmationMoongate()
            : this(Point3D.Zero, null) { }

        [Constructable]
        public NONPKEventConfirmationMoongate(Point3D target, Map targetMap)
            : base(target, targetMap) { }

        public NONPKEventConfirmationMoongate(Serial serial)
            : base(serial) { }
            ...
            )
            {
                ...
                from.SendGump(
                    new WarningGump(
                        m_TitleNumber,
                        m_TitleColor,
                        m_MessageString == null ? (object)m_MessageNumber : (object)m_MessageString,
                        m_MessageColor,
                        m_GumpWidth,
                        m_GumpHeight,
                        new WarningGumpCallback(Warning_Callback),
                        from
                    )
                );
            }
    ...
    public class NONPKEventMoongateConfirmGump : Gump
    {
        private Mobile m_From;
        private NONPKEventMoongate m_Gate;

        public NONPKEventMoongateConfirmGump(Mobile from, NONPKEventMoongate gate)
            : base(Core.AOS ? 110 : 20, Core.AOS ? 100 : 30)
        {
            ...
            if (Core.AOS)
            {
                ...
                if (from.Map != Map.Lodor && gate.TargetMap == Map.Lodor && gate.ShowLodorWarning)
                    AddHtmlLocalized(10, 40, 400, 200, 1062050, 32512, false, true); // This Gate goes to Lodor... Continue to enter the gate, Cancel to stay here
                else
