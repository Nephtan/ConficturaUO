...
namespace Server.Items
{
    public class JoinCityTarget : Target
    {
        ...
        public JoinCityTarget(CityManagementStone stone)
    ...
    public class CitySponsorTarget : Target
    {
        ...
        public CitySponsorTarget(CityManagementStone stone)
            : base(12, false, TargetFlags.None)
        {
            m_Stone = stone;
        }
    ...
    public class CityManagementStone : Item
    {
        ...
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(CityUpdate_OnCommand)
            );
        }
        ...
        {
            PlayerGovernmentSystem.AllCityStones.Remove(this);
            base.Delete();
        }
            ...
            if (from.InRange(this.GetWorldLocation(), 2))
            {
                if (from == m_Mayor || from.AccessLevel >= AccessLevel.GameMaster)
                {
                    ...
                    if (time.Days != 0)
                        from.SendMessage(
                            53,
                            "The city will update again in {0} days, {1} hours, {2} minutes, and {3} seconds.",
                            time.Days,
                            time.Hours,
                            time.Minutes,
                            time.Seconds
                        ...
                        from.SendMessage(
                            53,
                            "The city will update again in {0} hours, {1} minutes, and {2} seconds.",
                            time.Hours,
                            time.Minutes,
                            time.Seconds
                        );
                        ...
                        from.SendMessage(
                            53,
                            "The city will update again in {0} minutes, and {1} seconds.",
                            time.Minutes,
                            time.Seconds
                        );
                        ...
                        from.SendMessage(
                            53,
                            "The city will update again in {0} seconds.",
                            time.Seconds
                        );
                }
                ...
                {
                    from.SendMessage(
                        "You are not a citizen of this city, You must see the mayor to join this city."
                    );
                }
            }
            else
        ...
        public void DoJoin(Mobile mayor, Mobile from)
        {
            ...
            {
                mayor.SendMessage(
                    "You must lift the ban over them before you can add them to the city."
                );
            ...
            {
                from.SendMessage(
                    "You are already a mayor of another city. You cannot join this city."
                );
            ...
            {
                from.SendMessage(
                    "You are already a member of another city. You must leave that city before joining this one."
                );
            ...
            else if (PlayerGovernmentSystem.AreThereVendors(from))
            {
                ...
                mayor.SendMessage(
                    "They have existing vendors in their house and must remove them first."
                );
            }
            else
            {
                ...
                foreach (Mobile m in m_Members)
                {
                    ...
                    m.SendMessage(
                        53,
                        "The city now has {0} members and is at level {1}.",
                        m_Members.Count,
                        this.Level
                    );
#if ( ChatEnabled )
                    InternalChatMessage.SendInternalChatMsg(
                        m,
                        "City Join Message",
                        String.Format(" {0} has joined the city!", from.RawName)
                    );
#endif
                }
            }
        ...
        public void AdminDoJoin(Mobile admin, Mobile from)
        {
            ...
            {
                admin.SendMessage(
                    "User has a house in town with vendors that need to be removed first."
                );
            }
            else
            {
        ...
        public bool CalculateMaintenance()
        {
            ...
            {
                this.Mayor.SendMessage(
                    "Your cities maintenance has been paid, A total of {0} was taken out of your cites treasury you now have a total of {1} left in the city treasury.",
                    cost,
                ...
                InternalChatMessage.SendInternalChatMsg(
                    ...
                    "City System",
                    String.Format(
                        "City maintenance paid. {0} removed from treasury.  {1} remaning.",
                        cost,
                        (this.CityTreasury - cost)
                    )
                );
            ...
            else
            {
                if (m_Members != null)
                {
                    foreach (Mobile m in m_Members)
                    {
                        ...
                        m.SendMessage(
                            53,
                            "REASON: The city lacked the required funds in its treasury to pay its maintenance."
                        );
#if (ChatEnabled)
                        InternalChatMessage.SendInternalChatMsg(
                            m,
                            "City Message",
                            "City Disbanded due to lack of funds."
                        );
#endif
                    }
                }
        ...
        public void PitchMembers()
        {
            ...
            foreach (Mobile m in m_Members)
            {
                ...
                {
                    m.SendMessage(
                        "You must own a house in the city, Your membership has been revoked."
                    );
#if (ChatEnabled)
                    InternalChatMessage.SendInternalChatMsg(
                        m,
                        "City System",
                        "Your membership in the city has been revoked since you house is no longer in the limits."
                    );
#endif
                    removelist.Add(m);
        ...
        public void PickRandomNewMayor()
        {
            if (m_Members.Count > 0)
            {
                ...
                this.Mayor.SendMessage(
                    "You are the new mayor since the other has moved on! Because you were picked at random an election will take place in 24 hours!"
                );
#if (ChatEnabled)
                InternalChatMessage.SendInternalChatMsg(
                    this.Mayor,
                ...
                {
                    m.SendMessage(
                        "The mayor has left the town and there is no Assistant Mayor, {0} has been randomly chosen.  Please elect a new mayor within 24 hours!",
                        this.Mayor.Name
                    );
#if (ChatEnabled)
                    InternalChatMessage.SendInternalChatMsg(
        ...
        {
            if (
                (this.Mayor == null || !CheckMayorExist(this.Mayor))
                && (m_AssistMayor == null || !CheckAsstMayorExist(this.AssistMayor))
            )
            {
                PickRandomNewMayor();
                return true;
            ...
            if (incumbant.City == null || incumbant.City != this)
            {
                if (m_AssistMayor != null && CheckAsstMayorExist(m_AssistMayor))
                {
                    ...
                    m_AssistMayor.SendMessage(
                        "You are the new mayor since the other has moved on! Please remember to pick a new Assistant Mayor."
                    );
                    m_AssistMayor = null;
#if (ChatEnabled)
                    InternalChatMessage.SendInternalChatMsg(
                    ...
                    else
                    {
                        if (vend is CityRentedVendor)
                        {
                            ...
                            else
                            {
                                ...
                                owner.SendMessage(
                                    "Your vendor will delete in 2 days since your city is gone."
                                );
#if (ChatEnabled)
                                if (CheckVendorOwnerExist(vend.Owner, vend))
                                    InternalChatMessage.SendInternalChatMsg(
                                        owner,
                                        "City Message",
                                        " Your vendor will delete in 2 days because they are no longer in a town region."
                                    );
#endif
                                t.Start();
                            }
                            ...
                            (
                                vend.Region is PlayerCityRegion
                                || vend.Region is CityMarketRegion
                                || CheckIfVendorInTownhouse(vend)
                            ) && !citydelete
                        )
                            continue;
                        else
            ...
            else if (db1 == true)
            {
                if (m_Members != null)
                {
                    foreach (Mobile m in m_Members)
                    {
                        ...
                        m.SendMessage(
                            53,
                            "REASON: The city lacked the minumal amount of citizens."
                        );
                        m.SendMessage(53, "City Update Complete...");
#if (ChatEnabled)
                        InternalChatMessage.SendInternalChatMsg(
                            m,
                            "City Disbanded",
                            "City disbanded due to lack of citizens"
                        );
#endif
                    }
                }
            ...
            else
            {
                    ...
                    if (m_Members != null && m_Vendors != null)
                    {
                        foreach (Mobile m in m_Vendors)
                        {
                            if (m is CityPlayerVendor)
                            {
                                ...
                                mob.SendMessage(
                                    "Your income tax of {0} has been collected from your vendor.",
                                    vend.IncomeTax
                                );
#if (ChatEnabled)
                                InternalChatMessage.SendInternalChatMsg(
                    ...
                    if (m_Members != null)
                    {
                        foreach (Mobile m in m_Members)
                        {
                            if (m_HousingTax > 0)
                                ...
                                {
                                    m.SendMessage(
                                        "Your property tax has been collected, {0} has been withdrawn from your bank account.",
                                        m_HousingTax
                                    );
#if (ChatEnabled)
                                    InternalChatMessage.SendInternalChatMsg(
                                ...
                                else
                                {
                                    ...
                                    InternalChatMessage.SendInternalChatMsg(
                                        m,
                                        "You Owe Back Taxes",
                                        "You lack the gold to pay your taxes."
                                    );
#endif
                                    PlayerMobile pm = (PlayerMobile)m;
                ...
                if (m_CityTreasury <= 10000)
                {
                    if (m_Members != null)
                    {
                        foreach (Mobile m in m_Members)
                        {
                            m.SendMessage(
                                38,
                                "WARNING: City treasury is low, There is now less than 10k gold in the treasury, The city will disband if there is not enough gold in the treasury to cover the maintenance cost before the next city update."
                            );
#if (ChatEnabled)
                            InternalChatMessage.SendInternalChatMsg(
                                m,
                                "City Treasury",
                                "The city treasury is low, it will disband after the next maintainance, please add money!"
                            );
#endif
                        }
                    }
                ...
                if (m_Members != null)
                {
                    foreach (Mobile m in m_Members)
                    {
                        m.SendMessage(
                            53,
                            "The city has been updated, The city level is now {0}, with {1} citizens.",
                            Level,
                            Citizens.Count
                        );
                        m.SendMessage(53, "City Update Complete...");
        ...
        public CityManagementStone(Serial serial)
            : base(serial) { }
        ...
        private class CityUpdateTimer : Timer
        {
            ...
            public CityUpdateTimer(DateTime end, CityManagementStone stone)
