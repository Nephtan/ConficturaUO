...
namespace Server.Items
{
    public class CityBankStone : Item
    {
        ...
        public CityBankStone(Serial serial)
            : base(serial) { }
                    ...
                    switch (keyword)
                    {
                        ...
                        case 0x0001: // *balance*
                        {
                            ...
                            if (box != null)
                            {
                                this.Say(
                                    String.Format(
                                        "Thy current bank balance is {0} gold.",
                                        box.TotalGold.ToString()
                                    )
                                );
                            }
                            ...
                            if (split.Length >= 2)
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        this.Say(
                                            String.Format(
                                                "Into your bank box I have placed a check in the amount of: {0}",
                                                amount.ToString()
                                            )
                                        );
                                    }
