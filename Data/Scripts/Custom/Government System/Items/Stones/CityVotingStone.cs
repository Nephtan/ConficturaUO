...
namespace Server.Items
{
    public class CityVotingStone : Item
    {
        ...
        public void TallyVotes()
        {
            ...
            if (this.RunnerOne != null && this.RunnerTwo != null)
            {
                if (pm1.City != m_Stone || pm2.City != m_Stone)
                {
                    foreach (Mobile m in m_Stone.Citizens)
                    {
                        m.SendMessage(
                            53,
                            "CITY VOTING MESSAGE: One or both of the electees are no longer a member of the city! The Current mayor will stay in office this term."
                        );
                ...
                else if (this.RunnerOneVotes == this.RunnerTwoVotes)
                {
                    foreach (Mobile m in m_Stone.Citizens)
                    {
                        m.SendMessage(
                            53,
                            "CITY VOTING MESSAGE: The election was a tie! The Current mayor will stay in office this term."
                        );
                    }
                ...
                else if (this.RunnerOneVotes >= this.RunnerTwoVotes)
                {
                    ...
                    foreach (Mobile m in m_Stone.Citizens)
                    {
                        m.SendMessage(
                            53,
                            "CITY VOTING MESSAGE: {0} has been elected mayor over {1}!",
                            this.RunnerOne.Name,
                            this.Stone.CityName
                        );
                    }
            ...
            else
            {
                foreach (Mobile m in m_Stone.Citizens)
                {
                    m.SendMessage(
                        53,
                        "CITY VOTING MESSAGE: There was not enough people running for mayor, The current mayor will stay in office this term."
                    );
                }
        ...
        public CityVotingStone(Serial serial)
            : base(serial) { }
        ...
        private class CityElectionTimer : Timer
        {
            ...
            public CityElectionTimer(DateTime end, CityVotingStone stone)
