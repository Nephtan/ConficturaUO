...
namespace Server.Items
{
    public class CityPlacementTarget : MultiTarget
    {
        ...
        public CityPlacementTarget(CityDeed deed)
            ...
            if (ip != null)
            {
                ...
                if (from.AccessLevel >= AccessLevel.GameMaster || reg.AllowHousing(from, p))
                {
                    if (PlayerGovernmentSystem.CheckIfInUnderworld(from))
                    {
                        ...
                        m_Deed.Type == CivicStrutureType.FieldStoneCityHall
                        || m_Deed.Type == CivicStrutureType.SandstoneCityHall
                        || m_Deed.Type == CivicStrutureType.NecroCityHall
                        || m_Deed.Type == CivicStrutureType.MarbleCityHall
                        || m_Deed.Type == CivicStrutureType.StoneCityHall
                        || m_Deed.Type == CivicStrutureType.WoodCityHall
                        || m_Deed.Type == CivicStrutureType.AsianCityHall
                        || m_Deed.Type == CivicStrutureType.PlasterCityHall
                    )
                    {
                        ...
                        {
                            from.SendMessage(
                                "You lack the required skill to become a mayor at this time, You need at least 35.0 forensics."
                            );
                        }
                    }
                    else
    ...
    public abstract class CityDeed : Item
    {
        ...
        public CityDeed(int id, Point3D offset)
            : base(0x14F0)
        {
            Weight = 1.0;
        ...
        public void FinishPlacement(Mobile from, Point3D p)
        {
            if (Type == CivicStrutureType.FieldStoneCityHall)
            {
                ...
                building.MoveToWorld(
                    new Point3D(p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z),
                    from.Map
                );
