...
namespace Server.Items
{
    public class MediumCityGardenDeed : CityDeed
    {
        ...
        public MediumCityGardenDeed(Serial serial)
            : base(serial) { }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else if (pm.City != null)
            {
                ...
                else if (!PlayerGovernmentSystem.IsCityLevelReached(from, 2))
                {
                    ...
                    && from.Skills[SkillName.Forensics].Base < 50.0
                )
                {
                    from.SendMessage(
                        "You lack the required skill to place this building, You need at least 50.0 points in forensics."
                    );
                }
                else
                {
