...
namespace Server.Items
{
    ...
    {
        [Constructable]
        public LargeCityParkDeed()
            : base(0x147B, new Point3D(0, 10, 0))
        {
            Name = "a large park deed";
            Type = CivicStrutureType.LargeCityPark;
        }

        public LargeCityParkDeed(Serial serial)
            : base(serial) { }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else if (pm.City != null)
            {
                ...
                else if (!PlayerGovernmentSystem.IsCityLevelReached(from, 6))
                {
                    ...
                    && from.Skills[SkillName.Forensics].Base < 90.0
                )
                {
                    from.SendMessage(
                        "You lack the required skill to place this building, You need at least 90.0 points in forensics."
                    );
                }
                else
                {
