...
namespace Server.Items
{
    ...
    public class ResourceBox : BaseContainer
    {
        ...
        public bool TryAdd(Item item)
        {
            foreach (Type[] cat in m_AllTypes)
            {
                foreach (Type type in cat)
                {
                    ...
                    {
                        if (
                            m_Resources.ContainsKey(type)
                            && (int)m_Resources[type] + item.Amount >= 100000
                        )
                        {
                            this.PublicOverheadMessage(
                                ...
                                0,
                                false,
                                "I cannot hold more of that resource!"
                            );
                            return false;
                        }
        ...
        public ResourceBox(Serial serial)
            : base(serial) { }
