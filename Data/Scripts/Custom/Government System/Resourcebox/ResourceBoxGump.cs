...
namespace Server.Items
{
    public class ResourceBoxGump : Gump
    {
        ...
        public ResourceBoxGump(Mobile from, ResourceBox box, Pages page)
            : base(25, 25)
        {
            m_From = from;
            m_Box = box;
        ...
        private string AddLabelsButtonsAmounts()
        {
            ...
            switch ((int)m_Page)
            {
                case (int)Pages.Logs:
                {
                    m_Types = StorageTypes.Logs;
                    Label = "Logs";
                    Offset = 0;
                    break;
                }
                case (int)Pages.Boards:
                {
                    m_Types = StorageTypes.Boards;
                    Label = "Boards";
                    Offset = 0;
                    break;
                }
                case (int)Pages.Ingots:
                {
                    m_Types = StorageTypes.Ingots;
                    Label = "Ingots";
                    Offset = 0;
                    break;
                }
                case (int)Pages.Granites:
                {
                    m_Types = StorageTypes.Granites;
                    Label = "Granites";
                    Offset = 0;
                    break;
                }
                case (int)Pages.Scales:
                {
                    m_Types = StorageTypes.Scales;
                    Label = "Scales";
                    Offset = 0;
                    break;
                }
                case (int)Pages.Leathers:
                {
                    m_Types = StorageTypes.Leathers;
                    Label = "Leathers";
                    Offset = 0;
                    break;
                }
                case (int)Pages.Misc:
                {
                    m_Types = StorageTypes.Misc;
                    Label = "Misc";
                    Offset = 0;
                    break;
                }
                case (int)Pages.Reagents:
                {
                    m_Types = StorageTypes.Reagents;
                    Label = "Reagents";
                    Offset = 0;
                    break;
                }

                case (int)Pages.MoreLogs:
                {
                    m_Types = StorageTypes.Logs;
                    Label = "More Logs";
                    Offset = 16;
                ...
                if ((i - Offset) <= 7)
                {
                    ...
                    if ((int)m_Box.Resources[type] >= 100000)
                        ...
                        AddLabel(
                            ...
                            75 + ((i - Offset) * 25),
                            1152,
                            ((int)m_Box.Resources[type]).ToString()
                        );
                    AddButton(
                        75,
                        75 + ((i - Offset) * 25),
        ...
        private class ResourceBoxTarget : Target
        {
            ...
            public ResourceBoxTarget(ResourceBox box, Pages page)
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            else if (BID >= 100 && BID < 132)
            {
                ...
                {
                    from.SendMessage(
                        "Enter Amount, you may only take out 1 granite or tool at a time."
                    );
