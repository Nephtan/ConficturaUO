...
namespace Server.Mobiles
{
    public abstract class BaseLandLord : BaseVendor
    {
        ...
        public BaseLandLord(string title)
            : base(title)
        {
            Title = String.Format("the {0} ", title);
        }

        public BaseLandLord(Serial serial)
            : base(serial) { }
        ...
        private class InternalTarget : Target
        {
            ...
            public InternalTarget(BaseLandLord landlord)
                : base(30, false, TargetFlags.None) // range, allowground, flags
            {
                m_landlord = landlord;
            }
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is HouseSign)
                {
                    ...
                    {
                        from.SendMessage(
                            "That house sign does not seem to have a house associated with it."
                        );
                    }
                    else
                    {
