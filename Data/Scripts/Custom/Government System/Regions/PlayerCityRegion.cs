...
namespace Server.Regions
{
    public class CityLockDownTarget : Target
    {
        ...
        public CityLockDownTarget(CityManagementStone stone)
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Item)
            {
                ...
                {
                    from.SendMessage(
                        "That item cannot be locked down because it is already not movable."
                    );
                }
            }
        }
    ...
    public class CityReleaseTarget : Target
    {
        ...
        public CityReleaseTarget(CityManagementStone stone)
            : base(12, false, TargetFlags.None)
        {
            m_Stone = stone;
        }
    ...
    public class CityBanTarget : Target
    {
        ...
        public CityBanTarget(CityManagementStone stone)
            : base(12, false, TargetFlags.None)
        {
            m_Stone = stone;
        }
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (targeted is Mobile)
            {
                ...
                {
                    from.SendMessage(
                        "You cannot ban anymore players from this city, The ban list is full."
                    );
                }
    ...
    public class PlayerCityRegion : GuardedRegion
    {
        ...
        public PlayerCityRegion(CityManagementStone stone, Map map, Rectangle3D[] area)
            : base(PickRegionName(), map, DefaultPriority, area)
        {
            //LoadFromXml = false;
        ...
        public override void OnExit(Mobile m)
        {
            if (m_Stone != null)
            {
                if (m_Stone.Level == 1)
                    m.SendMessage(
                        "You have left the {0} of {1}.",
                        PlayerGovernmentSystem.Title1,
                        m_Stone.CityName
                    );
                else if (m_Stone.Level == 2)
                    m.SendMessage(
                        "You have left the {0} of {1}.",
                        PlayerGovernmentSystem.Title2,
                        m_Stone.CityName
                    );
                else if (m_Stone.Level == 3)
                    m.SendMessage(
                        "You have left the {0} of {1}.",
                        PlayerGovernmentSystem.Title3,
                        m_Stone.CityName
                    );
                else if (m_Stone.Level == 4)
                    m.SendMessage(
                        "You have left the {0} of {1}.",
                        PlayerGovernmentSystem.Title4,
                        m_Stone.CityName
                    );
                else if (m_Stone.Level == 5)
                    m.SendMessage(
                        "You have left the {0} of {1}.",
                        PlayerGovernmentSystem.Title5,
                        m_Stone.CityName
                    );
                else if (m_Stone.Level == 6)
                    m.SendMessage(
                        "You have left the {0} of {1}.",
                        PlayerGovernmentSystem.Title6,
                        m_Stone.CityName
                    );
                else
