    ...
    public class CityUpgradeSystem
    {
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(CityUpgrade_OnCommand)
            );
        }
            ...
            else
            {
                                    ...
                                    if (list[i] is CityManagementStone)
                                    {
                                        ...
                                        if (addons.Count > 0)
                                        {
                                            for (int j = 0; j < addons.Count; j++)
                                            {
                                                ...
                                                if (
                                                    Region.Find(p, m) == r
                                                    && !Server.CityTypes.IsCityType(type)
                                                )
                ...
                using (
                    StreamWriter sw = new StreamWriter(
                        Path.Combine(Core.BaseDirectory, "Data/GovernmentVersion.xml")
                    )
                )
                {
                ...
                from.SendMessage(
                    "City System successfully upgraded to {0}",
                    PlayerGovernmentSystem.SystemVersion
                );
                Console.WriteLine(
                    "Government system successfully upgraded to version {0}",
                    PlayerGovernmentSystem.SystemVersion
                );
            }
        }
