...
{
    public class SponsorGump : Gump
    {
        private CityManagementStone m_Stone;
        private Mobile m_From;

        public SponsorGump(CityManagementStone stone, Mobile from)
            : base(50, 50)
        {
            m_Stone = stone;
            m_From = from;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1)
            {
                m_Stone.Sponsored.Add(from);
                from.SendMessage("You must wait for the mayor to review your request.");
            }
