...
{
    public class DeclareCityAllegiancesGump : Gump
    {
        ...
        public DeclareCityAllegiancesGump(
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Previous page
            {
                if (m_ListPage > 0)
                    from.SendGump(
                        new DeclareCityAllegiancesGump(m_Stone, m_ListPage - 1, m_List, m_CountList)
                    );
            }
            ...
            if (info.ButtonID >= 100) // Declare Allegiances
            {
                ...
                if (stone == m_Stone)
                {
                    from.SendMessage("You cannot declare allegiances on your own city.");
                }
                else if (
                    stone.WarsInvited.Contains(m_Stone)
                    || m_Stone.WarsInvited.Contains(stone)
                    || m_Stone.WarsDeclared.Contains(stone)
                    || stone.WarsDeclared.Contains(m_Stone)
                    || m_Stone.Waring.Contains(stone)
                    || stone.Waring.Contains(m_Stone)
                )
                {
                    from.SendMessage("You are at war with this city.");
                }
                else if (
                    stone.AllegiancesInvited.Contains(m_Stone)
                    || m_Stone.AllegiancesInvited.Contains(stone)
                    || m_Stone.AllegiancesDeclared.Contains(stone)
                    || stone.AllegiancesDeclared.Contains(m_Stone)
                    || m_Stone.Allegiances.Contains(stone)
                    || stone.Allegiances.Contains(m_Stone)
                )
                {
                    from.SendMessage("You are already at allegiance with this city.");
                }
                else
                {
                    if (!m_Stone.AllegiancesDeclared.Contains(stone))
                    {
                        m_Stone.AllegiancesDeclared.Add(stone);

                        foreach (Mobile m in m_Stone.Citizens)
                        {
                            m.SendMessage(
                                "CITY MESSAGE: We have declared allegiance with {0}.",
                                stone.CityName
                            );
                        }
                    }
                    ...
                    {
                        stone.AllegiancesInvited.Add(m_Stone);

                        foreach (Mobile m in stone.Citizens)
                        {
                            m.SendMessage(
                                "CITY MESSAGE: {0} has declared allegiance with us.",
                                m_Stone.CityName
                            );
                        }
                    }
