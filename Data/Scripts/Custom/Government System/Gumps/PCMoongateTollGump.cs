...
namespace Server.Gumps
{
    public class PCMoongateTollGump : Gump
    {
        ...
        public PCMoongateTollGump(
            Item gate,
            CityManagementStone incomingCity,
            CityManagementStone outgoingCity
        )
            : base(50, 50)
        {
            m_Gate = gate;
            ...
            AddHtml(
                ...
                259,
                19,
                @"<BASEFONT COLOR= WHITE><CENTER>Travel Tax Report</CENTER></BASEFONT>",
                (bool)false,
                (bool)false
            );
            AddHtml(
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // OKAY
            {
                if (from.InRange(m_Gate.GetWorldLocation(), 3))
                {
                    ...
                    {
                        from.SendMessage(
                            "You do not have enough gold in your bank account to do this."
                        );
                    }
                }
                else
