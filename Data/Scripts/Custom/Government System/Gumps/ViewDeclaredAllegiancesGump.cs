...
{
    public class ViewDeclaredAllegiancesGump : Gump
    {
        ...
        public ViewDeclaredAllegiancesGump(
            ...
            : base(50, 50)
        {
            ...
            if (m_List == null)
            {
                ArrayList cities = m_Stone.AllegiancesDeclared;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Previous page
            {
                if (m_ListPage > 0)
                    from.SendGump(
                        new ViewDeclaredAllegiancesGump(
                            m_Stone,
                            m_ListPage - 1,
                            m_List,
                            m_CountList
            ...
            if (info.ButtonID >= 100) // Declare Allegiances
            {
                ...
                if (m_Stone.AllegiancesDeclared.Contains(stone))
                {
                    ...
                    {
                        m.SendMessage(
                            "CITY MESSAGE: We have resended our allegiance invite from {0}.",
                            stone.CityName
                        );
                    }
                }
                ...
                if (stone.AllegiancesInvited.Contains(m_Stone))
                {
                    ...
                    {
                        m.SendMessage(
                            "CITY MESSAGE: {0} has resended thier allegiance invite from us.",
                            m_Stone.CityName
                        );
                    }
                }
