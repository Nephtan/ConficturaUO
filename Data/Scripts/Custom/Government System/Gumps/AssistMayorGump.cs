...
{
    public class AssistMayorGump : Gump
    {
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Previous page
            {
                if (m_ListPage > 0)
                    from.SendGump(
                        new AssistMayorGump(m_Stone, m_ListPage - 1, m_List, m_CountList)
                    );
            }
            ...
            if (info.ButtonID >= 100) // Pick Assistant Mayor
            {
                ...
                {
                    PlayerMobile oldAM = (PlayerMobile)m_Stone.AssistMayor;
                    oldAM.CityTitle = "Citizen";
                }

                pm.CityTitle = "Assistant Mayor";
                m_Stone.AssistMayor = mob;
                from.SendMessage("You have selected {0} as your Assistant Mayor.", mob.Name);
                from.CloseGump(typeof(AssistMayorGump));
                from.SendGump(new CityManagementGump(m_Stone, from));
            }
        }
    }
