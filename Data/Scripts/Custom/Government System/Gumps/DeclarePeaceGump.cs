...
{
    public class DeclarePeaceGump : Gump
    {
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Previous page
            {
                if (m_ListPage > 0)
                    from.SendGump(
                        new DeclarePeaceGump(m_Stone, m_ListPage - 1, m_List, m_CountList)
                    );
            }
            ...
            if (info.ButtonID >= 100) // Declare War
            {
                ...
                if (m_Stone.Waring.Contains(stone))
                {
                    ...
                    {
                        m.SendMessage(
                            "CITY MESSAGE: We are now at peace with {0}.",
                            stone.CityName
                        );
                    }
                }
