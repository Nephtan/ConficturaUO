    ...
    public class DeclareCityWarGump : Gump
    {
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Previous page
            {
                if (m_ListPage > 0)
                    from.SendGump(
                        new DeclareCityWarGump(m_Stone, m_ListPage - 1, m_List, m_CountList)
                    );
            }
            ...
            if (info.ButtonID >= 100) // Declare War
            {
                ...
                else
                {
                    if (!m_Stone.WarsDeclared.Contains(stone))
                    {
                        ...
                        {
                            m.SendMessage(
                                "CITY MESSAGE: We have declared war against {0}.",
                                stone.CityName
                            );
                        }
                    }
                    ...
                    if (!stone.WarsInvited.Contains(m_Stone))
                    {
                        ...
                        {
                            m.SendMessage(
                                "CITY MESSAGE: {0} has declared war against us.",
                                m_Stone.CityName
                            );
                        }
                    }
