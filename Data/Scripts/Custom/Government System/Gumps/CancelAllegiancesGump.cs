...
{
    public class CancelAllegiancesGump : Gump
    {
            ...
            : base(50, 50)
        {
            ...
            if (m_List == null)
            {
                ArrayList cities = m_Stone.Allegiances;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Previous page
            {
                if (m_ListPage > 0)
                    from.SendGump(
                        new CancelAllegiancesGump(m_Stone, m_ListPage - 1, m_List, m_CountList)
                    );
            }
            ...
            if (info.ButtonID >= 100) // Declare Allegiances
            {
                ...
                if (m_Stone.Allegiances.Contains(stone))
                {
                    m_Stone.Allegiances.Remove(stone);

                    foreach (Mobile m in m_Stone.Citizens)
                    {
                        m.SendMessage(
                            "CITY MESSAGE: We are no longer allied with {0}.",
                            stone.CityName
                        );
                    }
                }

                if (stone.Allegiances.Contains(m_Stone))
                {
                    stone.Allegiances.Remove(m_Stone);
