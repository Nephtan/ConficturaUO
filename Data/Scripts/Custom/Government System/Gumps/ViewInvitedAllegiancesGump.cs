...
{
    public class ViewInvitedAllegiancesGump : Gump
    {
        ...
        public ViewInvitedAllegiancesGump(
            ...
            : base(50, 50)
        {
            ...
            if (m_List == null)
            {
                ArrayList cities = m_Stone.AllegiancesInvited;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Previous page
            {
                if (m_ListPage > 0)
                    from.SendGump(
                        new ViewInvitedAllegiancesGump(m_Stone, m_ListPage - 1, m_List, m_CountList)
                    );
            }
            ...
            if (info.ButtonID >= 100 && info.ButtonID <= 999) // Decline Allegiances
            {
                CityManagementStone stone = m_List[info.ButtonID - 100] as CityManagementStone;

                if (m_Stone.AllegiancesInvited.Contains(stone))
                {
                    m_Stone.AllegiancesInvited.Remove(stone);

                    foreach (Mobile m in m_Stone.Citizens)
                    {
                        m.SendMessage(
                            "CITY MESSAGE: We have declined {0} allegiance offer.",
                            stone.CityName
                        );
                    }
                }

                if (stone.AllegiancesDeclared.Contains(m_Stone))
                {
                    stone.AllegiancesDeclared.Remove(m_Stone);

                    foreach (Mobile m in stone.Citizens)
                    {
                        m.SendMessage(
                            "CITY MESSAGE: {0} has declined our allegiance offer.",
                            m_Stone.CityName
                        );
                    }
                }
            ...
            if (info.ButtonID >= 1000) // Accept Allegiances
            {
                ...
                {
                    m_Stone.AllegiancesInvited.Remove(stone);
                    m_Stone.Allegiances.Add(stone);

                    foreach (Mobile m in m_Stone.Citizens)
                    {
                        m.SendMessage("CITY MESSAGE: We are now allied with {0}.", stone.CityName);
                    }
                }
                ...
                {
                    stone.AllegiancesDeclared.Remove(m_Stone);
                    stone.Allegiances.Add(m_Stone);

                    foreach (Mobile m in stone.Citizens)
                    {
                        m.SendMessage(
                            "CITY MESSAGE: We are now allied with {0}.",
                            m_Stone.CityName
                        );
                    }
                }
