    ...
    public class ViewInvitedWarsGump : Gump
    {
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1) // Previous page
            {
                if (m_ListPage > 0)
                    from.SendGump(
                        new ViewInvitedWarsGump(m_Stone, m_ListPage - 1, m_List, m_CountList)
                    );
            }
            ...
            if (info.ButtonID >= 100 && info.ButtonID <= 999) // Decline War
            {
                ...
                if (m_Stone.WarsInvited.Contains(stone))
                {
                    ...
                    {
                        m.SendMessage(
                            "CITY MESSAGE: We have declined {0} war offer.",
                            stone.CityName
                        );
                    }
                }
                ...
                if (stone.WarsDeclared.Contains(m_Stone))
                {
                    ...
                    {
                        m.SendMessage(
                            "CITY MESSAGE: {0} has declined our war offer.",
                            m_Stone.CityName
                        );
                    }
                }
            ...
            if (info.ButtonID >= 1000) // Accept War
            {
                ...
                if (stone.WarsDeclared.Contains(m_Stone))
                {
                    ...
                    {
                        m.SendMessage(
                            "CITY MESSAGE: We are now at war with {0}.",
                            m_Stone.CityName
                        );
                    }
                }
