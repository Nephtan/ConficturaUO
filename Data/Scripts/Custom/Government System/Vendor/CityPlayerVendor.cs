...
namespace Server.Mobiles
{
    public class CityVendorBackpack : VendorBackpack
    {
        ...
        public CityVendorBackpack(Serial serial)
            : base(serial) { }
    ...
    public class CityPlayerVendor : PlayerVendor
    {
        ...
        public static void Initialize()
        {
            CommandSystem.Register(
                "MoveVendor",
                AccessLevel.Player,
                new CommandEventHandler(MoveVendor_OnCommand)
            );
            CommandSystem.Register(
                "MoveBox",
                AccessLevel.Player,
        ...
        private static void MoveVendor_OnCommand(CommandEventArgs e)
        {
            ...
            if (pm.City != null && pm.City.Mayor == pm)
            {
                ...
                if (
                    stone.PCRegion == r
                    || (r is CityMarketRegion && ((CityMarketRegion)r).Stone == stone)
                )
                {
            ...
            protected override void OnTarget(Mobile from, object o)
            {
                if (o is CityPlayerVendor)
                {
                    if (((CityPlayerVendor)o).City == null)
                        from.SendMessage(
                            "You may not move a vendor that is sceduled for termination!"
                        );

                    Mobile mob = o as Mobile;
                    if (
                        PlayerGovernmentSystem.IsAtCity(from, mob)
                        || CityRentedVendor.IsLegitmateVendor(from, mob)
                    )
                        from.Target = new MoveVendorTarget(o);
                    else
                        from.SendMessage(
                            "You may only move vendors that are in the public town areas."
                        );
                }
                else if (o is CityResourceBox)
                {
                    ...
                    if (PlayerGovernmentSystem.IsAtCity(item))
                        ...
                        from.SendMessage(
                            "You may only move boxes that are in the public town areas."
                        );
                }
                else
                    from.SendMessage(
                        "You may only use this to move City Vendors or Town Resource Boxes!"
                    );
            }
        }

        public CityPlayerVendor(Mobile owner, CityManagementStone stone)
            : base(owner, null)
        {
            Owner = owner;
            m_stone = stone;
        ...
        public CityPlayerVendor(Serial serial)
            : base(serial) { }
