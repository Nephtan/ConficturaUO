...
namespace Server.Items
{
    public class PublishedBook : BaseBook
    {
        ...
        [Constructable]
        public PublishedBook(BaseBook book)
            : base(book.ItemID, book.Title, book.Author, book.PagesCount, book.Writable)
        {
            // copy book
            Weight = book.Weight;
            Name = book.Name;
        ...
        [Constructable]
        public PublishedBook(string id, int pageCount)
            : base(0x0000, "", "", pageCount, false)
        {
            ...
            for (int i = 0; i < book.Contents.Length; i++)
            {
                ...
                this.Pages[book.Contents[i].Page - 1].Lines[book.Contents[i].Line - 1] =
                    book.Contents[i].Text;
            }
        }
        ...
        public bool IsPublishable()
        {
            if (!Writable)
                return false;
            // make sure something exists other than default information ...
            if (Title.Trim().Length < 5)
                return false;
            if (Author.Trim().Length < 5)
                return false;
            if (Title.ToLower().Trim() == "title")
                return false;
            if (Author.ToLower().Trim() == "author")
                return false;
            int count = 0;
            for (int i = 0; i < Pages.Length; i++)
                count += string.Join("", Pages[i].Lines).Trim().Length;
            if (count < 100)
                return false; // 100 characters or more.
        ...
        {
            get { return m_FirstPublished; }
        }
        public DateTime LastPublished
        {
            get { return m_LastPublished; }
        }
        public string PublishID
        {
            get { return m_PublishID; }
            set { m_PublishID = value; }
        }
        ...
        public PublishedBook(Serial serial)
            : base(serial) { }
