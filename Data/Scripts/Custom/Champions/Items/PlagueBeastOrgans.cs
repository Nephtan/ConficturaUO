...
namespace Server.Items
{
    public class PlagueBeastOrgan : PlagueBeastInnard
    {
        ...
        public override bool Scissor(Mobile from, Scissors scissors)
        {
            if (IsCuttable && IsAccessibleTo(from))
            {
                ...
                {
                    m_Timer = Timer.DelayCall<Mobile>(
                        TimeSpan.FromSeconds(3),
                        new TimerStateCallback<Mobile>(FinishOpening),
                        from
                    );
    ...
    public class PlagueBeastRubbleOrgan : PlagueBeastOrgan
    {
        ...
        private static int[] m_Hues = new int[] { 0xD, 0x17, 0x2B, 0x42, 0x54, 0x5D };
        ...
        public override bool OnDropped(Mobile from, Item item, PlagueBeastComponent to)
        {
            if (to.Hue == 0x1 && m_Gland == null && item is PlagueBeastGland)
            {
                ...
                m_Timer = Timer.DelayCall(
                    TimeSpan.FromSeconds(3),
                    new TimerCallback(FinishHealing)
                );
                from.SendAsciiMessage(
                    0x3B2,
                    "* You place the healthy gland inside the organ sac *"
                );
                item.Movable = false;
