...
namespace Server.Mobiles
{
    ...
    public class Ilhenir : BaseChampion
    {
        ...
        {
            get { return new Type[] { typeof(MonsterStatuette) }; }
        }
        ...
        public virtual void CacophonicAttack(Mobile to)
        {
            ...
            if (to.Alive && to.Player && m_Table[to] == null)
            {
                ...
                m_Table[to] = Timer.DelayCall(
                    TimeSpan.FromSeconds(30),
                    new TimerStateCallback(EndCacophonic_Callback),
                    to
                );
            }
        }
        ...
        public void Damage(Mobile m)
        {
            if (m_Corrosive)
            {
                ...
                for (int i = 0; i < items.Count; ++i)
                {
                    ...
                    if (
                        wearable != null
                        && wearable.HitPoints >= 10
                        && Utility.RandomDouble() < 0.25
                    )
                    {
                        wearable.HitPoints -=
                            (wearable.HitPoints == 10) ? Utility.Random(1, 5) : 10;
                        damaged = true;
                    }
                }
