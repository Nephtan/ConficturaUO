...
namespace Server.Engines.CannedEvil
{
    public class ChampionSpawn : Item
    {
        ...
        public void SetWhiteSkullCount(int val)
        {
            ...
            for (int i = m_WhiteSkulls.Count; i < val; ++i)
            {
                ...
                Effects.SendLocationEffect(
                    new Point3D(skull.X + 1, skull.Y + 1, skull.Z),
                    skull.Map,
                    0x3728,
                    10
                );
            ...
            for (int j = 0; j < pm.JusticeProtectors.Count; ++j)
            {
                ...
                if (chance > Utility.Random(100))
                {
                    try
                    {
                        ...
                        SpecialScroll scrollDupe =
                            Activator.CreateInstance(scroll.GetType()) as SpecialScroll;
                    ...
                    if (m.Deleted)
                    {
                        ...
                        if (killer is PlayerMobile)
                        {
                            ...
                            if (mobSubLevel >= 0)
                            {
                                ...
                                PlayerMobile.ChampionTitleInfo info = (
                                    (PlayerMobile)killer
                                ).ChampionTitles;
        ...
        public void AdvanceLevel()
        {
            ...
            if (Level < 16)
            {
                ...
                if (m_Altar != null)
                {
                    ...
                    Effects.SendLocationEffect(
                        new Point3D(m_Altar.X + 1, m_Altar.Y + 1, m_Altar.Z),
                        m_Altar.Map,
                        0x3728,
                        10
                    );
        ...
        public void SpawnChampion()
        {
            ...
            {
                m_Champion =
                    Activator.CreateInstance(ChampionSpawnInfo.GetInfo(m_Type).Champion) as Mobile;
            }
            catch { }
            ...
            while (m_Creatures.Count < (200 - (GetSubLevel() * 40)))
            {
                ...
                if (m is BaseCreature)
                {
                    ...
                    if (!m_ConfinedRoaming)
                    {
                        ...
                        bc.RangeHome = (int)(
                            Math.Sqrt(
                                m_SpawnArea.Width * m_SpawnArea.Width
                                    + m_SpawnArea.Height * m_SpawnArea.Height
                            ) / 2
                        );
                    }
                    else
                    {
                        ...
                        double minXDist = Math.Min(
                            bc.GetDistanceToSqrt(xWall1),
                            bc.GetDistanceToSqrt(xWall2)
                        );
                        double minYDist = Math.Min(
                            bc.GetDistanceToSqrt(yWall1),
                            bc.GetDistanceToSqrt(yWall2)
                        );
