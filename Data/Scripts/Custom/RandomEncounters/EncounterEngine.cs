...
namespace Server.Misc
{
    ...
    public class RandomEncounterEngine
    {
        ...
        private static string m_EncountersFile =
        ...
        public static float Cleanup
        {
            get { return m_Cleanup; }
        }
        public static int CleanupGrace
        {
            get { return m_CleanupGrace; }
        }
        ...
        public static bool DebugEffect
        {
            get { return m_DebugEffect; }
        }
        ...
        public static string EncountersFile
        {
            get { return m_EncountersFile; }
        }
        ...
        public static void Initialize()
        {
            ...
            if (MaybeLoadXml())
            {
                ...
                Console.WriteLine("RandomEncounters: **WE'RE LIVE, BABY***...");
                Console.WriteLine(
        ...
        private static bool LoadXml()
        {
            ...
            try
            {
                ...
                try
                {
                    language = root.Attributes["language"].Value;
                }
                catch { }
                try
                {
                    skipHidden = root.Attributes["skiphidden"].Value;
                }
                catch { }
                try
                {
                    picker = root.Attributes["picker"].Value;
                }
                catch { }
                try
                {
                    delay = root.Attributes["delay"].Value;
                }
                catch { }
                try
                {
                    interval = root.Attributes["interval"].Value;
                }
                catch { }
                try
                {
                    cleanup = root.Attributes["cleanup"].Value;
                }
                catch { }
                try
                {
                    cleanupgrace = root.Attributes["cleanupGrace"].Value;
                }
                catch { }
                try
                {
                    debug = root.Attributes["debug"].Value;
                }
                catch { }
                try
                {
                    debugEffect = root.Attributes["debugEffect"].Value;
                }
                catch { }
                try
                {
                    RTFM = root.Attributes["RTFM"].Value;
                }
                catch { }
                ...
                //  Dungeon Wilderness Guarded House Jail
                {
                    ...
                    if (tokens.Length >= 3)
                    {
                        m_Intervals[0] = float.Parse(tokens[0], m_Language); // VillageRegion
                        m_Intervals[1] = float.Parse(tokens[1], m_Language); // VillageRegion
                        m_Intervals[2] = float.Parse(tokens[2], m_Language); // VillageRegion
                    }

                    if (tokens.Length == 2)
                    {
                        m_Intervals[0] = float.Parse(tokens[0], m_Language); // DungeonRegion
                        m_Intervals[1] = float.Parse(tokens[1], m_Language); // OutDoorRegion
                        m_Intervals[2] = float.Parse(tokens[1], m_Language); // VillageRegion
                    }

                    if (tokens.Length == 1)
                    {
                        m_Intervals[0] = float.Parse(tokens[0], m_Language); // DungeonRegion
                        m_Intervals[1] = float.Parse(tokens[0], m_Language); // OutDoorRegion
                        m_Intervals[2] = float.Parse(tokens[0], m_Language); // VillageRegion
                    }
                }
                ...
                if (facetNodes.Count == 0)
                    Console.WriteLine(
                        "RandomEncounters: There appeared to be NO FACETS defined in the configuration file!"
                    );
                //------------------------------------------------------------------
                // Iterate over facets
                //------------------------------------------------------------------
                foreach (XmlNode facetNode in facetNodes)
                {
                    ...
                    {
                        string regionType = "OutDoorRegion";
                        string regionName = "default";
                        string failed = "";

                        try
                        {
                            regionType = regionNode.Attributes["type"].Value;
                        }
                        catch
                        {
                            failed = "type";
                        ...
                        try
                        {
                            regionName = regionNode.Attributes["name"].Value;
                        }
                        catch
                        {
                            ;
                        ...
                        {
                            Console.WriteLine(
                                "Attempted to add an element without a {0} at {1}. IGNORING.",
                                failed,
                        ...
                        foreach (XmlNode encounterNode in encounterNodes)
                        {
                            ...
                            try
                            {
                                encounterDistance = encounterNode.Attributes["distance"].Value;
                            }
                            catch { }
                            try
                            {
                                encounterProbability = encounterNode.Attributes["p"].Value;
                            }
                            catch { }
                            try
                            {
                                encounterLand = encounterNode.Attributes["landType"].Value;
                            }
                            catch { }
                            try
                            {
                                encounterWater = encounterNode.Attributes["water"].Value;
                            }
                            catch { }
                            try
                            {
                                encounterTime = encounterNode.Attributes["time"].Value;
                            }
                            catch { }
                            try
                            {
                                encounterLevel = encounterNode.Attributes["level"].Value;
                            }
                            catch { }
                            try
                            {
                                encounterScale = encounterNode.Attributes["scaleUp"].Value;
                            }
                            catch { }
            ...
            {
                Console.WriteLine(
                    "RandomEncounters: Exception caught attempting to load file: "
                        + m_EncountersFile
                );
        ...
        )
        {
            ...
            try
            {
                probability = childNode.Attributes["p"].Value;
            }
            catch { }
            try
            {
                n = childNode.Attributes["n"].Value;
            }
            catch { }
            try
            {
            ...
            try
            {
                forceAttack = childNode.Attributes["forceAttack"].Value;
            }
            catch { }
            try
            {
                effectStr = childNode.Attributes["effect"].Value;
            }
            catch { }
            ...
            string effectHue = "0";
            if (effectOptions.Length > 1)
                effectHue = effectOptions[1];

            try
            {
                pick = childNode.Attributes["pick"].Value;
            }
            catch
            {
                Console.WriteLine(
                    "Attempted to add an element without a pick at {0}. IGNORING.",
                    childNode.Attributes["lineNumber"].Value
                );
                return;
            }

            if (parentNode.Name == "Item" && childNode.Name == "Mobile")
                reject = true;
            ...
            EncounterElement newElement = new EncounterElement(
                childNode,
                probability,
                pick,
                id,
            ...
            {
                if (subNode.Name == "Prop")
                    AddProp(childNode, subNode, newElement);
                else
                    AddRecursiveMobilesAndItems(childNode, subNode, newElement);
            }
        ...
        public static void GenerateEncounters(string timerType)
        {
            ...
            foreach (PlayerMobile chosenMobile in chosenMobiles)
            {
                ...
                if (m_Debug)
                    Console.WriteLine(
                        "RandomEncounters: Picking encounter for player=\"{0}, map=\"{1}\", region=\"{2}\" ... ",
                        chosenMobile.Name,
                ...
                foreach (EncounterSet encounterSet in possibleEncounters)
                {
                    ...
                    if (draw <= encounterSet.Probability)
                    {
                        ...
                        if (encounterSet.Count != 0)
                        {
                            choice = Utility.RandomMinMax(0, encounterSet.Count - 1);
                        }

                        RandomEncounter encounter = (RandomEncounter)encounterSet[choice];

                        double playerLevel = Helpers.CalculateLevelForMobile(
                            chosenMobile,
                            encounter.LevelType
                        );
        ...
        )
        {
            ...
            {
                Console.WriteLine(
                    "RandomEncounters: Bad attempt to generate an element of type \"{0}\".",
                    parentElement.PickFrom
                );
                return;
            }
            ...
            //--------------------------------------------------------------
            {
                ...
                Console.WriteLine(
                    "RandomEncounters: Bad attempt to contruct an element of type \"{0}\", on line {1}",
                    parentElement.PickFrom,
                    line
                );
                Console.WriteLine("{0}", e);
            ...
            if (created is Mobile)
            {
                ...
                if (m is BaseCreature)
                {
                    ...
                    if (c.IsEnemy(playerMobile))
                    {
                        ...
                        if (c.AI == AIType.AI_Vendor)
                            ;
                        else if (parentElement.ForceAttack)
                            c.Combatant = playerMobile;
                    }
                ...
                {
                    double pLevel = Helpers.CalculateLevelForMobile(
                        playerMobile,
                        LevelType.Fighter
                    );
                    ...
                    if (scaleUp > 0)
                        for (int i = 0; i < scaleUp; i++)
                        {
                            ...
                            XmlAttach.AttachTo(
                                addonMob,
                                new XmlDateCount("RandomEncountersCreated")
                            );
            ...
            foreach (NetState state in NetState.Instances)
            {
                if (state.Mobile is PlayerMobile)
                {
                    ...
                    if (accessLevel != AccessLevel.Player)
                        continue; // staff doesn't qualify
                    if (!mobile.Alive)
                        continue; // no randoms for dead players
                    //if (pm!=null && pm.Young)                               continue; // young don't qualify
                    if (RandomEncounterEngine.SkipHidden && mobile.Hidden)
