    ...
    public class XmlAttach
    {
        ...
        public static void Initialize()
        {
            ...
            CommandSystem.Register(
                ...
                new CommandEventHandler(GetAttachments_OnCommand)
            );
                    ...
                    if (attachtype != null && attachtype.IsSubclassOf(typeof(XmlAttachment)))
                    {
                        ...
                        for (int i = 0; i < list.Count; ++i)
                        {
                            ...
                            if (o == null)
                            {
                                AddResponse(
                                    String.Format(
                                        "Unable to construct {0} with specified args",
                                        attachtype.Name
                                    )
                                );
                                break;
                            ...
                            if (XmlAttach.AttachTo(null, list[i], o, true))
                            {
                                ...
                                {
                                    AddResponse(
                                        String.Format("Added {0} to {1}", attachtype.Name, list[i])
                                    );
                                }
                                count++;
                            }
                ...
                if (e != null && list != null && e.Length >= 1)
                {
                    ...
                    if (attachtype != null && attachtype.IsSubclassOf(typeof(XmlAttachment)))
                    {
                        ...
                        if (count > 0)
                        {
                            AddResponse(
                                String.Format(
                                    "Attachment {0} has been deleted [{1}]",
                                    attachtype.Name,
                                    count
                                )
                            );
                        ...
                        {
                            AddResponse(
                                String.Format("Attachment {0} not deleted", attachtype.Name)
                            );
                        }
                    }
                    else
        ...
        public static void CleanUp()
        {
            ...
            foreach (XmlAttachment a in XmlAttach.Values)
            {
                if (
                    a.OwnedBy == null
                    || (a.OwnedBy is Mobile && ((Mobile)a.OwnedBy).Deleted)
        ...
        public static void EventSink_Movement(MovementEventArgs args)
        {
            ...
            if (from.Map != null)
            {
                IPooledEnumerable itemlist = from.Map.GetItemsInRange(
                    from.Location,
                    Map.SectorSize
                );
                if (itemlist != null)
                ...
                // check for mobiles
                IPooledEnumerable moblist = from.Map.GetMobilesInRange(
                    from.Location,
                    Map.SectorSize
                );
                if (moblist != null)
                        ...
                        if (alist != null)
                        {
                            foreach (XmlAttachment a in alist)
                            {
                                if (
                                    ...
                                    && (
                                        type == null
                                        || (a.GetType() == type || a.GetType().IsSubclassOf(type))
                                    )
                                    && (name == null || name == a.Name)
                                )
                                {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                switch (m_set)
                {
                    case "add":
                        ...
                        {
                            from.SendMessage(
                                "Unable to construct attachment {0}",
                                m_e.Arguments[0]
                            );
                        }
        ...
        public static ArrayList FindAttachments(
                ...
                if (list != null)
                {
                    ...
                    foreach (XmlAttachment i in list)
                    {
                        ...
                        if (
                            (
                                type == null
                                || (itype != null && (itype == type || itype.IsSubclassOf(type)))
        ...
        public static XmlAttachment FindAttachment(
            Hashtable attachments,
            object o,
            Type type,
            string name
        ...
        public static void OnWeaponHit(
            BaseWeapon weapon,
            Mobile attacker,
            Mobile defender,
            int damage
        ...
        public class ErrorReporter
        {
            private static void SendEmail(string filePath)
            {
                ...
                message.Body =
                    "Automated XmlSpawner2 Attachment Report. See attachment for details.";
