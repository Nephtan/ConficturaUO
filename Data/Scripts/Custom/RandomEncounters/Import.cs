...
namespace Server.Misc
{
    ...
    public class ImportRecord
    {
        ...
        public override string ToString()
        {
            string format = String.Format(
                "{0} ({1},{2},{3}) {4}/{5} n={6}, map={7}, region={8}, level={9:0.0}, hostile={10} p={11:0.000}",
                m_spawn,
                m_location.X,
                m_location.Y,
                m_location.Z,
    ...
    public class ImportHelper
    {
        ...
        public static void Import(Mobile mobile)
        {
            ...
            {
                Console.WriteLine(
                    "#### RandomEncounters: Error; no such directory \"{0}\" ... ",
                    topDir
                );
                return;
            }
        ...
        public static void ProcessSpawnLine(string spawn, string mapName)
        {
            ...
            {
                if (counts[i] > 0)
                    ProcessSpawnSet(sets[i], x, y, z, hRange, sRange, counts[i], mapName);
            }
        }
        ...
        )
        {
            ...
            if (region != null && region.Name != null && region.Name != "")
            {
        ...
        {
            string importName = Path.Combine(
                Core.BaseDirectory,
                "Data/Import/Premiums/RandomEncounters.xml"
            );
            ...
            using (StreamWriter sw = new StreamWriter(importName, false))
            {
                ...
                foreach (string facet in facetEncounters.Keys)
                {
                    ...
                    Dictionary<string, List<ImportRecord>> regionEncounters = facetEncounters[
                        facet
                    ];
                    ...
                    foreach (string regionName in regionEncounters.Keys)
                    {
                        ...
                        foreach (ImportRecord ir in encounterSet)
                        {
                            ...
                            if (ir.Hostile)
                                forceFlag = " forceAttack=\"true\"";

                            sw.WriteLine(
                                "            <Encounter p=\"{0:0.000}\" level=\"{1:0.000}\" distance=\"7:11\">",
                                ir.P,
                                level
                            );
                            sw.WriteLine(
                                "                <Mobile pick=\"{0}\" n=\"1:{1}\"{2}/> <!-- x={3},y={4},z={5} -->",
                                ir.Spawn,
                                n,
                                forceFlag,
                                ir.Location.X,
