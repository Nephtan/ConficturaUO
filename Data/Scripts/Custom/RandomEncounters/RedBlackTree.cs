    ...
    public class RedBlackTree : ICollection, IEnumerable
    {
                ...
                try
                {
                    if (root == null)
                        throw new InvalidOperationException(
                            "Unable to return Max because the RedBlackTree is empty."
                        );
        ...
        public object Min
        {
            get
            {
                ...
                try
                {
                    if (root == null)
                        throw new InvalidOperationException(
                            "Unable to return Min because the RedBlackTree is empty."
                        );
        ...
        private void RemoveRedBlackTreeNode(RedBlackTreeNode node)
        {
            ...
            if (nodeY == null || !nodeY.Color)
            {
                ...
                {
                    if (
                        nodeX == fatherX.Left /*&& nodeX != fatherX.Right*/
                    )
                    {
                        fatherY = fatherX;
                        nodeY = fatherX.Right;
        ...
        private int RecConform(RedBlackTreeNode node)
        {
            ...
            if (a != b)
                throw new ArgumentException(
                    "RedBlackTree : the paths do have not the  same number of black nodes."
                );
    ...
    public class RedBlackTreeEnumerator : IEnumerator
    {
        ...
        public object Current
        {
            get
            {
                if (!isValid)
                    throw new InvalidOperationException(
                        "The RedBlackTree was modified after the enumerator was created"
                    );
                if (!started)
                    throw new InvalidOperationException("Before first element");
                if (current == null)
                    throw new InvalidOperationException("After last element");
                return current.Key;
            }
        }
