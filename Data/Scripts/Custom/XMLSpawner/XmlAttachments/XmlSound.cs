...
namespace Server.Engines.XmlSpawner2
{
    public class XmlSound : XmlAttachment
    {
        ...
        public override string OnIdentify(Mobile from)
        {
            ...
            {
                msg = String.Format(
                    "Sound #{0} : {1} secs between uses - {2} charges left",
                    SoundValue,
        ...
        public override void OnTrigger(object activator, Mobile m)
        {
            ...
            if (AttachedTo is Mobile)
            {
                ...
                {
                    Effects.PlaySound(
                        ((Mobile)AttachedTo).Location,
                        ((IEntity)AttachedTo).Map,
                        SoundValue
                    );
                }
                catch { }
            ...
            else if (AttachedTo is Item)
            {
                ...
                else if (i.RootParent is IEntity)
                {
                    ...
                    {
                        Effects.PlaySound(
                            ((IEntity)i.RootParent).Location,
                            ((IEntity)i.RootParent).Map,
                            SoundValue
                        );
                    }
                    catch { }
