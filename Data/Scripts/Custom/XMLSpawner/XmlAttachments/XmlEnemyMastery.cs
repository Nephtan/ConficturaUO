    ...
    public class XmlEnemyMastery : XmlAttachment
    {
        ...
        )
        {
            ...
            if (defender != null && attacker != null && m_EnemyType != null)
            {
                ...
                if (
                    defender.GetType() == m_EnemyType
                    || defender.GetType().IsSubclassOf(m_EnemyType)
                )
                {
        ...
        public override void OnDelete()
        {
            ...
            if (AttachedTo is Mobile)
            {
                ...
                if (!m.Deleted)
                {
                    ...
                    m.SendMessage(
                        String.Format("Your power of Enemy Mastery over {0} fades..", Enemy)
                    );
                }
            }
        }
        ...
        public override string OnIdentify(Mobile from)
        {
            ...
            {
                msg = String.Format(
                    "Enemy Mastery : +{3}% damage vs {0}, {1}%, hitchance expires in {2} mins",
                    m_Enemy,
                    Chance,
            ...
            {
                msg = String.Format(
                    "Enemy Mastery : +{2}% damage vs {0}, {1}% hitchance",
                    m_Enemy,
                    Chance,
                    PercentIncrease
                );
