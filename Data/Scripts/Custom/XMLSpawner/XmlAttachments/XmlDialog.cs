    ...
    public class XmlDialog : XmlAttachment
    {
        ...
        private bool ValidSpeechTrig(Mobile m)
        {
            ...
            return (
                (
                    (m is PlayerMobile && (m.AccessLevel <= TriggerAccessLevel))
                    || (allownpctrigger && !(m is PlayerMobile))
                )
                && (
                    (!m.Body.IsGhost && !m_AllowGhostTriggering)
                    || (m.Body.IsGhost && m_AllowGhostTriggering)
        ...
        private SpeechEntry FindMatchingKeyword(Mobile from, string keyword, int currententryid)
        {
            ...
            foreach (SpeechEntry s in m_SpeechEntries)
            {
                ...
                if (
                    ((CheckDependsOn(s, -1) || CheckDependsOn(s, -2)) && !IsActive)
                        ...
                        for (int i = 0; i < arglist.Length; i++)
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ReportError(
                                        from,
                                        String.Format("Bad regular expression: {0} ", status_str)
                                    );
                                }
                            }
                        }
        ...
        private void ExecuteGump(Mobile mob, string gumpstring)
        {
            ...
            if (typeName == "GUMP")
            {
                BaseXmlSpawner.SpawnTypeKeyword(
                    ...
                    loc,
                    map,
                    new XmlGumpCallback(DialogGumpCallback),
                    out status_str
                );
        ...
        private void ReportError(Mobile mob, string status_str)
        {
            ...
            )
            {
                mob.SendMessage(
                    33,
                    String.Format("{0}:{1}", AttachedTo.GetType().Name, status_str)
                );
            }
        }
        ...
        public override void OnSpeech(SpeechEventArgs e)
        {
            ...
            if (
                ...
                && m_Running
                && m_ProximityRange >= 0
                && ValidSpeechTrig(e.Mobile)
                && ((e.Mobile == m_ActivePlayer) || !lockconversation || m_ActivePlayer == null)
            )
            {
            ...
            if (matchentry != null)
            {
                ...
                if (text != null)
                {
                    ...
                    if (AttachedTo is Item)
                    {
                        ...
                        ((Item)AttachedTo).PublicOverheadMessage(
                            MessageType.Regular,
                            speechhue,
                            true,
                            text
                    ...
                    else if (AttachedTo is Mobile)
                    {
                        ...
                        ((Mobile)AttachedTo).DoSpeech(
                            text,
                            new int[] { },
                            CurrentEntry.SpeechStyle,
                            speechhue
                        );
        ...
        public bool ProcessSpeech(Mobile m, string speech)
        {
            ...
            if (
                (speech != null || CheckDependsOn(matchentry, -2))
                && TriggerOnCarried != null
                && TriggerOnCarried.Length > 0
            )
            {
                bool found =
                    BaseXmlSpawner.CheckForCarried(m, TriggerOnCarried) || matchentry.IgnoreCarried;
            ...
            if (
                (speech != null || CheckDependsOn(matchentry, -2))
                && NoTriggerOnCarried != null
                && NoTriggerOnCarried.Length > 0
            )
            {
                bool notfound =
                    BaseXmlSpawner.CheckForNotCarried(m, NoTriggerOnCarried)
                    || matchentry.IgnoreCarried;
            ...
            m_HoldProcessing = true;
            Timer.DelayCall(
                TimeSpan.FromSeconds(prepause),
                new TimerStateCallback(DelayedSpeech),
                new object[] { matchentry, m }
            );
            ...
            if (System.IO.File.Exists(dirname) == true)
            {
                ...
                if (ds.Tables != null && ds.Tables.Count > 0)
                {
                    ...
                    if (ds.Tables[NPCPointName] != null && ds.Tables[NPCPointName].Rows.Count > 0)
                    {
                        ...
                        try
                        {
                            this.ResetTime = TimeSpan.FromMinutes(
                                double.Parse((string)dr["ResetTime"])
                            );
                        }
                        catch { }
                        try
                    ...
                    if (ds.Tables[NPCPointName] != null && ds.Tables[NPCPointName].Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow dr in ds.Tables[SpeechPointName].Rows)
                        {
                            ...
                            try
                            {
                                s.SpeechStyle = (MessageType)
                                    Enum.Parse(typeof(MessageType), (string)dr["SpeechStyle"]);
                            }
                            catch { }
                            try
                            {
        ...
        public new static void Initialize()
        {
            ...
            CommandSystem.Register(
                ...
                new CommandEventHandler(SaveNPC_OnCommand)
            );
            CommandSystem.Register(
                "LoadNPC",
                AccessLevel.Administrator,
                new CommandEventHandler(LoadNPC_OnCommand)
            );
        }
