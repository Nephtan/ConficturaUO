...
namespace Server.Engines.XmlSpawner2
{
    public class XmlMorph : XmlAttachment
    {
        ...
        public override string OnIdentify(Mobile from)
        {
            ...
            {
                msg = String.Format(
                    "Morph to {0} duration {1} mins",
                    m_MorphID,
                    m_Duration.TotalMinutes
                );
            }
        ...
        private class MorphTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                if (
                    m_Attachment != null
                    && !m_Attachment.Deleted
                    && m_Attachment.AttachedTo is Item
                    && !((Item)m_Attachment.AttachedTo).Deleted
                )
                {
