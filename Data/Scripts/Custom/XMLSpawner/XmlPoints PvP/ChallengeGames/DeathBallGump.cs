...
namespace Server.Gumps
{
    public class DeathBallGump : Gump
    {
        ...
        private DeathBallGauntlet m_gauntlet;
            ...
            : base(20, 30)
        {
            ...
            AddLabel(100, 10, 0, XmlPoints.GetText(from, 200570)); // "Deathball Challenge"
            AddLabel(
                20,
                30,
                0,
            ...
            if (gauntlet.Participants != null)
            {
                ...
                {
                    DeathBallGauntlet.ChallengeEntry entry = (DeathBallGauntlet.ChallengeEntry)
                        m_WorkingList[i];
            ...
            switch (info.ButtonID)
            {
                ...
                case 300:
                    ...
                    else
                    {
                        state.Mobile.SendGump(
                            new DeathBallGump(m_gauntlet, state.Mobile, viewpage)
                        );
                    }
                ...
                default:
                {
                    ...
                    if (info.ButtonID >= 4000)
                    {
                        ...
                        {
                            DeathBallGauntlet.ChallengeEntry entry =
                                (DeathBallGauntlet.ChallengeEntry)m_WorkingList[selection];
                            ...
                            if (m_gauntlet.Participants != null)
                            {
                                ...
                                foreach (
                                    DeathBallGauntlet.ChallengeEntry masterentry in m_gauntlet.Participants
                                )
                                {
                    ...
                    if (entry.Participant != null && entry.Status != ChallengeStatus.Forfeit)
                    {
                        if (forced || entry.Participant.HasGump(typeof(DeathBallGump)))
                        {
                            entry.Participant.SendGump(
                                new DeathBallGump(gauntlet, entry.Participant)
                            );
                        }
                    }
                }
