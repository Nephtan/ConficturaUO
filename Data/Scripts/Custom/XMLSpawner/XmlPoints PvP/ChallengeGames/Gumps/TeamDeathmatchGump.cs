            ...
            : base(20, 30)
        {
            ...
            if (gauntlet.Participants != null)
            {
                ...
                if (gauntlet.MatchLength > TimeSpan.Zero)
                    AddLabel(
                        ...
                        110,
                        0,
                        String.Format(XmlPoints.GetText(from, 200563), gauntlet.MatchLength)
                    ); // "Match Length: {0}"
                else
                ...
                {
                    TeamDeathmatchGauntlet.ChallengeEntry entry =
                        (TeamDeathmatchGauntlet.ChallengeEntry)m_WorkingList[i];
                ...
                if (gauntlet.GameInProgress && gauntlet.MatchLength > TimeSpan.Zero)
                {
                    AddLabelCropped(
                        ...
                        0,
                        String.Format(
                            XmlPoints.GetText(from, 200565), // "Time left {0}"
                            TimeSpan.FromSeconds(
                                (double)(
                                    (int)(
                                        (
                                            gauntlet.MatchStart
                                            + gauntlet.MatchLength
                                            - DateTime.Now
                                        ).TotalSeconds
                                    )
                                )
            ...
            if (from == gauntlet.Challenger && !m_gauntlet.GameLocked)
            {
                ...
                AddTextEntry(
                    ...
                    0,
                    40,
                    m_gauntlet.MatchLength.TotalMinutes.ToString()
                );
            ...
            switch (info.ButtonID)
            {
                case 1:
                    ...
                    state.Mobile.SendGump(
                        new TeamDeathmatchGump(m_gauntlet, state.Mobile, viewpage)
                    );
                    break;
                ...
                default:
                {
                    ...
                    if (info.ButtonID >= 4000)
                    {
                        ...
                        {
                            TeamDeathmatchGauntlet.ChallengeEntry entry =
                                (TeamDeathmatchGauntlet.ChallengeEntry)m_WorkingList[selection];
                            ...
                            if (m_gauntlet.Participants != null)
                            {
                                ...
                                foreach (
                                    TeamDeathmatchGauntlet.ChallengeEntry masterentry in m_gauntlet.Participants
                                )
                                {
                    ...
                    if (entry.Participant != null)
                    {
                        if (force || entry.Participant.HasGump(typeof(TeamDeathmatchGump)))
                        {
                            entry.Participant.SendGump(
                                new TeamDeathmatchGump(gauntlet, entry.Participant)
                            );
                        }
                    }
                }
            ...
            if (gauntlet.Challenger != null)
            {
                if (force || gauntlet.Challenger.HasGump(typeof(TeamDeathmatchGump)))
                {
                    gauntlet.Challenger.SendGump(
                        new TeamDeathmatchGump(gauntlet, gauntlet.Challenger)
                    );
                }
            }
        }
        ...
        private class MemberTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is Mobile && ((Mobile)targeted).Player)
                {
                    ...
                    {
                        foreach (
                            TeamDeathmatchGauntlet.ChallengeEntry entry in m_gauntlet.Participants
                        )
                        {
