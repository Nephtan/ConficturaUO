...
namespace Server.Engines.XmlSpawner2
{
    ...
    public abstract class BaseChallengeEntry : IChallengeEntry
    {
        ...
        {
            get { return m_LastCaution; }
            set { m_LastCaution = value; }
        }
        public virtual int PageBeingViewed
        {
            get { return m_PageBeingViewed; }
            set { m_PageBeingViewed = value; }
        }
    ...
    public abstract class BaseChallengeGame : Item, IChallengeGame
    {
        ...
        {
            get { return m_IsInChallengeGameRegion; }
            set { m_IsInChallengeGameRegion = value; }
        }
        ...
        public static void DoSetupChallenge(Mobile from, int nameindex, Type gametype)
        {
            if (from != null && gametype != null)
            {
                ...
                if (r is ChallengeGameRegion)
                {
                    ...
                    if (
                        cgr.ChallengeGame != null
                        && !cgr.ChallengeGame.Deleted
                        && !cgr.ChallengeGame.GameCompleted
                        && !cgr.ChallengeGame.IsOrganizer(from)
                    )
                    {
                        from.SendMessage(
                            String.Format(
                                XmlPoints.GetText(from, 100303),
                                XmlPoints.GetText(from, nameindex)
                            )
                ...
                if (r is ChallengeGameRegion)
                {
                    ...
                    XmlPoints.BroadcastMessage(
                        AccessLevel.Player,
                        0x482,
                        String.Format(
                            XmlPoints.SystemText(100307),
                            XmlPoints.SystemText(nameindex),
                            r.Name,
                            from.Name
                        )
                ...
                {
                    if (
                        afrom.ChallengeSetup != null
                        && !(
                            afrom.ChallengeSetup.GameInProgress
                            || afrom.ChallengeSetup.GameCompleted
                        )
                    )
                    {
            ...
            if (Participants != null)
            {
                foreach (IChallengeEntry entry in Participants)
                {
                    if (entry.Participant != null)
                    {
                        ...
                        XmlPoints afrom = (XmlPoints)
                            XmlAttach.FindAttachment(entry.Participant, typeof(XmlPoints));

                        // update the points gumps on the players if they are open
                        if (
                            afrom != null
                            && entry.Participant.HasGump(typeof(XmlPoints.PointsGump))
                        )
                        {
                ...
                foreach (IChallengeEntry entry in Participants)
                {
                    if (entry.Participant != from)
                    {
                        from.Send(
                            new MobileMoving(
                                entry.Participant,
                                Notoriety.Compute(from, entry.Participant)
                            )
                        );
                    }
                }
            ...
            if (Participants != null)
            {
                foreach (IChallengeEntry entry in Participants)
                {
                    if (entry.Participant != from)
                    {
                        ...
                        entry.Participant.Send(
                            new MobileMoving(from, Notoriety.Compute(entry.Participant, from))
                        );
                    }
                }
            }
