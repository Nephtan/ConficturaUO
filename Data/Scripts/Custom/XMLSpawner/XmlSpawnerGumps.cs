...
namespace Server.Mobiles
{
    ...
    public class TextEntryGump : Gump
    {
        ...
        public TextEntryGump(
            XmlSpawner spawner,
            XmlSpawnerGump spawnergump,
            int index,
            int X,
            int Y
        )
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (edit_entry)
            {
                ...
                if (origtext != null && oldtext != null && newtext != null)
                {
                    try
                    {
                        ...
                        if (firstindex >= 0)
                        {
                            ...
                            {
                                editedtext =
                                    origtext.Substring(0, firstindex)
                                    + newtext
                            ...
                            else
                            {
                                ...
                                m_Spawner.SpawnObjects = m_SpawnerGump.CreateArray(
                                    info,
                                    state.Mobile
                                );
                            }
                ...
                state.Mobile.SendGump(
                    new TextEntryGump(m_Spawner, m_SpawnerGump, m_index, this.X, this.Y)
                );
                return;
            }
            if (update_entry)
            ...
            state.Mobile.SendGump(
                new XmlSpawnerGump(
                    ...
                    this.X,
                    this.Y,
                    m_SpawnerGump.m_ShowGump,
                    m_SpawnerGump.xoffset,
                    m_SpawnerGump.page
                )
            );
        }
    ...
    public class XmlSpawnerGump : Gump
    {
            ...
            : base(X, Y)
        {
            ...
            if (
                spawner.SequentialSpawn >= 0
                || spawner.HasSubGroups()
                || spawner.HasIndividualSpawnTimes()
            )
            {
        ...
        */
            ...
            {
                if (page != (int)(i / MaxEntriesPerPage))
                    continue;
                ...
                if (i < m_Spawner.SpawnObjects.Length)
                {
                    ...
                    if (m_ShowGump > 1)
                    {
                        ...
                        if (
                            ...
                            && m_Spawner.SpawnObjects[i].SubGroup > 0
                        )
                        {
                        ...
                        if (m_Spawner.SpawnObjects[i].NextSpawn > DateTime.Now)
                        {
                            ...
                            if (
                                (DateTime.Now + m_Spawner.NextSpawn)
                                > m_Spawner.SpawnObjects[i].NextSpawn
                            )
                            {
                        ...
                        if (m_Spawner.SequentialSpawn >= 0)
                        {
                            ...
                            AddButton(
                                545 + xoffset + 30,
                                yoff,
                                m_Spawner.SpawnObjects[i].RestrictKillsToSubgroup ? 0xD3 : 0xD2,
                                m_Spawner.SpawnObjects[i].RestrictKillsToSubgroup ? 0xD2 : 0xD3,
                                300 + i,
                                GumpButtonType.Reply,
                                0
                            );
                            ...
                            AddButton(
                                565 + xoffset + 30,
                                yoff,
                                m_Spawner.SpawnObjects[i].ClearOnAdvance ? 0xD3 : 0xD2,
                                m_Spawner.SpawnObjects[i].ClearOnAdvance ? 0xD2 : 0xD3,
                                400 + i,
                                GumpButtonType.Reply,
                                0
                            );
                ...
                if (te != null)
                {
                    ...
                    if (str.Length > 0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                SpawnObjects.Add(
                                    new XmlSpawner.SpawnObject(from, m_Spawner, str, 0)
                                );
                            }
                            else
                                m_Spawner.status_str = String.Format(
        ...
        public static void RefreshSpawnerGumps(Mobile from)
        {
            ...
            if (ns != null && ns.Gumps != null)
            {
                ...
                foreach (XmlSpawnerGump g in refresh)
                {
                    ...
                    if (
                        g.m_Spawner != null /*&& g.m_Spawner.SpawnerGump == g */
                    )
                    {
                        // flag the current gump on the spawner as closed
                        g.m_Spawner.GumpReset = true;

                        XmlSpawnerGump xg = new XmlSpawnerGump(
                            g.m_Spawner,
                            g.X,
                            g.Y,
                            g.m_ShowGump,
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            for (int i = 0; i < m_Spawner.SpawnObjects.Length; i++)
            {
                ...
                if (m_ShowGump > 1)
                {
                    ...
                    int subgroupindex = m_Spawner.GetCurrentSequentialSpawnIndex(
                        m_Spawner.SpawnObjects[i].SubGroup
                    );
                    TextRelay tegrp;
            ...
            switch (info.ButtonID)
            {
                ...
                default:
                {
                    ...
                    if (info.ButtonID >= 800 && info.ButtonID < 800 + MaxSpawnEntries)
                    {
...
#if(BOOKTEXTENTRY)
                        // display a new gump
                        XmlSpawnerGump newgump = new XmlSpawnerGump(
                            m_Spawner,
                            this.X,
                            this.Y,
                            this.m_ShowGump,
                        ...
                        XmlTextEntryBook book = new XmlTextEntryBook(
                            0,
                            String.Empty,
                            m_Spawner.Name,
                            20,
                            true,
                            new XmlTextEntryBookCallback(ProcessSpawnerBookEntry),
                            args
                        );
                        ...
                        book.MoveToWorld(
                            new Point3D(
                                state.Mobile.Location.X,
                                state.Mobile.Location.Y,
                                state.Mobile.Location.Z - 100
                            ),
                            state.Mobile.Map
                        );
                    ...
                    if (info.ButtonID >= 1300 && info.ButtonID < 1300 + MaxSpawnEntries)
                    {
                        ...
                        if (index < m_Spawner.SpawnObjects.Length)
                        {
                            ...
                            if (scount > 0)
                            {
                                object so = m_Spawner.SpawnObjects[index].SpawnedObjects[
                                    nclicks % scount
                                ];
                    ...
                    if (
                        info.ButtonID >= 4000
                        && info.ButtonID < 4001 + (int)(MaxSpawnEntries / MaxEntriesPerPage)
                    )
                    {
                        // which page
                    ...
                    else if (info.ButtonID >= 5000 && info.ButtonID < 5000 + MaxSpawnEntries)
                    {
                        ...
                        {
                            XmlSpawnerGump newg = new XmlSpawnerGump(
                                m_Spawner,
                                this.X,
                                this.Y,
                                this.m_ShowGump,
                            ...
                            state.Mobile.SendGump(
                                new Server.Gumps.XmlCategorizedAddGump(state.Mobile, i, newg)
                            );
                        }
                        else
                        {
                            // use the XmlPartialCategorizedAddGump
                            state.Mobile.CloseGump(
                                typeof(Server.Gumps.XmlPartialCategorizedAddGump)
                            );
                            ...
                            XmlSpawnerGump.ReplacementEntry re =
                                new XmlSpawnerGump.ReplacementEntry();
                            ...
                            state.Mobile.SendGump(
                                new XmlPartialCategorizedAddGump(
                                    state.Mobile,
                                    categorystring,
                                    0,
                                    types,
