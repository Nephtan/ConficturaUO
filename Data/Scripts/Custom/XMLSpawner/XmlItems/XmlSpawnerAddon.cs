...
namespace Server.Engines.XmlSpawner2
{
    public class XmlSpawnerAddon : BaseAddon
    {
        ...
        public static int LoadAddonFromMulti(
            XmlSpawnerAddon newaddon,
            string filename,
                    ...
                    while ((line = sr.ReadLine()) != null)
                    {
                        ...
                        if (newcomponent != null && specs.Length > 1)
                        {
                            for (int j = 1; j < specs.Length; j++)
                            {
                                ...
                                try
                                {
                                    type = SpawnerType.GetType(
                                        BaseXmlSpawner.ParseObjectType(attachstring)
                                    );
                                }
                                catch { }
                                ...
                                {
                                    object newo = XmlSpawner.CreateObject(
                                        type,
                                        attachstring,
                                        false,
                                        true
