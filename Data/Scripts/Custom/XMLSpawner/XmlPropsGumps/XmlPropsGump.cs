...
namespace Server.Gumps
{
    public class XmlPropertiesGump : Gump
    {
        ...
        private static bool PrevLabel = OldStyle,
            NextLabel = OldStyle;
        ...
        private static readonly int TotalHeight =
            OffsetSize + ((EntryHeight + OffsetSize) * (EntryCount + 1));
        ...
        private void Initialize(int page)
        {
            ...
            int propcount = 0;
            for (
                int i = 0, index = page * EntryCount;
                i < count && index < m_List.Count;
                ++i, ++index
            )
            {
                ...
                */
                if (o is PropertyInfo)
                {
                    ...
                    Server.Mobiles.XmlSpawnerDefaults.DefaultEntry de =
                        new Server.Mobiles.XmlSpawnerDefaults.DefaultEntry();
        ...
        public static string[] m_PoisonNames = new string[]
        {
            "None",
            "Lesser",
            "Regular",
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                case 1: // Previous
                {
                    if (m_Page > 0)
                        from.SendGump(
                            new XmlPropertiesGump(from, m_Object, m_Stack, m_List, m_Page - 1)
                        );
                ...
                default:
                {
                    ...
                    if (index >= 0 && index < m_List.Count)
                    {
                        ...
                        if (IsType(type, typeofMobile) || IsType(type, typeofItem))
                            ...
                            from.Target = new XmlSetObjectTarget(
                                prop,
                                from,
                                m_Object,
                                m_Stack,
                            ...
                            from.SendGump(
                                new XmlSetPoint3DGump(prop, from, m_Object, m_Stack, m_Page, m_List)
                            );
                        else if (IsType(type, typeofPoint2D))
                            from.SendGump(
                                new XmlSetPoint2DGump(prop, from, m_Object, m_Stack, m_Page, m_List)
                            );
                        else if (IsType(type, typeofTimeSpan))
                            from.SendGump(
                                new XmlSetTimeSpanGump(
                                    prop,
                                    from,
                                    m_Object,
                                    m_Stack,
                            ...
                            IsType(type, typeofString)
                            || IsType(type, typeofReal)
                            || IsType(type, typeofNumeric)
                        )
                            from.SendGump(
                                new XmlSetGump(prop, from, m_Object, m_Stack, m_Page, m_List)
                            );
                        ...
                        else if (IsType(type, typeofSkills) && m_Object is Mobile)
                        {
                            ...
                            from.SendGump(
                                new XmlPropertiesGump(
                                    from,
                                    prop.GetValue(m_Object, null),
                                    m_Stack,
                                    m_Object
                                )
                            );
        ...
        private static Type[] typeofReal = new Type[] { typeof(Single), typeof(Double) };
        ...
        private class GroupComparer : IComparer
        {
            ...
            private int GetDistance(Type type)
            {
                ...
                for (
                    dist = 0;
                    current != null && current != typeofObject && current != type;
                    ++dist
                )
