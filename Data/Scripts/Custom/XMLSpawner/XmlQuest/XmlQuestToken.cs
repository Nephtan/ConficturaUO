    ...
    public abstract class XmlQuestToken : Item, IXmlQuest
    {
        ...
        public override void OnAdded(object target)
        {
            ...
            if ((target != null) && target is Container)
            {
                ...
                if (
                    (parentOfTarget != null)
                    && parentOfTarget is Container
                    && target is XmlQuestBook
                )
                {
        ...
        {
            if (
                (m_Pack == null || m_Pack.Deleted) && (Owner != null) /* && (this.RootParent is Mobile) */
            )
            {
            ...
            if (m_RewardItem != null && !m_RewardItem.Deleted && PlayerMade)
            {
                if (m_RewardItem is Container)
                {
                    UpdateTotal(
                        this,
                        TotalType.Weight,
                        m_RewardItem.TotalWeight + (int)m_RewardItem.Weight
                    );
                ...
                else
                {
                    UpdateTotal(
                        this,
                        TotalType.Weight,
                        (int)(m_RewardItem.Weight * m_RewardItem.Amount)
                    );
                    UpdateTotal(this, TotalType.Gold, m_RewardItem.TotalGold);
                }
            }
        ...
        */
        ...
        public XmlAttachment RewardAttachment
        {
            get
            {
                ...
                {
                    object o = XmlQuest.CreateItem(
                        this,
                        m_AttachmentString,
                        out m_status_str,
                        typeof(XmlAttachment)
                    );
                    if (o is Item)
                    {
        ...
        public Item RewardItem
        {
            get
            {
                ...
                // dont allow player made quests to use the rewardstring creation feature
                if (
                    (m_RewardItem == null || m_RewardItem.Deleted)
                    && (m_RewardString != null)
                    && !PlayerMade
                )
                {
                ...
                if (
                    (m_RewardItem != null && !m_RewardItem.Deleted)
                    && ((Pack == null) || Pack.Deleted || (m_RewardItem.Parent != Pack))
                )
                {
        ...
        public TimeSpan ExpiresIn
        {
            get
            {
                if (m_ExpirationDuration > 0)
                {
                     ...
                     */
                    return (
                        m_TimeCreated + TimeSpan.FromHours(m_ExpirationDuration) - DateTime.Now
                    );
                }
                else
                {
        ...
        public void CheckAutoReward()
        {
            ...
            )
            {
                ...
                Owner.SendMessage(
                    String.Format("{0} completed. You receive the quest reward!", Name)
                );
                this.Delete();
            }
        }
