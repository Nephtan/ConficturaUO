    ...
    public abstract class XmlQuest
    {
        ...
        public static void RemoveTemporaryQuestObjects(Mobile questowner, string questname)
        {
            ...
            foreach (XmlAttachment i in XmlAttach.Values)
            {
                ...
                if (
                    i != null
                    && !i.Deleted
                    && i is ITemporaryQuestAttachment
                    && ((ITemporaryQuestAttachment)i).QuestOwner == questowner
                    && i.Name == questname
                )
                {
                    list.Add(i);
        ...
        private static void ReturnCollected(IXmlQuest quest, Item item)
        {
            ...
            if (
                ...
                && quest.PlayerMade
                && (quest.Creator != null)
                && !quest.Creator.Deleted
                && (quest.Creator != quest.Owner)
            )
            {
                bool returned = false;
                ...
                quest.Creator.SendMessage(
                    "You receive {0} from quest {1}",
                    item.GetType().Name,
                    quest.Name
                );
            }
        ...
        private static void TakeGiven(Mobile to, IXmlQuest quest, Item item)
        {
            ...
            XmlSaveItem si = (XmlSaveItem)
                XmlAttach.FindAttachment(to, typeof(XmlSaveItem), "Given");
        ...
        )
        {
            ...
            else
            {
                ...
                if (
                    ...
                    && type != null
                    && type != typerestrict
                    && !type.IsSubclassOf(typerestrict)
                )
                {
                    return null;
                }
        ...
        public static void CheckArgList(
            string[] arglist,
            int argstart,
            object propobj,
            out string typestr,
        ...
        )
        {
            ...
            if (arglist.Length > argstart)
            {
                ...
                for (int i = argstart; i < arglist.Length; i++)
                {
                    ...
                    {
                        // its a prop arg
                        checkprop = BaseXmlSpawner.CheckPropertyString(
                            null,
                            propobj,
                            arglist[i],
                            null,
            ...
            if (target != null && !target.Deleted)
            {
                ...
                if (quest.PartyEnabled && p != null)
                {
                    ...
                    foreach (PartyMemberInfo mi in randlist)
                    {
                        ...
                        if (
                            quest.PartyRange < 0
                            || Utility.InRange(m.Location, member.Location, quest.PartyRange)
                        )
                        {
                            // find the quest item in their packs
                            Item questitem = BaseXmlSpawner.SearchMobileForItem(
                                member,
                                quest.Name,
                                "IXmlQuest",
                                false
        ...
        public static bool CheckCollectObjective(
            IXmlQuest quest,
            Item item,
            string objectivestr,
            string statestr,
            out string newstatestr,
            out bool collectstatus
        )
        {
            ...
            if (arglist.Length > 1)
            {
                ...
                if (arglist[0] == "COLLECT")
                {
                    ...
                    if (
                        item != null
                        && !item.Deleted
                        && BaseXmlSpawner.CheckType(item, arglist[1]) /*(item.GetType() == targettype)*/
                        && checkprop
                    )
                    {
                        // found a match
            ...
            if (target != null && !target.Deleted)
            {
                ...
                if (quest.PartyEnabled && p != null)
                {
                    ...
                    foreach (PartyMemberInfo mi in randlist)
                    {
                        ...
                        if (
                            quest.PartyRange < 0
                            || Utility.InRange(from.Location, member.Location, quest.PartyRange)
                        )
                        {
                            // find the quest item in their packs
                            Item questitem = BaseXmlSpawner.SearchMobileForItem(
        ...
        public static bool CheckGiveObjective(
            IXmlQuest quest,
            Mobile mob,
            Item item,
            string objectivestr,
            ...
            if (arglist.Length > 2)
            {
                ...
                if (arglist[0] == "GIVE")
                {
                    ...
                    if (
                        item != null
                        && !item.Deleted
                        && BaseXmlSpawner.CheckType(item, arglist[2]) /*(item.GetType() == targettype)*/
                        && checkprop
                    )
                    {
                        // found a match
        ...
        public static bool CheckKillObjective(
            IXmlQuest quest,
            Mobile m_killed,
            Mobile m_killer,
            string objectivestr,
        ...
        public static void RegisterKill(Mobile m_killed, Mobile m_killer)
        {
            ...
            if (p != null)
            {
                foreach (PartyMemberInfo mi in p.Members)
                {
                    ...
                    if (
                        member != null
                        && member is PlayerMobile
                        && ((PlayerMobile)member).GetFlag(CarriedXmlQuestFlag)
                    )
                    {
            ...
            else
            {
                if (
                    m_killer != null
                    && m_killer is PlayerMobile
                    && ((PlayerMobile)m_killer).GetFlag(CarriedXmlQuestFlag)
                )
                {
        ...
        public static bool CheckEscortObjective(
            IXmlQuest quest,
            Mobile m_escorted,
            Mobile m_escorter,
            string objectivestr,
        ...
        )
        {
            ...
            if (arglist.Length > 1)
            {
                ...
                if (
                    ...
                    && !m_escorted.Deleted
                    && (
                        arglist[1] == m_escorted.Name
                        || (arglist[1] == null || arglist[1] == String.Empty)
                    )
                    && checkprop
                )
                {
        ...
        public static bool CheckVisitObjective(
            IXmlQuest quest,
            PlayerMobile m_player,
            string objectivestr,
            string statestr,
            out string newstatestr,
            out bool visitstatus
        )
        {
            newstatestr = statestr;
            visitstatus = false;
