        ...
        public static DefaultEntry GetDefaults(string account, string name)
        {
            ...
            if (DefaultEntryList != null)
            {
                for (int i = 0; i < DefaultEntryList.Count; i++)
                {
                    ...
                    if (
                        entry != null
                        && string.Compare(entry.PlayerName, name, true) == 0
    ...
    public class XmlAddGump : Gump
    {
        ...
        private void DoLoadDefs(Mobile from, string filename)
        {
            ...
            if (System.IO.File.Exists(dirname) == true)
            {
                ...
                if (ds.Tables != null && ds.Tables.Count > 0)
                {
                    ...
                    if (
                        ds.Tables[DefsTablePointName] != null
                        && ds.Tables[DefsTablePointName].Rows.Count > 0
                    )
                    {
        ...
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(XmlAdd_OnCommand)
            );
        }
        ...
        public static void XmlAdd_OnCommand(CommandEventArgs e)
        {
            ...
            e.Mobile.SendGump(
                new XmlAddGump(e.Mobile, e.Mobile.Location, e.Mobile.Map, true, false, x, y)
            );
        }
        ...
        private class PlaceSpawnerTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                {
                    if (
                        defs.SelectionList != null
                        && i < defs.SelectionList.Length
                        && defs.SelectionList[i]
                        && defs.NameList != null
                        && i < defs.NameList.Length
                        && defs.NameList[i] != null
                        && defs.NameList[i].Length > 0
                    )
                    {
                        maxcount++;
                    }
                ...
                XmlSpawner spawner = new XmlSpawner(
                    ...
                    defs.Team,
                    defs.HomeRange,
                    defs.HomeRangeIsRelative,
                    new XmlSpawner.SpawnObject[0],
                    defs.RefractMin,
                    defs.RefractMax,
                    defs.TODStart,
                    defs.TODEnd,
                    null,
                    defs.TriggerObjectProp,
                    defs.ProximityMsg,
                    defs.TriggerOnCarried,
                    defs.NoTriggerOnCarried,
                    defs.SpeechTrigger,
                    null,
                    null,
                    defs.PlayerTriggerProp,
                    defs.TriggerProbability,
                    null,
                    defs.Group,
                    defs.TODMode,
                    defs.KillReset,
                    defs.ExternalTriggering,
                    defs.SequentialSpawn,
                    null,
                    defs.AllowGhostTrig,
                    defs.AllowNPCTrig,
                    defs.SpawnOnTrigger,
            ...
            switch (info.ButtonID)
            {
                ...
                case 156: // Delete last spawner
                {
                    ...
                    state.Mobile.SendGump(
                        new XmlAddConfirmDeleteGump(state.Mobile, defs.LastSpawner)
                    );
                    return;
                }
                ...
                default:
                {
                    ...
                    if (info.ButtonID >= 5000 && info.ButtonID < 5000 + MaxEntries)
                    {
                        ...
                        */
                        ...
                        XmlAddGump newg = new XmlAddGump(
                            state.Mobile,
                            defs.StartingLoc,
                            defs.StartingMap,
                            false,
                        ...
                        if (defs.NameList[i] == null || defs.NameList[i].Length == 0)
                        {
                            ...
                            state.Mobile.SendGump(
                                new Server.Gumps.XmlCategorizedAddGump(
                                    state.Mobile,
                                    defs.CurrentCategory,
                                    defs.CurrentCategoryPage,
                                    i,
                                    newg
                                )
                            );
