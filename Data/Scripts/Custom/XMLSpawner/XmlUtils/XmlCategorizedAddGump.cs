...
namespace Server.Gumps
{
    ...
    public class XmlAddCAGObject : XmlAddCAGNode
    {
        ...
        {
            get { return (m_Type == null ? "bad type" : m_Type.Name); }
        }
        ...
        public override void OnClick(Mobile from, int page, int index, Gump gump)
        {
            ...
            else
            {
                if (gump is XmlAddGump)
                {
                    ...
                    if (
                        xmladdgump != null
                        && xmladdgump.defs != null
                        && xmladdgump.defs.NameList != null
                        && index >= 0
                        && index < xmladdgump.defs.NameList.Length
                    )
                    {
                    ...
                    from.SendGump(
                        new XmlCategorizedAddGump(from, m_Parent, page, index, xmladdgump)
                    );
                }
    ...
    public class XmlCategorizedAddGump : Gump
    {
        ...
        public static readonly int SetOffsetX = PropsConfig.SetOffsetX,
            SetOffsetY =
                PropsConfig.SetOffsetY /*+ (((EntryHeight - 20) / 2) / 2)*/
        ;
        ...
        public static readonly int PrevOffsetX = PropsConfig.PrevOffsetX,
            PrevOffsetY =
                PropsConfig.PrevOffsetY /*+ (((EntryHeight - 20) / 2) / 2)*/
        ;
        ...
        public static readonly int NextOffsetX = PropsConfig.NextOffsetX,
            NextOffsetY =
                PropsConfig.NextOffsetY /*+ (((EntryHeight - 20) / 2) / 2)*/
        ;
        ...
        private static readonly int TotalWidth =
            OffsetSize + EntryWidth + OffsetSize + SetWidth + OffsetSize;
        ...
        public XmlCategorizedAddGump(
            Mobile owner,
            XmlAddCAGCategory category,
            int page,
            int index,
            Gump gump
        )
        ...
        public void Initialize(int page)
        {
            ...
            AddHtml(
                ...
                EntryHeight,
                String.Format("<center>{0}</center>", m_Category.Caption),
                false,
                false
            );
            ...
            for (
                ...
                i < EntryCount && index < nodes.Length;
                ++i, ++index
            )
            {
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                case 2: // Previous
                {
                    if (m_Page > 0)
                        from.SendGump(
                            new XmlCategorizedAddGump(from, m_Category, m_Page - 1, m_Index, m_Gump)
                        );
