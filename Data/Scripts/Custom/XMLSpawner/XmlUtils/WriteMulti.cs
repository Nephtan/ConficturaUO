    ...
    public class WriteMulti
    {
        ...
        )
        {
            ...
            BoundingBoxPicker.Begin(
                m,
                new BoundingBoxCallback(DefineMultiArea_Callback),
                multiargs
            );
        }
            ...
            if (from != null && multiargs != null && map != null)
            {
                ...
                if (includeitems)
                {
                    ...
                    IPooledEnumerable eable = map.GetItemsInBounds(
                        new Rectangle2D(sx, sy, ex - sx + 1, ey - sy + 1)
                    );
                    ...
                    foreach (Item item in eable)
                    {
                        ...
                        if (
                            ...
                            && (
                                zmin == int.MinValue
                                || (item.Location.Z >= zmin && item.Location.Z <= zmax)
                            )
                        )
                        {
                            // add the item
                        ...
                        for (int y = sy; y < ey; y++)
                        {
                            ...
                            for (int j = 0; j < statics.Length; j++)
                            {
                                if (
                                    (
                                        zmin == int.MinValue
                                        || (statics[j].Z >= zmin && statics[j].Z <= zmax)
                                    )
                                )
                                {
                                    staticlist.Add(
                                        new TileEntry(statics[j].ID & 0x3FFF, x, y, statics[j].Z)
                                    );
                                }
                            }
                        }
