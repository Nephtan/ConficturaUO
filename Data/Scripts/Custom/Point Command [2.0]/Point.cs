...
namespace Server.Commands
{
    public class Point
    {
        ...
        public static void Initialize()
        {
            CommandSystem.Register(
                "Point",
                AccessLevel.Player,
                new CommandEventHandler(Point_OnCommand)
            );
        }
    ...
    public class PointTarget : Target
    {
        ...
        public PointTarget(Mobile from, Item targeted)
            : base(-1, true, TargetFlags.None)
        {
            from.PublicOverheadMessage(
                ...
                20,
                true,
                from.Name.ToString() + " Points There"
            );
            targeted.PublicOverheadMessage(
                MessageType.Emote,
                20,
                true,
                from.Name.ToString() + " Points Here"
            );
        }
    ...
    public class PointTimer : Timer
    {
        ...
        public PointTimer(Item m)
            : base(TimeSpan.FromSeconds(5.0)) // Change the Message delay here for static tiles
        {
            m_item = m;
            Start();
        }
