    ...
    public class CharacterControlGump : Gump
    {
        public static void Initialize()
        {
            CommandSystem.Register(
                ...
                new CommandEventHandler(CharControl_OnCommand)
            );
        }
        ...
        public CharacterControlGump(Account first, Account second, string ErrorMessage, SwapInfo s)
            : base(50, 50)
        {
            Closable = true;
            Disposable = false;
                    ...
                    for (int i = 0; i < 6; i++) //6 because of 6th char slot and we can handle nulls & out of bounds fine
                    {
                        ...
                        if (!(InSwapMode && swap.AlreadyChose(first)))
                            AddButton(
                                ...
                                0
                            ); //The Swap Select button

                        AddLabel(
                            x,
                            y,
                            1152,
                            String.Format("{0} (0x{1:X})", m.Name, m.Serial.Value)
                        );
                ...
                else
                {
                    ...
                    switch (button)
                    {
                        ...
                        case 5: //Swap Selection And/Or Props
                        {
                            if (InSwapMode)
                            {
                                if (!swap.AlreadyChose(acct) && !swap.AlreadyChose(secondAcct))
                                {
                                    ...
                                    error =
                                        "Please choose a character from the other acct to swap with";
                                }
                                else if (
                                    (swap.AlreadyChose(swap.a1) && !swap.AlreadyChose(swap.a2))
                                ...
                                )
                                {
                                    ...
                                    if (swap.SwapEm())
                                    {
                                        error = String.Format(
                                            ...
                                            swap.a1[swap.a1CharIndex],
                                            ((Mobile)swap.a1[swap.a1CharIndex]).Serial.Value,
                                            swap.a2[swap.a2CharIndex],
                                            ((Mobile)swap.a2[swap.a2CharIndex]).Serial.Value,
                                            swap.a2.ToString(),
                                            swap.a1.ToString()
                                        );
                        ...
                        case 7: //Del
                        {
                            ...
                            m.SendGump(
                                new WarningGump(
                                    ...
                                    360,
                                    260,
                                    new WarningGumpCallback(CharacterDelete_Callback),
                                    o
                                )
                            );
                        ...
                        case 8: //Move
                        {
                            ...
                            {
                                error = String.Format(
                                    "Can't move Mobile {0} (0x{1:X} because the other account is null",
                                    mob.Name,
                                    mob.Serial.Value
                                );
                                break;
                            ...
                            error = String.Format(
                                "Mobile {0} (0x{1:X}) of Account {2} moved to Account {3}.",
                                mob.Name,
                                mob.Serial.Value,
                                acct.ToString(),
                                secondAcct.ToString()
        ...
        protected void CharacterDelete_Callback(Mobile from, bool okay, object state)
        {
            ...
            error = String.Format(
                "Mobile {0} (0x{1:X}) of Acct {2} {3} Deleted.",
                mob.Name,
                mob.Serial.Value,
                acct.ToString(),
                okay ? "" : "not"
