...
{
    public class AleKeg : Item, ICraftable
    {
            ...
            if (AllowBottling)
            {
                if (from.InRange(GetWorldLocation(), 2))
                {
                    if (m_Held > 0)
                    {
                        ...
                        if (pack != null && pack.ConsumeTotal(typeof(EmptyAleBottle), 1))
                        {
                            from.SendLocalizedMessage(502242); // You pour some of the keg's contents into an empty bottle...

                            BaseCraftAle ale = FillBottle();
                            ale.Crafter = m_Crafter;
                            ale.Quality = m_Quality;
                            ale.Variety = m_Variety;
                            if (this.Name != null)
                                ale.Name = this.Name;

                            if (pack.TryDropItem(from, ale, false))
                            {
        ...
        public BaseCraftAle FillBottle()
        {
            switch (m_Variety)
            {
                default:
