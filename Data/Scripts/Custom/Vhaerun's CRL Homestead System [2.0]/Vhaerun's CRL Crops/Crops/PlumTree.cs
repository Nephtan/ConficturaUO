...
{
    public class PlumTreeSeed : BaseCrop
    {
    ...
    public class PlumSapling : BaseCrop
    {
        ...
        {
            Movable = false;
            Name = "Plum tree sapling";

            init(this);
        }

        public static void init(PlumSapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(PlumTree), delay);
            plant.thisTimer.Start();
        }

        public PlumSapling(Serial serial)
            : base(serial) { }
    ...
    public class PlumTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public PlumTree(Point3D pnt, Map map)
            : base(0x26F1) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
        ...
        {
            private PlumTree i_plant;

            public FruitTimer(PlumTree plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        Plum fruit = new Plum((int)Utility.Random(13) + m_yield);
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(PlumTree));
        ...
        {
            private PlumTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(PlumTree Tree, FruitTreeStump Stump, Mobile from)
