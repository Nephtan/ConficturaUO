...
{
    public class CranberryTreeSeed : BaseCrop
    {
        ...
        [Constructable]
        public CranberryTreeSeed()
            : this(1) { }

        [Constructable]
        public CranberryTreeSeed(int amount)
            : base(0xF27)
        {
            Stackable = true;
            Weight = .5;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new CranberrySapling();
            item.Location = m_pnt;
            item.Map = m_map;
        }

        public CranberryTreeSeed(Serial serial)
            : base(serial) { }
    ...
    public class CranberrySapling : BaseCrop
    {
        ...
        [Constructable]
        public CranberrySapling()
            : base(Utility.RandomList(0xCC9, 0xCC9))
        {
            Movable = false;
            Name = "Cranberry Bush Sapling";
            Hue = 0x4C;

            init(this);
        }

        public static void init(CranberrySapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(CranberryTree), delay);
            plant.thisTimer.Start();
        }

        public CranberrySapling(Serial serial)
            : base(serial) { }
    ...
    public class CranberryTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public CranberryTree(Point3D pnt, Map map)
            : base(0xC9E) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
        ...
        public static void init(CranberryTree plant, bool full)
        {
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 2))
                {
                    ...
                    else //check skill
                    {
                        ...
                        Cranberries crop = new Cranberries(pick);
        ...
        {
            private CranberryTree i_plant;

            public FruitTimer(CranberryTree plant)
            ...
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    if ((lumberValue > .5) && (Utility.RandomDouble() <= lumberValue))
                    {
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(CranberryTree));
        ...
        {
            private CranberryTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(CranberryTree Tree, FruitTreeStump Stump, Mobile from)
        ...
        public CranberryTree(Serial serial)
            : base(serial) { }
