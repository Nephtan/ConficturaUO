...
{
    public class AlmondTreeSeed : BaseCrop
    {
        ...
        [Constructable]
        public AlmondTreeSeed()
            : this(1) { }

        [Constructable]
        public AlmondTreeSeed(int amount)
            : base(0xF27)
        {
            Stackable = true;
            Weight = .5;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new AlmondSapling();
            item.Location = m_pnt;
            item.Map = m_map;
        }

        public AlmondTreeSeed(Serial serial)
            : base(serial) { }
    ...
    public class AlmondSapling : BaseCrop
    {
        ...
        {
            Movable = false;
            Name = "Almond tree sapling";
            Hue = 0x466;

            init(this);
        }

        public static void init(AlmondSapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(AlmondTree), delay);
            plant.thisTimer.Start();
        }

        public AlmondSapling(Serial serial)
            : base(serial) { }
    ...
    public class AlmondTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public AlmondTree(Point3D pnt, Map map)
            : base(0xCE5) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
        ...
        public static void init(AlmondTree plant, bool full)
        {
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 2))
                {
                    ...
                    else //check skill
                    {
                        ...
                        m_yield -= pick;
                        from.SendMessage("You pick {0} Almond{1}!", pick, (pick == 1 ? "" : "s"));

                        //PublicOverheadMessage( MessageType.Regular, 0x3BD, false, string.Format( "{0}", m_yield ));

                        Almonds crop = new Almonds(pick);
        ...
        {
            private AlmondTree i_plant;

            public FruitTimer(AlmondTree plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        Almonds fruit = new Almonds((int)Utility.Random(13) + m_yield);
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(AlmondTree));
        ...
        {
            private AlmondTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(AlmondTree Tree, FruitTreeStump Stump, Mobile from)
        ...
        public AlmondTree(Serial serial)
            : base(serial) { }
