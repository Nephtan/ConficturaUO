...
{
    public class CherryTreeSeed : BaseCrop
    {
    ...
    public class CherrySapling : BaseCrop
    {
        ...
        public static void init(CherrySapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(CherryTree), delay);
            plant.thisTimer.Start();
        }

        public CherrySapling(Serial serial)
            : base(serial) { }
    ...
    public class CherryTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public CherryTree(Point3D pnt, Map map)
            : base(0xD99) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
        ...
        public static void init(CherryTree plant, bool full)
        {
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 2))
                {
                    ...
                    else //check skill
                    {
                        ...
                        m_yield -= pick;
                        from.SendMessage("You pick {0} Cherry{1}!", pick, (pick == 1 ? "" : "s"));

                        //PublicOverheadMessage( MessageType.Regular, 0x3BD, false, string.Format( "{0}", m_yield ));

                        Cherries crop = new Cherries(pick);
        ...
        {
            private CherryTree i_plant;

            public FruitTimer(CherryTree plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        Cherries fruit = new Cherries((int)Utility.Random(13) + m_yield);
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(CherryTree));
        ...
        {
            private CherryTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(CherryTree Tree, FruitTreeStump Stump, Mobile from)
        ...
        public CherryTree(Serial serial)
            : base(serial) { }
