...
{
    public class CoconutTreeSeed : BaseCrop
    {
        ...
        [Constructable]
        public CoconutTreeSeed()
            : this(1) { }

        [Constructable]
        public CoconutTreeSeed(int amount)
            : base(0xF27)
        {
            Stackable = true;
            Weight = .5;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new CoconutSapling();
            item.Location = m_pnt;
            item.Map = m_map;
        }

        public CoconutTreeSeed(Serial serial)
            : base(serial) { }
    ...
    public class CoconutSapling : BaseCrop
    {
        ...
        public static void init(CoconutSapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(CoconutTree), delay);
            plant.thisTimer.Start();
        }

        public CoconutSapling(Serial serial)
            : base(serial) { }
    ...
    public class CoconutTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public CoconutTree(Point3D pnt, Map map)
            : base(0xC95) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
        ...
        public static void init(CoconutTree plant, bool full)
        {
        ...
        {
            private CoconutTree i_plant;

            public FruitTimer(CoconutTree plant)
            ...
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    if ((lumberValue > .5) && (Utility.RandomDouble() <= lumberValue))
                    {
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(CoconutTree));
        ...
        {
            private CoconutTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(CoconutTree Tree, FruitTreeStump Stump, Mobile from)
        ...
        public CoconutTree(Serial serial)
            : base(serial) { }
