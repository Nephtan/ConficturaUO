...
{
    public class PineappleTreeSeed : BaseCrop
    {
        ...
        [Constructable]
        public PineappleTreeSeed()
            : this(1) { }

        [Constructable]
        public PineappleTreeSeed(int amount)
            : base(0xF27)
        {
            Stackable = true;
            Weight = .5;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new PineappleSapling();
            item.Location = m_pnt;
            item.Map = m_map;
        }

        public PineappleTreeSeed(Serial serial)
            : base(serial) { }
    ...
    public class PineappleSapling : BaseCrop
    {
        ...
        [Constructable]
        public PineappleSapling()
            : base(Utility.RandomList(0xCE9, 0xCEA))
        {
            Movable = false;
            Name = "Pineapple palm sapling";

            init(this);
        }

        public static void init(PineappleSapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(PineappleTree), delay);
            plant.thisTimer.Start();
        }

        public PineappleSapling(Serial serial)
            : base(serial) { }
    ...
    public class PineappleTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public PineappleTree(Point3D pnt, Map map)
            : base(0xCA8) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
        ...
        public static void init(PineappleTree plant, bool full)
        {
        ...
        {
            private PineappleTree i_plant;

            public FruitTimer(PineappleTree plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        Pineapple fruit = new Pineapple((int)Utility.Random(13) + m_yield);
        ...
        {
            private PineappleTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(PineappleTree Tree, FruitTreeStump Stump, Mobile from)
        ...
        public PineappleTree(Serial serial)
            : base(serial) { }
