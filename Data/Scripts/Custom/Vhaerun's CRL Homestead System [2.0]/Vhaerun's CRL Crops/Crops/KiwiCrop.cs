...
{
    public class KiwiSeed : BaseCrop
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new KiwiSeedling(from);
            item.Location = m_pnt;
            item.Map = m_map;
        }

        public KiwiSeed(Serial serial)
            : base(serial) { }
    ...
    public class KiwiSeedling : BaseCrop
    {
        ...
        [Constructable]
        public KiwiSeedling(Mobile sower)
            : base(Utility.RandomList(0x1A99, 0x1A9A))
        {
            Movable = false;
            Name = "Kiwi Seedling";
            m_sower = sower;
        ...
        public static void init(KiwiSeedling plant)
        {
            plant.thisTimer = new CropHelper.GrowTimer(plant, typeof(KiwiCrop), plant.Sower);
        ...
        public KiwiSeedling(Serial serial)
            : base(serial) { }
    ...
    public class KiwiCrop : BaseCrop
    {
        ...
        [Constructable]
        public KiwiCrop(Mobile sower)
            : base(Utility.RandomList(0xD37, 0xD38))
        {
            Movable = false;
            Name = "Kiwi Plant";
            Hue = 0x49;
        ...
        public static void init(KiwiCrop plant, bool full)
        {
            plant.PickGraphic = Utility.RandomList(0xD37, 0xD38);
            plant.FullGraphic = Utility.RandomList(0xD37, 0xD38);
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 1))
                {
                    ...
                    else //check skill and sower
                    {
                        ...
                        if (
                            SowerPickTime != TimeSpan.Zero
                            && m_lastvisit + SowerPickTime < DateTime.Now
                            && !(m_sower.AccessLevel > AccessLevel.Counselor)
                        )
                        {
                            this.UpRoot(from);
                            return;
        ...
        {
            private KiwiCrop i_plant;

            public CropTimer(KiwiCrop plant)
