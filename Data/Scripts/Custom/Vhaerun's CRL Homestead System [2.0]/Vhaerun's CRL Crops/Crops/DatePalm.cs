...
{
    public class DateTreeSeed : BaseCrop
    {
        ...
        [Constructable]
        public DateTreeSeed(int amount)
            : base(0xF27)
        {
            Stackable = true;
            Weight = .5;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new DateSapling();
            item.Location = m_pnt;
            item.Map = m_map;
        }

        public DateTreeSeed(Serial serial)
            : base(serial) { }
    ...
    public class DateSapling : BaseCrop
    {
        ...
        {
            Movable = false;
            Name = "Date Palm sapling";

            init(this);
        }

        public static void init(DateSapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(DateTree), delay);
            plant.thisTimer.Start();
        }

        public DateSapling(Serial serial)
            : base(serial) { }
    ...
    public class DateTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public DateTree(Point3D pnt, Map map)
            : base(0xC96) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
        ...
        public static void init(DateTree plant, bool full)
        {
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 2))
                {
                    ...
                    else //check skill
                    {
                        ...
                        m_yield -= pick;
                        from.SendMessage("You pick {0} Dates{1}!", pick, (pick == 1 ? "" : "s"));
        ...
        {
            private DateTree i_plant;

            public FruitTimer(DateTree plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        Dates fruit = new Dates((int)Utility.Random(13) + m_yield);
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(DateTree));
        ...
        {
            private DateTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(DateTree Tree, FruitTreeStump Stump, Mobile from)
