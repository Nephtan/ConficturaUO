...
{
    public class SquashSeed : BaseCrop
    {
    ...
    public class SquashSeedling : BaseCrop
    {
        ...
        public static void init(SquashSeedling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(SquashCrop), delay);
            plant.thisTimer.Start();
        }

        public SquashSeedling(Serial serial)
            : base(serial) { }
    ...
    public class SquashCrop : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public SquashCrop(Point3D pnt, Map map)
        ...
        public static void init(SquashCrop plant, bool full)
        {
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 2))
                {
                    ...
                    else //check skill
                    {
                        ...
                        m_yield -= pick;
                        from.SendMessage("You pick {0} Squash{1}!", pick, (pick == 1 ? "" : "s"));

                        //PublicOverheadMessage( MessageType.Regular, 0x3BD, false, string.Format( "{0}", m_yield ));

                        Squash crop = new Squash(pick);
        ...
        {
            private SquashCrop i_plant;

            public FruitTimer(SquashCrop plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        Squash fruit = new Squash((int)Utility.Random(13) + m_yield);
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(SquashCrop));
        ...
        {
            private SquashCrop i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(SquashCrop Tree, FruitTreeStump Stump, Mobile from)
