...
{
    public class BlackRaspberryTreeSeed : BaseCrop
    {
        ...
        [Constructable]
        public BlackRaspberryTreeSeed()
            : this(1) { }

        [Constructable]
        public BlackRaspberryTreeSeed(int amount)
            : base(0xF27)
        {
            Stackable = true;
            Weight = .5;
        ...
        public BlackRaspberryTreeSeed(Serial serial)
            : base(serial) { }
    ...
    public class BlackRaspberrySapling : BaseCrop
    {
        ...
        [Constructable]
        public BlackRaspberrySapling()
            : base(Utility.RandomList(0xCC9, 0xCC9))
        {
            Movable = false;
            Name = "Black Raspberry Bush Sapling";
            Hue = 0x298;

            init(this);
        }

        public static void init(BlackRaspberrySapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(BlackRaspberryTree), delay);
            plant.thisTimer.Start();
        }

        public BlackRaspberrySapling(Serial serial)
            : base(serial) { }
    ...
    public class BlackRaspberryTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public BlackRaspberryTree(Point3D pnt, Map map)
            : base(0xC9E) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
            Hue = 0x298;
            Name = "Black Raspberry Bush";

            init(this, false);
        }

        public static void init(BlackRaspberryTree plant, bool full)
        {
        ...
        {
            private BlackRaspberryTree i_plant;

            public FruitTimer(BlackRaspberryTree plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        BlackRaspberry fruit = new BlackRaspberry(
                            (int)Utility.Random(13) + m_yield
                        );
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(BlackRaspberryTree));
        ...
        {
            private BlackRaspberryTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(BlackRaspberryTree Tree, FruitTreeStump Stump, Mobile from)
        ...
        public BlackRaspberryTree(Serial serial)
            : base(serial) { }
