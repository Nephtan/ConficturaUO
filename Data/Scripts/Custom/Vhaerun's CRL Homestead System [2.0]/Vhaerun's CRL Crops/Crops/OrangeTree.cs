...
{
    public class OrangeTreeSeed : BaseCrop
    {
        ...
        [Constructable]
        public OrangeTreeSeed()
            : this(1) { }

        [Constructable]
        public OrangeTreeSeed(int amount)
            : base(0xF27)
        {
            Stackable = true;
            Weight = .5;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new OrangeSapling();
            item.Location = m_pnt;
            item.Map = m_map;
        }

        public OrangeTreeSeed(Serial serial)
            : base(serial) { }
    ...
    public class OrangeSapling : BaseCrop
    {
        ...
        public static void init(OrangeSapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(OrangeTree), delay);
            plant.thisTimer.Start();
        }

        public OrangeSapling(Serial serial)
            : base(serial) { }
    ...
    public class OrangeTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public OrangeTree(Point3D pnt, Map map)
            : base(0xDA2) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
        ...
        public static void init(OrangeTree plant, bool full)
        {
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 2))
                {
                    ...
                    else //check skill
                    {
                        ...
                        m_yield -= pick;
                        from.SendMessage("You pick {0} Orange{1}!", pick, (pick == 1 ? "" : "s"));
        ...
        {
            private OrangeTree i_plant;

            public FruitTimer(OrangeTree plant)
        ...
        {
            private OrangeTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(OrangeTree Tree, FruitTreeStump Stump, Mobile from)
        ...
        public OrangeTree(Serial serial)
            : base(serial) { }
