...
{
    public class BlackberryTreeSeed : BaseCrop
    {
        ...
        [Constructable]
        public BlackberryTreeSeed()
            : this(1) { }

        [Constructable]
        public BlackberryTreeSeed(int amount)
            : base(0xF27)
        {
            Stackable = true;
            Weight = .5;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new BlackberrySapling();
            item.Location = m_pnt;
            item.Map = m_map;
        }

        public BlackberryTreeSeed(Serial serial)
            : base(serial) { }
    ...
    public class BlackberrySapling : BaseCrop
    {
        ...
        [Constructable]
        public BlackberrySapling()
            : base(Utility.RandomList(0xCC9, 0xCC9))
        {
            Movable = false;
            Name = "Blackberry Bush Sapling";
            Hue = 0x252;

            init(this);
        }

        public static void init(BlackberrySapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(BlackberryTree), delay);
            plant.thisTimer.Start();
        }

        public BlackberrySapling(Serial serial)
            : base(serial) { }
    ...
    public class BlackberryTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public BlackberryTree(Point3D pnt, Map map)
            : base(0xC9E) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
        ...
        public static void init(BlackberryTree plant, bool full)
        {
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 2))
                {
                    ...
                    else //check skill
                    {
                        ...
                        Blackberries crop = new Blackberries(pick);
        ...
        {
            private BlackberryTree i_plant;

            public FruitTimer(BlackberryTree plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        Blackberries fruit = new Blackberries((int)Utility.Random(13) + m_yield);
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(BlackberryTree));
        ...
        {
            private BlackberryTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(BlackberryTree Tree, FruitTreeStump Stump, Mobile from)
        ...
        public BlackberryTree(Serial serial)
            : base(serial) { }
