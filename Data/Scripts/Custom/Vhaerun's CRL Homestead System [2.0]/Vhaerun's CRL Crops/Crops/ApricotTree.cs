...
{
    public class ApricotTreeSeed : BaseCrop
    {
        ...
        [Constructable]
        public ApricotTreeSeed()
            : this(1) { }

        [Constructable]
        public ApricotTreeSeed(int amount)
            : base(0xF27)
        {
            Stackable = true;
            Weight = .5;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new ApricotSapling();
            item.Location = m_pnt;
            item.Map = m_map;
        }

        public ApricotTreeSeed(Serial serial)
            : base(serial) { }
    ...
    public class ApricotSapling : BaseCrop
    {
        ...
        [Constructable]
        public ApricotSapling()
            : base(Utility.RandomList(0xCE9, 0xCEA))
        {
            Movable = false;
            Name = "Apricot tree sapling";

            init(this);
        }

        public static void init(ApricotSapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(ApricotTree), delay);
            plant.thisTimer.Start();
        }

        public ApricotSapling(Serial serial)
            : base(serial) { }
    ...
    public class ApricotTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public ApricotTree(Point3D pnt, Map map)
            : base(Utility.RandomList(0xDAA, 0xDA6)) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
            Name = "Apricot Tree";
        ...
        public static void init(ApricotTree plant, bool full)
        {
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 2))
                {
                    ...
                    else //check skill
                    {
                        ...
                        m_yield -= pick;
                        from.SendMessage("You pick {0} Apricot{1}!", pick, (pick == 1 ? "" : "s"));
        ...
        {
            private ApricotTree i_plant;

            public FruitTimer(ApricotTree plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        Apricot fruit = new Apricot((int)Utility.Random(13) + m_yield);
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(ApricotTree));
        ...
        {
            private ApricotTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(ApricotTree Tree, FruitTreeStump Stump, Mobile from)
        ...
        public ApricotTree(Serial serial)
            : base(serial) { }
