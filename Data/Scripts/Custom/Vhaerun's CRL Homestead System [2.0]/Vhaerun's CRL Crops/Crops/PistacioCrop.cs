...
{
    public class PistacioTreeSeed : BaseCrop
    {
        ...
        [Constructable]
        public PistacioTreeSeed()
            : this(1) { }

        [Constructable]
        public PistacioTreeSeed(int amount)
            : base(0xF27)
        {
            Stackable = true;
            Weight = .5;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new PistacioSapling();
            item.Location = m_pnt;
            item.Map = m_map;
        }

        public PistacioTreeSeed(Serial serial)
            : base(serial) { }
    ...
    public class PistacioSapling : BaseCrop
    {
        ...
        [Constructable]
        public PistacioSapling()
            : base(Utility.RandomList(0xCE9, 0xCEA))
        {
            Movable = false;
            Name = "Pistacio Tree Sapling";
            Hue = 0x466;

            init(this);
        }

        public static void init(PistacioSapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(PistacioTree), delay);
            plant.thisTimer.Start();
        }

        public PistacioSapling(Serial serial)
            : base(serial) { }
    ...
    public class PistacioTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public PistacioTree(Point3D pnt, Map map)
            : base(0xD99) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
        ...
        public static void init(PistacioTree plant, bool full)
        {
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 2))
                {
                    ...
                    else //check skill
                    {
                        ...
                        m_yield -= pick;
                        from.SendMessage("You pick {0} Pistacio{1}!", pick, (pick == 1 ? "" : "s"));

                        //PublicOverheadMessage( MessageType.Regular, 0x3BD, false, string.Format( "{0}", m_yield ));

                        Pistacios crop = new Pistacios(pick);
        ...
        {
            private PistacioTree i_plant;

            public FruitTimer(PistacioTree plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        Pistacios fruit = new Pistacios((int)Utility.Random(13) + m_yield);
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(PistacioTree));
        ...
        {
            private PistacioTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(PistacioTree Tree, FruitTreeStump Stump, Mobile from)
        ...
        public PistacioTree(Serial serial)
            : base(serial) { }
