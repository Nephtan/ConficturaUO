...
{
    public class LimeTreeSeed : BaseCrop
    {
    ...
    public class LimeSapling : BaseCrop
    {
        ...
        {
            Movable = false;
            Name = "Lime tree sapling";

            init(this);
        }

        public static void init(LimeSapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(LimeTree), delay);
    ...
    public class LimeTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public LimeTree(Point3D pnt, Map map)
            : base(0xDA6) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
        ...
        public static void init(LimeTree plant, bool full)
        {
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 2))
                {
                    ...
                    else //check skill
                    {
                        ...
                        m_yield -= pick;
                        from.SendMessage("You pick {0} Lime{1}!", pick, (pick == 1 ? "" : "s"));
        ...
        {
            private LimeTree i_plant;

            public FruitTimer(LimeTree plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        Lime fruit = new Lime((int)Utility.Random(13) + m_yield);
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(LimeTree));
        ...
        {
            private LimeTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(LimeTree Tree, FruitTreeStump Stump, Mobile from)
