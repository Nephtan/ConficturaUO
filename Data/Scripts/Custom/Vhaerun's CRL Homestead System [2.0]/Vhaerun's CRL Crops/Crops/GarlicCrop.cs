    ...
    public class GarlicCrop : BaseCrop
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                int mageValue = (int)from.Skills[SkillName.Magery].Value / 20;
                ...
                if (from.InRange(this.GetWorldLocation(), 1))
                {
                    ...
                    else //check skill and sower
                    {
                        ...
                        else if (alchemyValue > m_yield)
                        {
                            alchemyValue = m_yield + 1;
                        }

                        int pickmage = Utility.Random(mageValue);
                        int pickalchemy = Utility.Random(alchemyValue);

                        if (pickmage + pickalchemy == 0)
                        {
                            from.SendMessage("You do not manage to harvest any crops.");
                            return;
                        }

                        if (pickmage >= pickalchemy)
                        {
                            m_yield -= pickmage;
                            from.SendMessage(
                                "You harvest {0} crop{1}!",
                                pickmage,
                                (pickmage == 1 ? "" : "s")
                            );
                            ...
                            Garlic crop = new Garlic(pickmage);
