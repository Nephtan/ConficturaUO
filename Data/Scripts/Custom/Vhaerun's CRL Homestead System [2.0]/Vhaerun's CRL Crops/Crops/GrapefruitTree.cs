...
{
    public class GrapefruitTreeSeed : BaseCrop
    {
        ...
        [Constructable]
        public GrapefruitTreeSeed()
            : this(1) { }

        [Constructable]
        public GrapefruitTreeSeed(int amount)
            : base(0xF27)
        {
            Stackable = true;
            Weight = .5;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new GrapefruitSapling();
            item.Location = m_pnt;
            item.Map = m_map;
        }

        public GrapefruitTreeSeed(Serial serial)
            : base(serial) { }
    ...
    public class GrapefruitSapling : BaseCrop
    {
        ...
        {
            Movable = false;
            Name = "Grapefruit Tree Sapling";

            init(this);
        }

        public static void init(GrapefruitSapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(GrapefruitTree), delay);
            plant.thisTimer.Start();
        }

        public GrapefruitSapling(Serial serial)
            : base(serial) { }
    ...
    public class GrapefruitTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public GrapefruitTree(Point3D pnt, Map map)
            : base(Utility.RandomList(0xD9E, 0xDA2)) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
        ...
        public static void init(GrapefruitTree plant, bool full)
        {
        ...
        {
            private GrapefruitTree i_plant;

            public FruitTimer(GrapefruitTree plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        Grapefruit fruit = new Grapefruit((int)Utility.Random(13) + m_yield);
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(GrapefruitTree));
        ...
        {
            private GrapefruitTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(GrapefruitTree Tree, FruitTreeStump Stump, Mobile from)
        ...
        public GrapefruitTree(Serial serial)
            : base(serial) { }
