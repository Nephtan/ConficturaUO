...
{
    public class CocoaTreeSeed : BaseCrop
    {
        ...
        [Constructable]
        public CocoaTreeSeed()
            : this(1) { }

        [Constructable]
        public CocoaTreeSeed(int amount)
            : base(0xF27)
        {
            Stackable = true;
            Weight = .5;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            this.Consume();
            Item item = new CocoaSapling();
            item.Location = m_pnt;
            item.Map = m_map;
        }

        public CocoaTreeSeed(Serial serial)
            : base(serial) { }
    ...
    public class CocoaSapling : BaseCrop
    {
        ...
        {
            Movable = false;
            Name = "Cocoa tree sapling";

            init(this);
        }

        public static void init(CocoaSapling plant)
        {
            TimeSpan delay = TreeHelper.SaplingTime;
            plant.treeTime = DateTime.Now + delay;

            plant.thisTimer = new TreeHelper.TreeTimer(plant, typeof(CocoaTree), delay);
            plant.thisTimer.Start();
        }

        public CocoaSapling(Serial serial)
            : base(serial) { }
    ...
    public class CocoaTree : BaseTree
    {
        public Item i_trunk;
        ...
        [Constructable]
        public CocoaTree(Point3D pnt, Map map)
            : base(0xCFF) // leaves
        {
            Movable = false;
            MoveToWorld(pnt, map);
        ...
        public static void init(CocoaTree plant, bool full)
        {
            ...
            if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 2))
                {
                    ...
                    else //check skill
                    {
                        ...
                        m_yield -= pick;
                        from.SendMessage("You pick {0} Cocoa{1}!", pick, (pick == 1 ? "" : "s"));

                        //PublicOverheadMessage( MessageType.Regular, 0x3BD, false, string.Format( "{0}", m_yield ));

                        CocoaNut crop = new CocoaNut(pick);
        ...
        {
            private CocoaTree i_plant;

            public FruitTimer(CocoaTree plant)
        ...
        public void Chop(Mobile from)
        {
            if (from.InRange(this.GetWorldLocation(), 1))
            {
                if ((chopTimer == null) || (!chopTimer.Running))
                {
                    ...
                    {
                        CocoaNut fruit = new CocoaNut((int)Utility.Random(13) + m_yield);
                        ...
                        FruitTreeStump i_stump = new FruitTreeStump(typeof(CocoaTree));
        ...
        {
            private CocoaTree i_tree;
            private FruitTreeStump i_stump;
            private Mobile m_chopper;

            public StumpTimer(CocoaTree Tree, FruitTreeStump Stump, Mobile from)
        ...
        public CocoaTree(Serial serial)
            : base(serial) { }
