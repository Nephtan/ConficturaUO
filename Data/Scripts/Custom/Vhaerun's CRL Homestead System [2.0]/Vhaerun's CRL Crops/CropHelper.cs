...
namespace Server.Items.Crops
{
    public class BaseCrop : Item
    {
        ...
        public virtual bool CanGrowFarm
        {
            get { return true; }
        }
        public virtual bool CanGrowHouseTiles
        {
            get { return true; }
        }
        public virtual bool CanGrowDirt
        {
            get { return true; }
        }
        public virtual bool CanGrowGround
        {
            get { return false; }
        }
        public virtual bool CanGrowSwamp
        {
            get { return false; }
        }
        public virtual bool CanGrowSand
        {
            get { return false; }
        }

        // For Player Deeded Plots
        public virtual bool CanGrowGarden
        {
            get { return false; }
        }
        ...
        } //changed FromDays ( 14 ) to mins...for testing

        // If true any Player can destroy a crop, overriddable in Crop scripts
        public virtual bool PlayerCanDestroy
        {
            get { return true; }
        }
    ...
    public class CropHelper
    {
        ...
        public static bool CanWorkMounted
        {
            get { return false; }
        }
        ...
        public static bool ValidateGardenPlot(Map map, int x, int y)
        {
            ...
            if (!ground)
            {
                Server.StaticTile[] tiles = map.Tiles.GetStaticTiles(x, y);
        ...
        public static bool ValidateHouseTiles(Map map, int x, int y) //HOUSETILES
        {
            ...
            Server.StaticTile[] tiles = map.Tiles.GetStaticTiles(x, y, true);
            for (int i = 0; !ground && i < tiles.Length; ++i)
            {
                int tileID = tiles[i].ID & 0x3FFF;
        ...
        public class GrowTimer : Timer
        {
            ...
            //private int rnd; //- Jeremy

            public GrowTimer(Item seedling, Type croptype, Mobile sower)
                //: base(TimeSpan.FromSeconds(600), TimeSpan.FromSeconds(12))
                : base(TimeSpan.FromSeconds(3600), TimeSpan.FromSeconds(36)) //- Jeremy
            {
                Priority = TimerPriority.OneSecond;
                //Priority = TimerPriority.OneMinute; //- Jeremy
                i_seedling = seedling;
                t_crop = croptype;
                m_sower = sower;
                cnt = 0;
                ...
                //rnd = Utility.RandomMinMax(60, 120);  //- Jeremy
        ...
        public class Weeds : BaseCrop
        {
            ...
            [Constructable]
            public Weeds(Mobile sower)
                : base(Utility.RandomList(0xCAD, 0xCAE, 0xCAF))
            {
                Movable = false;
                Name = "Weed";
            ...
            public override void OnDoubleClick(Mobile from)
            {
                ...
                if (from.InRange(this.GetWorldLocation(), 1))
                {
                    if ((from == m_sower) || (DateTime.Now >= planted.AddDays(3)))
                    {
                        ...
                        from.SendMessage("The weed is still too tough to pull.");
                }
                else
                {
