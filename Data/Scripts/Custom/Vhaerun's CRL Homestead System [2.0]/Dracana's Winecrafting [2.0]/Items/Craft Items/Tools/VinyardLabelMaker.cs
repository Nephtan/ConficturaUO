...
namespace Server.Items
{
    public class VinyardLabelMaker : Item
    {
        ...
        public VinyardLabelMaker(Serial serial)
            : base(serial) { }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.SendMessage(
                    "Click on this label tool to name your vinyard or a wine keg/wine bottle you crafted to add a new label."
                );
        ...
        private class RenameItemTarget : Target
        {
            ...
            public RenameItemTarget(VinyardLabelMaker labelmaker)
            ...
            protected override void OnTarget(Mobile from, object target)
            {
                ...
                {
                    from.SendMessage(
                        "Invalid Target.  Only crafted Wine Kegs, Wine Bottles, or this label maker can be labeled."
                    );
                }
                    ...
                    if (target == m_LabelMaker || target is BaseCraftWine || target is WineKeg)
                    {
                        ...
                        else
                        {
                            if (item is BaseCraftWine)
                            {
                                ...
                                {
                                    from.SendMessage(
                                        "That bottle is either not worth labeling or was not crafted at your vinyard!"
                                    );
                                }
                                else
                                {
                            ...
                            else if (item is WineKeg)
                            {
                                ...
                                {
                                    from.SendMessage(
                                        "That keg is either not worth labeling or was not crafted at your vinyard!"
                                    );
                                }
                                else
                                {
        ...
        private class LabelPrompt2 : Prompt
        {
            ...
            public override void OnResponse(Mobile from, string text)
            {
                ...
                from.SendMessage(
                    "As long as you keep this tool in your pack, all future wine you create will be labeled with this vinyard name."
                );
            }
        }
    }
