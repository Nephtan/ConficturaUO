//================================================//
// Created by dracana				  //
// Desc: Core and rules for placing of grapevines //
//       by both players and admins.              //
//       Called from AddGrapevineGump.cs.         //
//================================================//
using System;
...
namespace Server.Misc
{
    ...
    {
        public static bool AllowPlayerYards = false;
        public static bool AllowAllHouseTiles = false;
        public static bool AllowAllYardTiles = false;
        ...
        public static bool ValidatePlayerPlacement(Point3D loc, Mobile m_From, Map m_Map, object o)
        {
            ...
            if (!AllowPlayerYards)
            {
                ...
                else
                {
                    if (
                        !AllowAllHouseTiles
                        && (
                            !ValidateHouseTiles(m_Map, loc.X, loc.Y)
                            && !ValidateVinyardPlot(m_Map, loc.X, loc.Y)
                        )
                    )
                    {
                        m_From.SendMessage(
                            "The grapevines must be placed on dirt tiles or a vinyard ground addon."
                        );
                        return false;
                    }
                }
            ...
            else
            {
                if (CheckHouse(m_From, loc, m_Map, 20))
                {
                    ...
                    {
                        m_From.SendMessage(
                            "Grapevines placed inside your house must be placed on dirt tiles or a vinyard ground addon."
                        );
                        return false;
                    }
                }
                else
                {
                    ...
                    {
                        m_From.SendMessage(
                            "This is outside of your yard. Please re-try the placement"
                        );
                        return false;
                    }
                    ...
                    {
                        if (
                            !ValidateFarmLand(m_Map, loc.X, loc.Y)
                            && !ValidateDirt(m_Map, loc.X, loc.Y)
                            && !ValidateGround(m_Map, loc.X, loc.Y)
                        )
                        {
                            m_From.SendMessage(
                                "Grapevines must be placed on dirt, ground, or farm tiles."
                            );
                            return false;
                        }
                    }
        ...
        public static bool ValidateVinyardPlot(Map map, int x, int y)
        {
            ...
            if (!ground)
            {
                StaticTile[] tiles = map.Tiles.GetStaticTiles(x, y); // Use StaticTile instead of Tile
        ...
        public static bool ValidateHouseTiles(Map map, int x, int y) // HOUSETILES
        {
            ...
            StaticTile[] tiles = map.Tiles.GetStaticTiles(x, y, true); // Use StaticTile instead of Tile

            for (int i = 0; i < tiles.Length; ++i)
            {
                StaticTile t = tiles[i]; // Use StaticTile instead of Tile
        ...
        public static bool PayForVine(Mobile m_From)
        {
            ...
            {
                m_From.SendLocalizedMessage(1060398, m_VinePrice.ToString()); // ~1_AMOUNT~ gold has been withdrawn from your bank box.
            ...
            else
            {
                ...
                {
                    m_From.SendMessage(
                        m_VinePrice.ToString() + " gold has been removed from your pack"
                    );
            ...
            if (c.TryDropItem(m_From, t, true))
            {
                m_From.SendMessage(
                    "You have been refunded "
                        + m_VinePrice.ToString()
                        + " gold for the deleted vine."
                );
