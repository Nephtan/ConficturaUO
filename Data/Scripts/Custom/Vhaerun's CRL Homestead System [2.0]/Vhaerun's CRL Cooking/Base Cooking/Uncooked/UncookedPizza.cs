...
namespace Server.Items
{
    public class UncookedPizza : Item
    {
        ...
        {
            get { return new Pizza(this.Desc, this.Hue); }
        }
        ...
        [Constructable]
        public UncookedPizza(int Color)
            : this("cheese", Color) { }

        [Constructable]
        public UncookedPizza(string desc, int Color)
            : base(0x1083)
        {
            Weight = 1.0;
            if (Color != 0)
        ...
        public class InternalTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                // meat
                else if (targeted is RawBacon || targeted is Bacon) // no baconslab, 'too big'
                {
                    if (!((Item)targeted).Movable)
                        return;
                ...
                else if (targeted is RawHamSlices || targeted is HamSlices) // not 'ham', too big
                {
                    if (!((Item)targeted).Movable)
                        return;
                ...
                // reagents - should I add them all?
                ...
                else if (targeted is FishHeads) // ick! ;)
                {
                    if (!((Item)targeted).Movable)
                        return;
                ...
                // too much
                    ...
                    targeted is RawHam
                    || targeted is Ham
                    || targeted is RawBaconSlab
                    || targeted is BaconSlab
                    || targeted is CheeseWheel
                    || targeted is CheeseWedge
                )
                {
                    from.SendMessage(
                        "That portion is too large. Use a bladed object to cut it up first."
                    );
                    return;
                }
                // generic
                ...
                {
                    if (
                        targeted is FromageDeChevre
                        || targeted is FromageDeChevreWedge
                        || targeted is FromageDeBrebis
                        || targeted is FromageDeBrebisWedge
                        || targeted is FromageDeVache
                        || targeted is FromageDeVacheWedge
                    )
                    {
                        from.SendMessage(
                            "That portion is too large. Use a bladed object to cut it up first."
                    ...
                    else if (targeted is FromageDeVacheWedgeSmall) // FromageDeVache = cow cheese
                    {
                        if (!((Item)targeted).Movable)
                            return;
                        from.SendMessage("You add extra cheese to the pizza.");
    ...
    // temp, convert to "uncookedpizza" call with a desc of "cheese"
    ...
    // likewise, uncookedpizza, desc sausage
