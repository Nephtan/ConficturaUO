...
namespace Server.Items
{
    ...
    {
        public int Laitage; //quantit� de lait de base du seau
    ...
    public class OnVaVoirLesVaches : Target
    {
        ...
        public OnVaVoirLesVaches(MilkBucket m_saut)
        ...
        * si c'est une bouteille il extrait 1L du seau et fait une bouteille pleine , chaque lait a une propri�t� soif
            ...
            else if ((o is BaseBeverage)) // added by Alari
            {
                ...
                if ((m_varsaut.m_laitage >= p.MaxQuantity) && (p.Quantity == 0))
                {
                    ...
                    else
                    {
                        from.SendMessage(
                            0x84B,
                            "This isn't a drink container, drink container has liquid in it already, or the milk bucket doesn't have enough milk left in it."
                        );
                        from.CloseGump(typeof(LaitageHelpGump));
                        from.SendGump(new LaitageHelpGump(from));
                    }
