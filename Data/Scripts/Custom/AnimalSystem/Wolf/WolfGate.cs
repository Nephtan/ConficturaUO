/* Wolf Gate Script for RunUO 2.0
 * 	by Lokai
 *
 * 	Additional scripting by Mum_Chamber and Alcore
 * 
 * Completely rescripted for RunUO 2.0
 *        and completely redesigned for maximum performance.
 * 
 ...
 *	an Event Gate.
 * 
 * To Use:  Simply add this script to your RunUO 2.0 custom scripts folder. Then, use the [add
 *	command to create an WolfGate, and use [props command to adjust the many
 *	built-in features. 
 *	
...
{
    public class WolfGate : Moongate
    {
        // Private variable declarations.
        private bool m_Decays;
        private bool m_DoesRename = false; //Used for Perma-Death shard, where the original player has died.
        ...
        private int m_Dex;
        private int m_Str;
        private string m_GateMessage = "Wolf Gate";

        //these are Public for use by the GateDialogGump
        public int[] gAllSkills = new int[55];
        ...
        // Items
        private bool m_GivesItems = false;
        ...
        [Constructable]
        public WolfGate(bool decays, Point3D loc, Map map)
            : this(decays)
        {
            MoveToWorld(loc, map);
        ...
        public override bool OnMoveOver(Mobile m)
        {
            ...
            if (pm != null)
            {
                if (bp != null)
                {
                    ...
                    {
                        Item toDisarm = pm.FindItemOnLayer(layer);
                        if (toDisarm != null)
                            bp.DropItem(toDisarm);
                    }

                    while ((Item)bp.FindItemByType(typeof(Item)) != null)
                    {
                        Item item = (bp.FindItemByType(typeof(Item)));
                ...
                Item robe = new WolfGuise();
                pm.EquipItem(robe);

                Item chest = new WolfChest();
                pm.EquipItem(chest);

                Item legs = new WolfLegs();
                pm.EquipItem(legs);

                Item arms = new WolfArms();
                pm.EquipItem(arms);

                Item gorget = new WolfGorget();
                pm.EquipItem(gorget);

                Item cap = new WolfCap();
                pm.EquipItem(cap);
                
                Item weapon = new WolfClaw();
                pm.EquipItem(weapon);

                Item guise = new WolfGuise();
                pm.EquipItem(guise);
                ...
                bracelet.Name = "Wolf Bracelet";
                bracelet.Movable = false;
                bracelet.LootType = LootType.Blessed;
                pm.EquipItem(bracelet);

                Item earrings = new GoldEarrings();
                bracelet.Name = "Wolf Earrings";
                bracelet.Movable = false;
                bracelet.LootType = LootType.Blessed;
                pm.EquipItem(earrings);

                Item dagger = new WolfsTooth();
                bp.DropItem(dagger);

                Item huestone = new WolfHueStone();
                bp.DropItem(huestone);
            }
    ...
    public class GateDialogGump : Gump
    {
        ...
        private static WolfGate m_Sender;
        ...
        public GateDialogGump(int mStr, int mDex, int mInt, string mMessage,
            ...
            string mPlayerCountExeededMessage, WolfGate sender)
            : base(48, 36)
        {
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            m.CloseGump(typeof(GateDialogGump));
