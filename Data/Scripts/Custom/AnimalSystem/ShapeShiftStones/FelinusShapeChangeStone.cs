...
{
    public class FelinusShapeShiftStone : Item
    {
        [Constructable]
        public FelinusShapeShiftStone()
            : base(0x1870)
        {
            Name = "a Felinus ShapeShift Stone";
            Movable = false;
            LootType = LootType.Blessed;
            Hue = 1952;
        }

        public FelinusShapeShiftStone(Serial serial)
            : base(serial)
        {
        }
        ...
        {
            private FelinusShapeShiftStone wss;
            private Mobile user;

            public HueGump(FelinusShapeShiftStone shiftstone, Mobile from)
                : base(150, 150)
            {
                wss = shiftstone;
                user = from;
        ...
        private class ChangeHue : ContextMenuEntry
        {
            ...
            public ChangeHue(FelinusShapeShiftStone wsss, Mobile from)
                : base(0313, 1)
            {
                WSS = wsss;
                m_From = from;
