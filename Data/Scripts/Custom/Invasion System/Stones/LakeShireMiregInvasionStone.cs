...
{
    public class LakeShireMiregInvasionStone : Item
    {
        [Constructable]
        public LakeShireMiregInvasionStone()
            : base(0xED4)
        {
            Movable = false;
            Hue = 33;
        ...
        public virtual void CleanUpLakeShireMiregUnderworld()
        {
            CleanUpSpawns("LakeShireMiregInvasionUnderworld");
            CleanUpWayPoints("LakeShireMiregInvasionUnderworld");
        }

        public virtual void StopLakeShireMiregUnderworld()
        {
            ArrayList lakilsh = new ArrayList(World.Items.Values);
            foreach (Item item in lakilsh)
            {
                if (item is LakeShireMiregInvasionStone)
                {
                    ((LakeShireMiregInvasionStone)item).CleanUpLakeShireMiregUnderworld();
                }
            }
        }

        public LakeShireMiregInvasionStone(Serial serial)
            : base(serial) { }
        ...
        {
            from.SendMessage(" Lake Shire and Mireg are being invaded");
        }
    }
}
