...
{
    public class RegVolonInvasionStone : Item
    {
        [Constructable]
        public RegVolonInvasionStone()
            : base(0xED4)
        {
            Movable = false;
            Hue = 33;
            Name = "a  reg volon invasion stone";
        }
        ...
        public virtual void CleanUpRegVolonUnderworld()
        {
            CleanUpSpawns("RegVolonInvasionUnderworld");
            CleanUpWayPoints("RegVolonInvasionUnderworld");
        }

        public virtual void StopRegVolonUnderworld()
        {
            ArrayList regilsh = new ArrayList(World.Items.Values);
            foreach (Item item in regilsh)
            {
                if (item is RegVolonInvasionStone)
                {
                    ((RegVolonInvasionStone)item).CleanUpRegVolonUnderworld();
                }
            }
        }

        public RegVolonInvasionStone(Serial serial)
            : base(serial) { }
        ...
        {
            from.SendMessage(" Reg Volon is being invaded");
        }
    }
}
