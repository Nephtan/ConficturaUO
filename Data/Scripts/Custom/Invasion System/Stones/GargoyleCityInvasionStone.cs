...
{
    public class GargoyleCityInvasionStone : Item
    {
        [Constructable]
        public GargoyleCityInvasionStone()
            : base(0xED4)
        {
            Movable = false;
            Hue = 33;
            Name = "a  GargoyleCity invasion stone";
        }
        ...
        public virtual void CleanUpGargoyleCityUnderworld()
        {
            CleanUpSpawns("GargoyleCityInvasionUnderworld");
            CleanUpWayPoints("GargoyleCityInvasionUnderworld");
        }

        public virtual void StopGargoyleCityUnderworld()
        {
            ArrayList garcitilsh = new ArrayList(World.Items.Values);
            foreach (Item item in garcitilsh)
            {
                if (item is GargoyleCityInvasionStone)
                {
                    ((GargoyleCityInvasionStone)item).CleanUpGargoyleCityUnderworld();
                }
            }
        }

        public GargoyleCityInvasionStone(Serial serial)
            : base(serial) { }
        ...
        {
            from.SendMessage("Gargoyle City is being invaded");
        }
    }
}
