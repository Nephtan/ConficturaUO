...
{
    public class SavageCampInvasionStone : Item
    {
        [Constructable]
        public SavageCampInvasionStone()
            : base(0xED4)
        {
            Movable = false;
            Hue = 33;
            Name = "a  savage camp invasion stone";
        }
        ...
        public virtual void CleanUpSavageCampUnderworld()
        {
            CleanUpSpawns("SavageCampInvasionUnderworld");
            CleanUpWayPoints("SavageCampInvasionUnderworld");
        }

        public virtual void StopSavageCampUnderworld()
        {
            ArrayList savilsh = new ArrayList(World.Items.Values);
            foreach (Item item in savilsh)
            {
                if (item is SavageCampInvasionStone)
                {
                    ((SavageCampInvasionStone)item).CleanUpSavageCampUnderworld();
                }
            }
        }

        public SavageCampInvasionStone(Serial serial)
            : base(serial) { }
        ...
        {
            from.SendMessage(" Savage Camp is being invaded");
        }
    }
}
