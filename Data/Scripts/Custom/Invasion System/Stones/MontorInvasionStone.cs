...
{
    public class MontorInvasionStone : Item
    {
        [Constructable]
        public MontorInvasionStone()
            : base(0xED4)
        {
            Movable = false;
            Hue = 33;
            Name = "a  montor invasion stone";
        }
        ...
        public virtual void CleanUpMontorUnderworld()
        {
            CleanUpSpawns("MontorInvasionUnderworld");
            CleanUpWayPoints("MontorInvasionUnderworld");
        }

        public virtual void StopMontorUnderworld()
        {
            ArrayList monilsh = new ArrayList(World.Items.Values);
            foreach (Item item in monilsh)
            {
                if (item is MontorInvasionStone)
                {
                    ((MontorInvasionStone)item).CleanUpMontorUnderworld();
                }
            }
        }

        public MontorInvasionStone(Serial serial)
            : base(serial) { }
        ...
        {
            from.SendMessage(" Montor is being invaded");
        }
    }
}
