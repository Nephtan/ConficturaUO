...
{
    public class TerortSkitasInvasionStone : Item
    {
        [Constructable]
        public TerortSkitasInvasionStone()
            : base(0xED4)
        {
            Movable = false;
            Hue = 33;
            Name = "a  terort skitas invasion stone";
        }
        ...
        public virtual void CleanUpTerortSkitasUnderworld()
        {
            CleanUpSpawns("TerortSkitasInvasionUnderworld");
            CleanUpWayPoints("TerortSkitasInvasionUnderworld");
        }

        public virtual void StopTerortSkitasUnderworld()
        {
            ArrayList terilsh = new ArrayList(World.Items.Values);
            foreach (Item item in terilsh)
            {
                if (item is TerortSkitasInvasionStone)
                {
                    ((TerortSkitasInvasionStone)item).CleanUpTerortSkitasUnderworld();
                }
            }
        }

        public TerortSkitasInvasionStone(Serial serial)
            : base(serial) { }
        ...
        {
            from.SendMessage(" Terort Skitas is being invaded");
        }
    }
}
