...
{
    public class PirateCaptainInvader : BaseCreature
    {
        ...
        public static string path = "Data/The Pirate Captain Invader/Speech.txt";
        ...
        public override void OnThink()
        {
            ...
            if (boatspawn == false)
            {
                ...
                for (int i = 0; i < 5; ++i)
                {
                    PirateCrewInvader m_PirateCrewInvader = new PirateCrewInvader();
                    m_PirateCrewInvader.MoveToWorld(loccrew, map);
                }
            }
            ...
            IPooledEnumerable nearbyItems = this.GetItemsInRange(200);

            try
            {
                foreach (Item enemy in nearbyItems)
                {
                    if (enemy is BaseBoat && enemy != m_PirateShip_Boat && !(enemy is PirateShip_Boat))
                    {
                        List<Mobile> targets = new List<Mobile>();
                        IPooledEnumerable enemyMobiles = enemy.GetMobilesInRange(16);

                        try
                        {
                            foreach (Mobile m in enemyMobiles)
                            {
            ...
            if (m_PirateShip_Boat != null && m_enemyboat != null)
            {
                ...
                if (
                    m_PirateShip_Boat != null
                    && this.InRange(m_enemyboat, 10)
                    && m_PirateShip_Boat.IsMoving == true
                )
                {
        ...
        public void SpawnPirate(Mobile target)
        {
            ...
            {
                BaseCreature PirateCrewInvader = new PirateCrewInvader();
                ...
                PirateCrewInvader.MoveToWorld(loc, map);

                PirateCrewInvader.Combatant = target;
            }
        }
        ...
        public PirateCaptainInvader(Serial serial)
            : base(serial) { }
