    ...
    public class Robot : BaseCreature
    {
        ...
        public override bool OnDragDrop(Mobile from, Item dropped)
        {
            if (dropped is BaseLog)
            {
                ...
                {
                    from.SendMessage(
                        "You have no idea how to have the robot cut this type of wood!"
                    );
                    return false;
                }
                ...
                if (from.CheckTargetSkill(SkillName.Lumberjacking, this, minSkill, maxSkill))
                {
                    ...
                    else
                    {
                        ...
                        from.SendMessage(
                            "The robot cuts the logs and you put some boards in your backpack."
                        );
                    }
                }
                else
                {
                    ...
                    if (amount < 2 || lose == amount)
                    {
                        m_Log.Delete();
                        from.SendMessage(
                            "The robot tries to cut the logs but ruins all of the wood."
                        );
                    }
                    else
                    {
                        ...
                        from.SendMessage(
                            "The robot tries to cut the logs but ruins some of the wood."
                        );
                    }
