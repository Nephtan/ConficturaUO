...
namespace Server.Items
{
    ...
    public class FishingQuestBoard : Item
    {
        ...
        public override void OnDoubleClick(Mobile e)
        {
            if (e.InRange(this.GetWorldLocation(), 4))
            {
                ...
                e.SendGump(
                    new BoardGump(
                        e,
                        "SEEKING BRAVE SAILORS",
                        "The townsfolk are looking for brave sailors of the high seas, "
                            + e.Name
        ...
        public class FishingQuestEntry : ContextMenuEntry
        {
            ...
            public FishingQuestEntry(Mobile from)
                : base(6120, 12)
            {
                m_Mobile = from;
            }
            ...
            public override void OnClick()
            {
                ...
                if (PlayerSettings.GetQuestState(m_Mobile, "FishingQuest"))
                {
                    m_Mobile.PrivateOverheadMessage(
                        ...
                        1150,
                        false,
                        "You are already on a quest. Return here when you are done.",
                        m_Mobile.NetState
                    );
                }
                else if (nWhenForAnotherQuest > 0)
                {
                    m_Mobile.PrivateOverheadMessage(
                        ...
                        1150,
                        false,
                        "There are no quests at the moment. Check back in "
                            + sAllowedForAnotherQuest
                            + " minutes.",
                        m_Mobile.NetState
                    );
                }
        ...
        public class FishingQuestComplete : ContextMenuEntry
        {
            ...
            public FishingQuestComplete(Mobile from)
                : base(548, 12)
            {
                m_Mobile = from;
            }
            ...
            if (dropped is Gold)
            {
                ...
                if (dropped.Amount == nPenalty)
                {
                    ...
                    from.PrivateOverheadMessage(
                        ...
                        1153,
                        false,
                        "Someone else will eventually take care of this.",
                        from.NetState
                    );
                    dropped.Delete();
                }
        ...
        public FishingQuestBoard(Serial serial)
            : base(serial) { }
