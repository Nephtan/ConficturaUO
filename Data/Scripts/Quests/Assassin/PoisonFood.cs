    ...
    public class PoisonFood : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else
            {
                ...
                if (m_Poison == 5)
                {
                    from.ApplyPoison(from, PoisonImpl.Lethal);
                }
                else if (m_Poison == 4)
                {
                    from.ApplyPoison(from, PoisonImpl.Deadly);
                }
                else if (m_Poison == 3)
                {
                    from.ApplyPoison(from, PoisonImpl.Greater);
                }
                else if (m_Poison == 2)
                {
                    from.ApplyPoison(from, PoisonImpl.Regular);
                }
                else
                {
                    from.ApplyPoison(from, PoisonImpl.Lesser);
                }
        ...
        public override void AddNameProperties(ObjectPropertyList list)
        {
            ...
            if (m_Poison == 5)
            {
                sPoison = "Lethally Poisoned";
            }
            else if (m_Poison == 4)
            {
                sPoison = "Deathly Poisoned";
            }
            else if (m_Poison == 3)
            {
                sPoison = "Greatly Poisoned";
            }
            else if (m_Poison == 2)
            {
                sPoison = "Poisoned";
        ...
        public PoisonFood(Serial serial)
            : base(serial) { }
