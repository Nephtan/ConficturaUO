    ...
    class StandardQuestFunctions
    {
        ...
        public static void CheckTarget(Mobile m, Mobile target, Item box)
        {
            ...
            if (PlayerSettings.GetQuestState(m, "StandardQuest"))
            {
                ...
                if (sPCCategory == "Item" && target != null)
                {
                    if (
                        sPCCategory == "Item"
                        && StandardQuestFunctions.ChanceToFindQuestedItem()
                            >= Utility.RandomMinMax(1, 100)
                        && Server.Misc.Worlds.GetRegionName(m.Map, m.Location) == sPCRegion
                        && nPCDone != 1
                    )
                    {
                ...
                if (xCount == aCount)
                {
                    if (Utility.RandomMinMax(1, 2) == 1) // KILL SOMETHING
                    {
                        ...
                        string myexplorer =
                            ...
                            + killName
                            + "#"
                            + Server.Misc.Worlds.GetRegionName(theone.Map, theone.Location)
                            + "#0#"
                            + kDollar
                            + "#"
                            + kWorld
                        ...
                        string theStory =
                            myexplorer + "#" + StandardQuestFunctions.QuestSentence(m); // ADD THE STORY PART
        ...
        public static string QuestStatus(Mobile m)
        {
            ...
            if (PlayerSettings.GetQuestState(m, "StandardQuest"))
            {
                ...
                {
                    sexplorerQuest =
                        "Return to any quest bulletin board for your " + sWorth + " gold payment";
                }
            }
