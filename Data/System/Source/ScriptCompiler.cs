...
namespace Server
{
    public static class ScriptCompiler
    {
        ...
        {
            //Console.Write( "Scripts: Compiling script..." );
        ...
        public static void Display(CompilerResults results)
        {
            if (results.Errors.Count > 0)
            {
                Dictionary<string, List<CompilerError>> errors = new Dictionary<
                    string,
                    List<CompilerError>
                >(results.Errors.Count, StringComparer.OrdinalIgnoreCase);
                Dictionary<string, List<CompilerError>> warnings = new Dictionary<
                    string,
                    List<CompilerError>
                >(results.Errors.Count, StringComparer.OrdinalIgnoreCase);
                ...
                foreach (CompilerError e in results.Errors)
                {
                    ...
                    Dictionary<string, List<CompilerError>> table = (
                        e.IsWarning ? warnings : errors
                    );
                ...
                string scriptRoot = Path.GetFullPath(
                    Path.Combine(Core.BaseDirectory, "Data/Scripts" + Path.DirectorySeparatorChar)
                );
                ...
                foreach (KeyValuePair<string, List<CompilerError>> kvp in warnings)
                {
                    ...
                    string usedPath = Uri.UnescapeDataString(
                        scriptRootUri.MakeRelativeUri(new Uri(fullPath)).OriginalString
                    );
        ...
        public static void Invoke(string method)
        {
            ...
            for (int a = 0; a < m_Assemblies.Length; ++a)
            {
                ...
                {
                    MethodInfo m = types[i].GetMethod(
                        method,
        ...
        private static Dictionary<Assembly, TypeCache> m_TypeCaches =
            new Dictionary<Assembly, TypeCache>();
    ...
    public class TypeTable
    {
        ...
        public TypeTable(int capacity)
        {
            ...
            m_Insensitive = new Dictionary<string, Type>(
                capacity,
