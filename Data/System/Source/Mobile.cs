...
namespace Server
{
    ...
    public class DamageEntry
    {
        ...
        {
            get { return m_DamageGiven; }
            set { m_DamageGiven = value; }
        }
        ...
        {
            get { return (DateTime.Now > (m_LastDamage + m_ExpireDelay)); }
        }
    ...
    public delegate bool SkillCheckLocationHandler(
        Mobile from,
        SkillName skill,
        double minSkill,
        double maxSkill
    ...
    public delegate bool SkillCheckDirectTargetHandler(
        Mobile from,
        SkillName skill,
        object target,
        double chance
    );
    public delegate bool SkillCheckDirectLocationHandler(
        Mobile from,
        SkillName skill,
        double chance
    );
    ...
    public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable
    {
        ...
        private static readonly TimeSpan WarmodeSpamCatch = TimeSpan.FromSeconds(
            (Core.SE ? 1.0 : 0.5)
        );
        private static readonly TimeSpan WarmodeSpamDelay = TimeSpan.FromSeconds(
            (Core.SE ? 4.0 : 2.0)
        );
        ...
        public virtual void AddNameProperties(ObjectPropertyList list)
        {
            ...
            if (
                guild != null
                && (m_DisplayGuildTitle || (m_Player && guild.Type != GuildType.Regular))
            )
            {
                string type;
                ...
                else
                {
                    if (title.Length > 0)
                        list.Add(
                            "{0}, {1} Guild{2}",
                            Utility.FixHtml(title),
                            Utility.FixHtml(guild.Name),
                            type
                        );
                    else
        ...
        private void CheckAggrExpire()
        {
            for (int i = m_Aggressors.Count - 1; i >= 0; --i)
            {
                ...
                if (info.Expired)
                {
                    ...
                    if (
                        m_NetState != null
                        && this.CanSee(attacker)
                        && Utility.InUpdateRange(m_Location, attacker.m_Location)
                    )
                    {
            ...
            for (int i = m_Aggressed.Count - 1; i >= 0; --i)
            {
                ...
                if (info.Expired)
                {
                    ...
                    if (
                        m_NetState != null
                        && this.CanSee(defender)
                        && Utility.InUpdateRange(m_Location, defender.m_Location)
                    )
                    {
                        if (m_NetState.StygianAbyss)
                        {
        ...
        {
            get { return m_AllowedStealthSteps; }
                ...
                if (m_Holding != value)
                {
                    if (m_Holding != null)
                    {
                        UpdateTotal(
                            m_Holding,
                            TotalType.Weight,
                            -(m_Holding.TotalWeight + m_Holding.PileWeight)
                        );
        ...
        {
            get { return this.Alive && (RegenThroughPoison || !this.Poisoned); }
        }
            ...
            protected override void OnTick()
            {
                if (DateTime.Now > m_Mobile.m_NextCombatTime)
                {
                    ...
                    if (
                        ...
                        || combatant.m_Deleted
                        || m_Mobile.m_Deleted
                        || combatant.m_Map != m_Mobile.m_Map
                        || !combatant.Alive
                        || !m_Mobile.Alive
                    ...
                    if (m_Mobile.InLOS(combatant))
                    {
                        ...
                        m_Mobile.m_NextCombatTime =
                            DateTime.Now + weapon.OnSwing(m_Mobile, combatant);
                    }
                }
            }
        }
        ...
        private class ExpireAggressorsTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                if (
                    m_Mobile.Deleted
                    || (m_Mobile.Aggressors.Count == 0 && m_Mobile.Aggressed.Count == 0)
                )
        ...
        public virtual Mobile Combatant
        {
            ...
            set
            {
                ...
                if (m_Combatant != value && value != this)
                {
                    ...
                    if (
                        (m_Combatant != null && !CanBeHarmful(m_Combatant, false))
        ...
        public virtual void AggressiveAction(Mobile aggressor, bool criminal)
        {
            ...
            AggressiveActionEventArgs args = AggressiveActionEventArgs.Create(
                this,
                aggressor,
                criminal
            );
        ...
        private class SimpleStateTarget<T> : Target
        {
            ...
            public SimpleStateTarget(
                ...
                TargetFlags flags,
                bool allowGround,
                TargetStateCallback<T> callback,
                T state
            )
                : base(range, allowGround, flags)
            {
        ...
        private class SimpleStatePrompt : Prompt
        {
            ...
            public SimpleStatePrompt(
                PromptStateCallback callback,
                PromptStateCallback cancelCallback,
                object state
            )
            {
                m_Callback = callback;
        ...
        private class SimpleStatePrompt<T> : Prompt
        {
            ...
            public SimpleStatePrompt(
                PromptStateCallback<T> callback,
                PromptStateCallback<T> cancelCallback,
                T state
            )
            {
                m_Callback = callback;
        ...
        {
            get { return m_FwdAccessOverride; }
            set { m_FwdAccessOverride = value; }
        }
        public static bool FwdEnabled
        {
            get { return m_FwdEnabled; }
            set { m_FwdEnabled = value; }
        }
        public static bool FwdUOTDOverride
        {
            get { return m_FwdUOTDOverride; }
            set { m_FwdUOTDOverride = value; }
        }
        public static int FwdMaxSteps
        {
            get { return m_FwdMaxSteps; }
            set { m_FwdMaxSteps = value; }
        }
        ...
        public virtual bool Move(Direction d)
        {
            ...
            if ((m_Direction & Direction.Mask) == (d & Direction.Mask))
            {
                ...
                if (CheckMovement(d, out newZ))
                {
                    ...
                    if (map != null)
                    {
                        ...
                        if (oldSector != newSector)
                        {
                            ...
                            for (int i = 0; i < oldSector.Items.Count; ++i)
                            {
                                ...
                                if (
                                    item.AtWorldPoint(oldX, oldY)
                                    && (
                                        item.Z == oldZ
                                        || (
                                            (item.Z + item.ItemData.Height) > oldZ
                                            && (oldZ + 15) > item.Z
                                        )
                                    )
                                    && !item.OnMoveOff(this)
                            ...
                            for (int i = 0; i < newSector.Items.Count; ++i)
                            {
                                ...
                                if (
                                    ...
                                    && (
                                        item.Z == newZ
                                        || (
                                            (item.Z + item.ItemData.Height) > newZ
                                            && (newZ + 15) > item.Z
                                        )
                                    )
                    ...
                    if (
                        m_FwdEnabled
                        && m_NetState != null
                        && m_AccessLevel < m_FwdAccessOverride
            ...
            if (m_Map != null)
            {
                IPooledEnumerable eable = m_Map.GetObjectsInRange(
                    m_Location,
                ...
                for (int i = 0; i < m_MoveList.Count; ++i)
                {
                    ...
                    if (o is Mobile)
                    {
                        ...
                        if (
                            ns != null
                            && Utility.InUpdateRange(m_Location, m.m_Location)
                            && m.CanSee(this)
                        )
                        {
                            Packet p = null;
                            ...
                            if (ns.StygianAbyss)
                            {
                                ...
                                if (p == null)
                                    cache[0][noto] = p = Packet.Acquire(
                                        new MobileMoving(this, noto)
                                    );
                            }
                            else
                            {
                                ...
                                if (p == null)
                                    cache[1][noto] = p = Packet.Acquire(
                                        new MobileMovingOld(this, noto)
                                    );
                            }
        ...
        {
            get { return !m_Deleted && (!m_Player || !m_Body.IsGhost); }
        }
        ...
        public virtual void Kill()
        {
            ...
            Container c = (
                m_CreateCorpse == null
                    ? null
                    : m_CreateCorpse(this, hair, facialhair, content, equip)
            );
        ...
        {
            get { return m_AutoManifestTimeout; }
            set { m_AutoManifestTimeout = value; }
        }
                ...
                if (from.CheckAlive())
                {
                    ...
                    else
                    {
                        ...
                        if (
                            root != null
                            && root is Mobile
                            && !((Mobile)root).CheckNonlocalLift(from, item)
                        )
                        {
                    ...
                    foreach (NetState ns in eable)
                    {
                        if (!ns.StygianAbyss && ns.Mobile != this && ns.Mobile.CanSee(this))
                        {
                            if (p == null)
                            {
                                ...
                                p = Packet.Acquire(
                                    new DragEffect(this, trg, item.ItemID, item.Hue, item.Amount)
                                );
                            }
            ...
            if (m_Map != null)
            {
                ...
                foreach (object o in eable)
                {
                    if (o is Mobile)
                    {
                        ...
                        if (
                            heard.CanSee(this)
                            && (m_NoSpeechLOS || !heard.Player || heard.InLOS(this))
                        )
                        {
        ...
        public DamageEntry FindMostTotalDamageEntry(bool allowSelf)
        {
            ...
            for (int i = m_DamageEntries.Count - 1; i >= 0; --i)
            {
                ...
                if (de.HasExpired)
                    ...
                    (allowSelf || de.Damager != this)
                    && (mostTotal == null || de.DamageGiven > mostTotal.DamageGiven)
                )
                    mostTotal = de;
            }
                ...
                switch (m_VisibleDamageType)
                {
                    case VisibleDamageType.Related:
                    {
                        NetState ourState = m_NetState,
                            theirState = (from == null ? null : from.m_NetState);
        ...
        public virtual void Deserialize(GenericReader reader)
        {
            ...
            switch (version)
            {
                ...
                case 32:
                {
                    ...
                    DataStoreStr5 = reader.ReadString();
        ...
        public virtual void GetChildContextMenuEntries(
            Mobile from,
        ...
        public void AddItem(Item item)
        {
            ...
            if (
                item.PhysicalResistance != 0
                || item.FireResistance != 0
                || item.ColdResistance != 0
                || item.PoisonResistance != 0
                || item.EnergyResistance != 0
            )
                UpdateResistances();
        ...
        {
            if (item != null && !Item.isModded(item))
            {
                Item.doMod(item);
            }
        }
        ...
        public void ShowHair()
        {
            ...
            if (HairItemID >= 80000)
            {
                HairItemID = RecordHair - 80000;
            }
            if (FacialHairItemID >= 80000)
            {
                FacialHairItemID = RecordBeard - 80000;
            }
        }
        ...
        public void RecordsHair(bool again)
        {
            if (again)
            {
                if (HairItemID < 80000)
                {
                    RecordHair = HairItemID + 80000;
                }
                if (FacialHairItemID < 80000)
                {
                    RecordBeard = FacialHairItemID + 80000;
                }
                ProcessHair();
            }
            else
            {
                if (HairItemID > 0 && HairItemID < 80000 && RecordHair < 80000)
                {
                    RecordHair = HairItemID + 80000;
                }
                if (FacialHairItemID > 0 && FacialHairItemID < 80000 && RecordBeard < 80000)
                {
                    RecordBeard = FacialHairItemID + 80000;
                }
            }
        ...
        public void ProcessClothing()
        {
            ...
            if (Utility.ClothingMod())
            {
                ...
                if (
                    FindItemOnLayer(Layer.Special) != null
                    && Item.isRaceCostume(FindItemOnLayer(Layer.Special))
                )
                {
                    ...
                    FindItemOnLayer(Layer.OuterTorso) != null
                    && Item.isCoat(FindItemOnLayer(Layer.OuterTorso))
                )
                {
                    HideItem(FindItemOnLayer(Layer.MiddleTorso));
                    HideItem(FindItemOnLayer(Layer.InnerTorso));
                    ...
                    FindItemOnLayer(Layer.OuterTorso) != null
                    && Item.isRobe(FindItemOnLayer(Layer.OuterTorso))
                )
                {
                    HideItem(FindItemOnLayer(Layer.MiddleTorso));
                    HideItem(FindItemOnLayer(Layer.InnerLegs));
                    ...
                    HideItem(FindItemOnLayer(Layer.Shirt));
                    if (
                        FindItemOnLayer(Layer.Pants) != null
                        && !Item.isFullLegs(FindItemOnLayer(Layer.Pants))
                    )
                    {
                        HideItem(FindItemOnLayer(Layer.Pants));
                    }
                    ...
                    FindItemOnLayer(Layer.OuterTorso) != null
                    && (
                        Item.isShroud(FindItemOnLayer(Layer.OuterTorso))
                        || Item.isHoodedRobe(FindItemOnLayer(Layer.OuterTorso))
                    )
                )
                {
                    HideItem(FindItemOnLayer(Layer.Helm));
                ...
                {
                    if (
                        FindItemOnLayer(Layer.MiddleTorso) != null
                        && Item.isArmor(FindItemOnLayer(Layer.MiddleTorso))
                    )
                    {
                        HideItem(FindItemOnLayer(Layer.InnerTorso));
                        HideItem(FindItemOnLayer(Layer.Shirt));
                        ...
                        FindItemOnLayer(Layer.Shirt) != null
                        && Item.isArmor(FindItemOnLayer(Layer.Shirt))
                    )
                    {
                        HideItem(FindItemOnLayer(Layer.MiddleTorso));
                        HideItem(FindItemOnLayer(Layer.InnerTorso));
                        ...
                        FindItemOnLayer(Layer.InnerTorso) != null
                        && Item.isArmor(FindItemOnLayer(Layer.InnerTorso))
                    )
                    {
                        HideItem(FindItemOnLayer(Layer.MiddleTorso));
                        HideItem(FindItemOnLayer(Layer.Shirt));
                ...
                {
                    if (
                        FindItemOnLayer(Layer.InnerLegs) != null
                        && Item.isFullLegs(FindItemOnLayer(Layer.InnerLegs))
                    )
                    {
                        HideItem(FindItemOnLayer(Layer.Shoes));
                    }
                    else if (
                        FindItemOnLayer(Layer.OuterLegs) != null
                        && Item.isFullLegs(FindItemOnLayer(Layer.OuterLegs))
                    )
                    {
                        HideItem(FindItemOnLayer(Layer.Shoes));
                    }
        ...
        public void ProcessHair()
        {
            ...
            if (Utility.ClothingMod())
            {
                ...
                )
                {
                    ...
                    FindItemOnLayer(Layer.Helm) != null
                    && Item.isPartialHat(FindItemOnLayer(Layer.Helm))
                )
                {
                    HideHair(false);
                }
                    ...
                    FindItemOnLayer(Layer.Helm) != null
                    && Item.isFullHat(FindItemOnLayer(Layer.Helm))
                )
                {
                    HideHair(true);
                }
        ...
        public static int AnimateMod(int action, int body, int raceID)
        {
            ...
            else if (raceID > 0)
            {
                ...
                {
                    action = Utility.RandomList(4, 5, 6);
                }
                else if (action > 25)
                {
                    action = 18;
            ...
            {
                if (action == 3 && CorrectL)
                {
                    action = 6;
                }
                else if (action == 4 && (CorrectL || CorrectL_04))
                {
                    action = 6;
                }
                else if (action == 7 && CorrectL)
                {
                    action = 9;
                }
                else if (action == 10 && (CorrectL || CorrectL_10))
                {
                    action = 9;
                }
                else if (action == 11 && (CorrectL || CorrectL_11))
                {
                    action = 6;
                }
                else if (action == 12 && CorrectL)
                {
                    action = 8;
                }
            }
        ...
        public int RaceSound(int soundID)
        {
            ...
            {
                if (soundID == 0x310 || soundID == 1055)
                {
                    soundID = RaceIdleSound;
                } // clears throat
                else if (soundID == 0x31B || soundID == 0x42B)
                {
                    soundID = RaceAttackSound;
                } // groans
                else if (soundID == 0x31C || soundID == 0x42C)
                {
                    soundID = RaceAngerSound;
                } // yells
                else if (soundID == 0x32E || soundID == 0x440)
                {
                    soundID = RaceAttackSound;
                } // ahhhh!
                else if (soundID == 0x338 || soundID == 0x44A)
                {
                    soundID = RaceAngerSound;
                } // growls
                else if (soundID == 778 || soundID == 1049)
                {
                    soundID = RaceIdleSound;
                } // ah!
                else if (soundID == 779 || soundID == 1050)
                {
                    soundID = RaceIdleSound;
                } // ah ha!
                else if (soundID == 780 || soundID == 1051)
                {
                    soundID = RaceIdleSound;
                } // applauds
                else if (soundID == 781 || soundID == 1052)
                {
                    soundID = RaceIdleSound;
                } // blows nose
                else if (soundID == 782 || soundID == 1053)
                {
                    soundID = RaceIdleSound;
                } // burp!
                else if (soundID == 783 || soundID == 1054)
                {
                    soundID = RaceIdleSound;
                } // woohoo!
                ...
                else if (soundID == 786 || soundID == 1057)
                {
                    soundID = RaceIdleSound;
                } // bs cough
                else if (soundID == 787 || soundID == 1058)
                {
                    soundID = RaceHurtSound;
                } // cries
                else if (soundID == 791 || soundID == 1063)
                {
                    soundID = RaceDeathSound;
                } // faints
                else if (soundID == 792 || soundID == 1064)
                {
                    soundID = RaceHurtSound;
                } // farts
                else if (soundID == 793 || soundID == 1065)
                {
                    soundID = RaceAttackSound;
                } // gasp!
                else if (soundID == 794 || soundID == 1066)
                {
                    soundID = RaceAttackSound;
                } // giggles
                else if (soundID == 797 || soundID == 1069)
                {
                    soundID = RaceAngerSound;
                } // hey!
                else if (soundID == 798 || soundID == 1070)
                {
                    soundID = RaceIdleSound;
                } // hiccup!
                else if (soundID == 799 || soundID == 1071)
                {
                    soundID = RaceIdleSound;
                } // huh?
                else if (soundID == 800 || soundID == 1072)
                {
                    soundID = RaceIdleSound;
                } // kisses
                else if (soundID == 801 || soundID == 1073)
                {
                    soundID = RaceAttackSound;
                } // laughs
                else if (soundID == 802 || soundID == 1074)
                {
                    soundID = RaceAttackSound;
                } // no!
                else if (soundID == 803 || soundID == 1075)
                {
                    soundID = RaceIdleSound;
                } // oh!
                else if (soundID == 811 || soundID == 1085)
                {
                    soundID = RaceIdleSound;
                } // oooh
                else if (soundID == 812 || soundID == 1086)
                {
                    soundID = RaceAngerSound;
                } // oops
                else if (soundID == 813 || soundID == 1087)
                {
                    soundID = RaceAngerSound;
                } // pukes
                else if (soundID == 815 || soundID == 1089)
                {
                    soundID = RaceIdleSound;
                } // shhh!
                else if (soundID == 816 || soundID == 1090)
                {
                    soundID = RaceIdleSound;
                } // sigh
                else if (soundID == 817 || soundID == 1091)
                {
                    soundID = RaceIdleSound;
                } // ahh-choo!
                else if (soundID == 818 || soundID == 1092)
                {
                    soundID = RaceIdleSound;
                } // sniff
                else if (soundID == 819 || soundID == 1093)
                {
                    soundID = RaceIdleSound;
                } // snore
                else if (soundID == 820 || soundID == 1094)
                {
                    soundID = RaceIdleSound;
                } // spits
                else if (soundID == 821 || soundID == 1095)
                {
                    soundID = RaceIdleSound;
                } // whistles
                else if (soundID == 822 || soundID == 1096)
                {
                    soundID = RaceIdleSound;
                } // yawns
                else if (soundID == 823 || soundID == 1097)
                {
                    soundID = RaceAttackSound;
                } // yea!
                else if (
        ...
        public AccessLevel AccessLevel
        {
            ...
            set
            {
                ...
                if (oldValue != value)
                {
                    ...
                    SendMessage(
                        "Your access level has been changed. You are now {0}.",
        ...
        public int Fame
        {
            ...
            set
            {
                ...
                if (oldValue != value)
                {
                    ...
                    if (
                        ShowFameTitle
                        && (m_Player || m_Body.IsHuman)
                        && (oldValue >= 10000) != (value >= 10000)
                    )
                        InvalidateProperties();
        ...
        public virtual bool CanBeBeneficial(Mobile target, bool message, bool allowDead)
        {
            ...
            if (
                ...
                || !Alive
                || IsDeadBondedPet
                || (!allowDead && (!target.Alive || IsDeadBondedPet))
            )
            {
                if (message)
            ...
            if ( /*m_Player &&*/
                !Region.AllowBeneficial(this, target)
            )
            {
                // TODO: Pets
        ...
        {
            get { return (m_Female ? 0x2107 : 0x2106); }
        }
        ...
        public virtual bool CanSee(Item item)
        {
            ...
            return !item.Deleted
                && item.Map == m_Map
                && (item.Visible || m_AccessLevel > AccessLevel.Counselor);
        }
                    ...
                    && (
                        ...
                        || (
                            ...
                            && (
                                ...
                                || m_AccessLevel >= AccessLevel.Developer
                            )
                        )
                    )
                    && (
                        (m.Alive || (Core.SE && Skills.Spiritualism.Value >= 100.0))
                        || !Alive
                        || m_AccessLevel > AccessLevel.Player
                        || m.Warmode
                    )
        ...
        public virtual void OnPoisoned(Mobile from, Poison poison, Poison oldPoison)
        {
            if (poison != null)
            {
                ...
                this.NonlocalOverheadMessage(
                    MessageType.Regular,
                    0x21,
                    1042858 + (poison.Level * 2),
                    Name
                );
            }
        }
        ...
        {
            get { return m_WalkRegion; }
            set { m_WalkRegion = value; }
        }
        ...
        {
            get { return (m_Poison != null); }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public bool IsBodyMod
        {
            get { return (m_BodyMod.BodyID != 0); }
        }
                    ...
                    if (ourState != null)
                    {
                        ...
                        foreach (object o in eable)
                        {
                            if (o is Item)
                            {
                                ...
                                if (
                                    !Utility.InRange(oldLocation, loc, range)
                    ...
                    else
                    {
                        ...
                        foreach (NetState ns in eable)
                        {
                            if (
                                (
                                    isTeleport
                                    || !Utility.InUpdateRange(oldLocation, ns.Mobile.Location)
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public int RaceAngerSound
        {
            get { return m_RaceAngerSound; }
            set { m_RaceAngerSound = value; }
        }

        public int m_RaceIdleSound;

        [CommandProperty(AccessLevel.GameMaster)]
        public int RaceIdleSound
        {
            get { return m_RaceIdleSound; }
            set { m_RaceIdleSound = value; }
        }

        public int m_RaceDeathSound;

        [CommandProperty(AccessLevel.GameMaster)]
        public int RaceDeathSound
        {
            get { return m_RaceDeathSound; }
            set { m_RaceDeathSound = value; }
        }

        public int m_RaceAttackSound;

        [CommandProperty(AccessLevel.GameMaster)]
        public int RaceAttackSound
        {
            get { return m_RaceAttackSound; }
            set { m_RaceAttackSound = value; }
        }

        public int m_RaceHurtSound;

        [CommandProperty(AccessLevel.GameMaster)]
        public int RaceHurtSound
        {
            get { return m_RaceHurtSound; }
            set { m_RaceHurtSound = value; }
        }

        public bool m_RaceMakeSounds;

        [CommandProperty(AccessLevel.GameMaster)]
        public bool RaceMakeSounds
        {
            get { return m_RaceMakeSounds; }
            set { m_RaceMakeSounds = value; }
        }

        public int m_RaceMagicSchool;

        [CommandProperty(AccessLevel.GameMaster)]
        public int RaceMagicSchool
        {
            get { return m_RaceMagicSchool; }
            set { m_RaceMagicSchool = value; }
        }

        public bool m_RaceWasFemale;

        [CommandProperty(AccessLevel.GameMaster)]
        public bool RaceWasFemale
        {
            get { return m_RaceWasFemale; }
            set { m_RaceWasFemale = value; }
        }

        public int m_RaceSection;

        [CommandProperty(AccessLevel.GameMaster)]
        public int RaceSection
        {
            get { return m_RaceSection; }
            set { m_RaceSection = value; }
        }

        public int m_RaceHomeLand;

        [CommandProperty(AccessLevel.GameMaster)]
        public int RaceHomeLand
        {
            get { return m_RaceHomeLand; }
            set { m_RaceHomeLand = value; }
        }

        public bool m_RainbowMsg;

        [CommandProperty(AccessLevel.GameMaster)]
        public bool RainbowMsg
        {
            get { return m_RainbowMsg; }
            set { m_RainbowMsg = value; }
        }
        ...
        {
            get { return m_NoAutoAttack; }
            set { m_NoAutoAttack = value; }
        }

        public int m_DataStoreInt1;

        [CommandProperty(AccessLevel.GameMaster)]
        public int DataStoreInt1
        {
            get { return m_DataStoreInt1; }
            set { m_DataStoreInt1 = value; }
        }

        public int m_DataStoreInt2;

        [CommandProperty(AccessLevel.GameMaster)]
        public int DataStoreInt2
        {
            get { return m_DataStoreInt2; }
            set { m_DataStoreInt2 = value; }
        }

        public int m_DataStoreInt3;

        [CommandProperty(AccessLevel.GameMaster)]
        public int DataStoreInt3
        {
            get { return m_DataStoreInt3; }
            set { m_DataStoreInt3 = value; }
        }

        public int m_DataStoreInt4;

        [CommandProperty(AccessLevel.GameMaster)]
        public int DataStoreInt4
        {
            get { return m_DataStoreInt4; }
            set { m_DataStoreInt4 = value; }
        }

        public int m_DataStoreInt5;

        [CommandProperty(AccessLevel.GameMaster)]
        public int DataStoreInt5
        {
            get { return m_DataStoreInt5; }
            set { m_DataStoreInt5 = value; }
        }

        public string m_DataStoreStr1;

        [CommandProperty(AccessLevel.GameMaster)]
        public string DataStoreStr1
        {
            get { return m_DataStoreStr1; }
            set { m_DataStoreStr1 = value; }
        }

        public string m_DataStoreStr2;

        [CommandProperty(AccessLevel.GameMaster)]
        public string DataStoreStr2
        {
            get { return m_DataStoreStr2; }
            set { m_DataStoreStr2 = value; }
        }

        public string m_DataStoreStr3;

        [CommandProperty(AccessLevel.GameMaster)]
        public string DataStoreStr3
        {
            get { return m_DataStoreStr3; }
            set { m_DataStoreStr3 = value; }
        }

        public string m_DataStoreStr4;

        [CommandProperty(AccessLevel.GameMaster)]
        public string DataStoreStr4
        {
            get { return m_DataStoreStr4; }
            set { m_DataStoreStr4 = value; }
        }

        public string m_DataStoreStr5;

        [CommandProperty(AccessLevel.GameMaster)]
        public string DataStoreStr5
        {
            get { return m_DataStoreStr5; }
            set { m_DataStoreStr5 = value; }
        }
        ...
        public int Record_Hair
        {
            get { return RecordHair; }
            set
            {
                RecordHair = value;
                InvalidateProperties();
        ...
        [CommandProperty(AccessLevel.Owner)]
        public int Record_Beard
        {
            get { return RecordBeard; }
            set
            {
                RecordBeard = value;
                InvalidateProperties();
        ...
        [CommandProperty(AccessLevel.Owner)]
        public int Record_HairColor
        {
            get { return RecordHairColor; }
            set
            {
                RecordHairColor = value;
                InvalidateProperties();
            }
        }
        ...
        [CommandProperty(AccessLevel.Owner)]
        public int Record_BeardColor
        {
            get { return RecordBeardColor; }
            set
            {
                RecordBeardColor = value;
                InvalidateProperties();
            }
        }
        ...
        [CommandProperty(AccessLevel.Owner)]
        public int Record_SkinColor
        {
            get { return RecordSkinColor; }
            set
            {
                RecordSkinColor = value;
                InvalidateProperties();
            }
        }
        ...
        public virtual bool CheckNonlocalLift(Mobile from, Item item)
        {
            if (
                from == this
                || (
                    from.AccessLevel > this.AccessLevel
                    && from.AccessLevel >= AccessLevel.GameMaster
                )
            )
                return true;
        ...
        public virtual bool CheckEquip(Item item)
        {
            for (int i = 0; i < m_Items.Count; ++i)
                if (
                    m_Items[i].CheckConflictingLayer(this, item, item.Layer)
        ...
        public virtual bool AllowEquipFrom(Mobile mob)
        {
            return (
                mob == this
                || (mob.AccessLevel >= AccessLevel.GameMaster && mob.AccessLevel > this.AccessLevel)
            );
        }
        ...
        public virtual void ProcessDelta()
        {
            ...
            bool sendHits = false,
                ...
                sendAll = false,
                sendAny = false;
            bool sendIncoming = false,
                sendNonlocalIncoming = false;
            bool sendUpdate = false,
                sendRemove = false;
            bool sendPublicStats = false,
                sendPrivateStats = false;
            bool sendMoving = false,
                sendNonlocalMoving = false;
            bool sendOPLUpdate =
                ObjectPropertyList.Enabled && (delta & MobileDelta.Properties) != 0;

            bool sendHair = false,
                sendFacialHair = false,
                removeHair = false,
                removeFacialHair = false;

            bool sendHealthbarPoison = false,
                sendHealthbarYellow = false;
            ...
            if (
                (
                    delta
                    & (
                        MobileDelta.WeaponDamage
                        | MobileDelta.Resistances
                        | MobileDelta.Stat
                        | MobileDelta.Weight
                        | MobileDelta.Gold
                        | MobileDelta.Armor
                        | MobileDelta.StatCap
                        | MobileDelta.Followers
                        | MobileDelta.TithingPoints
                        | MobileDelta.Race
                    )
                ) != 0
            )
            ...
            if (ourState != null)
            {
                ...
                else
                {
                    ...
                    if (sendMoving || sendHealthbarPoison || sendHealthbarYellow)
                    {
                        int noto = Notoriety.Compute(m, m);
                        ourState.Send(
                            cache[1][noto] = Packet.Acquire(new MobileMovingOld(m, noto))
                        );
                    }
                }
                        ...
                        if (sendPublicStats)
                        {
                            if (m.CanBeRenamedBy(beholder))
                            {
                                if (statPacketTrue == null)
                                    statPacketTrue = Packet.Acquire(
                                        new MobileStatusCompact(true, m)
                                    );
                            ...
                            else
                            {
                                if (statPacketFalse == null)
                                    statPacketFalse = Packet.Acquire(
                                        new MobileStatusCompact(false, m)
                                    );
        ...
        )
        {
            ...
            if (ascii)
                ...
                state.Send(
                    new UnicodeMessage(m_Serial, Body, type, hue, 3, m_Language, Name, text)
                );
        }
        ...
        {
            get { return m_DisableDismountInWarmode; }
            set { m_DisableDismountInWarmode = value; }
        }
        ...
        public IMount Mount
        {
            get
            {
                ...
                if (mountItem == null)
                    m_MountItem =
                        (mountItem = (FindItemOnLayer(Layer.Mount) as IMountItem)) as Item;
        ...
        {
            get { return m_OldPropertyTitles ? ClickTitle : true; }
        }
        ...
        {
            get { return m_DisableHiddenSelfClick; }
            set { m_DisableHiddenSelfClick = value; }
        }
        public static bool AsciiClickMessage
        {
            get { return m_AsciiClickMessage; }
            set { m_AsciiClickMessage = value; }
        }
        public static bool GuildClickMessage
        {
            get { return m_GuildClickMessage; }
            set { m_GuildClickMessage = value; }
        }
        public static bool OldPropertyTitles
        {
            get { return m_OldPropertyTitles; }
            set { m_OldPropertyTitles = value; }
        }
        ...
        {
            get { return FindItemOnLayer(Layer.TwoHanded) as Item; }
        }

        public Item NeckArmor
        {
            get { return FindItemOnLayer(Layer.Neck) as Item; }
        }

        public Item HandArmor
        {
            get { return FindItemOnLayer(Layer.Gloves) as Item; }
        }

        public Item HeadArmor
        {
            get { return FindItemOnLayer(Layer.Helm) as Item; }
        }

        public Item ArmsArmor
        {
            get { return FindItemOnLayer(Layer.Arms) as Item; }
        }
        ...
        {
            get { return FindItemOnLayer(Layer.Talisman) as Item; }
        }

        public Item Special
        {
            get { return FindItemOnLayer(Layer.Special) as Item; }
        }
        ...
        {
            get { return m_CanHearGhosts || AccessLevel >= AccessLevel.Counselor; }
        ...
        {
            get { return RawStr + RawDex + RawInt; }
        }
