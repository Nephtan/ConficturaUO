        ...
        public static MultiComponentList Load(int multiID)
        {
            try
            {
                ...
                return new MultiComponentList(
                    m_StreamReader,
                    length / (MultiComponentList.PostHSFormat ? 16 : 12)
                );
            }
            catch
            {
            ...
            if (vx >= 0 && vx < m_Width && vy >= 0 && vy < m_Height)
            {
                ...
                for (int i = oldTiles.Length - 1; i >= 0; --i)
                {
                    ...
                    if (oldTiles[i].Z == z && (oldTiles[i].Height > 0 == data.Height > 0))
                    {
                        ...
                        bool oldIsRoof =
                            (
                                TileData.ItemTable[oldTiles[i].ID & TileData.MaxItemValue].Flags
                                & TileFlag.Roof
                            ) != 0;
        ...
        public void RemoveXYZH(int x, int y, int z, int minHeight)
        {
            ...
            if (vx >= 0 && vx < m_Width && vy >= 0 && vy < m_Height)
            {
                ...
                for (int i = 0; i < oldList.Length; ++i)
                {
                    ...
                    if (
                        tile.m_OffsetX == (short)x
                        && tile.m_OffsetY == (short)y
                        && tile.m_OffsetZ == (short)z
                        && TileData.ItemTable[tile.m_ItemID & TileData.MaxItemValue].Height
                            >= minHeight
                    )
                    {
                        MultiTileEntry[] newList = new MultiTileEntry[oldList.Length - 1];
            ...
            for (int x = 0; x < newWidth; ++x)
            {
                for (int y = 0; y < newHeight; ++y)
                {
                    ...
                    for (int i = 0; i < tiles.Length; ++i)
                    {
                        ...
                        m_List[index++] = new MultiTileEntry(
                            (ushort)tile.ID,
                            (short)vx,
                            (short)vy,
                            (short)tile.Z,
        ...
        public MultiComponentList(GenericReader reader)
        {
            ...
            for (int i = 0; i < allTiles.Length; ++i)
            {
                if (i == 0 || allTiles[i].m_Flags != 0)
                {
                    ...
                    tiles[xOffset]
                        [yOffset]
                        .Add((ushort)allTiles[i].m_ItemID, (sbyte)allTiles[i].m_OffsetZ);
                }
            }
