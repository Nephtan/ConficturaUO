...
namespace Server.Network
{
    ...
    public sealed class DisplayContextMenu : Packet
    {
            ...
            : base(0xBF)
        {
            ...
            for (int i = 0; i < length; ++i)
            {
                ...
                CMEFlags flags =
                    (e.Enabled && menu.From.InRange(p, range)) ? CMEFlags.None : CMEFlags.Disabled;
    ...
    public sealed class DisplayGumpPacked : Packet, IGumpWriter
    {
        ...
        public void AppendLayout(int val)
        {
            ...
            int bytes =
                System.Text.Encoding.ASCII.GetBytes(toString, 0, toString.Length, m_Buffer, 1) + 1;
        ...
        {
            string toString = val.ToString();
            int bytes = System.Text.Encoding.ASCII.GetBytes(
                toString,
                0,
                toString.Length,
                m_Buffer,
    ...
    {
        public static readonly Packet InvalidInstance = Packet.SetStatic(
            new PlayMusic(MusicName.Invalid)
        );
    ...
    public sealed class MobileStatus : Packet
    {
            ...
            : base(0x11)
        {
            ...
            if (beholder == beheld)
            {
                ...
                m_Stream.Write(
                    (short)(Core.AOS ? beheld.PhysicalResistance : (int)(beheld.ArmorRating + 0.5))
                );
    ...
    public sealed class CityInfo
    {
        ...
        public CityInfo(string city, string building, int description, int x, int y, int z)
            : this(city, building, description, x, y, z, Map.Sosaria) { }

        public CityInfo(string city, string building, int x, int y, int z)
            : this(city, building, 0, x, y, z, Map.Sosaria) { }
    ...
    public sealed class CharacterList : Packet
    {
            ...
            : base(0xA9)
        {
            ...
            if (count > 6)
                flags |= (
                    CharacterListFlags.SeventhCharacterSlot | CharacterListFlags.SixthCharacterSlot
    ...
    public abstract class Packet
    {
                ...
                else
                {
                    if ((m_State & State.Warned) == 0)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                op.WriteLine(
                                    "Redundant compile for packet {0}, use Acquire() and Release()",
                                    this.GetType()
                                );
        ...
        private void InternalCompile(bool compress)
        {
            ...
            if (compress)
            {
                m_CompiledBuffer = Compression.Compress(m_CompiledBuffer, 0, length, ref length);
