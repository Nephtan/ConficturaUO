...
namespace Server.Network
{
    ...
    public static class Compression
    {
        ...
        private const int DefiniteOverflow =
            ((BufferSize * 8) - TerminalCodeLength) / MinimalCodeLength;

        // If our input exceeds this length, we may potentially overflow the buffer
        private const int PossibleOverflow =
            ((BufferSize * 8) - TerminalCodeLength) / MaximalCodeLength;
        ...
        public static unsafe byte[] Compress(byte[] input, int offset, int count, ref int length)
        {
            ...
            lock (_syncRoot)
            {
                ...
                fixed (int* pTable = _huffmanTable)
                {
                    ...
                    {
                        byte* pInput = pInputBuffer + offset,
                            pInputEnd = pInput + count;

                        fixed (byte* pOutputBuffer = _outputBuffer)
                        {
                            byte* pOutput = pOutputBuffer,
                                pOutputEnd = pOutput + BufferSize;
        ...
        public static ZLibError Unpack(
            byte[] dest,
            ref int destLength,
            byte[] source,
            int sourceLength
    ...
    public sealed class Compressor32 : ICompressor
    {
        ...
        [DllImport("zlib32")]
        private static extern ZLibError compress(
            byte[] dest,
            ref int destLength,
            byte[] source,
            int sourceLength
        ...
        [DllImport("zlib32")]
        private static extern ZLibError uncompress(
            byte[] dest,
            ref int destLen,
            byte[] source,
            int sourceLen
