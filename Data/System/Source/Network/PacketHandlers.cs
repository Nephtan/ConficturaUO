...
namespace Server.Network
{
    ...
    public static class PacketHandlers
    {
        ...
        public static void Register6017(
            int packetID,
            int length,
            bool ingame,
            OnPacketReceive onReceive
        ...
        public static void RegisterExtended(int packetID, bool ingame, OnPacketReceive onReceive)
        {
            if (packetID >= 0 && packetID < 0x100)
                ...
                m_ExtendedHandlersHigh[packetID] = new PacketHandler(
                    packetID,
                    0,
                    ingame,
                    onReceive
        ...
        public static void RegisterEncoded(
            int packetID,
            bool ingame,
        ...
        {
            if (packetID >= 0 && packetID < 0x100)
                m_EncodedHandlersLow[packetID] = new EncodedPacketHandler(
                    packetID,
                    ingame,
                    onReceive
                );
            else
                m_EncodedHandlersHigh[packetID] = new EncodedPacketHandler(
                    packetID,
                    ingame,
                    onReceive
                );
        ...
        public static void RenameRequest(NetState state, PacketReader pvSrc)
        {
            ...
            if (targ != null)
                EventSink.InvokeRenameRequest(
                    new RenameRequestEventArgs(from, targ, pvSrc.ReadStringSafe())
                );
        }
            ...
            switch (pvSrc.ReadByte())
            {
                case 1: // Cancel
                {
                    ...
                    if (
                        ...
                        && cont.Trade != null
                        && (
                            cont.Trade.From.Mobile == state.Mobile
                            || cont.Trade.To.Mobile == state.Mobile
                        )
                    )
                        cont.Trade.Cancel();
        ...
        public static void VendorBuyReply(NetState state, PacketReader pvSrc)
        {
            ...
            if (vendor == null)
            {
                ...
                vendor.Deleted || !Utility.RangeCheck(vendor.Location, state.Mobile.Location, 10)
            )
            {
        ...
        public static void GodviewQuery(NetState state, PacketReader pvSrc)
        {
            ...
            {
                Console.WriteLine(
                    "God Client: {0}: Godview query 0x{1:X}",
                    state,
                    pvSrc.ReadByte()
                );
            }
        ...
        public static void ObjectHelpRequest(NetState state, PacketReader pvSrc)
        {
            ...
            if (serial.IsItem)
            {
                ...
                if (
                    item != null
                    && from.Map == item.Map
                    && Utility.InUpdateRange(item.GetWorldLocation(), from.Location)
                    && from.CanSee(item)
                )
            ...
            else if (serial.IsMobile)
            {
                ...
                if (
                    m != null
                    && from.Map == m.Map
                    && Utility.InUpdateRange(m.Location, from.Location)
                    && from.CanSee(m)
                )
                    m.OnHelpRequest(m);
            }
        ...
        public static void Edit(NetState state, PacketReader pvSrc)
        {
            if (VerifyGC(state))
            {
                ...
                Console.WriteLine(
                    "God Client: {0}: Edit {6} ({1}, {2}, {3}) 0x{4:X} (0x{5:X})",
                    state,
                    x,
                    y,
                    z,
        ...
        public static void DeleteStatic(NetState state, PacketReader pvSrc)
        {
            if (VerifyGC(state))
            {
                ...
                Console.WriteLine(
                    "God Client: {0}: Delete Static ({1}, {2}, {3}) 0x{4:X}",
                    state,
                    x,
                    y,
                    z,
        ...
        public static void NewTerrain(NetState state, PacketReader pvSrc)
        {
            if (VerifyGC(state))
            {
                ...
                Console.WriteLine(
                    "God Client: {0}: New Terrain ({1}, {2})+({3}, {4}) 0x{5:X4}",
                    state,
                    x,
                    y,
                    width,
        ...
        public static void TextCommand(NetState state, PacketReader pvSrc)
        {
            ...
            switch (type)
            {
                ...
                case 0x43: // Open spellbook
                {
                    ...
                    EventSink.InvokeOpenSpellbookRequest(
                        new OpenSpellbookRequestEventArgs(m, booktype)
                    );
                ...
                case 0x27: // Cast spell from book
                {
                    ...
                    if (split.Length > 0)
                    {
                        ...
                        EventSink.InvokeCastSpellRequest(
                            new CastSpellRequestEventArgs(m, spellID, World.FindItem(serial))
                        );
                    }
                ...
                case 0x56: // Cast spell from macro
                {
                    ...
                    EventSink.InvokeCastSpellRequest(
                        new CastSpellRequestEventArgs(m, spellID, null)
                    );
                ...
                {
                    Console.WriteLine(
                        "Client: {0}: Unknown text-command type 0x{1:X2}: {2}",
                        state,
                        type,
                        command
                    );
        ...
        public static void ProfileReq(NetState state, PacketReader pvSrc)
        {
            ...
            switch (type)
            {
                ...
                case 0x01: // edit request
                {
                    ...
                    EventSink.InvokeChangeProfileRequest(
                        new ChangeProfileRequestEventArgs(beholder, beheld, text)
                    );
                    ...
                    else
                    {
                        ...
                        if (type == 1)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    StaticTile[] tiles = map.Tiles.GetStaticTiles(
                                        x,
                                        y,
                                        !t.DisallowMultis
                                    );
                                    ...
                                    if (state.HighSeas)
                                    {
                                        ItemData id = TileData.ItemTable[
                                            graphic & TileData.MaxItemValue
                                        ];
                                        if (id.Surface)
                                        {
                                            z -= id.Height;
        ...
        public static void ExtendedCommand(NetState state, PacketReader pvSrc)
        {
            ...
            if (ph != null)
            {
                ...
                {
                    Console.WriteLine(
                        "Client: {0}: Sent ingame packet (0xBFx{1:X2}) before having been attached to a mobile",
                        state,
                        packetID
                    );
                    state.Dispose();
        ...
        public static void CastSpell(NetState state, PacketReader pvSrc)
        {
            ...
            EventSink.InvokeCastSpellRequest(
                new CastSpellRequestEventArgs(from, spellID, spellbook)
            );
        }
        ...
        public static void PartyMessage_PrivateMessage(NetState state, PacketReader pvSrc)
        {
            if (PartyCommands.Handler != null)
                PartyCommands.Handler.OnPrivateMessage(
                    state.Mobile,
                    World.FindMobile(pvSrc.ReadInt32()),
        ...
        public static void ContextMenuRequest(NetState state, PacketReader pvSrc)
        {
            ...
            if (from != null && target != null && from.Map == target.Map && from.CanSee(target))
            {
                if (target is Mobile && !Utility.InUpdateRange(from.Location, target.Location))
                    ...
                    target is Item
                    && !Utility.InUpdateRange(from.Location, ((Item)target).GetWorldLocation())
                )
                    return;
                ...
                if (c.Entries.Length > 0)
                {
                    if (target is Item)
                    {
                        ...
                        if (
                            root is Mobile
                            && root != from
                            && ((Mobile)root).AccessLevel >= from.AccessLevel
                        )
                        {
            ...
            if (m != null)
            {
                switch (type)
                {
                    case 0x00: // Unknown, sent by godclient
                    {
                        if (VerifyGC(state))
                            Console.WriteLine(
                                "God Client: {0}: Query 0x{1:X2} on {2} '{3}'",
                                state,
                                type,
                                m.Serial,
                                m.Name
        ...
        private static Dictionary<int, AuthIDPersistence> m_AuthIDWindow = new Dictionary<
            int,
            AuthIDPersistence
        >(m_AuthIDWindowSize);
