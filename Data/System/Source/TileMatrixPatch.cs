...
namespace Server
{
    public class TileMatrixPatch
    {
        ...
        private unsafe int PatchLand(TileMatrix matrix, string dataPath, string indexPath)
        {
            ...
            {
                using (
                    FileStream fsIndex = new FileStream(
                        indexPath,
                        FileMode.Open,
                        FileAccess.Read,
                        FileShare.Read
...
#if !MONO
                            NativeReader.Read(
                                fsData.SafeFileHandle.DangerousGetHandle(),
                                pTiles,
                                192
                            );
#else
        ...
        private unsafe int PatchStatics(
            TileMatrix matrix,
            string dataPath,
            string indexPath,
            string lookupPath
        )
        {
            ...
            )
            {
                ...
                {
                    using (
                        FileStream fsLookup = new FileStream(
                            lookupPath,
                            FileMode.Open,
                            FileAccess.Read,
                            FileShare.Read
