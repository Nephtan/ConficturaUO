...
namespace Server
{
    ...
    public class Region : IComparable
    {
        ...
        public virtual bool OnMoveInto(
            Mobile m,
            Direction d,
            Point3D newLocation,
            Point3D oldLocation
        ...
        private static void LoadRegions(XmlElement xml, Map map, Region parent)
        {
            foreach (XmlElement xmlReg in xml.SelectNodes("region"))
            {
                ...
                {
                    region = (Region)
                        Activator.CreateInstance(type, new object[] { xmlReg, map, parent });
                }
                catch (Exception ex)
                {
                    Console.WriteLine(
                        "Error during the creation of region type '{0}': {1}",
                        type.FullName,
                        ex
                    );
                    continue;
        ...
        protected static string GetAttribute(XmlElement xml, string attribute, bool mandatory)
        {
            ...
            else
            {
                if (mandatory)
                    Console.WriteLine(
                        "Missing attribute '{0}' in element '{1}'",
                        attribute,
                        xml.Name
                    );
        ...
        )
        {
            ...
            {
                Console.WriteLine(
                    "Could not parse integer attribute '{0}' in element '{1}'",
                    attribute,
                    xml.Name
                );
                return false;
        ...
        )
        {
            ...
            {
                Console.WriteLine(
                    "Could not parse boolean attribute '{0}' in element '{1}'",
                    attribute,
                    xml.Name
                );
                return false;
        ...
        public static bool ReadDateTime(
            XmlElement xml,
            string attribute,
            ref DateTime value,
            bool mandatory
        )
        {
            ...
            {
                Console.WriteLine(
                    "Could not parse DateTime attribute '{0}' in element '{1}'",
                    attribute,
                    xml.Name
                );
                return false;
        ...
        public static bool ReadTimeSpan(
            XmlElement xml,
            string attribute,
            ref TimeSpan value,
            bool mandatory
        )
        {
            ...
            {
                Console.WriteLine(
                    "Could not parse TimeSpan attribute '{0}' in element '{1}'",
                    attribute,
                    xml.Name
                );
                return false;
        ...
        public static bool ReadMap(XmlElement xml, string attribute, ref Map value, bool mandatory)
        {
            ...
            {
                Console.WriteLine(
                    "Could not parse Map attribute '{0}' in element '{1}'",
                    attribute,
                    xml.Name
                );
                return false;
        ...
        )
        {
            ...
            {
                Console.WriteLine(
                    "Could not parse Type attribute '{0}' in element '{1}'",
                    attribute,
                    xml.Name
                );
                return false;
        ...
        public static bool ReadPoint3D(XmlElement xml, Map map, ref Point3D value, bool mandatory)
        {
            ...
            bool xyOk =
                ReadInt32(xml, "x", ref x, mandatory) & ReadInt32(xml, "y", ref y, mandatory);
