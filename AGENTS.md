...
* **Project Outputs:** `Data/System/Source/Server.csproj` builds `ConficturaServer.exe`; `Debug|x86` outputs to the repository root and `Release|x86` outputs to `Data/System/Source/bin/Release/`. `Data/Scripts/Scripts.csproj` outputs to `Data/Scripts/` for `Debug|AnyCPU` and `Data/Scripts/bin/Release/` for `Release|AnyCPU`.
